...
class HeliconeCostAggregator:
    ...
    def add_gateway_operation(
        ...
        **governance_kwargs,
        ...
        logger.debug(
            f"Added gateway operation {operation_id}: {provider}/{model} - ${total_cost:.6f}"
        )
    ...
    def _update_provider_summary(self, operation: GatewayCostBreakdown):
        ...
        total_request_time = (
            getattr(summary, "_total_request_time", 0.0) + operation.request_time
        )
        total_gateway_overhead = (
            getattr(summary, "_total_gateway_overhead", 0.0)
            + operation.gateway_overhead
        )
        ...
        # Update model usage
        summary.models_used[operation.model] = (
            summary.models_used.get(operation.model, 0) + 1
        )
        summary.operation_types[operation.operation_type] = (
            summary.operation_types.get(operation.operation_type, 0) + 1
        )

    def _track_routing_decision(
        self, operation: GatewayCostBreakdown, governance_kwargs: dict[str, Any]
    ):
    ...
    def _generate_optimization_insights(self):
        ...
        if len(provider_costs) > 1:
            avg_costs = {
                p: sum(costs) / len(costs) for p, costs in provider_costs.items()
            }
            cheapest_provider = min(avg_costs.keys(), key=lambda x: avg_costs[x])
            most_expensive = max(avg_costs.keys(), key=lambda x: avg_costs[x])

            potential_savings = (
                avg_costs[most_expensive] - avg_costs[cheapest_provider]
            ) * sum(op.total_tokens for op in recent_ops)
            ...
            if potential_savings > 0.01:  # Only suggest if savings > 1 cent
                insights.append(
                    CostOptimizationInsight(
                        ...
                        affected_operations=len(
                            [op for op in recent_ops if op.provider == most_expensive]
                        ),
        ...
        high_overhead_ops = [
            op for op in recent_ops if op.gateway_overhead > 0.5
        ]  # > 500ms
        if (
            len(high_overhead_ops) > len(recent_ops) * 0.2
        ):  # More than 20% have high overhead
            avg_overhead = sum(op.gateway_overhead for op in high_overhead_ops) / len(
                high_overhead_ops
            )
        ...
        for model, usage_data in model_usage.items():
            ...
            if (
                model in ["gpt-4", "claude-3-opus", "mistral-large"]
                and avg_tokens < 100
            ):
                ...
                insights.append(
                    CostOptimizationInsight(
                        ...
                        potential_savings=sum(
                            cost * 0.6 for _, cost in usage_data
                        ),  # Estimate 60% savings
                        confidence=0.7,
                        actionable_steps=[
    ...
    def get_cost_summary(self) -> GatewayCostSummary:
        ...
        # Calculate averages
        avg_request_time = (
            sum(op.request_time for op in self.operations) / total_operations
        )
        avg_gateway_overhead = (
            sum(op.gateway_overhead for op in self.operations) / total_operations
        )
    ...
    def get_routing_analysis(self) -> dict[str, Any]:
        ...
        strategies_used = list(
            {decision.get("routing_strategy") for decision in self.routing_decisions}
        )
        total_cost_impact = sum(
            decision.get("cost_impact", 0.0) for decision in self.routing_decisions
        )
        ...
        return {
            ...
            "avg_savings_per_decision": total_cost_impact
    ...
    def export_cost_data(self, format: str = "json") -> Union[str, dict[str, Any]]:
        ...
        export_data = {
            ...
            "recent_operations": [
                asdict(op)
                for op in self.operations[-20:]  # Last 20 operations
            ],
            "optimization_insights": [asdict(insight) for insight in insights],
...
def aggregate_multi_session_costs(
    aggregators: list[HeliconeCostAggregator],
) -> dict[str, Any]:
    ...
    total_operations = sum(
        agg.get_cost_summary().total_operations for agg in aggregators
    )
