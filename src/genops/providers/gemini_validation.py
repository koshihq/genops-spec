...
try:
    from genops.core.telemetry import GenOpsTelemetry  # noqa: F401
...
class GeminiValidationResult:
    ...
    def has_errors(self) -> bool:
        ...
        return len(self.errors) > 0 or any(
            check.level == ValidationLevel.ERROR for check in self.checks
        )

    def has_warnings(self) -> bool:
        """Check if validation has any warnings."""
        return len(self.warnings) > 0 or any(
            check.level == ValidationLevel.WARNING for check in self.checks
        )

    def get_error_count(self) -> int:
        """Get total number of errors."""
        return len(self.errors) + len(
            [c for c in self.checks if c.level == ValidationLevel.ERROR]
        )

    def get_warning_count(self) -> int:
        """Get total number of warnings."""
        return len(self.warnings) + len(
            [c for c in self.checks if c.level == ValidationLevel.WARNING]
        )
...
def validate_gemini_setup(
        ...
        try:
            ...
            if response and hasattr(response, "text"):
                ...
                result.warnings.append(
                    "API connectivity test returned unexpected response"
                )
    ...
    if test_model_access and effective_api_key and not result.has_errors():
        models_to_test = ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-1.5-flash"]
        ...
        try:
            ...
            if inaccessible_models:
                ...
                result.warnings.append(
                    f"Some models inaccessible: {', '.join(model_names)}"
                )
    ...
    if performance_test and effective_api_key and not result.has_errors():
        try:
            ...
            test_prompts = [
                ...
                (
                    "large",
                    "Write a detailed analysis of the impact of artificial intelligence on modern society, covering economic, social, and technological aspects. Include examples and future predictions.",
                ),
            ]
            ...
            for size, prompt in test_prompts:
                try:
                    ...
                    performance_results[f"{size}_request_error"] = str(e)  # type: ignore[assignment]
            ...
            avg_latency = sum(
                ...
                1,
                len(
                    [k for k in performance_results.keys() if k.endswith("_latency_ms")]
                ),
            )
    ...
    if not GENOPS_AVAILABLE:
        recommendations.append(
            "Consider installing full GenOps core for complete telemetry capabilities"
        )
    ...
    if "connectivity_latency_ms" in result.performance_metrics:
        ...
        if latency > 2000:
            recommendations.append(
                "High latency detected - consider optimizing network or using regional endpoints"
            )
    ...
    if "accessible_models" in result.performance_metrics:
        ...
        if accessible_count < 2:
            recommendations.append(
                "Limited model access - consider upgrading API tier for more model options"
            )

    # Environment-specific recommendations
    if not os.getenv("GEMINI_API_KEY"):
        recommendations.append(
            "Set GEMINI_API_KEY environment variable for easier configuration management"
        )
...
def print_validation_result(
    result: GeminiValidationResult, detailed: bool = False
) -> None:
    """
    ...
    success_count = len(
        [c for c in result.checks if c.level == ValidationLevel.SUCCESS]
    )
    ...
    if error_count > 0:
        ...
        has_auth_error = any(
            "authentication" in str(err).lower() for err in result.errors
        )
        has_quota_error = any("quota" in str(err).lower() for err in result.errors)
        has_network_error = any(
            any(
                net_term in str(err).lower()
                for net_term in ["network", "connection", "timeout"]
            )
        ...
        print("   🔧 Test Your Fix:")
        print(
            '      python -c "from genops.providers.gemini import validate_setup; validate_setup()"'
        )
        print()
...
def quick_validate() -> None:
    ...
    if validate_gemini_quick():
        ...
        print(
            '   python -c "from genops.providers.gemini_validation import validate_gemini_setup, print_validation_result; print_validation_result(validate_gemini_setup(), detailed=True)"'
        )
        print()
