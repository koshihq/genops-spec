...
class GenOpsCohereAdapter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.api_key:
            logger.warning(
                "No Cohere API key provided. Set CO_API_KEY environment variable or pass api_key parameter"
            )

        # Initialize Cohere client
        client_kwargs = {"api_key": self.api_key, "timeout": timeout, **kwargs}
        if base_url:
        ...
        logger.info(
            f"GenOpsCohereAdapter initialized with cost tracking: {cost_tracking_enabled}"
        )
    ...
    def _check_budget_limit(self, estimated_cost: float) -> bool:
        ...
        if projected_total > self.budget_limit:
            logger.warning(
                f"Operation would exceed budget limit: ${projected_total:.6f} > ${self.budget_limit:.6f}"
            )
            return False
        ...
        if projected_total > (self.budget_limit * self.cost_alert_threshold):
            logger.warning(
                f"Approaching budget limit: ${projected_total:.6f} / ${self.budget_limit:.6f}"
            )
    ...
    def _update_usage_stats(self, usage: CohereUsageMetrics):
        ...
        if self.debug:
            logger.debug(
                f"Operation {usage.operation_id}: {usage.operation_type.value} - ${usage.total_cost:.6f}"
            )
    ...
    def chat(
        ...
        **governance_kwargs,
        ...
        with self._create_span("chat", **governance_attrs, model=model):
            try:
                ...
                if temperature is not None:
                    request_params["temperature"] = temperature  # type: ignore
                if max_tokens is not None:
                    request_params["max_tokens"] = max_tokens  # type: ignore
                if stream and self.enable_streaming:
                    request_params["stream"] = True  # type: ignore
    ...
    def generate(
        ...
        **governance_kwargs,
        ...
        with self._create_span("generate", **governance_attrs, model=model):
            try:
                # Prepare request parameters
                request_params = {"model": model, "prompt": prompt}

                if temperature is not None:
                    request_params["temperature"] = temperature  # type: ignore[assignment]
                if max_tokens is not None:
                    request_params["max_tokens"] = max_tokens  # type: ignore[assignment]
                if stop_sequences:
                    request_params["stop_sequences"] = stop_sequences  # type: ignore
    ...
    def _process_generate_response(
        ...
        usage = CohereUsageMetrics(
            ...
            operation_cost=operation_cost,
            latency_ms=latency_ms,
            **governance_attrs,  # type: ignore[arg-type]
        )
    ...
    def embed(
        ...
        **governance_kwargs,
        ...
        with self._create_span(
            "embed", **governance_attrs, model=model, text_count=len(texts)
        ):
            try:
                # Prepare request parameters
                request_params = {
    ...
    def rerank(
        ...
        **governance_kwargs,
        ...
        with self._create_span(
            "rerank", **governance_attrs, model=model, document_count=len(documents)
        ):
            try:
                # Prepare request parameters
                request_params = {
    ...
    def _process_rerank_response(
        ...
        if hasattr(response, "meta") and hasattr(response.meta, "billed_units"):
            search_units = getattr(
                response.meta.billed_units, "search_units", search_units
            )

        # Calculate costs
        input_cost, output_cost, operation_cost = self._calculate_cost(
            model=model, operation=CohereOperation.RERANK, operation_units=search_units
        )
    ...
    def get_usage_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_cost_per_operation": round(
                self._total_cost / max(1, self._operation_count), 6
            ),
    ...
    try:
        ...
        if HAS_OTEL:
            ...
            with tracer.start_as_current_span(
                f"genops.cohere.workflow.{workflow_name}"
            ) as span:
