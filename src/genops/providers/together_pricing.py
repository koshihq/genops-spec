...
class ModelPricing:
    ...
    context_length: int
    cost_per_image: Optional[Decimal] = None  # For multimodal models
    fine_tuning_cost_per_million: Optional[Decimal] = None
...
class TogetherPricingCalculator:
    ...
    def get_model_pricing(self, model_id: str) -> ModelPricing:
        ...
        for known_model in self.pricing_data:
            if model_id.lower() in known_model.lower() or known_model.lower() in model_id.lower():
                logger.info(f"Using pricing for similar model '{known_model}' for '{model_id}'")
    ...
    def recommend_model(
        ...
        for comp in comparisons:
            ...
            elif task_complexity == "moderate" and tier in [TogetherModelTier.STANDARD, TogetherModelTier.LARGE]:
                filtered_comparisons.append(comp)
            elif task_complexity == "complex" and tier in [TogetherModelTier.LARGE, TogetherModelTier.PREMIUM]:
                filtered_comparisons.append(comp)
            else:
    ...
    def analyze_costs(
        ...
        return {
            ...
            'potential_savings': {
                ...
                'best_alternative': cheaper_alternatives[0] if cheaper_alternatives else None,
                'potential_daily_savings': float(
                    daily_cost - (comparisons[0]['estimated_cost'] * operations_per_day)
                ) if comparisons else 0,
                'potential_monthly_savings': float(
                    monthly_cost - (comparisons[0]['estimated_cost'] * operations_per_day * days_to_analyze)
                ) if comparisons else 0
            }
        }
