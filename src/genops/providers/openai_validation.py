...
def check_dependencies() -> list[ValidationIssue]:
    ...
    # Core dependencies
    core_deps = {"opentelemetry": "OpenTelemetry SDK", "openai": "OpenAI Python client"}
...
def print_openai_validation_result(result: ValidationResult) -> None:
    ...
    if not result.is_valid:
        ...
        print(
            '   2. Run validation again: python -c "from genops.providers.openai_validation import validate_openai_setup, print_openai_validation_result; print_openai_validation_result(validate_openai_setup())"'
        )
