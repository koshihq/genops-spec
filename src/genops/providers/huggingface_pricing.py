...
HUGGINGFACE_PRICING = {
    ...
    "openai": {
        ...
        "whisper-1": {"input": 0.006, "output": 0.0},  # per minute, converted to token equiv
...
def estimate_model_size_category(model: str) -> str:
    ...
    elif any(img_type in model_lower for img_type in ["diffus", "dalle", "imagen", "midjourney"]):
        return "image_models"
    else:
...
def get_model_pricing(provider: str, model: str, task: str = "text-generation") -> Dict[str, float]:
    ...
    if provider == "huggingface_hub":
        ...
        base_pricing = pricing_data.get(size_category, pricing_data.get("medium_models", {"input": 0.0001, "output": 0.0002}))
    else:
...
def calculate_huggingface_cost(
    ...
    try:
        ...
        if task == "text-to-image" and images_generated > 0:
            ...
            total_cost = max(total_cost, image_cost)  # Use higher of token-based or image-based cost
...
def get_provider_info(model: str) -> Dict[str, any]:
    ...
    info["cost_estimates"] = {
        "short_chat": calculate_huggingface_cost(provider, model, 100, 50, "chat-completion"),
        "long_generation": calculate_huggingface_cost(provider, model, 500, 2000, "text-generation"),
        "embedding": calculate_huggingface_cost(provider, model, 1000, 0, "feature-extraction"),
    }

    return info


def compare_model_costs(models: list[str], input_tokens: int = 1000, output_tokens: int = 500, task: str = "text-generation") -> Dict[str, Dict[str, any]]:
    """Compare costs across multiple models for the same workload."""
    comparison = {}

    for model in models:
        provider = detect_model_provider(model)
        cost = calculate_huggingface_cost(provider, model, input_tokens, output_tokens, task)
    ...
    if comparison:
        ...
        for model_info in comparison.values():
            model_info["relative_cost"] = model_info["cost"] / min_cost if min_cost > 0 else 1.0

    return comparison


def get_cost_optimization_suggestions(model: str, task: str = "text-generation") -> Dict[str, any]:
        ...
        for alt_model, alt_pricing in provider_models.items():
            if alt_pricing["input"] < current_pricing["input"]:
                suggestions["alternatives"].append({
                    ...
                    "savings": round((current_pricing["input"] - alt_pricing["input"]) / current_pricing["input"] * 100, 1)
                })
