...
def validate_setup(raindrop_api_key: Optional[str] = None) -> ValidationResult:
    ...
    if is_valid:
        ...
        recommendations.append(
            "Fix the error-level issues above to enable Raindrop AI integration"
        )
...
def _validate_sdk_installation(
    issues: list[ValidationIssue],
    warnings: list[ValidationIssue],
    recommendations: list[str],
):
    ...
    try:
        ...
        try:
            from genops.providers.raindrop import auto_instrument  # noqa: F401
    ...
    if raindrop_spec is None:
        ...
        recommendations.append(
            "Consider installing the Raindrop AI SDK for enhanced features"
        )
    else:
        try:
            import raindrop
            ...
            try:
                ...
                recommendations.append(
                    "Raindrop AI SDK client instantiation successful"
                )
    ...
    try:
        ...
        recommendations.append(
            f"Python packages location: {site_packages[0] if site_packages else user_site}"
        )
    except Exception:
        pass
    ...
    if active_venv:
        recommendations.append(
            f"Virtual environment detected: {os.getenv(active_venv, 'active')}"
        )
    else:
        warnings.append(
            ValidationIssue(
...
def _validate_authentication(
    ...
    try:
        ...
        raindrop.Client(api_key=api_key)
    ...
    if budget_limit:
        try:
            ...
            if budget_value <= 0:
                ...
                recommendations.append(
                    f"Daily budget limit configured: ${budget_value}"
                )
        except ValueError:
            warnings.append(
                ValidationIssue(
...
def _validate_governance_setup(
    issues: list[ValidationIssue],
    warnings: list[ValidationIssue],
    recommendations: list[str],
):
...
def print_validation_result(result: ValidationResult, verbose: bool = True) -> None:
    ...
    if verbose:
        ...
        print("\n📊 Validation Summary:")
        print(
            f"  • SDK Installation: {_count_issues_by_category(result.issues + result.warnings, 'sdk')} issues"
        )
        print(
            f"  • Authentication: {_count_issues_by_category(result.issues + result.warnings, 'auth')} issues"
        )
        print(
            f"  • Configuration: {_count_issues_by_category(result.issues + result.warnings, 'config')} issues"
        )
        print(
            f"  • Governance: {_count_issues_by_category(result.issues + result.warnings, 'governance')} issues"
        )
    ...
    print("\n🚀 Next Steps:")
    if result.is_valid:
        print("  1. You can now use GenOps Raindrop integration with confidence")
        print("  2. Try the basic example: python examples/raindrop/basic_tracking.py")
        print(
            "  3. Explore advanced features: python examples/raindrop/advanced_features.py"
        )
    else:
        print("  1. Fix the error-level issues listed above")
        print("  2. Re-run validation: python setup_validation.py")
        print("  3. Check the troubleshooting guide if issues persist")
...
def validate_setup_interactive() -> ValidationResult:
    ...
    if not api_key:
        ...
        api_key = input(
            "Please enter your Raindrop AI API key (or press Enter to skip): "
        ).strip()
    ...
    if not os.getenv("GENOPS_TEAM"):
        team = input(
            "Enter your team name for cost attribution (default: 'default'): "
        ).strip()
        if team:
    ...
    if not os.getenv("GENOPS_PROJECT"):
        project = input(
            "Enter your project name for cost attribution (default: 'default'): "
        ).strip()
        if project:
