...
def check_promptlayer_installation() -> ValidationCheck:
    ...
    try:
        ...
        from promptlayer import PromptLayer  # noqa: F401
...
@_measure_duration
def check_genops_configuration(
...
def check_genops_promptlayer_integration() -> ValidationCheck:
    ...
    try:
        ...
        adapter = GenOpsPromptLayerAdapter(
            team="validation-test", project="integration-check"
        )
...
def check_governance_features() -> ValidationCheck:
    ...
    try:
        ...
        with adapter.track_prompt_operation(
            prompt_name="test_prompt", operation_type="validation"
        ) as span:
...
def check_performance_overhead() -> ValidationCheck:
    ...
    try:
        ...
        for i in range(iterations):
            with adapter.track_prompt_operation(
                prompt_name=f"perf_test_{i}", operation_type="performance_test"
            ) as span:
...
def validate_setup(
    ...
    summary = {
        ...
        "categories": list({c.category for c in checks}),
...
def print_validation_result(result: ValidationResult, detailed: bool = False) -> None:
    ...
    if result.overall_status == ValidationStatus.PASSED:
        print(
            "   ✅ All checks passed! You're ready to use PromptLayer with GenOps governance."
        )
