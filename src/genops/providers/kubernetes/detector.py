...
class KubernetesContext:
    ...
    annotations: dict[str, str] = None  # type: ignore[assignment]
...
class KubernetesDetector:
    ...
    def _detect_environment(self) -> None:
        ...
        if self.context.is_running_in_kubernetes:
            logger.info(
                f"🚢 Kubernetes context detected: {self.context.pod_namespace}/{self.context.pod_name}"
            )
        else:
        ...
        if self.context.pod_name and not self.context.is_running_in_kubernetes:
            ...
            if any(Path(p).exists() for p in ["/proc/1/cgroup", "/etc/resolv.conf"]):
                try:
                    ...
                    if any(
                        pattern in cgroup_content
                        for pattern in ["kubepods", "docker", "containerd"]
                    ):
                        self.context.is_running_in_kubernetes = True
                ...
                for line in cgroup_content.split("\n"):
                    if "docker" in line and "/" in line:
                        ...
                        if len(parts) > 1:
                            ...
                            if (
                                len(container_id) >= 12
                            ):  # Docker container IDs are at least 12 chars
    ...
    def _detect_cluster_info(self) -> None:
        ...
        self.context.cluster_name = os.getenv("CLUSTER_NAME") or os.getenv(
            "K8S_CLUSTER_NAME"
        )
