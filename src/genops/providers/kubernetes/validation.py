...
def validate_kubernetes_setup(
    enable_resource_monitoring: bool = True, cluster_name: Optional[str] = None
...
def _validate_kubernetes_context(
    detector: KubernetesDetector, issues: list[ValidationIssue]
) -> None:
...
def _validate_resource_monitoring(
    issues: list[ValidationIssue], result: KubernetesValidationResult
) -> None:
    ...
    try:
        ...
        try:
            from opentelemetry.instrumentation.system_metrics import (
                SystemMetricsInstrumentor,  # noqa: F401
            )
...
def print_kubernetes_validation_result(result: KubernetesValidationResult) -> None:
    ...
    if result.is_kubernetes_environment:
        ...
        print(
            f"   Resource Monitoring: {'✅' if result.has_resource_monitoring else '❌'}"
        )
    ...
    print("\n📚 Documentation:")
    print(
        "   • Kubernetes Guide: https://github.com/KoshiHQ/GenOps-AI/tree/main/docs/kubernetes"
    )
    print(
        "   • Troubleshooting: https://github.com/KoshiHQ/GenOps-AI/tree/main/docs/troubleshooting"
    )
