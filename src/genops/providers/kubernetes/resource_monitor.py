...
class KubernetesResourceMonitor:
    ...
    def _detect_cgroup_v1_limits(self) -> None:
        ...
        try:
            # Memory limit
            memory_limit_path = Path(self.cgroup_v1_paths["memory"]) / "memory.limit_in_bytes"
        ...
        try:
            ...
            if cpu_usage is not None:
                ...
                if self.limits.cpu_limit_millicores:
                    usage.cpu_usage_percent = (cpu_usage / self.limits.cpu_limit_millicores) * 100
            ...
            if memory_usage is not None:
                ...
                if self.limits.memory_limit_bytes:
                    usage.memory_usage_percent = (memory_usage / self.limits.memory_limit_bytes) * 100
    ...
    def _get_memory_usage(self) -> Optional[int]:
        ...
        try:
            ...
            # Try cgroup v1
            memory_usage_path = Path(self.cgroup_v1_paths["memory"]) / "memory.usage_in_bytes"
    ...
    def get_current_resources(self) -> Dict[str, Any]:
        ...
        if self.limits.memory_request_bytes:
            resources["memory_request"] = self._format_memory(self.limits.memory_request_bytes)
        if self.limits.memory_limit_bytes:
            resources["memory_limit"] = self._format_memory(self.limits.memory_limit_bytes)
        ...
        try:
            ...
            if usage.memory_usage_bytes:
                resources["memory_usage"] = self._format_memory(usage.memory_usage_bytes)
    ...
    def get_telemetry_attributes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # CPU metrics
            if usage.cpu_usage_millicores:
                attributes["k8s.container.cpu.usage_millicores"] = usage.cpu_usage_millicores
            if usage.cpu_usage_percent:
                attributes["k8s.container.cpu.usage_percent"] = usage.cpu_usage_percent
            
            # Memory metrics
            if usage.memory_usage_bytes:
                attributes["k8s.container.memory.usage_bytes"] = usage.memory_usage_bytes
            if usage.memory_usage_percent:
                attributes["k8s.container.memory.usage_percent"] = usage.memory_usage_percent
            ...
            # Resource limits
            if self.limits.cpu_limit_millicores:
                attributes["k8s.container.cpu.limit_millicores"] = self.limits.cpu_limit_millicores
            if self.limits.memory_limit_bytes:
                attributes["k8s.container.memory.limit_bytes"] = self.limits.memory_limit_bytes
    ...
    def check_quota_compliance(self, estimated_usage: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (self.limits.cpu_limit_millicores and 
                current_usage.cpu_usage_millicores and
                estimated_cpu):
                
                projected_cpu = current_usage.cpu_usage_millicores + estimated_cpu
                if projected_cpu > self.limits.cpu_limit_millicores:
                    logger.warning(f"CPU usage would exceed limit: {projected_cpu}m > {self.limits.cpu_limit_millicores}m")
                    return False
            
            # Check memory compliance
            estimated_memory = estimated_usage.get("memory_bytes", 0)
            if (self.limits.memory_limit_bytes and 
                current_usage.memory_usage_bytes and
                estimated_memory):
                
                projected_memory = current_usage.memory_usage_bytes + estimated_memory
                if projected_memory > self.limits.memory_limit_bytes:
                    logger.warning(f"Memory usage would exceed limit: {self._format_memory(projected_memory)} > {self._format_memory(self.limits.memory_limit_bytes)}")
                    return False
