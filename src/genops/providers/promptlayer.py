...
class MockPromptLayer:
    ...
    def run(self, *args, **kwargs):
        logger.warning(
            "MockPromptLayer.run() called - install PromptLayer for full functionality"
        )
        return {"error": "PromptLayer not installed", "mock": True}

    def track(self, *args, **kwargs):
        logger.warning(
            "MockPromptLayer.track() called - install PromptLayer for full functionality"
        )
        return {"error": "PromptLayer not installed", "mock": True}
...
class EnhancedPromptLayerSpan:
    ...
    def __init__(
        ...
        logger.info(
            f"Created PromptLayer span: {self.operation_name} (ID: {self.operation_id})"
        )
    ...
    def update_cost(self, cost: float) -> None:
        ...
        if self.max_cost and cost > self.max_cost:
            violation = (
                f"Operation cost ${cost:.6f} exceeds maximum ${self.max_cost:.6f}"
            )
...
class GenOpsPromptLayerAdapter:
    ...
    def __init__(
        ...
        """
        # Store configuration
        self.promptlayer_api_key = promptlayer_api_key or os.getenv(
            "PROMPTLAYER_API_KEY"
        )
        ...
        if not self.promptlayer_api_key:
            logger.warning(
                "PromptLayer API key not provided. Set PROMPTLAYER_API_KEY or pass promptlayer_api_key"
            )

        if not self.team:
            logger.warning(
                "Team not specified. Set GENOPS_TEAM or pass team parameter for cost attribution"
            )
        ...
        logger.info(
            f"GenOpsPromptLayerAdapter initialized for team: {self.team}, project: {self.project}"
        )
    ...
    def track_prompt_operation(
        ...
        final_operation_name = (
            operation_name or f"{prompt_name or 'prompt'}_{operation_type}"
        )
    ...
    def _check_governance_policies(self, span: EnhancedPromptLayerSpan) -> None:
        ...
        if self.max_operation_cost and span.estimated_cost > self.max_operation_cost:
            violations.append(
                f"Operation cost ${span.estimated_cost:.6f} exceeds limit ${self.max_operation_cost:.6f}"
            )

        # Check daily budget
        if (
            self.daily_budget_limit
            and (self.daily_usage + span.estimated_cost) > self.daily_budget_limit
        ):
            violations.append(
                f"Operation would exceed daily budget ${self.daily_budget_limit:.2f}"
            )
...
def instrument_promptlayer(
    ...
    **kwargs,
    ...
    return GenOpsPromptLayerAdapter(
        promptlayer_api_key=promptlayer_api_key, team=team, project=project, **kwargs
    )
...
def auto_instrument(
    ...
    **kwargs,
    ...
    logger.info(
        f"Auto-instrumentation enabled for PromptLayer with team: {team}, project: {project}"
    )
