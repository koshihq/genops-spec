...
def get_detailed_cost_breakdown(
    ...
    if "per_image_price" in model_info and images_generated > 0:
        input_cost = (input_tokens / 1000.0) * model_info["input_price_per_1k"] * regional_multiplier
        output_cost = model_info["per_image_price"] * images_generated * regional_multiplier
        total_tokens = input_tokens + images_generated * 100  # Rough equivalent for cost per token
    else:
        input_cost = (input_tokens / 1000.0) * model_info["input_price_per_1k"] * regional_multiplier
        output_cost = (output_tokens / 1000.0) * model_info["output_price_per_1k"] * regional_multiplier
...
def estimate_monthly_cost(
    ...
    return {
        ...
        "operations_per_dollar": 1.0 / (daily_cost / daily_operations) if daily_cost > 0 else 0
    }
...
def get_cheapest_model_for_task(
    ...
    if not suitable_models:
        ...
        suitable_models = [
            (model_id, calculate_bedrock_cost(model_id, input_tokens, output_tokens, region))
            for model_id, info in BEDROCK_MODELS.items()
