...
def validate_setup(skyrouter_api_key: Optional[str] = None) -> ValidationResult:
    ...
    _validate_skyrouter_configuration(
        skyrouter_api_key, errors, warnings, recommendations
    )
    ...
    if is_valid:
        recommendations.append(
            "All validation checks passed! SkyRouter integration is ready for use."
        )
...
def _validate_python_environment(
    ...
    if sys.version_info >= (3, 12):
        recommendations.append(
            "Using latest Python version - excellent for performance!"
        )
...
def _validate_dependencies(
    ...
    if not skyrouter_available:
        ...
        recommendations.append(
            "SkyRouter SDK detected - enhanced integration available"
        )


def _validate_authentication(
    skyrouter_api_key: Optional[str],
    errors: list[ValidationIssue],
    warnings: list[ValidationIssue],
    recommendations: list[str],
):
...
def _validate_skyrouter_configuration(
    skyrouter_api_key: Optional[str],
    errors: list[ValidationIssue],
    warnings: list[ValidationIssue],
    recommendations: list[str],
    ...
    if preferred_models and preferred_models != [""]:
        recommendations.append(
            f"Preferred models configured: {', '.join(preferred_models[:3])}"
        )
    else:
...
def _validate_genops_configuration(
    errors: list[ValidationIssue],
    warnings: list[ValidationIssue],
    recommendations: list[str],
):
...
def print_validation_result(result: ValidationResult, verbose: bool = True):
    ...
    if result.is_valid:
        ...
        print(
            '  2. Run validation again: python -c "from genops.providers.skyrouter_validation import validate_setup; validate_setup()"'
        )
...
def validate_setup_interactive() -> ValidationResult:
    ...
    if any(error.category == "auth" for error in result.errors):
        ...
        if api_key:
            print("Setting SKYROUTER_API_KEY environment variable...")
