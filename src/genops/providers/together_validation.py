...
class TogetherSetupValidator:
    ...
    def __init__(self, together_api_key: str | None = None):
        """
    ...
    def validate_model_access(
        self, test_models: list[str] | None = None
    ) -> Tuple[list[ValidationIssue], list[str]]:  # type: ignore  # noqa: F821
    ...
    def run_comprehensive_validation(
        ...
        if accessible_models:
            try:
                model_comparisons = self.pricing_calculator.compare_models(
                    accessible_models[:5]
                )
                pricing_info = {
                    "accessible_models": len(accessible_models),
                    "cost_comparison": model_comparisons,
                    "recommended_starter": model_comparisons[0]
        ...
        if accessible_models:
            recommendations.append(
                f"🎯 {len(accessible_models)} models available for use"
            )
        ...
        if warning_count > 0:
            recommendations.append(
                f"⚠️ {warning_count} configuration improvements recommended"
            )
...
def print_validation_result(result: ValidationResult, config: dict[str, Any]) -> None:
    ...
    if result.pricing_info and "cost_comparison" in result.pricing_info:
        ...
        for comp in comparisons:
            print(
                f"   • {comp['model']}: ${comp['cost_per_1k_tokens']:.4f} ({comp['tier']} tier)"
            )
