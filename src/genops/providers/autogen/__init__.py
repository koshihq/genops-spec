...
try:
    ...
    def create_chain_cost_context(chain_id: str):
        ...
        from genops.providers.autogen.cost_aggregator import (
            create_chain_cost_context as _real_func,
        )
...
try:
    ...
    HAS_AUTOGEN = True
    logger.info(
        f"GenOps AutoGen integration loaded - AutoGen {autogen.__version__} detected"
    )
except ImportError:
    HAS_AUTOGEN = False
    logger.warning(
        "AutoGen not installed - integration available but limited functionality"
    )
...
def quick_validate(*args, **kwargs):  # type: ignore  # noqa: F811
    """Lazy-loaded quick_validate function."""
    real_func = __getattr__("quick_validate")
    globals()["quick_validate"] = real_func
    return real_func(*args, **kwargs)
...
def enable_governance(**kwargs):
    ...
    budget = kwargs.get(
        "daily_budget_limit", float(os.getenv("GENOPS_BUDGET_LIMIT", "50.0"))
    )
    ...
    return auto_instrument(
        ...
        **{
            ...
            if k not in ["team", "project", "daily_budget_limit"]
        },
    )
...
def analyze_conversation_costs(
    adapter: "GenOpsAutoGenAdapter", time_period_hours: int = 24
) -> dict:
    """
    ...
    analysis = adapter.cost_aggregator.get_cost_analysis(
    ...
    return {
        ...
        "most_expensive_agent": max(
            analysis.cost_by_agent.items(), key=lambda x: x[1], default=(None, 0)
        )[0],
        "recommendations": [
            {
...
def get_conversation_insights(
    monitor: "AutoGenConversationMonitor", conversation_id: str
) -> dict:
    """
