...
class AutoGenCostAggregator:
    ...
    def __init__(
        ...
        self.alert_threshold = self.daily_budget_limit * Decimal(
            str(alert_threshold_percentage / 100)
        )
    ...
    def _calculate_cost(
        ...
        input_cost = (
            Decimal(str(model_pricing.get("input", 0)))
            * Decimal(str(input_tokens))
            / 1000
        )
        output_cost = (
            Decimal(str(model_pricing.get("output", 0)))
    ...
    ) -> Optional[ConversationCostSummary]:
        """Get cost summary for a specific conversation."""
        with self._lock:
            return self.conversation_summaries.get(conversation_id)

    def get_daily_cost(self, date: str = None) -> Decimal:  # type: ignore[assignment]
    ...
    def get_cost_analysis(self, time_period_hours: int = 24) -> CostAnalysisResult:
        ...
        with self._lock:
            ...
            for provider_type in cost_by_provider.keys():
                provider_entries = [
                    e for e in relevant_entries if e.provider == provider_type
                ]
                ...
                provider_summaries[provider_type] = ProviderCostSummary(
                    ...
                    avg_cost_per_operation=cost_by_provider[provider_type]
                    / max(len(provider_entries), 1),
                    cost_by_model={
                        model: sum(e.cost for e in provider_entries if e.model == model)
                        for model in {e.model for e in provider_entries}
                    },
                )
    ...
    def _generate_optimization_recommendations(
        ...
        if (
            ...
            and self._recommendation_cache
        ):
        ...
        for agent_name, agent_entries in agent_usage.items():
            ...
            avg_input_tokens = sum(e.input_tokens for e in agent_entries) / len(
                agent_entries
            )
            avg_output_tokens = sum(e.output_tokens for e in agent_entries) / len(
                agent_entries
            )
            ...
            current_provider = max(provider_usage, key=provider_usage.get)  # type: ignore
    ...
    def export_cost_data(self, format_type: str = "dict") -> Union[dict, str]:
        ...
        with self._lock:
            data = {
                ...
                "conversations": {
                    conv_id: {
                        ...
                        "agent_costs": {
                            k: str(v) for k, v in summary.agent_costs.items()
                        },
                        "provider_costs": {
                            k.value: str(v) for k, v in summary.provider_costs.items()
                        },
            ...
            if format_type == "dict":
                ...
                lines = [
                    "agent_name,provider,model,input_tokens,output_tokens,cost,timestamp,conversation_id"
                ]
...
def create_autogen_cost_context(conversation_id: str, **kwargs) -> AutoGenCostContext:
    ...
    aggregator = AutoGenCostAggregator(
        team=team, project=project, daily_budget_limit=budget_limit
    )
