...
def validate_autogen_setup(
    ...
    logger.info(f"AutoGen validation completed in {duration:.2f}s - Score: {result.overall_score:.1f}/100")
...
def quick_validate() -> bool:
    ...
    try:
        ...
        critical_errors = [issue for issue in result.issues if issue.severity == "error"]
...
def _gather_environment_info() -> Dict[str, Any]:
    ...
    env_vars = [
        ...
        'AWS_ACCESS_KEY_ID', 'COHERE_API_KEY', 'HUGGINGFACE_API_TOKEN',
            ...
            try:
                ...
                if current_ver < pkg_version.parse("0.2.0"):
                    result.issues.append(ValidationIssue(
                        ...
                        details={"current_version": version, "recommended_min": "0.2.0"}
                    ))
...
def _diagnose_autogen_install_issues(result: ValidationResult):
    ...
    try:
        import subprocess
        result_pip = subprocess.run([sys.executable, '-m', 'pip', 'show', 'pyautogen'], 
...
def _check_genops_integration(result: ValidationResult, team: str, project: str):
    ...
    try:
        from genops.providers.autogen import GenOpsAutoGenAdapter, auto_instrument
        ...
        if value:
            ...
            if key == 'OPENAI_API_KEY':
                if not value.startswith('sk-'):
                    invalid_keys.append((key, "OpenAI API keys should start with 'sk-'"))
                elif len(value) < 40:
                    invalid_keys.append((key, "OpenAI API key appears too short"))
            elif key == 'ANTHROPIC_API_KEY':
                if not value.startswith('sk-ant-'):
                    invalid_keys.append((key, "Anthropic API keys should start with 'sk-ant-'"))
...
def _check_model_availability(result: ValidationResult, models: List[str]):
    ...
    for model in models:
        ...
        if any(provider in model.lower() for provider in ['gpt', 'claude', 'gemini', 'command']):
...
def _check_api_connectivity(result: ValidationResult, timeout: int):
    ...
    if os.getenv('OPENAI_API_KEY'):
        try:
            ...
            if response.status_code == 200:
                ...
                connectivity_tests.append(("OpenAI", False, f"HTTP {response.status_code}"))
        except Exception as e:
    ...
    for provider, success, message in connectivity_tests:
        ...
        result.issues.append(ValidationIssue(
            ...
            fix_suggestion="Check API key and network connection" if not success else "No action needed",
...
def _run_performance_tests(result: ValidationResult):
    ...
    try:
        ...
        adapter = GenOpsAutoGenAdapter(team="test", project="perf-test")
...
def _generate_recommendations(result: ValidationResult):
    ...
    has_autogen = any("AutoGen" in issue.title for issue in result.issues 
                      and issue.severity != "error")
    has_api_keys = any("API Keys" in issue.title for issue in result.issues)
    
    if has_errors:
        recommendations.append("Fix all error-level issues before proceeding to production")
