...
class AutoGenConversationContext:
    ...
    def add_turn(self, cost: Decimal, tokens: int, agent_name: str = None):
...
class GenOpsAutoGenAdapter(BaseFrameworkProvider):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._cost_aggregator: Optional['AutoGenCostAggregator'] = None
        self._conversation_monitor: Optional['AutoGenConversationMonitor'] = None
    ...
    def setup_governance_attributes(self) -> None:
        ...
        self.REQUEST_ATTRIBUTES.update({
            'conversation_id', 'agent_name', 'agent_role', 'group_chat_id',
            'message_type', 'code_execution', 'function_call', 'turn_number'
        })
    ...
    def conversation_monitor(self) -> 'AutoGenConversationMonitor':
        ...
        return self._conversation_monitor
    ...
    def instrument_agent(self, agent, agent_name: str = None) -> Any:
        """
        ...
        if hasattr(agent, 'generate_reply'):
            ...
            def instrumented_generate_reply(*args, **kwargs):
                ...
                tracer = trace.get_tracer(__name__)
                with tracer.start_span(f"autogen.agent.{agent_name}.generate_reply") as span:
                    span.set_attributes({
                        "genops.framework": "autogen",
                    ...
                    try:
                        ...
                        span.set_attribute("genops.agent.response_time_ms", response_time_ms)
                        
                        # Estimate tokens (simplified - could be enhanced with actual counting)
                        if isinstance(result, str):
                            estimated_tokens = len(result.split()) * 1.3  # Rough estimation
                            span.set_attribute("genops.agent.estimated_tokens", int(estimated_tokens))
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        total_turns = sum(conv.turns_count for conv in self.session_context.conversations)
        ...
        return {
            ...
            "budget_utilization": (total_cost / float(self.session_context.budget_limit)) * 100,
