...
class ConversationTracker:
    ...
    def add_turn(
        ...
        **metadata,
        ...
        logger.debug(
            f"Added turn {self.current_turn} for {agent_name} in {self.conversation_id}"
        )

    def add_code_execution(
        self, language: str, success: bool, execution_time_ms: float = 0
    ):
        ...
        logger.debug(
            f"Code execution: {language} ({'success' if success else 'failed'})"
        )

    def add_function_call(self, function_name: str, parameters: dict[str, Any] = None):  # type: ignore[assignment]
    ...
    def get_metrics(self) -> ConversationMetrics:
        ...
        agent_participation = {
            agent: count / max(total_turns, 1) for agent, count in agent_turns.items()
        }
    ...
    def _efficiency_score(self) -> float:
        ...
        token_efficiency = min(
            1.0, avg_tokens_per_turn / 500
        )  # Normalize around 500 tokens
...
class AutoGenConversationMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized AutoGen conversation monitor - Team: {team}, Project: {project}"
        )
    ...
    def get_conversation_summary(self, time_period_hours: int = 24) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_conversations = [
                metrics
                for metrics in self.completed_conversations.values()
            ...
            avg_duration = statistics.mean(
                conv.duration_seconds for conv in recent_conversations
            )
            avg_turns = statistics.mean(
                conv.turns_count for conv in recent_conversations
            )
            avg_cost = statistics.mean(
                float(conv.total_cost) for conv in recent_conversations
            )
            avg_success_rate = statistics.mean(
                conv.success_rate for conv in recent_conversations
            )
            avg_quality_score = statistics.mean(
                conv.quality_score for conv in recent_conversations
            )
            ...
            return {
                ...
                "code_executions": sum(
                    conv.code_executions_count for conv in recent_conversations
                ),
                "function_calls": sum(
                    conv.function_calls_count for conv in recent_conversations
                ),
                "most_active_agents": dict(
                    sorted(
    ...
    def _update_agent_metrics(self, conversation_metrics: ConversationMetrics):
        ...
        for turn in conversation_metrics.conversation_turns:
            ...
            metrics.avg_response_time_ms = (
                metrics.avg_response_time_ms * (metrics.total_turns - 1)
    ...
    def _cleanup_old_conversations(self):
        ...
        old_conversations = [
            conv_id
            for conv_id, metrics in self.completed_conversations.items()
        ...
        with self._lock:
            data = {
                ...
                "code_execution_stats": {
                    **{
                        k: v
                        for k, v in self.code_execution_stats.items()
                        if k != "languages_used"
                    },
                    "languages_used": list(self.code_execution_stats["languages_used"]),  # type: ignore
                },
            }
