    ...
    **kwargs
    ...
    with _state_lock:
        if _instrumentation_state['enabled']:
            ...
            return _instrumentation_state['adapter']
...
def is_instrumented() -> bool:
    ...
    """
    with _state_lock:
        return _instrumentation_state['enabled']
...
def get_current_adapter() -> Optional[GenOpsAutoGenAdapter]:
    ...
    """
    with _state_lock:
        return _instrumentation_state.get('adapter')
...
def get_current_monitor() -> Optional[AutoGenConversationMonitor]:
    ...
    """
    with _state_lock:
        return _instrumentation_state.get('monitor')
    ...
    with _state_lock:
        ...
        try:
            ...
            return {
                ...
                "cost_by_provider": {k.value: str(v) for k, v in analysis.cost_by_provider.items()},
...
def get_instrumentation_stats() -> Dict[str, Any]:
    ...
    with _state_lock:
        ...
        return {
            ...
            "instrumented_classes": len(_instrumentation_state['instrumented_classes']),
            "instrumented_instances": len(_instrumentation_state['instrumented_instances']),
            "stats": stats,
            "config": _instrumentation_state.get('config', {}).__dict__ if _instrumentation_state.get('config') else {}
        }
...
def _instrument_autogen_classes():
    ...
    try:
        ...
        logger.info(f"Instrumented {len(_instrumentation_state['instrumented_classes'])} AutoGen classes")
...
def _instrument_conversable_agent(agent_class, adapter: GenOpsAutoGenAdapter):
    ...
    def instrumented_generate_reply(self, messages=None, sender=None, **kwargs):
        ...
        try:
            ...
            result = agent_class._genops_original_generate_reply(self, messages, sender, **kwargs)
            ...
            return agent_class._genops_original_generate_reply(self, messages, sender, **kwargs)
...
def _instrument_group_chat_manager(manager_class, adapter: GenOpsAutoGenAdapter):
    ...
    def instrumented_run_chat(self, messages=None, **kwargs):
        ...
        if adapter.conversation_monitor:
            participants = [agent.name for agent in self.groupchat.agents] if hasattr(self, 'groupchat') else []
            
            with adapter.track_group_chat(group_chat_id, participants) as context:
                try:
                    result = manager_class._genops_original_run_chat(self, messages, **kwargs)
...
def _restore_autogen_classes():
    ...
    try:
        ...
        if hasattr(autogen, 'ConversableAgent') and hasattr(autogen.ConversableAgent, '_genops_original_generate_reply'):
            autogen.ConversableAgent.generate_reply = autogen.ConversableAgent._genops_original_generate_reply
            autogen.ConversableAgent.send = autogen.ConversableAgent._genops_original_send
            autogen.ConversableAgent.receive = autogen.ConversableAgent._genops_original_receive
        ...
        if hasattr(autogen, 'GroupChatManager') and hasattr(autogen.GroupChatManager, '_genops_original_run_chat'):
            autogen.GroupChatManager.run_chat = autogen.GroupChatManager._genops_original_run_chat
            delattr(autogen.GroupChatManager, '_genops_original_run_chat')
            
        # Restore GroupChat
        if hasattr(autogen, 'GroupChat') and hasattr(autogen.GroupChat, '_genops_original_init'):
