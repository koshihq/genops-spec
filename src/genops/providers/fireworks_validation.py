...
class FireworksValidation:
    ...
    def validate_setup(
        ...
        **kwargs,
        ...
        try:
            ...
            result.connectivity_ok = self._test_connectivity(api_key, result)  # type: ignore
            ...
            if test_model_access:
                result.model_access = self._test_model_access(api_key, result)  # type: ignore
            ...
            result.is_valid = (
                ...
                and result.api_key_valid
                and result.connectivity_ok
                and (not test_model_access or len(result.model_access) > 0)
            )
    ...
    def _validate_dependencies(self, result: ValidationResult) -> bool:
        ...
        if not HAS_OPENAI:
            result.warnings.append(
                "OpenAI library not installed (optional for OpenAI compatibility)"
            )
    ...
    def _validate_api_key(
        self, api_key: Optional[str], result: ValidationResult
    ) -> bool:
        ...
        if api_key.startswith("sk-"):
            result.warnings.append(
                "API key format looks like OpenAI key, not Fireworks"
            )
            result.recommendations.append(
                "Ensure you're using a Fireworks API key, not OpenAI"
            )
    ...
    def _test_connectivity(self, api_key: str, result: ValidationResult) -> bool:
        ...
        try:
            ...
            try:
                ...
                result.performance_metrics["connectivity_latency_ms"] = (
                    connectivity_time * 1000
                )
                ...
                if "unauthorized" in error_str or "invalid api key" in error_str:
                    ...
                    result.recommendations.append(
                        "Check your Fireworks account billing status"
                    )
                elif "model" in error_str:
                    result.warnings.append(
                        "Model access issue (but API key may be valid)"
                    )
    ...
    def _test_model_access(self, api_key: str, result: ValidationResult) -> list[str]:
        ...
        if len(accessible_models) == 0:
            result.recommendations.append(
                "No models accessible - check account status and permissions"
            )
        elif len(accessible_models) < len(self.test_models) // 2:
            result.recommendations.append(
                "Limited model access - some models may require special permissions"
            )

        return accessible_models

    def _test_performance(
        self, api_key: str, result: ValidationResult
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if total_time < 2.0:
                ...
                result.recommendations.append(
                    "Check network connection for optimal performance"
                )
    ...
    def _validate_config(self, config: dict[str, Any], result: ValidationResult):
        ...
        for key in required_config:
            if key not in config or not config[key]:
                ...
                result.recommendations.append(
                    f"Set {key} in your GenOpsFireworksAdapter configuration"
                )
        ...
        for key in recommended_config:
            if key not in config:
                result.recommendations.append(
                    f"Consider setting '{key}' for better governance"
                )
        ...
        if "daily_budget_limit" in config:
            try:
                ...
                if daily_limit <= 0:
                    ...
                    result.recommendations.append(
                        "Very low daily budget limit may restrict usage"
                    )
        ...
        if "governance_policy" in config:
            ...
            if config["governance_policy"] not in valid_policies:
                result.warnings.append(
                    f"Invalid governance policy. Use: {', '.join(valid_policies)}"
                )
    ...
    def _generate_recommendations(self, result: ValidationResult):
        ...
        if result.performance_metrics:
            ...
            if tokens_per_sec > 50:
                result.recommendations.append(
                    "Excellent throughput - suitable for high-volume applications"
                )
...
def print_validation_result(result: ValidationResult):
    ...
    # Detailed results
    print(
        f"✅ Dependencies: {'✅ Installed' if result.dependencies_installed else '❌ Missing'}"
    )
    print(f"✅ API Key: {'✅ Valid' if result.api_key_valid else '❌ Invalid'}")
    print(
        f"✅ Connectivity: {'✅ Connected' if result.connectivity_ok else '❌ Failed'}"
    )
    print(
        f"✅ Model Access: ✅ {len(result.model_access)} models accessible"
        if result.model_access
        else "❌ No models accessible"
    )
...
def validate_api_key(api_key: Optional[str] = None) -> bool:
    ...
    result = validate_fireworks_setup(
        api_key=api_key, test_model_access=False, test_performance=False
    )
...
def check_api_key_validity(api_key: Optional[str] = None) -> tuple[bool, Optional[str]]:
    """Check if the Fireworks API key is valid.
    ...
    if not HAS_FIREWORKS:
        return False, "Fireworks SDK not installed"
    try:
        client = Fireworks(api_key=api_key)
        client.models.list()
        return True, None
...
def test_model_access(api_key: str) -> tuple[list[str], list[str]]:
    """Test access to available Fireworks models.

    Returns (accessible_models, failed_models).
    """
    if not HAS_FIREWORKS:
        return [], []
    try:
        ...
        logger.error(f"Failed to list Fireworks models: {exc}")
    ...
    for model_id in model_ids:
        try:
            ...
            logger.warning("Model %s failed access test: %s", model_id, exc)
            failed.append(model_id)
    return accessible, failed


def benchmark_performance(
    api_key: Optional[str] = None, iterations: int = 3
) -> dict[str, Any]:
    """Run a simple performance benchmark against Fireworks API."""
    if not api_key or not HAS_FIREWORKS:
        return {"error": "API key or Fireworks SDK not available"}
    results: list[float] = []
    try:
        client = Fireworks(api_key=api_key)
    ...
    return {
        ...
        "avg_latency_s": sum(results) / len(results) if results else 0,
        "min_latency_s": min(results) if results else 0,
        "max_latency_s": max(results) if results else 0,
    }


def collect_diagnostics(api_key: Optional[str] = None) -> dict[str, Any]:
    """Collect diagnostic information about the Fireworks setup."""
    diag: dict[str, Any] = {
        "fireworks_sdk_installed": HAS_FIREWORKS,
        "requests_installed": HAS_REQUESTS,
        "api_key_set": bool(api_key or os.getenv("FIREWORKS_API_KEY")),
    }
    if api_key:
        valid, msg = check_api_key_validity(api_key)
        diag["api_key_valid"] = valid
        if msg:
            diag["api_key_error"] = msg
    return diag


def generate_setup_report(api_key: Optional[str] = None) -> str:
    """Generate a human-readable setup report."""
    diag = collect_diagnostics(api_key)
    lines = ["Fireworks AI Setup Report", "=" * 40]
