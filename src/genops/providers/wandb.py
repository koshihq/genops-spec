...
class GenOpsWandbAdapter:
    ...
    def __init__(
        ...
        logger.info(
            f"GenOps W&B adapter initialized for team={self.team}, project={self.project}"
        )
        ...
        **kwargs,
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                logger.info(
                    f"Starting experiment lifecycle tracking: {experiment_name}"
                )
                ...
                duration = (
                    datetime.utcnow() - experiment_context.start_time
                ).total_seconds()
                ...
                span.set_attributes(
                    {
                        ...
                        "genops.governance.violations": len(
                            experiment_context.policy_violations
                        ),
                    }
                )
                ...
                logger.info(
                    f"Experiment completed: {experiment_name}, cost: ${total_cost:.4f}"
                )
        ...
        def enhanced_init(*args, **kwargs):
            ...
            try:
                ...
                if run:
                    ...
                    run.genops_update_cost = lambda cost: self._update_run_cost(
                        run.id, cost
                    )
                    run.genops_log_violation = lambda violation: (
                        self._log_policy_violation(run.id, violation)
                    )
                    ...
                    span.set_attributes(
                        {
                            ...
                            "genops.metrics.count": len(log_data)
    ...
    def get_experiment_cost_summary(
        self, experiment_id: str
    ) -> Optional[ExperimentCostSummary]:
        ...
        return ExperimentCostSummary(
            ...
            resource_efficiency=run_context.estimated_cost
            / max(duration / 3600, 0.01),  # Cost per hour
        )
...
def instrument_wandb(
    ...
    **kwargs,
    ...
    return GenOpsWandbAdapter(
        wandb_api_key=wandb_api_key, team=team, project=project, **kwargs
    )
