...
class InstrumentationRegistry:
    ...
    def __init__(self):
        ...
        self.component_patterns = {
            ...
            "converters": ["HTMLToDocument", "PDFToDocument", "TextFileToDocument"],
        }
...
def get_instrumentation_stats() -> dict[str, Any]:
    ...
    return {
        ...
        "instrumented_classes": [
            cls.__name__ for cls in _registry.instrumented_classes
        ],
        "config": _registry.config.copy(),
        "has_adapter": _registry.adapter is not None,
        "has_monitor": _registry.monitor is not None,
        "pipeline_executions": len(_registry.monitor._execution_results)
...
def _create_instrumented_pipeline_run():
    ...
    def instrumented_run(self, inputs: dict[str, Any], **kwargs):
        ...
        pipeline_name = (
            getattr(self, "name", "unnamed-pipeline") or f"pipeline-{id(self)}"
        )
                    ...
                    for node_name in self.graph.nodes():
                        try:
                            ...
                            if node:
                                ...
                                estimated_cost = _estimate_component_cost(
                                ...
                                component_result = HaystackComponentResult(
                                    ...
                                    cost=estimated_cost,
                                    provider=_get_provider_for_component(
                                        component_type
                                    ),
                                )
...
def _estimate_component_cost(component_type: str, inputs: dict[str, Any]) -> "Decimal":  # type: ignore  # noqa: F821
    ...
    def instrumented_run(self, **kwargs):
        ...
        if _registry.monitor and _registry.config["enable_component_tracking"]:
            try:
                ...
                logger.debug(
                    f"Enhanced component monitoring failed for {component_name}: {e}"
                )
        ...
        try:
            ...
            if HAS_HAYSTACK:
                ...
                for method_key, original_method in _registry.original_methods.items():
                    if "." in method_key and method_key not in [
                        "Pipeline.run",
                        "Component.run",
                    ]:
            ...
            logger.info(
                "Auto-instrumentation disabled - original Haystack behavior restored"
            )
