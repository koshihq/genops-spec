...
try:
    ...
    HAS_HAYSTACK = True
    logger.info(
        f"GenOps Haystack integration loaded - Haystack {haystack.__version__} detected"
    )
except ImportError:
    HAS_HAYSTACK = False
    logger.warning(
        "Haystack not installed - integration available but limited functionality"
    )
...
def analyze_pipeline_costs(
    adapter: "GenOpsHaystackAdapter", time_period_hours: int = 24
) -> dict:
    """
    ...
    return {
        ...
        "most_expensive_component": max(
            analysis.cost_by_component.items(), key=lambda x: x[1], default=(None, 0)
        )[0],
        "recommendations": [
            {
