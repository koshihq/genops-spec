...
class DatabricksCostSummary:
    ...
    def get_most_expensive_workspace(self) -> Optional[str]:
        """Get the workspace with highest cost."""
        if not self.cost_by_workspace:
            return None
        return max(self.cost_by_workspace, key=self.cost_by_workspace.get)

    def get_cost_per_gb_processed(self) -> Optional[float]:
...
class DatabricksUnityCatalogCostAggregator:
    ...
    def add_sql_warehouse_cost(
        ...
        **governance_attrs
        ...
        dbu_per_hour = self.sql_warehouse_pricing.get(warehouse_size, 0.44)  # Default to X-Small
    ...
    def add_compute_cluster_cost(
        ...
        **governance_attrs
        ...
        dbu_per_node_hour = self.compute_pricing.get(cluster_type, 0.15)  # Default to standard
    ...
    def get_summary(self, context_id: Optional[str] = None) -> DatabricksCostSummary:
        """
...
_cost_aggregator: Optional[DatabricksUnityCatalogCostAggregator] = None
