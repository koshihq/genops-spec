...
def validate_setup(
    ...
    **kwargs,
    ...
    if result.has_errors():
        result.passed = False
        logger.warning(
            f"Databricks Unity Catalog validation failed with {len(result.get_issues_by_severity('error'))} errors"
        )
    else:
...
def _validate_dependencies(result: ValidationResult) -> None:
    ...
    try:
        ...
        result.configuration["databricks_sdk_version"] = getattr(
            databricks, "__version__", "unknown"
        )
    except ImportError:
        result.add_issue(
            ValidationIssue(
...
def _validate_configuration(
    result: ValidationResult, workspace_url: str | None = None
) -> None:
    ...
    try:
        ...
        if workspace_url:
            ...
            try:
                ...
                result.configuration["authenticated_user"] = (
                    current_user.user_name or "unknown"
                )
            ...
            try:
                ...
                result.configuration["available_catalogs"] = len(catalogs)  # type: ignore[assignment]
            except Exception as e:
                result.add_issue(
                    ValidationIssue(
                        severity="warning",
            ...
            try:
                ...
                result.configuration["available_warehouses"] = len(warehouses)  # type: ignore[assignment]
            except Exception as e:
                result.add_issue(
                    ValidationIssue(
                        severity="info",
...
def _validate_governance_features(result: ValidationResult) -> None:
    ...
    try:
        ...
        try:
            from . import adapter, cost_aggregator, governance_monitor  # noqa: F401
...
def print_validation_result(result: ValidationResult) -> None:
    ...
    if result.passed:
        ...
        print("   Try running: python basic_tracking.py")
    else:
        print("\n🔧 Please fix the errors above and run validation again.")
        print("   Command: python setup_validation.py")
