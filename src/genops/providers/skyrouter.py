...
class SkyRouterSession:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if OTEL_AVAILABLE and self.span_context:
            self.span_context.set_attribute("genops.skyrouter.session.duration", duration)
            self.span_context.set_attribute("genops.skyrouter.session.total_cost", float(total_cost))
            self.span_context.set_attribute("genops.skyrouter.session.operation_count", len(self.operations))
            
            # Add governance attributes
            for key, value in self.governance_attrs.to_dict().items():
                self.span_context.set_attribute(key, value)
        
        logger.info(f"SkyRouter session completed: {self.session_name}, cost: ${total_cost:.3f}, operations: {len(self.operations)}")
    ...
    def track_model_call(
        ...
        logger.debug(f"Tracked SkyRouter model call: {model}, cost: ${cost_result.total_cost:.3f}")
        return cost_result
    ...
    def track_multi_model_routing(
        ...
        logger.debug(f"Tracked multi-model routing: {selected_model}, cost: ${cost_result.total_cost:.3f}")
        return cost_result
    ...
    def track_agent_workflow(
        ...
        primary_model = agent_steps[0].get("model", "unknown") if agent_steps else "unknown"
        ...
        if cost is not None:
            cost_result = SkyRouterCostResult(
                ...
                total_cost=Decimal(str(cost)),
                input_tokens=sum(self._estimate_input_tokens(step.get("input", {})) for step in agent_steps),
                output_tokens=sum(self._estimate_output_tokens("moderate") for step in agent_steps),
                governance_attrs=self.governance_attrs
            )
        else:
        ...
        cost_result.metadata.update({
            ...
            "models_used": list(set(step.get("model", "unknown") for step in agent_steps)),
            "session": self.session_name
        })
        ...
        logger.debug(f"Tracked agent workflow: {workflow_name}, cost: ${cost_result.total_cost:.3f}")
        return cost_result
    
    def _estimate_input_tokens(self, input_data: Dict[str, Any]) -> int:
        """Estimate input tokens from input data."""
        if isinstance(input_data, dict):
            text_content = str(input_data.get("messages", "")) + str(input_data.get("prompt", ""))
            return max(len(text_content.split()) * 1.3, 10)  # Rough token estimation
    ...
    def _check_budget_constraints(self, cost_result: SkyRouterCostResult):
        ...
        if hasattr(self.adapter, 'daily_budget_limit') and self.adapter.daily_budget_limit:
            ...
            if current_total > Decimal(str(self.adapter.daily_budget_limit)):
                logger.warning(f"Daily budget limit exceeded: ${current_total:.3f} > ${self.adapter.daily_budget_limit:.3f}")
    ...
    def total_cost(self) -> Decimal:
        ...
        return sum(Decimal(str(op.total_cost)) for op in self.operations)
...
class GenOpsSkyRouterAdapter:
    ...
    def __init__(
        ...
        logger.info(f"SkyRouter adapter initialized for team: {team}, project: {project}")
    ...
    def _estimate_fallback_cost(self, model: str, input_data: Dict[str, Any], complexity: str) -> float:
