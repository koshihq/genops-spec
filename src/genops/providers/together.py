...
try:
    ...
    Together = None
    logger.warning(
        "Together AI client not installed. Install with: pip install together"
    )
...
class TogetherSession:
    ...
    results: list[TogetherResult] = None  # type: ignore
...
class GenOpsTogetherAdapter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if HAS_TOGETHER:
            ...
            self.client = None
            logger.warning(
                "Together client not available. Some features may be limited."
            )

        logger.info(
            f"GenOps Together adapter initialized for team='{self.team}', project='{self.project}'"
        )

    def _build_base_tags(
    ...
    def chat_with_governance(
        ...
        model: str | TogetherModel = None,  # type: ignore[assignment]
        session_id: str | None = None,
        max_tokens: int = 1000,
        ...
        **governance_attributes,
        ...
        if not HAS_TOGETHER:
            raise GenOpsConfigurationError(
                "Together client required for Together AI integration"
            )
        ...
        estimated_cost = self.pricing_calculator.estimate_chat_cost(
            model=model_name, estimated_tokens=max_tokens
        )
        ...
        try:
            ...
            with self.telemetry.trace_operation(
                "together.chat_completion", **operation_attrs
            ) as span:
            ...
            if "span" in locals():
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))  # type: ignore  # noqa: F821
            raise
    ...
    def complete_with_governance(
        ...
        **governance_attributes,
        ...
        try:
            ...
            with self.telemetry.trace_operation(
                "together.completion", **operation_attrs
            ) as span:
                ...
                actual_cost = self.pricing_calculator.calculate_completion_cost(
                    model=model_name, tokens_used=tokens_used
                )
...
_current_adapter: GenOpsTogetherAdapter | None = None
...
def auto_instrument(
    ...
    **adapter_kwargs,
    ...
    _current_adapter = GenOpsTogetherAdapter(
        together_api_key=together_api_key, team=team, project=project, **adapter_kwargs
    )
...
def instrument_together(
    ...
    **kwargs,
    ...
    return GenOpsTogetherAdapter(
        together_api_key=together_api_key, team=team, project=project, **kwargs
    )


def get_current_adapter() -> GenOpsTogetherAdapter | None:
