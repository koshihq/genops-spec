...
class GriptapeStructureMetrics:
    ...
    def get_efficiency_metrics(self) -> Dict[str, Any]:
        ...
        if self.total_duration and self.total_duration > 0:
            throughput = self.tasks_completed / self.total_duration
        ...
        return {
            ...
            "memory_efficiency": 1.0 - min(1.0, (self.peak_memory_usage or 0) / 1000),  # Assume 1GB baseline
...
class GriptapeWorkflowMonitor:
    ...
    def record_reasoning_step(self, request_id: str, step_type: str = "generic") -> None:
    ...
    def get_performance_insights(
        ...
        success_rates = [s.get_task_performance()["success_rate"] for s in recent_sessions]
        ...
        if durations:
            ...
            if avg_duration > baseline * 1.5:
                insights.append(f"Average execution time ({avg_duration:.2f}s) is significantly above baseline")
                recommendations.append("Consider optimizing task sequence or using faster models")
        ...
        if success_rates:
            ...
            if avg_success_rate < baseline_success - 5:
                insights.append(f"Success rate ({avg_success_rate:.1f}%) is below baseline")
                recommendations.append("Review error patterns and improve error handling")
        
        # Resource utilization analysis
        high_memory_sessions = [s for s in recent_sessions if s.peak_memory_usage and s.peak_memory_usage > 500]
        ...
        if len(failed_tasks) > 0:
            ...
            if error_patterns:
                ...
                if most_common_error[0] == "timeout":
                    ...
                    recommendations.append("Implement rate limiting and backoff strategies")
