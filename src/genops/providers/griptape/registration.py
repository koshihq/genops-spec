...
def _wrap_structure_method(
    ...
    def wrapped_method(self, *args, **kwargs):
        ...
        if structure_type == "agent":
            context_manager = adapter.track_agent(
                structure_id, operation_type=operation_type
            )
        elif structure_type == "pipeline":
            context_manager = adapter.track_pipeline(
                structure_id, operation_type=operation_type
            )
        elif structure_type == "workflow":
            context_manager = adapter.track_workflow(
                structure_id, operation_type=operation_type
            )
        else:
...
def _wrap_structure_class(
    structure_class: type, structure_type: str, adapter: "GenOpsGriptapeAdapter"
) -> type:
    ...
    class WrappedStructure(structure_class):
        ...
        if hasattr(structure_class, "run"):
            run = _wrap_structure_method(
                structure_class.run, structure_type, "run", adapter
            )

        if hasattr(structure_class, "execute"):
            execute = _wrap_structure_method(
                structure_class.execute, structure_type, "execute", adapter
            )

        if hasattr(structure_class, "kickoff"):
            kickoff = _wrap_structure_method(
                structure_class.kickoff, structure_type, "kickoff", adapter
            )
    ...
    **kwargs,
    ...
    with _instrumentation_registry["lock"]:
        if _instrumentation_registry["enabled"]:
            ...
            return _instrumentation_registry["adapter"]  # type: ignore[return-value]

    # Validate Griptape availability
    if not _is_griptape_available():
        error_msg = (
            "Griptape framework not found. Please install it with: pip install griptape"
        )
    ...
    try:
        ...
        with _instrumentation_registry["lock"]:
            ...
            _instrumentation_registry["adapter"] = adapter  # type: ignore[assignment]
...
class InstrumentedGriptape:
    ...
    def __init__(self, adapter: "GenOpsGriptapeAdapter"):
        ...
        if _is_griptape_available():
            from griptape.engines import ExtractionEngine, RagEngine, SummaryEngine
            from griptape.structures import Agent, Pipeline, Workflow
    ...
    def track_engine_operation(self, engine_type: str, engine_id: str = None):  # type: ignore[assignment]
...
def is_instrumented() -> bool:
    ...
    with _instrumentation_registry["lock"]:
        return _instrumentation_registry["enabled"]  # type: ignore[return-value]


def get_instrumentation_adapter() -> Optional["GenOpsGriptapeAdapter"]:
    """Get the current auto-instrumentation adapter, if enabled."""
    with _instrumentation_registry["lock"]:
        return _instrumentation_registry["adapter"]  # type: ignore
...
def validate_griptape_setup() -> dict[str, Any]:
    ...
    if _is_griptape_available():
        ...
        validation_result["griptape_version"] = _detect_griptape_version()  # type: ignore

        try:
            from griptape.structures import Agent, Pipeline, Workflow  # noqa: F401

            validation_result["supported_structures"] = [
                "Agent",
                "Pipeline",
                "Workflow",
            ]
            ...
            try:
                from griptape.engines import RagEngine  # noqa: F401
            ...
            try:
                from griptape.tasks import PromptTask  # noqa: F401
        ...
        validation_result["recommendations"].append(
            "Install Griptape: pip install griptape"
        )
    ...
    if validation_result["griptape_version"]:
        ...
        if version == "unknown":  # type: ignore
