...
# Import existing cost calculators from GenOps providers
...
class GriptapeCostAggregator:
    ...
    def __init__(self):
        ...
        self.calculators = {
            ...
            "anthropic": AnthropicCostCalculator(), 
            "google": GoogleCostCalculator(),
            "bedrock": BedrockCostCalculator(),
        }
        ...
        try:
            ...
            if provider_lower in self.calculators:
                ...
                if hasattr(calculator, 'calculate_cost'):
                    result = calculator.calculate_cost(model, input_tokens, output_tokens)
                ...
                for method_name in ['get_cost', 'calculate_pricing', 'get_pricing']:
                    if hasattr(calculator, method_name):
                        result = getattr(calculator, method_name)(model, input_tokens, output_tokens)
                        if result:
                            total_cost = result if isinstance(result, (Decimal, float)) else result.get('total_cost', 0)
                            return {
                                "input_cost": Decimal(str(total_cost)) * Decimal("0.6"),  # Estimate
                                "output_cost": Decimal(str(total_cost)) * Decimal("0.4"),  # Estimate
            ...
            logger.warning(f"No pricing data for {provider}/{model}, using generic fallback")
    ...
    def get_cost_summary(
        ...
        for breakdown in filtered_breakdowns:
            ...
            summary.cost_by_structure_type[breakdown.structure_type] += breakdown.total_cost
            ...
            if breakdown.environment:
                ...
                summary.cost_by_environment[breakdown.environment] += breakdown.total_cost
    ...
    def get_monthly_costs(self, date: Optional[datetime] = None) -> Decimal:
        ...
        target_date = date or datetime.now()
        start_of_month = datetime.combine(target_date.replace(day=1).date(), datetime.min.time())
        ...
        summary = self.get_cost_summary(start_time=start_of_month, end_time=end_of_month)
        return summary.total_cost
    ...
    def clear_old_data(self, days_to_keep: int = 30) -> int:
        ...
        with self._lock:
            ...
            self.cost_breakdowns = [
                breakdown for breakdown in self.cost_breakdowns
                if breakdown.timestamp >= cutoff_date
            ]
            removed_count = original_count - len(self.cost_breakdowns)
        
        if removed_count > 0:
            logger.info(f"Cleared {removed_count} old cost records (>{days_to_keep} days)")
