...
class GriptapeRequest:
    ...
    def add_provider_cost(self, provider: str, model: str, cost: Union[Decimal, float]) -> None:
...
class GenOpsGriptapeAdapter(BaseProvider):
    ...
    def _export_telemetry(self, request: GriptapeRequest) -> None:
        ...
        try:
            ...
            self.telemetry_exporter.export_span(
                ...
                status=Status(StatusCode.OK if request.status == "completed" else StatusCode.ERROR)
            )
            
            logger.debug(f"Exported telemetry for request {request.request_id}")
            
        except Exception as e:
            logger.error(f"Failed to export telemetry for request {request.request_id}: {e}")
    ...
    def track_agent(self, agent_id: str, **kwargs):
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                if self.enable_performance_monitoring:
                    metrics = self.workflow_monitor.stop_structure_monitoring(request.request_id)
                    if metrics:
    ...
    def track_engine(self, engine_id: str, engine_type: str = "generic", **kwargs):
        ...
        request = self._create_request(STRUCTURE_ENGINE, engine_id, operation_type=engine_type, **kwargs)
    ...
    def track_memory(self, memory_id: str, operation_type: str = "access", **kwargs):
        ...
        request = self._create_request(STRUCTURE_MEMORY, memory_id, operation_type, **kwargs)
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                logger.debug(f"Starting Memory tracking: {memory_id} ({operation_type})")
                yield request
