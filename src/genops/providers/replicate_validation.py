...
class ReplicateValidator:
    ...
    def _validate_environment(self, result: ValidationResult) -> dict[str, Any]:
        ...
        if self.api_token:
            if self.api_token.startswith("r8_") and len(self.api_token) > 10:
                ...
                result.errors.append(
                    "   2. Token should start with 'r8_' and be ~40 characters"
                )
                result.errors.append(
                    "   3. export REPLICATE_API_TOKEN='r8_your_actual_token_here'"
                )
        else:
            ...
            result.errors.append(
                "   3. export REPLICATE_API_TOKEN='r8_your_token_here'"
            )
    ...
    def _validate_dependencies(self, result: ValidationResult):
        ...
        try:
            ...
            if version < "0.20.0":  # Adjust based on minimum supported version
                result.warnings.append(
                    f"Replicate SDK version {version} may be outdated"
                )
        ...
        try:
            ...
            result.warnings.append(
                "OpenTelemetry not available - telemetry will be disabled"
            )
    ...
    def _validate_authentication(self, result: ValidationResult):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result.errors.append(
                    "   1. Verify token: https://replicate.com/account/api-tokens"
                )
                result.errors.append("   2. Copy the correct token (starts with 'r8_')")
                result.errors.append(
                    "   3. export REPLICATE_API_TOKEN='r8_your_correct_token'"
                )
            ...
            result.errors.append("   1. Check internet connection")
            result.errors.append(
                "   2. Verify DNS resolution: nslookup api.replicate.com"
            )
    ...
    def _validate_api_connectivity(self, result: ValidationResult):
        ...
        try:
            ...
            if api_latency > 5000:  # 5 seconds
                ...
                result.warnings.append(
                    "   2. Consider using replicate.stream() for long operations"
                )
            ...
            result.errors.append(
                "   2. Check https://replicate.com/status for service status"
            )
            result.errors.append("   3. Try again in a few minutes")
    ...
    def _validate_model_availability(self, result: ValidationResult) -> dict[str, bool]:
        ...
        for model_name, category in test_models:
            ...
            if not test_result.available:
                result.warnings.append(
                    f"Model {model_name} not available: {test_result.error}"
                )

        # Store detailed results in performance metrics
        result.performance_metrics = result.performance_metrics or {}
        result.performance_metrics["model_tests"] = [
            asdict(t) for t in model_test_results
        ]
    ...
    def _test_model_availability(
        ...
        test_result = ModelTestResult(
            model_name=model_name, available=False, category=category
        )
    ...
    def _generate_recommendations(self, result: ValidationResult) -> list[str]:
        ...
        if result.performance_metrics:
            ...
            if api_latency > 2000:
                recommendations.append(
                    "High API latency detected - consider caching responses"
                )
                recommendations.append(
                    "Use replicate.stream() for long-running operations"
                )

            if api_latency < 500:
                recommendations.append(
                    "Good API performance - suitable for real-time applications"
                )

        # Environment recommendations
        if result.environment_info:
            if not result.environment_info.get("environment_variables", {}).get(
                "OTEL_EXPORTER_OTLP_ENDPOINT"
            ):
                recommendations.append(
            ...
            if not result.environment_info.get("environment_variables", {}).get(
                ...
                recommendations.append(
                    "Set GENOPS_ENVIRONMENT (dev/staging/prod) for proper attribution"
                )
        ...
        if result.model_availability:
            ...
            if available_models < total_models:
                recommendations.append(
                    "Some test models unavailable - check model names and access permissions"
                )
            else:
                recommendations.append(
                    "All test models available - ready for production use"
                )

        # Setup completion recommendations
        if result.success:
            recommendations.append(
                "✅ Setup validation passed - ready to use GenOps with Replicate!"
            )
            recommendations.append(
                "Next steps: Try the hello_genops_minimal.py example"
            )
        else:
            recommendations.append(
                "❌ Setup issues found - fix errors above before proceeding"
            )
...
def print_validation_result(result: ValidationResult, detailed: bool = False):
    ...
    if result.environment_info and detailed:
        ...
        print(
            f"   API Token: {'✅ Set' if env.get('replicate_token_set') else '❌ Missing'}"
        )
    ...
    if result.success:
        print("🎯 NEXT STEPS:")
        print(
            "   1. Try the examples: python examples/replicate/hello_genops_minimal.py"
        )
        ...
        print(
            '   2. Run validation again: python -c "from genops.providers.replicate_validation import validate_setup, print_validation_result; print_validation_result(validate_setup())"'
        )
