...
class DatabricksPricingConfig:
    ...
    sql_warehouse_pricing: Dict[str, float] = None
    
    # Compute cluster pricing (DBU per hour by type)
    compute_cluster_pricing: Dict[str, float] = None
...
class DatabricksUnityCatalogPricingCalculator:
    ...
    def __init__(self, pricing_config: Optional[DatabricksPricingConfig] = None):
        """
    ...
    def calculate_storage_cost(
        ...
        cost_usd = (
            data_size_gb * 
            self.config.storage_pricing_gb_month * 
            duration_months
        )
    ...
    def calculate_data_transfer_cost(
        ...
        logger.debug(
            f"Data transfer cost: {transfer_gb} GB, {transfer_type}, "
            f"${cost_usd:.6f}"
        )
...
_pricing_calculator: Optional[DatabricksUnityCatalogPricingCalculator] = None


def get_pricing_calculator(
    pricing_config: Optional[DatabricksPricingConfig] = None
