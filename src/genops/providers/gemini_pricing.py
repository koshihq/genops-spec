...
def calculate_gemini_cost(
    ...
    if context_cache_tokens and model_info.context_cache_price_per_1m_tokens:
        context_cache_cost = (
            context_cache_tokens / 1_000_000
        ) * model_info.context_cache_price_per_1m_tokens
...
def compare_gemini_models(
    ...
    comparisons.sort(key=lambda x: x[sort_by], reverse=reverse)  # type: ignore
...
def get_cost_optimization_recommendations(
    ...
    for alt_model in alternatives:
        ...
        if current_info:
            ...
            if (
                not alt_info.supports_code_execution
                and current_info.supports_code_execution
            ):
    ...
    recommendations.sort(key=lambda x: x["savings_percent"], reverse=True)  # type: ignore
...
def estimate_monthly_cost(
    ...
    """
    cost_per_operation = calculate_gemini_cost(
