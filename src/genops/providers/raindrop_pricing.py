...
class RaindropPricingConfig:
    ...
    dashboard_analytics_daily_cost: Decimal = field(
        default_factory=lambda: Decimal("0.10")
    )

    # Volume discount tiers (monthly interaction thresholds)
    volume_tiers: dict[int, Decimal] = field(
    ...
    signal_complexity_multipliers: dict[str, Decimal] = field(
        default_factory=lambda: {
    ...
    alert_complexity_multipliers: dict[str, Decimal] = field(
        default_factory=lambda: {
    ...
    search_complexity_multipliers: dict[str, Decimal] = field(
        default_factory=lambda: {
    ...
    currency_rates: dict[str, Decimal] = field(
        default_factory=lambda: {
            "USD": Decimal("1.00"),
            "EUR": Decimal("0.85"),
            "GBP": Decimal("0.75"),
...
class RaindropPricingCalculator:
    ...
    def calculate_alert_cost(
        ...
        daily_management_cost = (
            self.config.alert_management_daily_cost * complexity_multiplier
        )
    ...
    def calculate_experiment_cost(
        ...
        duration_multiplier = self._calculate_experiment_duration_multiplier(
            experiment_config
        )
        base_cost *= duration_multiplier

        # Apply complexity multiplier based on number of variants and metrics
        complexity_multiplier = self._calculate_experiment_complexity_multiplier(
            experiment_config
        )
    ...
    def _calculate_data_size_multiplier(
    ...
    def _calculate_signal_frequency_multiplier(
    ...
    def _calculate_notification_multiplier(
    ...
    def _calculate_search_scope_multiplier(
    ...
    def _calculate_experiment_duration_multiplier(
    ...
    def _calculate_experiment_complexity_multiplier(
        self, experiment_config: dict[str, Any]
    ) -> Decimal:
    ...
    def get_volume_discount_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "next_tier_discount_rate": float(next_tier_discount)
            if next_tier_discount
            else None,
            "next_tier_discount_percentage": float(next_tier_discount * 100)
            if next_tier_discount
            else None,
        }
