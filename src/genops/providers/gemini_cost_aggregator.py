...
class GeminiCostSummary:
    ...
    def get_average_cost_per_operation(self) -> float:
        ...
        return (
            self.total_cost / self.total_operations
...
class GeminiCostContext:
        ...
        if self.telemetry:
            ...
            self.span = self.telemetry.start_span(
                ...
                attributes={
                    ...
                    **{
                        f"genops.{k}": str(v)
                        for k, v in self.governance_attributes.items()
                    },
                },
            )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
            ...
            self.span.set_attributes(
                {
                    ...
                    "genops.context.duration_ms": (self.end_time - self.start_time)
                    * 1000,
                }
            )
        ...
        logger.info(
            f"Finalized Gemini cost context {self.context_id}: ${summary.total_cost:.6f}"
        )
    ...
    def add_operation(
        ...
        logger.debug(
            f"Added Gemini operation {operation_id}: {model_id}, ${cost_usd:.6f}"
        )
    ...
    def get_current_summary(self) -> GeminiCostSummary:
        ...
        for op in self.operations:
            cost_by_model[op.model_id] = (
                cost_by_model.get(op.model_id, 0.0) + op.cost_usd
            )
    ...
    def _check_budget_alerts(self) -> None:
        ...
        for threshold, alert_level, message in thresholds:
            if (
                ...
                and alert_level not in self.budget_warnings_sent
            ):
    ...
    def _create_budget_alert(
        self, level: CostAlertLevel, message: str, utilization: float
    ) -> None:
        ...
        log_level = (
            logging.WARNING
            if level in [CostAlertLevel.CRITICAL, CostAlertLevel.BUDGET_EXCEEDED]
            else logging.INFO
        )
        logger.log(
            log_level,
            f"Budget alert [{level.value}]: {message} (${self.total_cost:.4f}/${self.budget_limit:.4f})",
        )

    def _generate_optimization_recommendations(
        self, summary: GeminiCostSummary
    ) -> None:
        ...
        if (
            "gemini-2.5-pro" in summary.cost_by_model
            and summary.cost_by_model["gemini-2.5-pro"] > 0
        ):
    ...
    def get_model_performance_comparison(self) -> dict[str, dict[str, float]]:
        ...
        for model in self.get_current_summary().unique_models:
            ...
            if model_operations:
                ...
                total_tokens = sum(
                    op.input_tokens + op.output_tokens for op in model_operations
                )
                avg_latency = sum(op.latency_ms for op in model_operations) / len(
                    model_operations
                )
                cost_per_1k_tokens = (
                ...
                model_stats[model] = {
                    ...
                    "cost_efficiency_score": cost_per_1k_tokens
                    / avg_latency
