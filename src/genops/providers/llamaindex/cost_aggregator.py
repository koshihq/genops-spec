...
class LlamaIndexCostAggregator:
    ...
    def _calculate_operation_cost(self, operation: Dict[str, Any]) -> float:
        ...
        if operation_type == 'embed':
            ...
            if provider in self.provider_pricing and model in self.provider_pricing[provider]:
                embedding_rate = self.provider_pricing[provider][model].get('embedding', self.embedding_cost_per_1k)
    ...
    def get_current_summary(self) -> LlamaIndexCostSummary:
        ...
        breakdown = RAGCostBreakdown(
            ...
            synthesis_cost=self._cost_by_operation.get('llm_call', 0.0) + self._cost_by_operation.get('synthesize', 0.0),
    ...
    def enforce_budget_constraints(self, operation_cost: float, customer_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        enforcement_result = {
            ...
            "budget_status": {
                "daily_remaining": self.budget_limit - self.daily_cost if self.budget_limit else float('inf'),
    ...
    def implement_cost_circuit_breaker(self, cost_threshold: float, time_window_seconds: int = 3600) -> Dict[str, Any]:
