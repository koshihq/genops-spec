...
from .adapter import GenOpsLlamaIndexAdapter  # noqa: E402
from .cost_aggregator import LlamaIndexCostAggregator, set_cost_aggregator  # noqa: E402
from .rag_monitor import LlamaIndexRAGInstrumentor, set_rag_monitor  # noqa: E402
    ...
    def register(
        ...
        **governance_defaults,
        ...
        if not HAS_LLAMAINDEX:
            logger.warning(
                "Cannot register LlamaIndex instrumentation - LlamaIndex not available"
            )
            return False
        ...
        try:
            ...
            if enable_cost_tracking:
                self.cost_aggregator = LlamaIndexCostAggregator(
                    context_name="global_llamaindex", **governance_defaults
                )
                set_cost_aggregator(self.cost_aggregator)

                # Connect cost aggregator to adapter
                self.adapter.cost_aggregator = self.cost_aggregator  # type: ignore[assignment]
    ...
    def unregister(self) -> bool:
        ...
        try:
            ...
            if "callback_manager" in self._original_settings:
                ...
                if (
                    self.adapter
                    and self.adapter.callback_handler
                    in Settings.callback_manager.handlers
                ):
                    Settings.callback_manager.handlers.remove(
                        self.adapter.callback_handler
                    )

            # Clear global references
            set_cost_aggregator(None)  # type: ignore[arg-type]
            set_rag_monitor(None)  # type: ignore[arg-type]
...
def auto_register() -> None:
    ...
    try:
        ...
        if os.getenv("GENOPS_LLAMAINDEX_AUTO_REGISTER", "true").lower() in (
            "true",
            "1",
            "yes",
        ):
...
def validate_llamaindex_setup() -> dict[str, Any]:
    ...
    if not HAS_LLAMAINDEX:
        ...
        results["recommendations"].append(
            "Install LlamaIndex: pip install llama-index>=0.10.0"
        )
    else:
        ...
        if not status["registered"]:
            results["recommendations"].append(
                "Register LlamaIndex provider: register_llamaindex_provider()"
            )

        if not status["components"]["cost_aggregator"]:
            results["recommendations"].append(
                "Enable cost tracking for comprehensive governance"
            )

        if not status["components"]["rag_monitor"]:
            results["recommendations"].append(
                "Enable RAG monitoring for pipeline optimization"
            )
...
from .adapter import auto_instrument, instrument_llamaindex  # noqa: E402
