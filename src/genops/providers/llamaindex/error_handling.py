        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    logger.warning(f"Retry attempt {attempt + 1} after {delay:.2f}s delay: {str(e)}")
                    time.sleep(delay)

        # All retries exhausted
        raise RetryExhaustedError(f"Failed after {self.config.max_retries + 1} attempts") from last_exception
...
class ProviderHealthMonitor:
    ...
    def call_with_protection(self, provider: str, func: Callable[..., T], *args, **kwargs) -> T:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._get_health_recommendations(healthy_providers, all_providers)
        }

    def _get_health_recommendations(self, healthy: list[str], all_providers: list[str]) -> list[str]:
        """Generate health recommendations."""
        recommendations = []

        if len(healthy) == 0:
            recommendations.append("CRITICAL: All providers unavailable - check network and API keys")
        elif len(healthy) == 1:
            recommendations.append(f"WARNING: Only {healthy[0]} available - single point of failure")
        elif len(healthy) < len(all_providers):
            failed = set(all_providers) - set(healthy)
            recommendations.append(f"INFO: {len(failed)} provider(s) degraded: {', '.join(failed)}")
...
@contextmanager
def graceful_degradation(primary_provider: str, fallback_providers: list[str], health_monitor: ProviderHealthMonitor):
