...
try:
    ...
    from llama_index.core.callbacks import CallbackManager  # noqa: F401
...
class ValidationResult:
    ...
    success: bool
    issues: list[ValidationIssue] = None  # type: ignore
    environment_info: Optional[dict[str, Any]] = None
    performance_metrics: Optional[dict[str, Any]] = None
    component_status: Optional[dict[str, Any]] = None
    optimization_recommendations: list[str] = None  # type: ignore
...
class LlamaIndexValidator:
    ...
    def _validate_environment(self, result: ValidationResult) -> dict[str, Any]:
        ...
        if HAS_LLAMAINDEX:
            try:
                env_info["llamaindex_version"] = getattr(
                    llama_index, "__version__", "unknown"
                )
        ...
        for var in env_vars_to_check:
            value = os.getenv(var)
            env_info["environment_variables"][var] = bool(
                value
            )  # Don't store actual values
            if not value and var in [
                "OPENAI_API_KEY",
                "ANTHROPIC_API_KEY",
                "GOOGLE_API_KEY",
            ]:
        ...
        try:
            ...
            env_info["system_resources"]["note"] = (
    ...
    def _validate_dependencies(self, result: ValidationResult):
        ...
        try:
            ...
            from opentelemetry.sdk import trace as trace_sdk  # noqa: F401
        except ImportError:
            result.issues.append(
                ValidationIssue(
                    severity="warning",
    ...
    def _run_performance_benchmarks(self, result: ValidationResult) -> dict[str, Any]:
        ...
        try:
            ...
            LlamaIndexCostAggregator("test_context")
            LlamaIndexRAGInstrumentor()
            metrics["component_creation_time_ms"] = (
    ...
    def _generate_recommendations(self, result: ValidationResult) -> list[str]:
        ...
        if result.environment_info:
            ...
            if "system_resources" in env and "memory_gb" in env["system_resources"]:
                ...
                if memory_gb >= 16:
                    recommendations.append(
                        "Excellent system resources - suitable for complex RAG pipelines"
                    )
                elif memory_gb >= 8:
                    recommendations.append(
                        "Good system resources - suitable for most RAG applications"
                    )
            ...
            if available_providers == 0:
                recommendations.append(
                    "Set up API keys for LLM providers to enable RAG functionality"
                )
            elif available_providers == 1:
                recommendations.append(
                    "Consider setting up multiple LLM providers for cost optimization"
                )
            else:
                recommendations.append(
                    "Multiple LLM providers available - great for cost optimization"
                )
        ...
        if result.component_status:
            ...
            if status.get("adapter_available") and status.get(
                "cost_aggregator_available"
            ):
                recommendations.append(
                    "All GenOps components available - ready for production RAG monitoring"
                )

            reg_status = status.get("registration_status", {})
            if reg_status.get("registered", False):
                recommendations.append(
                    "GenOps integration active - RAG operations will be automatically tracked"
                )
            else:
                recommendations.append(
                    "Register GenOps provider for automatic RAG pipeline monitoring"
                )
        ...
        if result.performance_metrics:
            ...
            if perf.get("system_ready", False):
                if perf.get("component_creation_time_ms", 0) < 100:
                    recommendations.append(
                        "Fast component initialization - optimal for high-frequency RAG operations"
                    )
                else:
                    recommendations.append(
                        "Consider component reuse patterns for better performance"
                    )
        ...
        if error_count == 0 and warning_count == 0:
            recommendations.append(
                "Perfect setup - ready for production LlamaIndex RAG workflows"
            )
        elif error_count == 0:
            recommendations.append("Setup complete with minor optimizations available")
        else:
            recommendations.append(
                "Address error issues before proceeding to production"
            )
...
def print_validation_result(result: ValidationResult, detailed: bool = False):
    ...
    if result.component_status and detailed:
        ...
        print(
            f"   {status_symbols.get(components.get('cost_aggregator_available'))} Cost Aggregator"
        )
        print(
            f"   {status_symbols.get(components.get('rag_monitor_available'))} RAG Monitor"
        )
    ...
    if result.environment_info and detailed:
        ...
        print(
            f"   LlamaIndex: {'✅' if env.get('llamaindex_available') else '❌'} {env.get('llamaindex_version', 'N/A')}"
        )
    ...
    if result.success:
        print("🎯 NEXT STEPS:")
        print(
            "   1. Try the examples: python examples/llamaindex/hello_genops_minimal.py"
        )
        ...
        print(
            '   2. Run validation again: python -c "from genops.providers.llamaindex.validation import validate_setup, print_validation_result; print_validation_result(validate_setup())"'
        )
