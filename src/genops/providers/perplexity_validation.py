...
class PerplexitySetupValidator:
    ...
    def _validate_authentication(self, perplexity_api_key: str | None) -> None:
        ...
        if not api_key:
            self.issues.append(
                ValidationIssue(
                    ...
                    documentation_links=["https://docs.perplexity.ai/getting-started"],
                )
            )
            return
        ...
        if not api_key.startswith("pplx-"):
            self.issues.append(
                ValidationIssue(
                    ...
                    affected_functionality=["API authentication may fail"],
                )
            )
        else:
            self.issues.append(
    ...
    def _validate_governance_configuration(
        self, team: str | None, project: str | None
    ) -> None:
    ...
    def _validate_connectivity_and_models(self, perplexity_api_key: str | None) -> None:
            ...
            if (
                ...
                self.issues.append(
                    ValidationIssue(
                        ...
                        affected_functionality=["All Perplexity API operations"],
                ...
                self.issues.append(
                    ValidationIssue(
                        ...
                        affected_functionality=["High-volume API operations"],
                        technical_details=str(e),
                    )
                )
                ...
                self.issues.append(
                    ValidationIssue(
                        ...
                        affected_functionality=["All network-dependent operations"],
                        technical_details=str(e),
                    )
                )
                ...
                self.issues.append(
                    ValidationIssue(
                        ...
                        affected_functionality=["Perplexity API operations"],
                        technical_details=str(e),
                    )
                )

    def _validate_search_features(self, perplexity_api_key: str | None) -> None:
    ...
    def print_validation_result(
        ...
        print(
            f"   ℹ️ Info: {len([i for i in result.issues if i.level == ValidationLevel.INFO])}"
        )
                ...
                for issue in issues:
                    ...
                    if issue.level in [ValidationLevel.ERROR, ValidationLevel.WARNING]:
                        ...
                        if issue.affected_functionality:
                            print(
                                f"      Affects: {', '.join(issue.affected_functionality[:2])}"
                            )
...
def validate_setup(
    ...
    **kwargs,
    ...
    return validator.validate_complete_setup(
        perplexity_api_key=perplexity_api_key, team=team, project=project, **kwargs
    )
...
def interactive_setup_wizard() -> dict[str, Any]:
    ...
    api_key = input(
        "Enter your Perplexity API key (or press Enter to use PERPLEXITY_API_KEY env var): "
    ).strip()
    if api_key:
    ...
    try:
        ...
        config["daily_budget_limit"] = float(daily_budget) if daily_budget else 100.0  # type: ignore[assignment]
    except ValueError:
        config["daily_budget_limit"] = 100.0  # type: ignore[assignment]

    # Governance Policy
    print("\n🛡️ Step 5: Governance Policy")
    print(
        "Policies: advisory (warnings), enforced (blocks on budget), strict (maximum control)"
    )
