...
class OptimizationInsight:
    ...
    category: (
        str  # model_selection, token_efficiency, usage_patterns, european_advantages
    )
...
class MistralCostAggregator:
    ...
    def record_operation(
        ...
        **governance_attrs,
        ...
        record = OperationRecord(
            ...
            request_time=performance_metrics.get("request_time", 0.0)
    ...
    def _update_real_time_aggregations(self, record: OperationRecord):
        ...
        if record.tokens_per_second > 0:
            self.token_efficiency_history.append(
                (record.timestamp, record.tokens_per_second)
            )

        if record.cost_per_token > 0:
            self.cost_efficiency_history.append(
                (record.timestamp, record.cost_per_token)
            )
    ...
    def _check_alerts(self, record: OperationRecord):
        ...
        if (
            record.total_cost > self.cost_alert_threshold / 100
        ):  # Per operation threshold
            logger.warning(
                f"High-cost operation detected: ${record.total_cost:.6f} for {record.model}"
            )

        # Session cost alerts
        if self.current_session_cost > self.cost_alert_threshold:
            logger.warning(
                f"Session cost threshold exceeded: ${self.current_session_cost:.2f}"
            )
        ...
        if record.team and record.team in self.team_budgets:
            ...
            if team_cost > team_budget * 0.8:  # 80% of budget
                logger.warning(
                    f"Team {record.team} approaching budget: ${team_cost:.2f}/${team_budget:.2f}"
                )

        # Efficiency alerts
        if record.efficiency_score < self.efficiency_alert_threshold:
            logger.warning(
                f"Low efficiency operation: {record.efficiency_score:.2f} for {record.model}"
            )
    ...
    def get_cost_summary(
        ...
        filtered_ops = [
            ...
            and (not team or op.team == team)
            and (not project or op.project == project)
            and (not customer_id or op.customer_id == customer_id)
        ]
        ...
        avg_tokens_per_sec = sum(op.tokens_per_second for op in filtered_ops) / max(
            total_operations, 1
        )
    ...
    def _calculate_cost_trend(
        self, operations: list[OperationRecord], time_window: TimeWindow
        ...
        for bucket in sorted(cost_by_bucket.keys()):
            ...
            time_str = datetime.fromtimestamp(timestamp).strftime(
                "%H:%M" if time_window == TimeWindow.HOUR else "%m-%d"
            )
    ...
    def get_cost_optimization_insights(
        ...
        insights.extend(
            self._analyze_model_optimization(recent_ops, min_savings_threshold)
        )

        # Analyze token efficiency
        insights.extend(
            self._analyze_token_efficiency(recent_ops, min_savings_threshold)
        )
        ...
        # European AI advantages
        insights.extend(
            self._analyze_european_advantages(recent_ops, min_savings_threshold)
        )
    ...
    def _analyze_model_optimization(
        self, operations: list[OperationRecord], min_savings: float
    ) -> list[OptimizationInsight]:
                ...
                if potential_savings >= min_savings:
                    insights.append(
                        OptimizationInsight(
                            category="model_selection",
                            priority="medium",
                            insight="Consider mistral-small-latest for simple queries",
                            potential_savings_usd=potential_savings,
    ...
    def _analyze_token_efficiency(
        ...
        high_output_ops = [
            op for op in operations if op.output_tokens > op.input_tokens * 2
        ]

        if len(high_output_ops) > len(operations) * 0.2:  # >20% of operations
            output_cost_waste = sum(
                op.output_cost * 0.3 for op in high_output_ops
            )  # 30% potential reduction
                ...
                insights.append(
                    OptimizationInsight(
                        ...
                        supporting_metrics={
                            ...
                            "avg_output_input_ratio": sum(
                                op.output_tokens / max(op.input_tokens, 1)
                                for op in high_output_ops
                            )
                            / len(high_output_ops),
                        },
                    )
    ...
    def _analyze_usage_patterns(
        ...
        if usage_by_hour:
            peak_hours = [
                hour
                for hour, count in usage_by_hour.items()
            ...
            if len(peak_hours) < 8:  # Concentrated usage
                ...
                batch_savings = (
                    sum(op.total_cost for op in operations) * 0.15
                )  # 15% batch discount
                    ...
                    insights.append(
                        OptimizationInsight(
                            ...
                            supporting_metrics={
                                ...
                                "concentration_ratio": len(peak_hours) / 24,
                            },
                        )
                    )
            ...
            if total_cost > 50.0:  # For significant workloads
                ...
                insights.append(
                    OptimizationInsight(
                        ...
                        implementation_effort="medium",
                        supporting_metrics={
                            "cost_competitiveness": "vs_us_providers",
                            "market_position": "european_ai",
                        },
                    )
                )
    ...
    def get_budget_status(self) -> dict[str, Any]:
        ...
        status = {"teams": {}, "projects": {}, "customers": {}}
...
if __name__ == "__main__":
    ...
    for _i in range(10):
        ...
        cost_breakdown["total_cost"] = (
            cost_breakdown["input_cost"] + cost_breakdown["output_cost"]
        )
        cost_breakdown["cost_per_token"] = (
            cost_breakdown["total_cost"] / cost_breakdown["total_tokens"]
        )
