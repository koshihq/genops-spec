...
class GenOpsOllamaAdapter(BaseFrameworkProvider):
    ...
    def _test_connection(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully connected to Ollama server at {self.ollama_base_url}"
            )
        except Exception as e:
                ...
                try:
                    if self.client:
                        ...
                        response = requests.get(
                            f"{self.ollama_base_url}/api/tags", timeout=10
                        )
                        response.raise_for_status()
                    ...
                    if self.cost_tracking_enabled:
                        operation.infrastructure_cost = self._calculate_operation_cost(
                            operation
                        )
    ...
    def generate(
        ...
        governance_attrs = {
            ...
            if k.startswith(("team", "project", "customer", "environment"))
        }
        generation_kwargs = {
            k: v
            for k, v in kwargs.items()
            if not k.startswith(("team", "project", "customer", "environment"))
        }
            ...
            with tracer.start_as_current_span("ollama.generate") as span:
                ...
                try:
                    ...
                    if self.cost_tracking_enabled:
                        ...
                        operation.gpu_hours = operation.duration_ms / (
                            1000 * 3600
                        )  # Convert to hours
                    ...
                    span.set_attributes(
                        {
                            ...
                            "genops.infrastructure_cost": operation.infrastructure_cost
                            or 0.0,
                        }
                    )
                    ...
                    if operation.response:
                        span.set_attribute(
                            "genops.response_length", len(operation.response)
                        )
                    ...
                    logger.info(
                        f"Generated text with model {model}: {operation.inference_time_ms:.0f}ms"
                    )
                    return response
                ...
                try:
                    ...
                    if self.client:
                        ...
                        response = self.client.chat(
                            model=model, messages=messages, stream=stream, **chat_kwargs
                        )
                    else:
                        # Use HTTP API
                        payload = {
                    ...
                    logger.info(
                        f"Chat with model {model}: {operation.inference_time_ms:.0f}ms"
                    )
                    return response
    ...
    def _update_model_metrics(self, model: str, operation: OllamaOperation):
        ...
        if model not in self.model_metrics:
            self.model_metrics[model] = LocalModelMetrics(
                model_name=model, total_operations=0, total_inference_time_ms=0.0
            )
        ...
        if operation.inference_time_ms:
            ...
            metrics.avg_inference_latency_ms = (
                metrics.total_inference_time_ms / metrics.total_operations
            )
        ...
        if operation.infrastructure_cost:
            ...
            metrics.cost_per_operation = (
                metrics.total_infrastructure_cost / metrics.total_operations
            )
        ...
        if operation.gpu_hours:
            ...
            if metrics.gpu_hours_consumed > 0:
                ...
                if metrics.total_infrastructure_cost > 0:
                    metrics.operations_per_dollar = (
                        metrics.total_operations / metrics.total_infrastructure_cost
                    )

    def get_model_metrics(
        self, model: str | None = None
    ) -> LocalModelMetrics | dict[str, LocalModelMetrics]:
        """
        ...
        """
        if model:
            return self.model_metrics.get(model)  # type: ignore
    ...
    def get_operation_summary(self) -> dict[str, Any]:
        ...
        total_inference_time = sum(
            op.inference_time_ms or 0.0 for op in self.operations
        )
        models_used = list({op.model for op in self.operations})
        ...
        return {
            ...
            "avg_cost_per_operation": total_cost / len(self.operations)
            if self.operations
            else 0.0,
            "models_used": models_used,
            "unique_models_count": len(models_used),
            "avg_inference_time_ms": total_inference_time / len(self.operations)
            if self.operations
            else 0.0,
...
# Module-level storage for original functions so uninstrument() can restore them
_orig_generate: Callable | None = None
_orig_chat: Callable | None = None


def auto_instrument(**kwargs) -> bool:
    """
    Auto-instrument the Ollama client for governance tracking.

    Patches ``ollama.generate`` and ``ollama.chat`` so that every call
    is wrapped with GenOps telemetry.

    Returns:
        True if instrumentation was applied, False otherwise.
    """
    global _orig_generate, _orig_chat
    ...
    adapter = GenOpsOllamaAdapter(**kwargs)

    # Save originals at module level so uninstrument() can restore them
    _orig_generate = _ollama_client.generate
    _orig_chat = _ollama_client.chat

    def _wrapped_generate(*args, **kw):
        model = kw.get("model") or (args[0] if args else "unknown")
        prompt = kw.get("prompt") or (args[1] if len(args) > 1 else "")
        # Pass all remaining kwargs through to preserve stream, temperature, etc.
        extra_kw = {k: v for k, v in kw.items() if k not in ("model", "prompt")}
        return adapter.generate(model, prompt, **extra_kw)

    def _wrapped_chat(*args, **kw):
        model = kw.get("model") or (args[0] if args else "unknown")
        messages = kw.get("messages") or (args[1] if len(args) > 1 else [])
        # Pass all remaining kwargs through to preserve stream, temperature, etc.
        extra_kw = {k: v for k, v in kw.items() if k not in ("model", "messages")}
        return adapter.chat(model, messages, **extra_kw)

    _ollama_client.generate = _wrapped_generate
    _ollama_client.chat = _wrapped_chat

    return True


def uninstrument() -> bool:
    """Remove auto-instrumentation from the Ollama client."""
    global _orig_generate, _orig_chat

    if not HAS_OLLAMA_CLIENT:
        return False

    if _orig_generate is None and _orig_chat is None:
        return False

    import ollama as _ollama_client

    if _orig_generate is not None:
        _ollama_client.generate = _orig_generate
        _orig_generate = None
    if _orig_chat is not None:
        _ollama_client.chat = _orig_chat
        _orig_chat = None
