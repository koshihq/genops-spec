...
class ModelInfo:
    ...
    def _categorize_model(self):
        ...
        if any(keyword in name_lower for keyword in ["code", "codellama", "starcoder"]):
            ...
            self.model_type = (
                ModelType.INSTRUCT if "instruct" in name_lower else ModelType.CHAT
            )
        elif "embed" in name_lower:
            self.model_type = ModelType.EMBEDDING
        elif any(
            keyword in name_lower for keyword in ["vision", "multimodal", "llava"]
        ):
    ...
    def update_performance_stats(
        ...
        if inference_time_ms > 0:
            self.avg_inference_latency_ms = (
                self.avg_inference_latency_ms * (self.total_inferences - 1)
                + inference_time_ms
        ...
        if tokens > 0 and inference_time_ms > 0:
            ...
            self.avg_tokens_per_second = (
                self.avg_tokens_per_second * (self.total_inferences - 1)
                + tokens_per_second
            ) / self.total_inferences
    ...
    def mark_error(self):
        ...
        self.error_count += 1
        self.success_rate = (
            (self.total_inferences - self.error_count) / max(self.total_inferences, 1)
        ) * 100
...
class ModelOptimizer:
    ...
    def add_recommendation(
        self, category: str, description: str, impact: str = "medium"
    ):
        """Add an optimization recommendation."""
        self.optimization_opportunities.append(
            f"[{impact.upper()}] {category}: {description}"
        )
...
class ModelComparison:
    ...
    def add_metric(self, metric_name: str, model_values: dict[str, float]):
        ...
        if metric_name == "cost_per_inference":
            ...
            self.best_for_speed = max(
                model_values.keys(), key=lambda m: model_values[m]
            )
        elif metric_name == "success_rate":
            self.best_for_quality = max(
                model_values.keys(), key=lambda m: model_values[m]
            )
...
class OllamaModelManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Ollama model manager (optimization: {enable_auto_optimization})"
        )
    ...
    def get_model_performance_summary(self, model_name: str = None) -> dict[str, Any]:  # type: ignore[assignment]
        """
    ...
    def compare_models(
        ...
        if comparison.best_for_cost:
            comparison.recommendations["cost"] = (
                f"Use {comparison.best_for_cost} for lowest cost per inference"
            )
        if comparison.best_for_speed:
            comparison.recommendations["speed"] = (
                f"Use {comparison.best_for_speed} for highest throughput"
            )
        if comparison.best_for_quality:
            comparison.recommendations["quality"] = (
                f"Use {comparison.best_for_quality} for highest success rate"
            )
    ...
    def _suggest_model_alternatives(self, optimizer: ModelOptimizer, model: ModelInfo):
        ...
        if model.size_category in [ModelSize.LARGE, ModelSize.XLARGE]:
            if model.cost_per_inference > 0.005:
                optimizer.suggest_alternative(
                    "llama3.2:3b", "smaller model with good performance for most tasks"
                )
        ...
        if model.avg_inference_latency_ms > 3000:
            if "llama" in model_name_lower:
                optimizer.suggest_alternative(
                    "llama3.2:1b", "fastest LLaMA variant for simple tasks"
                )
    ...
    def export_model_data(self, format: str = "json") -> str:
        ...
        if format.lower() == "json":
            export_data = {"export_timestamp": time.time(), "models": {}}
