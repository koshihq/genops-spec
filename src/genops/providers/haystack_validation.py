...
def validate_haystack_installation() -> tuple[
    bool, list[ValidationIssue], Optional[str]
]:
    ...
    try:
        ...
        try:
            ...
            from genops.providers.haystack_cost_aggregator import (
                HaystackCostAggregator,  # noqa: F401
            )
            from genops.providers.haystack_monitor import HaystackMonitor  # noqa: F401
...
def validate_ai_providers() -> tuple[dict[str, dict[str, Any]], list[ValidationIssue]]:
    ...
    available_providers = [
        name for name, info in provider_status.items() if info["status"] == "available"
    ]
...
def benchmark_performance() -> tuple[dict[str, float], list[ValidationIssue]]:
    ...
    start_time = time.perf_counter()
    try:
        GenOpsHaystackAdapter(team="test", project="benchmark")
...
def validate_haystack_setup() -> ValidationResult:
    ...
    if result.get_error_count() == 0 and result.get_warning_count() == 0:
        result.recommendations.append(
            "Setup is optimal! You're ready to build with Haystack + GenOps"
        )
    elif result.get_error_count() == 0:
        result.recommendations.append(
            "Setup is functional with minor optimizations available"
        )
...
def print_validation_result(result: ValidationResult) -> None:
    ...
    if result.is_valid:
        print("\n🚀 You're ready! Try:")
        print("   from genops.providers.haystack import auto_instrument")
        print("   auto_instrument()")
        print("\n📚 Next steps:")
        print("   • Examples: python examples/haystack/basic_pipeline_tracking.py")
        print("   • Docs: docs/integrations/haystack.md")
    else:
        print("\n🔧 Quick Fixes:")
        print("   Interactive setup: ./validate --fix-issues")
        print("   Or manually:")
        print("     1. Fix the errors listed above")
        print("     2. Re-run: python scripts/validate_setup.py")
        print("     3. Provider setup: ./validate --provider openai")
        print("\n📚 Help:")
        print("   • Documentation: docs/integrations/haystack.md#troubleshooting")
        print("   • Examples: examples/haystack/README.md")
