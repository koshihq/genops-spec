...
class CostMetrics:
    ...
    def update(
        ...
        if latency_ms > 0:
            total_latency = (
                self.avg_latency_ms * (self.total_operations - 1) + latency_ms
            )
...
class CohereCostAggregator:
    ...
    def record_operation(
        ...
        **governance_attrs,
        ...
        self.costs_by_operation[operation_type].update(
            cost_breakdown, latency_ms, timestamp
        )
        ...
        if customer_id != "unknown":
            self.costs_by_customer[customer_id].update(
                cost_breakdown, latency_ms, timestamp
            )
        if environment != "unknown":
            self.costs_by_environment[environment].update(
                cost_breakdown, latency_ms, timestamp
            )
        ...
        if (
            ...
            and self.total_metrics.total_cost >= self.cost_alert_threshold
        ):
            logger.warning(
                f"Cost alert: Total cost ${self.total_metrics.total_cost:.6f} exceeded threshold ${self.cost_alert_threshold:.6f}"
            )
    ...
    def get_cost_summary(
        self, time_window: TimeWindow = TimeWindow.ALL_TIME
    ) -> dict[str, Any]:
        """
        ...
        summary = {
            ...
            "usage_metrics": {
                ...
                "avg_tokens_per_operation": round(
                    base_metrics.avg_tokens_per_operation, 1
                ),
            ...
            "budget_tracking": {
                ...
                "period_utilization": round(
                    (self.current_period_cost / self.cost_alert_threshold * 100)
    ...
    def get_operation_summary(
        ...
        for op in operations:
            ...
            summary.costs_by_type[op_type] = (
                summary.costs_by_type.get(op_type, 0.0) + cost
            )

            # Model breakdown
            summary.operations_by_model[model] = (
                summary.operations_by_model.get(model, 0) + 1
            )
            summary.costs_by_model[model] = (
                summary.costs_by_model.get(model, 0.0) + cost
            )
            ...
            if team != "unknown":
                summary.costs_by_team[team] = (
                    summary.costs_by_team.get(team, 0.0) + cost
                )
            if project != "unknown":
                summary.costs_by_project[project] = (
                    summary.costs_by_project.get(project, 0.0) + cost
                )
            if customer_id != "unknown":
                summary.costs_by_customer[customer_id] = (
                    summary.costs_by_customer.get(customer_id, 0.0) + cost
                )
            if environment != "unknown":
                summary.costs_by_environment[environment] = (
                    summary.costs_by_environment.get(environment, 0.0) + cost
                )

        # Generate top lists
        summary.top_models_by_cost = sorted(
            summary.costs_by_model.items(), key=lambda x: x[1], reverse=True
        )[:10]
        summary.top_teams_by_cost = sorted(
            summary.costs_by_team.items(), key=lambda x: x[1], reverse=True
        )[:10]
        summary.top_operations_by_cost = sorted(
            summary.costs_by_type.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def get_cost_optimization_insights(self) -> dict[str, Any]:
        ...
        if not self.calculator:
            insights["recommendations"].append(
                "⚠️ Cost calculator unavailable - install pricing module for optimization insights"
            )
            return insights
        ...
        if len(model_efficiency) > 1:
            ...
            sorted_by_efficiency = sorted(
                model_efficiency.items(), key=lambda x: x[1]["cost_per_operation"]
            )
            ...
            efficiency_diff = (
                least_efficient[1]["cost_per_operation"]
                / most_efficient[1]["cost_per_operation"]
            )
        ...
        for op in self.operation_history:
            if op["timestamp"] > cutoff_time:
                filtered_metrics.update(
                    op["cost_breakdown"], op["latency_ms"], op["timestamp"]
                )
    ...
    def export_cost_data(self, format: str = "dict") -> Any:
        ...
        if format == "json":
            ...
            for op in self.operation_history:
                lines.append(
                    f"{op['timestamp']},{op['model']},{op['operation_type']},{op['cost_breakdown'].total_cost},{op.get('team', '')},{op.get('project', '')}"
                )
            return "\n".join(lines)
...
class ModelSummary:
    ...
    total_operations: int = 0
    avg_cost_per_operation: float = 0.0
    usage_teams: list[str] = field(default_factory=list)
...
class CostSummary:
    ...
    overview: OverviewSummary = field(default_factory=OverviewSummary)
    by_team: dict[str, TeamSummary] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "overview": {
                "total_cost": self.overview.total_cost,
                "total_operations": self.overview.total_operations,
                "avg_cost_per_operation": self.overview.avg_cost_per_operation,
            },
            "time_window": self.time_window.name,
        }


@dataclass
class OptimizationInsight:
    """A cost optimization insight."""
