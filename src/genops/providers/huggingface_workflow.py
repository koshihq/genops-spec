...
def production_workflow_context(
    workflow_name: str, customer_id: str, **kwargs
) -> tuple[Any, str]:
    """
    ...
    with create_huggingface_cost_context(workflow_id) as cost_context:
        with telemetry.trace_operation(
            operation_name=f"huggingface.workflow.{workflow_name}", **governance_attrs
        ) as span:
            ...
            logger.info(
                f"Started production workflow: {workflow_name} (ID: {workflow_id})"
            )
            ...
            try:
                ...
                if final_summary:
                    span.set_attribute(
                        "genops.workflow.total_cost", final_summary.total_cost
                    )
                    span.set_attribute(
                        "genops.workflow.providers_used",
                        len(final_summary.unique_providers),
                    )
                    span.set_attribute(
                        "genops.workflow.models_used", len(final_summary.unique_models)
                    )
                    span.set_attribute(
                        "genops.workflow.total_tokens_input",
                        final_summary.total_tokens_input,
                    )
                    span.set_attribute(
                ...
                logger.info(
                    f"Completed production workflow: {workflow_name} (ID: {workflow_id})"
                )
...
class ProductionWorkflowSpan:
    ...
    def record_step(self, step_name: str, metadata: dict[str, Any] = None) -> None:  # type: ignore[assignment]
        """Record a workflow step with optional metadata."""
        self._step_counter += 1

        # Record step in span
        self.span.set_attribute(
            f"genops.workflow.step.{self._step_counter}.name", step_name
        )
        self.span.set_attribute(
            f"genops.workflow.step.{self._step_counter}.timestamp", time.time()
        )
        ...
        if metadata:
            for key, value in metadata.items():
                if isinstance(value, (str, int, float, bool)):
                    self.span.set_attribute(
                        f"genops.workflow.step.{self._step_counter}.{key}", value
                    )

        logger.debug(
            f"Workflow {self.workflow_name} step {self._step_counter}: {step_name}"
        )
    ...
    def record_checkpoint(
        ...
        self.span.set_attribute(
            f"genops.workflow.checkpoint.{len(self._checkpoints)}.name", checkpoint_name
        )
        self.span.set_attribute(
            f"genops.workflow.checkpoint.{len(self._checkpoints)}.id", checkpoint_id
        )

        logger.info(
            f"Workflow {self.workflow_name} checkpoint: {checkpoint_name} (ID: {checkpoint_id})"
        )
    ...
    def record_hf_operation(
        ...
        **metadata,
        ...
        self.span.set_attribute(
            f"genops.workflow.operation.{operation_count}.name", operation_name
        )
        self.span.set_attribute(
            f"genops.workflow.operation.{operation_count}.provider", provider
        )
        self.span.set_attribute(
            f"genops.workflow.operation.{operation_count}.model", model
        )
        self.span.set_attribute(
            f"genops.workflow.operation.{operation_count}.task",
            task or "text-generation",
        )

        if call_cost:
            self.span.set_attribute(
                f"genops.workflow.operation.{operation_count}.cost", call_cost.cost
            )

        logger.debug(
            f"Workflow {self.workflow_name} operation {operation_count}: {operation_name} ({provider}/{model})"
        )

    def get_current_cost_summary(self) -> Any | None:
    ...
    def record_performance_metric(
        ...
        metric_name: str,
        value: float,
        unit: str = None,  # type: ignore[assignment]
    ) -> None:
        ...
        logger.debug(
            f"Workflow {self.workflow_name} metric: {metric_name} = {value} {unit or ''}"
        )
    ...
    def record_alert(
        ...
        self.span.set_attribute(
            f"genops.workflow.alert.{alert_count}.severity", severity
        )
        self.span.set_attribute(
            f"genops.workflow.alert.{alert_count}.timestamp", time.time()
        )

        if severity in ["warning", "error", "critical"]:
            logger.warning(
                f"Workflow {self.workflow_name} {severity}: {alert_type} - {message}"
            )
        else:
            logger.info(
                f"Workflow {self.workflow_name} alert: {alert_type} - {message}"
            )
    ...
    def get_workflow_metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "providers_used": list(current_summary.unique_providers)
            if current_summary
            else [],
            "models_used": list(current_summary.unique_models)
            if current_summary
            else [],
        }
