...
class HaystackPipelineContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        pipeline_result = HaystackPipelineResult(
            ...
            successful_components=len(
                [r for r in self.component_results if r.status == "success"]
            ),
            failed_components=len(
                [r for r in self.component_results if r.status == "error"]
            ),
        ...
        self.span.set_attribute(
            "genops.pipeline.components.successful",
            pipeline_result.successful_components,
        )
        self.span.set_attribute(
            "genops.pipeline.components.failed", pipeline_result.failed_components
        )
        self.span.set_attribute(
            "genops.pipeline.execution_time_seconds", total_execution_time
        )
    ...
    def add_component_result(self, result: HaystackComponentResult):
        ...
        with self.adapter.telemetry.tracer.start_as_current_span(
            ...
            component_span.set_attribute(
                "genops.execution_time_seconds", result.execution_time_seconds
            )
            ...
            if result.tokens_output:
                component_span.set_attribute(
                    "genops.tokens.output", result.tokens_output
                )
            ...
            if result.status == "error":
                component_span.set_status(
                    Status(StatusCode.ERROR, result.error_message or "Component failed")
                )
            else:
...
class GenOpsHaystackAdapter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"GenOps Haystack adapter initialized for team '{team}', project '{project}'"
        )
    ...
    def _lazy_init_components(self, team: str, project: str):
        ...
        try:
            ...
            self.cost_aggregator = HaystackCostAggregator()  # type: ignore[assignment]
            self.monitor = HaystackMonitor(team=team, project=project)  # type: ignore[assignment]
        ...
        try:
            ...
            with self.telemetry.tracer.start_as_current_span(
                ...
                session_span.set_attribute(
                    "genops.session.total_pipelines", session_context.total_pipelines
                )
                session_span.set_attribute(
                    "genops.session.total_cost", float(session_context.total_cost)
                )
    ...
    def estimate_component_cost(
        ...
        return total_cost, provider  # type: ignore[return-value]

    def track_component_execution(
        self, component_name: str, component_type: str, execution_func, *args, **kwargs
        ...
        for pipeline_result in self._pipeline_results.values():
            for provider, cost in pipeline_result.cost_by_provider.items():
                cost_by_provider[provider] = (
                    cost_by_provider.get(provider, Decimal("0")) + cost
                )
    ...
    def get_error_diagnostics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "error_types": dict(self.error_stats["error_types"]),  # type: ignore
    ...
    def get_recent_pipeline_results(
        self, limit: int = 10
    ) -> list[HaystackPipelineResult]:
        """Get the most recent pipeline execution results."""
        return sorted(
            self._pipeline_results.values(), key=lambda x: x.end_time, reverse=True
        )[:limit]
    ...
    def get_initialization_status(self) -> dict[str, Any]:
        ...
        if not status["initialized"] or not all(components.values()):
            status["summary"] = (
                "⚠️ Initialization incomplete - some features may not work properly"
            )
        else:
...
def auto_instrument():
    ...
    logger.info(
        "Haystack auto-instrumentation enabled - all pipeline executions will be tracked"
    )
...
class GenOpsComponentMixin:
    ...
    def with_retry_decorator(
        self, max_retries: Optional[int] = None, base_delay: Optional[float] = None
    ):
        """
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    result = self._execute_with_retry(
                        func.__name__, "DecoratedFunction", func, *args, **kwargs
                    )
                    ...
                    if result.status == "success":
                        return func(
                            *args, **kwargs
                        )  # Execute one final time to get actual result
                    else:
