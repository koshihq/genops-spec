...
def check_environment_variables() -> List[ValidationIssue]:
    ...
    required_vars = {
        "OPENROUTER_API_KEY": "OpenRouter API key for multi-provider LLM access"
    }
...
def check_genops_configuration() -> List[ValidationIssue]:
    ...
    try:
        ...
        if hasattr(instrumentor, 'provider_patches') and 'openrouter' in instrumentor.provider_patches:
            issues.append(ValidationIssue(
                level="info",
                component="configuration",
    ...
    try:
        ...
        try:
            ...
            if hasattr(adapter, 'client') and adapter.client:
                ...
                if hasattr(adapter.client, '_base_url') and 'openrouter.ai' in str(adapter.client._base_url):
                    issues.append(ValidationIssue(
                        level="info",
                        component="functionality",
        ...
        try:
            from genops.providers.openrouter_pricing import get_pricing_engine, calculate_openrouter_cost
            ...
            for model, expected_provider in test_models:
                try:
                    cost = calculate_openrouter_cost(model, input_tokens=10, output_tokens=10)
                    if cost > 0:
    ...
    if otel_endpoint:
        if not (otel_endpoint.startswith("http://") or otel_endpoint.startswith("https://")):
            issues.append(ValidationIssue(
                ...
                fix_suggestion=f"Update endpoint format: export OTEL_EXPORTER_OTLP_ENDPOINT='https://your-endpoint'"
            ))
...
def validate_openrouter_setup() -> ValidationResult:
    ...
    all_issues.extend(check_common_issues())           # New: Check common configuration problems first
    ...
    summary = {
        ...
        "validation_score": _calculate_validation_score(error_count, warning_count, info_count),
        "recommendations": []
    }
    
    # Add specific recommendations based on validation results
    if error_count > 0:
        summary["recommendations"].append("❌ Fix error-level issues before using OpenRouter provider")
        if error_count == 1:
            summary["recommendations"].append("💡 Focus on the single error - usually API key or connectivity")
        else:
            summary["recommendations"].append("🔧 Check installation first: pip install --upgrade genops-ai openai")
    
    if warning_count > 3:
        summary["recommendations"].append("⚠️  Multiple warnings detected - review configuration carefully")
    elif warning_count > 0:
        summary["recommendations"].append("📝 Review warning-level issues for optimal configuration")
    
    if error_count == 0 and warning_count <= 1:
        summary["recommendations"].append("✅ Setup looks excellent! Ready to use OpenRouter with GenOps")
        summary["recommendations"].append("🚀 Try the examples: python examples/openrouter/basic_tracking.py")
    elif error_count == 0:
        summary["recommendations"].append("✅ Setup is functional - minor optimizations recommended")
        summary["recommendations"].append("📖 See troubleshooting guide for warning resolution")
    
    # Add context-specific recommendations
    if info_count > 5:
        summary["recommendations"].append("ℹ️  Rich configuration detected - good setup coverage")
    
    return ValidationResult(is_valid, all_issues, summary)


def _calculate_validation_score(error_count: int, warning_count: int, info_count: int) -> float:
...
def print_openrouter_validation_result(result: ValidationResult) -> None:
    ...
    print(f"   • Functional: {'Yes' if result.summary.get('is_functional', False) else 'No'}")
    ...
    if result.is_valid:
        ...
        print("     python -c \"from genops.providers.openrouter import instrument_openrouter; print('OpenRouter ready!')\"")
    else:
        print("   • Fix the error-level issues above")
        print("   • Re-run validation: python -c \"from genops.providers.openrouter import validate_setup, print_validation_result; print_validation_result(validate_setup())\"")
