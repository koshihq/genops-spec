...
def validate_dependencies() -> list[ValidationResult]:
    ...
    start_time = time.time()
    try:
        from genops.providers.traceloop import instrument_traceloop  # noqa: F401
...
def validate_governance() -> list[ValidationResult]:
    ...
    try:
        ...
        instrument_traceloop(
            team="validation-test", project="governance-check", environment="test"
        )
...
def validate_performance() -> list[ValidationResult]:
    ...
    try:
        ...
        adapter = instrument_traceloop(team="perf-test", project="baseline")

        # Measure governance overhead
        governance_start = time.time()
        with adapter.track_operation(
            operation_type="performance_test", operation_name="baseline_test"
        ) as span:
...
def validate_setup(
    include_connectivity_tests: bool = True, include_performance_tests: bool = False
) -> ValidationSummary:
    """
