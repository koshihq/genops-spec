...
try:
    ...
    from genops.providers.bedrock_pricing import (
        BedrockCostBreakdown,  # noqa: F401
        calculate_bedrock_cost,
        compare_bedrock_models,  # noqa: F401
        get_cost_optimization_recommendations,  # noqa: F401
        get_detailed_cost_breakdown,  # noqa: F401
    )
...
class BedrockCostSummary:
    ...
    def get_cost_breakdown_percentage(self) -> dict[str, dict[str, float]]:
        ...
        breakdown = {"by_model": {}, "by_provider": {}, "by_region": {}}
...
class BedrockCostContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.telemetry:
            with self.telemetry.trace_operation(
                ...
                span.set_attribute(
                    "bedrock.context.total_operations", summary.total_operations
                )
                span.set_attribute("bedrock.context.duration_ms", duration * 1000)
                span.set_attribute(
                    "bedrock.context.unique_models", len(summary.unique_models)
                )
    ...
    def get_current_summary(self) -> BedrockCostSummary:
        ...
        unique_providers = {op.provider for op in self.operations}
        unique_regions = {op.region for op in self.operations}
    ...
    def _generate_optimization_recommendations(
        ...
        high_volume_models = [
            model
            for model, ops in operations_by_model.items()
    ...
    def export_cost_report(
        self, format: str = "json", include_operations: bool = False
    ) -> str:
        """
...
def get_global_cost_summary(timespan_hours: Optional[int] = None) -> dict[str, Any]:
    """
