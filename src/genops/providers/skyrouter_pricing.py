...
class SkyRouterPricingConfig:
    ...
    optimization_multipliers: Dict[str, float] = field(default_factory=lambda: {
    ...
    volume_tiers: Dict[int, float] = field(default_factory=lambda: {
    ...
    modal_multipliers: Dict[str, float] = field(default_factory=lambda: {
        "text": 1.0,
        "vision": 1.8,
        "audio": 1.5,
        "multimodal": 2.2
    ...
    currency_rates: Dict[str, float] = field(default_factory=lambda: {
        "USD": 1.0,
        "EUR": 1.08,
        "GBP": 1.25,
        "JPY": 0.0067,
...
class SkyRouterPricingCalculator:
    ...
    def calculate_model_call_cost(
        ...
        optimization_multiplier = self.config.optimization_multipliers.get(route_optimization, 1.0)
        optimization_adjustment = base_cost * (Decimal(str(optimization_multiplier)) - 1)
        ...
        standard_cost = base_cost * Decimal(str(self.config.complexity_multipliers["moderate"]))
        ...
        breakdown = SkyRouterCostBreakdown(
            ...
            efficiency_score=float(potential_savings / standard_cost) if standard_cost > 0 else 0.0,
            currency=currency,
            region=region
        )
        
        # Import here to avoid circular import
        from .skyrouter import SkyRouterCostResult, SkyRouterGovernanceAttrs
    ...
    def calculate_multi_model_cost(
        ...
        if routing_strategy == "cost_optimized":
            selected_model, selected_result = min(model_costs, key=lambda x: x[1].total_cost)
        elif routing_strategy == "latency_optimized":
            # Use model priority (first in list for latency)
            selected_model, selected_result = model_costs[0] if model_costs else (models[0], None)
        else:
        ...
        selected_result.metadata.update({
            ...
            "model_costs": {model: float(result.total_cost) for model, result in model_costs}
        })
    ...
    def calculate_agent_workflow_cost(
        ...
        workflow_discount = self._get_volume_discount(total_tokens) * (total_input_cost + total_output_cost)
        ...
        return SkyRouterCostResult(
            ...
            route_efficiency_score=float(total_savings / final_cost) if final_cost > 0 else 0.0,
            metadata={
    ...
    def _estimate_output_tokens(self, input_data: Dict[str, Any], complexity: str) -> int:
        ...
        if input_tokens > 1000:
            base_output = int(base_output * 1.2)  # Longer inputs often need longer outputs
    ...
    def _select_balanced_model(self, model_costs: List[Tuple[str, Any]]) -> Tuple[str, Any]:
    ...
    def update_monthly_volume(self, token_count: int):
        ...
        logger.debug(f"Updated monthly volume: {self.monthly_volume}, current discount: {self.current_discount:.1%}")
    ...
    def get_volume_discount_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tokens_to_next_tier": next_threshold - self.monthly_volume if next_threshold else 0
        }
    ...
    def estimate_monthly_cost(
        ...
        return {
            ...
            "cost_per_token": float(final_monthly_cost / monthly_tokens) if monthly_tokens > 0 else 0,
