...
class FireworksResult:
    ...
    governance_attributes: dict[str, Any] = None  # type: ignore[assignment]
...
class FireworksSessionContext:
    ...
    total_tokens: int = 0
    operations_by_model: dict[str, int] = None  # type: ignore[assignment]
...
class GenOpsFireworksAdapter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"GenOps Fireworks adapter initialized for team={self.team}, project={self.project}"
        )
    ...
    def _init_clients(self):
        ...
        if HAS_OPENAI:
            ...
            logger.warning(
                "OpenAI client not available for OpenAI-compatible interface"
            )

    def chat_with_governance(
        self,
        messages: list[dict[str, Any]],
        model: str | FireworksModel = FireworksModel.LLAMA_3_1_8B_INSTRUCT,
        ...
        try:
            ...
            if stream:
                return self._handle_streaming_completion(
                    request_data, governance_attrs, start_time
                )
            else:
                ...
                if session_id and session_id in self.active_sessions:
                    ...
                    session.operations_by_model[model_name] = (
                        session.operations_by_model.get(model_name, 0) + 1
                    )
    ...
    def _handle_streaming_completion(
        ...
        try:
            ...
            self.telemetry.record_error(
                "fireworks_streaming_completion_failed", error_attrs
            )
            raise GenOpsValidationError(
                f"Fireworks streaming completion failed: {e}"
            ) from e

    def embeddings_with_governance(
        self,
        input_texts: str | list[str],
        model: str | FireworksModel = FireworksModel.NOMIC_EMBED_TEXT,
        # Governance parameters
        session_id: str | None = None,
        feature: str | None = None,
        use_case: str | None = None,
        ...
        **kwargs,
        ...
        try:
            ...
            response = self.client.embeddings.create(
                model=model_name, input=input_texts, **kwargs
            )
            ...
            estimated_tokens = sum(
                len(text) // 4 for text in input_texts
            )  # Rough estimate
            cost = self.pricing_calc.estimate_embedding_cost(
    ...
    def get_cost_summary(self) -> dict[str, Any]:
        ...
        monthly_utilization = (
            float(self._monthly_costs) / self.monthly_budget_limit
        ) * 100
    ...
    def _check_cost_alerts(
        self, operation_cost: Decimal, governance_attrs: dict[str, Any]
    ):
    ...
    def _emit_completion_telemetry(
        self, result: FireworksResult, governance_attrs: dict[str, Any]
    ):
        ...
        self.telemetry.record_metric(
            "fireworks_cost", float(result.cost), telemetry_attrs
        )
        self.telemetry.record_metric(
            "fireworks_tokens", result.tokens_used, telemetry_attrs
        )
        self.telemetry.record_metric(
            "fireworks_latency", result.execution_time_seconds, telemetry_attrs
        )
    ...
    try:
        ...
        def instrumented_chat_create(self, **kwargs):
            ...
            try:
                ...
                return MockResponse(
                    result.response, result.model_used, result.tokens_used
                )

            except Exception as e:
                logger.warning(
                    f"Governance tracking failed, falling back to original: {e}"
                )
        ...
        if original_chat_create:
            ...
            logger.info(
                "✅ Fireworks chat completions auto-instrumented with GenOps governance"
            )

        if original_embeddings_create:
            Fireworks.embeddings.create = instrumented_embeddings_create
            logger.info(
                "✅ Fireworks embeddings auto-instrumented with GenOps governance"
            )
