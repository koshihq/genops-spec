...
class MistralPricingCalculator:
    ...
    def __init__(
        self, pricing_tier: MistralPricingTier = MistralPricingTier.PAY_AS_YOU_GO
    ):
        """
        ...
        logger.info(
            f"Mistral pricing calculator initialized with {len(self.pricing_data)} models"
        )
    ...
    def calculate_cost(
        ...
        **kwargs,
        ...
        tier_multiplier = self._get_tier_multiplier(
            kwargs.get("pricing_tier", self.pricing_tier)
        )
    ...
    def compare_models(
        ...
        comparisons.sort(key=lambda x: x["tokens_per_dollar"], reverse=True)  # type: ignore
    ...
    def get_optimization_insights(
        ...
        """
        insights = []
        current_breakdown = self.get_cost_breakdown(
            current_model, operation, input_tokens, output_tokens
        )

        # Model selection insights
        all_models = list(self.pricing_data.keys())
        comparisons = self.compare_models(
            all_models, operation, input_tokens, output_tokens
        )
        ...
        for comp in comparisons:
            if (
                comp["model"] != current_model
                and comp["total_cost"] < current_cost * 0.8
            ):  # 20%+ savings
                ...
                insight = PricingInsight(
                    ...
                    confidence="medium"
                    if comp["performance_tier"] == "standard"
                    else "low",
                )
        ...
        if output_tokens > input_tokens * 2:  # High output ratio
            potential_savings = (
                current_breakdown.output_cost * 0.3
        ...
        if current_breakdown.total_cost > 0.001:  # For significant costs
            insights.append(
                PricingInsight(
                    ...
                    potential_savings=current_breakdown.total_cost
                    * 0.2,  # Estimated 20% vs OpenAI equivalent
    ...
    def get_model_recommendations(self, use_case: str) -> list[dict[str, Any]]:
        ...
        for model_name, pricing in self.pricing_data.items():
            ...
            for rec_use_case in pricing.recommended_use_cases:
                if any(
                    ...
                    for keyword in rec_use_case.lower().split()
                ):
                    relevance_score += 2
        ...
        recommendations.sort(
            key=lambda x: (x["relevance_score"], -x["cost_per_operation"]), reverse=True
        )
...
def calculate_mistral_cost(
    model: str, input_tokens: int, output_tokens: int, operation: str = "chat"
) -> float:
...
def compare_mistral_models(
    models: list[str], tokens: tuple[int, int] = (1000, 500)
) -> list[dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    input_cost, output_cost, total_cost = calc.calculate_cost(
        model, "chat", input_tokens, output_tokens
    )
    ...
    for comp in comparisons:
        print(
            f"  {comp['model']}: ${comp['total_cost']:.6f} ({comp['cost_per_1k_tokens']:.4f}/1k tokens)"
        )
