...
from .raindrop_pricing import RaindropCostResult, RaindropPricingCalculator
from .raindrop_validation import print_validation_result, validate_setup
        ...
        if self.governance_policy == "enforced":
            try:
                ...
                if not validation_result.is_valid:
                    ...
                    raise ValueError(
                        "Raindrop AI setup validation failed:\n"
                        + "\n".join(error_messages)
                ...
                if self.governance_policy == "enforced":
                    ...
                    logger.warning(
                        f"Validation failed but continuing in advisory mode: {validation_error}"
                    )

        logger.info(
            f"GenOps Raindrop adapter initialized for team='{team}', project='{project}'"
        )
    ...
    def _setup_telemetry(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Telemetry will work locally but won't export to external collectors"
                )
            ...
            logger.error(
                "Install with: pip install genops[raindrop] to enable telemetry"
            )
            if self.governance_policy == "enforced":
                raise ValueError(
                    "OpenTelemetry is required in enforced governance mode.\n"
                    "Install with: pip install genops[raindrop]\n"
                    "Or switch to advisory mode: governance_policy='advisory'"
                ) from import_error
        except Exception as e:
            logger.warning(f"Failed to setup OpenTelemetry: {e}")
            logger.info(
                "Continuing without telemetry export - local tracking will still work"
            )
            if self.governance_policy == "enforced":
    ...
    def track_agent_monitoring_session(self, session_name: str, **kwargs):
        ...
        if self.tracer:
            with self.tracer.start_as_current_span(
                f"raindrop.monitoring.session.{session_name}"
            ) as span:
                ...
                try:
                    ...
                    span.set_attribute(
                        "genops.session.operations", session.operation_count
                    )
                    span.set_attribute(
                        "genops.session.duration_seconds", session.duration_seconds
                    )
...
class RaindropMonitoringSession:
    ...
    def __init__(
        ...
        name: str,
        adapter: GenOpsRaindropAdapter,
        governance_attrs: RaindropGovernanceAttributes,
        **kwargs,
    ):
        self.name = name
    ...
    def track_agent_interaction(
        ...
        if cost is not None:
            ...
            cost_result = self.adapter.pricing_calculator.calculate_interaction_cost(
                agent_id=agent_id, interaction_data=interaction_data
            )
        ...
        if self.adapter.daily_budget_limit and self.adapter.enable_cost_alerts:
            if float(self.total_cost) > self.adapter.daily_budget_limit:
                if self.adapter.governance_policy == "enforced":
                    raise ValueError(
                        f"Session would exceed daily budget limit: ${self.adapter.daily_budget_limit}"
                    )
                else:
                    logger.warning(
                        f"Session cost ${float(self.total_cost)} exceeds budget ${self.adapter.daily_budget_limit}"
                    )
    ...
    def track_performance_signal(
        ...
        if cost is not None:
            ...
            cost_result = self.adapter.pricing_calculator.calculate_signal_cost(
                signal_name=signal_name, signal_data=signal_data
            )
    ...
    def create_alert(
        ...
        if cost is not None:
            ...
            cost_result = self.adapter.pricing_calculator.calculate_alert_cost(
                alert_name=alert_name, alert_config=alert_config
            )
    ...
    try:
        ...
        class InstrumentedRaindropClient(_original_raindrop_client):
            ...
            def track_interaction(self, *args, **kwargs):
                ...
                with adapter.track_agent_monitoring_session(
                    "auto_instrumented"
                ) as session:
        ...
        logger.info(
            "📋 Raindrop AI SDK not found - governance adapter ready for manual use"
        )
    except Exception as e:
