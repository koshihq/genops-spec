...
class CostBreakdown:
    ...
    optimization_suggestions: list[str] = None  # type: ignore
...
class ReplicatePricingCalculator:
    ...
    def get_model_info(self, model_name: str) -> "ReplicateModelInfo":  # type: ignore  # noqa: F821
        """
    ...
    def _estimate_model_info(self, model_name: str) -> "ReplicateModelInfo":  # type: ignore  # noqa: F821
        ...
        if any(
            term in model_lower
            for term in ["llama", "chat", "gpt", "claude", "mistral", "falcon"]
        ):
            category = "text"
            pricing_type = "token"
            ...
            term in model_lower
            for term in ["flux", "sdxl", "stable", "diffusion", "midjourney", "dalle"]
        ):
            category = "image"
            pricing_type = "output"
            base_cost = 0.01  # Default $0.01 per image
        elif any(
            term in model_lower for term in ["video", "gen-", "runway", "veo", "pika"]
        ):
            category = "video"
            pricing_type = "output"
            ...
            term in model_lower
            for term in ["whisper", "music", "audio", "speech", "voice"]
        ):
            category = "audio"
            pricing_type = "time"
            base_cost = 0.001  # Default $0.001/second
    ...
    def calculate_cost(
        self,
        model_info: "ReplicateModelInfo",  # type: ignore  # noqa: F821
    ...
    def calculate_cost_breakdown(
        ...
        if model_info.pricing_type == "token":
            breakdown = self._calculate_token_cost(
                model_info, input_data, output, breakdown
            )
        elif model_info.pricing_type == "output":
            breakdown = self._calculate_output_cost(
                model_info, input_data, output, breakdown
            )
            ...
            breakdown = self._calculate_hybrid_cost(
                model_info, input_data, output, time_seconds, breakdown
            )
        else:
        ...
        breakdown.optimization_suggestions = self._get_optimization_suggestions(
            model_info, breakdown
        )

        # Round to reasonable precision
        breakdown.total_cost = float(
            Decimal(str(breakdown.total_cost)).quantize(
    ...
    def _calculate_token_cost(
        ...
        if output and isinstance(output, (str, list)):
            output_text = (
                str(output) if isinstance(output, str) else " ".join(map(str, output))
            )
    ...
    def _calculate_output_cost(
        ...
        if model_info.category == "image":
            ...
            duration = input_data.get(
                "duration", input_data.get("length", 5.0)
            )  # Default 5 seconds
    ...
    def _calculate_time_cost(
        ...
        if model_info.base_cost:
            ...
            model_info.hardware_type
            and model_info.hardware_type in self.hardware_pricing
        ):
    ...
    def _get_optimization_suggestions(
        self,
        model_info: "ReplicateModelInfo",  # type: ignore  # noqa: F821
        breakdown: CostBreakdown,  # noqa: F821
    ) -> list[str]:
        ...
        if breakdown.time_seconds > 30:
            suggestions.append(
                "Consider using a faster model variant for time-sensitive tasks"
            )

        # Hardware optimizations
        if model_info.hardware_type == "a100-80gb" and breakdown.time_seconds < 5:
            suggestions.append(
                "Consider using smaller GPU for short tasks to reduce costs"
            )

        # Model-specific suggestions
        if (
            model_info.category == "image"
            and breakdown.output_units
            and breakdown.output_units > 1
        ):
            suggestions.append(
                "Batch multiple images in single request to reduce overhead"
            )

        if (
            model_info.category == "video"
            and breakdown.output_units
            and breakdown.output_units > 10
        ):
            suggestions.append(
                "Consider shorter video clips - cost scales linearly with duration"
            )

        # Cost threshold suggestions
        if breakdown.total_cost > 1.0:
            suggestions.append(
                f"High cost operation (${breakdown.total_cost:.2f}) - verify necessity"
            )

        return suggestions

    def get_model_alternatives(
        self, model_name: str, category: Optional[str] = None
        ...
        for cached_model, pricing in self._pricing_cache.items():
            if (
                pricing.category == target_category
                and cached_model != model_name
                and pricing.official
            ):
...
# Export main classes
__all__ = ["ReplicatePricingCalculator", "ModelPricing", "CostBreakdown"]
