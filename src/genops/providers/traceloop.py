...
try:
    ...
    logger.info(
        f"OpenLLMetry loaded successfully (version: {getattr(openllmetry, '__version__', 'unknown')})"
    )
except ImportError:
...
try:
    from opentelemetry import context, trace  # noqa: F401
    from opentelemetry.semconv.ai import SpanAttributes  # noqa: F401
...
class GenOpsTraceloopAdapter:
    ...
    def __init__(
        ...
        enable_traceloop_platform: bool = None,  # type: ignore[assignment]
        # Advanced settings
        ...
        **kwargs,
        ...
        logger.info(
            f"GenOps Traceloop adapter initialized: team={self.team}, project={self.project}"
        )
    ...
    def _initialize_traceloop_platform(self, api_key: Optional[str], base_url: str):
        ...
        if not HAS_TRACELOOP_SDK:
            logger.warning(
                "Traceloop SDK not available. Install with: pip install traceloop-sdk"
            )
            return
        ...
        try:
            ...
            if not api_key:
                logger.info(
                    "No Traceloop API key provided, commercial features disabled"
                )
                return
    ...
    def track_operation(
        ...
        operation_type_str = (
            ...
            if isinstance(operation_type, TraceloopOperationType)
            else operation_type
        )

        # Create enhanced span with governance attributes
        with self.tracer.start_span(
            operation_name, kind=trace.SpanKind.CLIENT if HAS_OTEL else None
        ) as span:
    ...
    def _check_governance_policies(self, enhanced_span):
        ...
        if (
            ...
            violations.append(
                f"Operation cost ${enhanced_span.estimated_cost:.6f} exceeds limit ${self.max_operation_cost}"
            )

        # Check daily budget limits
        if self.daily_budget_limit and self._daily_usage > self.daily_budget_limit:
            violations.append(
                f"Daily usage ${self._daily_usage:.2f} exceeds budget ${self.daily_budget_limit}"
            )
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "budget_remaining": (self.daily_budget_limit - self._daily_usage)
...
def auto_instrument(
    team: str, project: str, environment: str = "development", **kwargs
) -> None:
    """
...
def get_budget_status() -> dict[str, Any]:
    ...
    if "_global_adapter" in globals():
        return {
            ...
            "remaining": (
                _global_adapter.daily_budget_limit - _global_adapter._daily_usage
            )
            if _global_adapter.daily_budget_limit
            else None,
...
def traceloop_create(
    ...
    return instrument_traceloop(
        team=team, project=project, environment=environment, **kwargs
    )
