...
class ValidationResult:
    ...
    def add_issue(self, component: str, status: ValidationStatus, message: str, 
                  fix_suggestion: Optional[str] = None, documentation_link: Optional[str] = None):
...
def validate_provider_api_keys() -> Tuple[List[ValidationIssue], Dict[str, ValidationStatus]]:
...
def validate_litellm_setup(
    ...
    try:
        ...
        error_count = sum(1 for issue in result.issues if issue.status == ValidationStatus.ERROR)
        warning_count = sum(1 for issue in result.issues if issue.status == ValidationStatus.WARNING)
...
def print_validation_result(result: ValidationResult, verbose: bool = True) -> None:
    ...
    if verbose:
        ...
        status_order = [ValidationStatus.ERROR, ValidationStatus.WARNING, 
