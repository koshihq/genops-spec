...
class FireworksPricingCalculator:
    ...
    def _initialize_model_catalog(self) -> dict[str, ModelInfo]:
        ...
        small_models = [
            ...
            (
                "accounts/deepseek-ai/models/deepseek-coder-v2-lite-instruct",
                "16B",
                65536,
            ),
        ]
        ...
        specialized_models = [
            ...
            (
                "accounts/deepseek-ai/models/deepseek-r1",
                "70B",
                None,
                32768,
                ["text"],
                {"input": Decimal("1.35"), "output": Decimal("5.40")},
            ),
            (
                ...
                32768,
                ["text"],
                {"input": Decimal("0.14"), "output": Decimal("0.56")},
            ),
            # Multimodal Models
            (
                "accounts/mistral/models/pixtral-12b-2409",
                "12B",
                Decimal("0.15"),
                128000,
                ["text", "image"],
            ...
            (
                "accounts/fireworks/models/nomic-embed-text-v1p5",
                "137M",
                Decimal("0.02"),
                8192,
                ["text"],
            ...
            (
                "accounts/fireworks/models/bge-base-en-v1p5",
                "109M",
                Decimal("0.02"),
                512,
                ["text"],
        ...
        for model_data in specialized_models:
            model, params, base_cost, context, modalities = model_data[:5]  # type: ignore
            ...
            catalog[model] = ModelInfo(
                ...
                specialized_pricing=specialized_pricing,  # type: ignore
            )
    ...
    def estimate_chat_cost(
        ...
        **kwargs,
        ...
        if model not in self.model_catalog:
            ...
            return self._estimate_unknown_model_cost(
                tokens or (input_tokens or 0) + (output_tokens or 0)
            )
        ...
        if model_info.specialized_pricing:
            return self._calculate_specialized_cost(
                model_info, input_tokens, output_tokens, tokens, is_batch
            )
        ...
        # Calculate base cost
        cost = (
            Decimal(str(total_tokens)) / Decimal("1000000")
    ...
    def _calculate_specialized_cost(
        ...
        if (
            "input" in model_info.specialized_pricing
            and "output" in model_info.specialized_pricing
        ):
            if input_tokens:
                input_cost = (
                    Decimal(str(input_tokens)) / Decimal("1000000")
                ) * model_info.specialized_pricing["input"]
                total_cost += input_cost

            if output_tokens:
                output_cost = (
                    Decimal(str(output_tokens)) / Decimal("1000000")
                ) * model_info.specialized_pricing["output"]
            ...
            if tokens and not (input_tokens and output_tokens):
                ...
                input_cost = (
                    Decimal(str(half_tokens)) / Decimal("1000000")
                ) * model_info.specialized_pricing["input"]
                output_cost = (
                    Decimal(str(tokens - half_tokens)) / Decimal("1000000")
                ) * model_info.specialized_pricing["output"]
    ...
    def estimate_embedding_cost(self, model: str, tokens: int, **kwargs) -> Decimal:
        """
        ...
        if model not in self.model_catalog:
            logger.warning(
                f"Embedding model {model} not in catalog, using default pricing"
            )
            return Decimal(str(tokens)) * Decimal(
                "0.00002"
            )  # Default $0.02 per 1M tokens
        ...
        cost = (
            Decimal(str(tokens)) / Decimal("1000000")
        ) * model_info.cost_per_million_tokens
        return cost.quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)

    def estimate_audio_cost(
        self, model: str, duration_minutes: float, **kwargs
    ) -> Decimal:
        """
        ...
        if model not in self.model_catalog:
            ...
            return Decimal(str(duration_minutes)) * Decimal(
                "0.006"
            )  # Default $0.006 per minute
        ...
        for model in models:
            ...
            if include_batch_pricing:
                batch_cost = self.estimate_chat_cost(
                    model, tokens=estimated_tokens, is_batch=True
                )
        ...
        **kwargs,
        ...
        for model_name, model_info in self.model_catalog.items():
            ...
            if (
                ...
                and model_info.context_length < min_context_length
            ):
                continue

            # Check modality requirements
            if not all(
                modality in model_info.modalities for modality in required_modalities
            ):
                continue

            # Estimate cost for typical operation
            estimated_tokens = self._get_tokens_for_complexity(task_complexity)
            estimated_cost = self.estimate_chat_cost(
                model_name, tokens=estimated_tokens, is_batch=prefer_batch
            )
        ...
        if not suitable_models:
            ...
            return ModelRecommendation(
                recommended_model=None,  # type: ignore[arg-type]
        ...
        suitable_models.sort(
            key=lambda x: (float(x["estimated_cost"]), self._get_model_score(x["info"]))
        )
    ...
    def analyze_costs(
        ...
        standard_cost_per_op = self.estimate_chat_cost(
            model, tokens=avg_tokens_per_operation
        )
        batch_cost_per_op = self.estimate_chat_cost(
            model, tokens=avg_tokens_per_operation, is_batch=True
        )
        ...
        daily_cost = (standard_ops * float(standard_cost_per_op)) + (
            batch_ops * float(batch_cost_per_op)
        )
        ...
        if best_alternative:
            alt_daily_cost = operations_per_day * float(
                best_alternative["estimated_cost"]
            )
        ...
        return {
            ...
            "optimization": {
                ...
                "batch_optimization_potential": float(
    ...
    def _find_alternative_models(
        ...
        for model_name, model_info in self.model_catalog.items():
            estimated_tokens = self._get_tokens_for_complexity(task_complexity)
            estimated_cost = self.estimate_chat_cost(
                model_name, tokens=estimated_tokens
            )
            ...
            # Modality fit
            modality_match = sum(
                1 for mod in modalities if mod in model_info.modalities
            ) / len(modalities)
            fit_score += modality_match * 5  # type: ignore[assignment]

            # Context length fit
            if (
                model_info.context_length
                and model_info.context_length >= context_length
            ):
                fit_score += 3
            elif (
                model_info.context_length
                and model_info.context_length >= context_length * 0.5
            ):
                fit_score += 1
    ...
    def _find_cost_alternatives(
        self, current_model: str, tokens: int
    ) -> list[dict[str, Any]]:
    ...
    def _generate_recommendation_reasoning(
        ...
        if cost <= budget * 0.5:
            reasoning_parts.append(
                f"Excellent cost efficiency at ${cost:.4f} (well under ${budget:.3f} budget)"
            )
        elif cost <= budget * 0.8:
            reasoning_parts.append(
                f"Good cost efficiency at ${cost:.4f} (within ${budget:.3f} budget)"
            )
        else:
            reasoning_parts.append(
                f"Cost-effective at ${cost:.4f} (fits ${budget:.3f} budget)"
            )
        ...
        if prefer_batch:
            reasoning_parts.append(
                "optimized for batch processing with 50% cost savings"
            )
...
# Pricing constants
PRICING_TIERS = {tier.value: tier for tier in FireworksPricingTier}
MODEL_PRICING = {
    "tiny": Decimal("0.10"),
    "small": Decimal("0.20"),
    "medium": Decimal("0.20"),
    "large": Decimal("0.90"),
    "mixture_of_experts": Decimal("0.50"),
}
