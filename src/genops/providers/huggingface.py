...
class GenOpsHuggingFaceAdapter:
    ...
    def __init__(self, client: Any | None = None, **client_kwargs: Any):
        ...
        self.circuit_breaker_enabled = (
            os.getenv("GENOPS_CIRCUIT_BREAKER", "true").lower() == "true"
        )
    ...
    def _record_circuit_breaker_success(self):
        ...
        if self._circuit_breaker_failures > 0:
            logger.debug(
                "Circuit breaker - resetting failure count after successful operation"
            )
            self._circuit_breaker_failures = 0

    def _async_export_telemetry(self, span_data: dict, cost_data: dict = None):  # type: ignore[assignment]
    ...
    def _fallback_cost_estimation(
        self, input_tokens: int, output_tokens: int, provider: str
    ) -> float:
    ...
    def text_generation(self, prompt: str, **kwargs) -> Any:
        ...
        provider = self._detect_provider(model)  # type: ignore
        ...
        with self.telemetry.trace_operation(
            ...
            **governance_attrs,
            ...
            try:
                ...
                cost_data = (
                    {"cost": cost, "currency": "USD", "provider": provider}
                    if cost > 0
                    else None
                )
                self._async_export_telemetry(span_data, cost_data)  # type: ignore[arg-type]
    ...
    def feature_extraction(self, inputs: str | list, **kwargs) -> Any:
...
def instrument_huggingface(**config):
    ...
    try:
        ...
        if hasattr(InferenceClient, "chat") and hasattr(
            InferenceClient.chat, "completions"
        ):
        ...
        InferenceClient._genops_original_chat_completions_create = (
            original_chat_completions_create
        )
        InferenceClient._genops_original_feature_extraction = (
            original_feature_extraction
        )
    ...
    try:
        ...
        if hasattr(InferenceClient, "_genops_original_text_generation"):
            InferenceClient.text_generation = (
                InferenceClient._genops_original_text_generation
            )
        ...
        if hasattr(InferenceClient, "_genops_original_text_to_image"):
            InferenceClient.text_to_image = (
                InferenceClient._genops_original_text_to_image
            )
