...
class CohereValidator:
    ...
    def _validate_configuration(self):
        ...
        for var, description in env_vars.items():
            value = os.getenv(var)
            if value:
                self.result.system_info[f"env_{var.lower()}"] = (
                    f"Set ({len(value)} chars)" if "key" in var.lower() else value
                )
                self.result.add_issue(
                    ValidationIssue(
                        category=ValidationCategory.CONFIGURATION,
    ...
    def _validate_connectivity(self):
        ...
        try:
            ...
            if response.status_code in [
                200,
                401,
            ]:  # 401 is expected without valid auth, but shows connectivity
                self.result.add_passed_check("API connectivity")
                self.result.performance_metrics["connectivity_time_ms"] = (
                    connectivity_time
                )
            else:
                self.result.add_issue(
    ...
    def _validate_models(self):
        ...
        for model_name, model_type, description in model_tests:
            try:
                if model_type == "generation":
                    ...
                    self.client.embed(
                        model=model_name, texts=["test"], input_type="classification"
                    )
                    available_models.append((model_name, description))
        ...
        if available_models:
            ...
            self.result.system_info["available_models"] = [
                f"{name} ({desc})" for name, desc in available_models
            ]
    ...
    def _validate_performance(self):
        ...
        for test_name, test_func in performance_tests:
            try:
                ...
                self.result.performance_metrics[f"{test_name}_response_time_ms"] = (
                    response_time
                )
    ...
    def _generate_recommendations(self):
        ...
        if auth_issues:
            recommendations.append(
                "🔑 Configure valid Cohere API key for full integration testing"
            )

        # Based on model access
        if not self.result.system_info.get("available_models"):
            recommendations.append(
                "🤖 Verify API key has access to required Cohere models"
            )
        ...
        if slow_operations:
            recommendations.append(
                "⚡ Consider optimizing slow API operations or checking network latency"
            )

        # Success recommendations
        if self.result.success and not self.result.has_errors:
            recommendations.append(
                "✅ Your setup looks good! You can proceed with GenOps Cohere integration"
            )
...
def quick_validate(api_key: Optional[str] = None) -> bool:
    ...
    """
    validator = CohereValidator(api_key=api_key, include_performance_tests=False)
