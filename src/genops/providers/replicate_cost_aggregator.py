...
class ReplicateCostAggregator:
    ...
    def __init__(
        ...
        try:
            ...
            self._pricing_calculator = None  # type: ignore[assignment]
    ...
    def get_current_summary(self) -> ReplicateCostSummary:
        ...
        if not self.operations:
            return ReplicateCostSummary(total_cost=0.0, operation_count=0)

        # Calculate aggregated metrics
        total_tokens = sum(
            (op.input_tokens or 0) + (op.output_tokens or 0) for op in self.operations
        )

        total_output_units = sum(op.output_units or 0 for op in self.operations)

        total_time_ms = sum(op.latency_ms or 0 for op in self.operations)

        unique_models = {op.model for op in self.operations}
        unique_categories = {op.category for op in self.operations}
    ...
    def _generate_optimization_recommendations(self) -> list[str]:
        ...
        for category, _cost in self._cost_by_category.items():
            category_ops = [op for op in self.operations if op.category == category]
    ...
    def get_model_performance(self, model: str) -> Optional[dict[str, Any]]:
        ...
        return {
            ...
            "cost_percentage": (total_cost / self.total_cost) * 100
