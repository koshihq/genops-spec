...
class PrometheusValidator:
    ...
    def __init__(self, port: int = 8000, prometheus_url: str = None, namespace: str = "genops"):
        """
    ...
    def _check_configuration(self, result: ValidationResult):
        ...
        if 1024 <= self.port <= 65535:
            ...
            result.add_issue(ValidationIssue(
                ...
                fix_suggestion=f"Use a port between 1024 and 65535",
    ...
    def _generate_recommendations(self, result: ValidationResult):
        ...
        if any(issue.title == "Port Already in Use" for issue in result.issues):
            result.recommendations.append(
                f"Use a different port: export PROMETHEUS_EXPORTER_PORT=8001"
            )

        # No Prometheus server
        if any("Prometheus Server Not Reachable" in issue.title for issue in result.issues):
            result.recommendations.append(
...
def print_validation_result(result: ValidationResult) -> None:
    ...
    for category in ValidationCategory:
        ...
        for issue in category_issues:
            print(f"   {issue}")
            if issue.fix_suggestion and issue.level in [ValidationLevel.ERROR, ValidationLevel.CRITICAL, ValidationLevel.WARNING]:
