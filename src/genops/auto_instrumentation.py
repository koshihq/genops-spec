...
class GenOpsInstrumentor:
    ...
    def instrument(
        ...
        if providers is None and patch_all:
            providers_to_patch = [
                name for name, available in self.available_providers.items() 
                if available
            ]
        elif providers:
            providers_to_patch = [
                name for name in providers 
                if name in self.provider_patches and self.available_providers.get(name, False)
            ]
        else:
            providers_to_patch = []
        ...
        logger.info(f"🎉 GenOps AI initialized successfully!")
