...
def demonstrate_comprehensive_rag_monitoring():
    ...
    with create_llamaindex_cost_context(
        "team_rag_demo", budget_limit=2.0, enable_alerts=True
    ) as cost_context:
        for i, query_info in enumerate(team_queries, 1):
            ...
            with rag_monitor.monitor_rag_operation(
                ...
                print(
                    f"   🏷️  Attribution: {query_info['team']}/{query_info['project']}"
                )
        ...
        print("\nComponent Costs:")
        print(
            f"  • Embeddings: ${breakdown.embedding_cost:.6f} ({breakdown.embedding_cost / summary.total_cost * 100:.1f}%)"
        )
        print(
            f"  • Retrieval: ${breakdown.retrieval_cost:.6f} ({breakdown.retrieval_cost / summary.total_cost * 100:.1f}%)"
        )
        print(
            f"  • Synthesis: ${breakdown.synthesis_cost:.6f} ({breakdown.synthesis_cost / summary.total_cost * 100:.1f}%)"
        )
    ...
    print(
        f"  • Success Rates: Embedding {analytics.embedding_success_rate * 100:.1f}%, Retrieval {analytics.retrieval_success_rate * 100:.1f}%, Synthesis {analytics.synthesis_success_rate * 100:.1f}%"
    )

    if analytics.avg_retrieval_relevance:
        print(
            f"  • Average Retrieval Relevance: {analytics.avg_retrieval_relevance:.3f}"
        )
...
def demonstrate_team_cost_attribution():
    ...
    docs = [
        Document(
            text="Customer support best practices include quick response times, accurate information, and empathetic communication."
        ),
        Document(
            text="Product development requires user research, iterative design, and continuous testing to ensure market fit."
        ),
        Document(
            text="Engineering teams focus on code quality, system reliability, and scalable architecture design."
        ),
    ]
    ...
    for scenario in team_scenarios:
        ...
        with create_llamaindex_cost_context(
            ...
            print(
                f"   📊 Budget Used: {team_summary.total_cost / scenario['budget'] * 100:.1f}%"
            )

            if team_summary.budget_status and team_summary.budget_status["alerts"]:
                print(
                    f"   ⚠️  Budget Alerts: {len(team_summary.budget_status['alerts'])}"
                )
...
def main():
    ...
    try:
        ...
        print(
            f"   • Most expensive component: {'Synthesis' if summary.cost_breakdown.synthesis_cost > summary.cost_breakdown.embedding_cost else 'Embeddings'}"
        )
