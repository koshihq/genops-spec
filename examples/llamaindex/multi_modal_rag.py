...
def check_multimodal_capabilities():
    ...
    try:
        import fitz  # PyMuPDF, F401  # noqa: F401
...
def setup_multimodal_llm_provider():
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        return (
            provider_info,
            Settings.llm,
        )  # Claude can handle multi-modal in single model

    else:
        raise ValueError(
            "No supported API key found. Set OPENAI_API_KEY or ANTHROPIC_API_KEY for multi-modal capabilities"
        )
...
def create_sample_multimodal_documents(
    ...
    if capabilities.get("image_processing", False):
        try:
            ...
            # Draw bars
            bars = [("Q1", 200, "blue"), ("Q2", 300, "green"), ("Q3", 400, "orange")]
...
def process_text_document(
    doc: MultiModalDocument, cost_tracker: MultiModalRAGCostTracker
) -> MultiModalDocument:
    ...
    print(
        f"📄 Processing text document: {doc.metadata.get('document_type', 'unknown')}"
    )
    ...
    print(
        f"   ✅ Text processed: {estimated_tokens} tokens, ${processing_cost:.6f}, {processing_time:.0f}ms"
    )
...
def process_image_document(
    ...
    multimodal_llm,
    cost_tracker: MultiModalRAGCostTracker,
    capabilities: dict[str, bool],
) -> MultiModalDocument:
    ...
    if not doc.image_data or not capabilities.get("vision_models", False):
        ...
        doc.image_description = (
            "Image processing not available - would describe visual content"
        )
    ...
    try:
        ...
        base64.b64encode(doc.image_data).decode("utf-8")
...
def create_multimodal_knowledge_base(
    documents: list[MultiModalDocument], cost_tracker: MultiModalRAGCostTracker
):
...
def demonstrate_multimodal_rag_queries(
    query_engine, cost_tracker: MultiModalRAGCostTracker
):
    ...
    for i, query_info in enumerate(queries, 1):
        ...
        print(
            f"   💰 Costs: Retrieval ${retrieval_cost:.6f}, Synthesis ${synthesis_cost:.6f}"
        )
        ...
        if hasattr(response, "source_nodes") and response.source_nodes:
            ...
            for node in response.source_nodes[:2]:  # Show first 2 sources
                ...
                sources.append(
                    f"{content_type}{'📷' if has_image else '📄'} (quality: {quality:.2f})"
                )
...
def main():
    ...
    try:
        ...
        for doc in documents:
            print(
                f"   • {doc.content_type}: {doc.metadata.get('document_type', 'general')}"
            )
        ...
        for doc in documents:
            if doc.content_type == "text" or doc.content_type == "mixed":
                ...
                processed_doc = process_image_document(
                    doc, multimodal_llm, cost_tracker, capabilities
                )
                processed_documents.append(processed_doc)
            else:
        ...
        query_engine = create_multimodal_knowledge_base(
            processed_documents, cost_tracker
        )
