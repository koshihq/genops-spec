...
class ProductionAlertManager:
    ...
    def check_performance_alerts(self, metrics: ProductionMetrics) -> List[Dict[str, Any]]:
    ...
    def send_alerts(self, alerts: List[Dict[str, Any]]) -> None:
        ...
        for alert in alerts:
            ...
            print(f"{level_icon} ALERT [{alert['level'].value.upper()}]: {alert['message']}")
...
class ProductionRAGDeployment:
    ...
    def select_optimal_provider(self, complexity: str, budget_remaining: float) -> Dict[str, Any]:
        ...
        if complexity == "high":
            ...
            premium_providers = [p for p in self.provider_configs if p["quality_score"] >= 0.9]
        ...
        balanced = min(self.provider_configs, key=lambda p: abs(p["quality_score"] - 0.85))
    ...
    def track_request(self, customer_id: str, query_type: str, **kwargs):
        ...
        if self.metrics.current_concurrent_requests > self.config.max_concurrent_requests:
            self.metrics.current_concurrent_requests -= 1
            raise Exception(f"Max concurrent requests exceeded: {self.metrics.current_concurrent_requests}")
        ...
        try:
            ...
            self.metrics.p95_response_time_ms = sorted(self.response_times)[int(len(self.response_times) * 0.95)]
            ...
            customer_stats["avg_response_time"] = (
                (customer_stats["avg_response_time"] * (customer_stats["requests"] - 1) + duration_ms)
            ...
            self.metrics.daily_budget_utilization = self.metrics.daily_cost / self.config.daily_budget_limit
            self.metrics.monthly_budget_utilization = (self.metrics.total_cost / self.config.monthly_budget_limit)

            # Calculate cost per request
            if self.metrics.total_requests > 0:
                self.metrics.cost_per_request = self.metrics.total_cost / self.metrics.total_requests

            logger.info(f"Request {request_id} completed: {duration_ms:.0f}ms, ${request_data['cost']:.6f}")
...
def create_production_knowledge_base():
    ...
    logger.info(f"Production knowledge base ready with {len(production_documents)} documents")
    return query_engine
...
def simulate_production_traffic(deployment: ProductionRAGDeployment, query_engine):
    ...
    print(f"🎯 Simulating {sum(len(s['queries']) for s in customer_scenarios)} queries across {len(customer_scenarios)} customers")
    ...
    for scenario in customer_scenarios:
        ...
        for query, complexity in scenario["queries"]:
            ...
            customer_stats = deployment.customer_usage.get(customer_id, {"daily_cost": 0.0})
            if customer_stats["daily_cost"] > deployment.config.per_customer_daily_limit:
                print(f"   ⚠️  Customer daily budget exceeded, skipping: {query[:50]}...")
                continue
            ...
            try:
                with deployment.track_request(
                    ...
                    budget_remaining = deployment.config.daily_budget_limit - deployment.metrics.daily_cost
                    provider = deployment.select_optimal_provider(complexity, budget_remaining)

                    print(f"   🤖 Query: {query}")
                    print(f"   🔧 Provider: {provider['name']} (${provider['cost_per_1k']:.3f}/1K)")
                    ...
                    # Calculate costs
                    estimated_tokens = 100 + {"low": 50, "medium": 100, "high": 200}[complexity]
                    ...
                    quality_score = provider["quality_score"] + (0.05 if tier == "enterprise" else 0.0)
    ...
    if deployment.quality_scores:
        deployment.metrics.avg_response_quality = sum(deployment.quality_scores) / len(deployment.quality_scores)
        deployment.metrics.avg_retrieval_relevance = deployment.metrics.avg_response_quality * 0.9  # Estimate
...
def demonstrate_production_monitoring(deployment: ProductionRAGDeployment):
    ...
    performance_alerts = deployment.alert_manager.check_performance_alerts(deployment.metrics)
    ...
    print(f"   Requests: {deployment.metrics.successful_requests}/{deployment.metrics.total_requests}")
    print(f"   Concurrent: {deployment.metrics.current_concurrent_requests}/{deployment.config.max_concurrent_requests}")
    ...
    print(f"   Daily Budget: {deployment.metrics.daily_budget_utilization:.1%} (${deployment.metrics.daily_cost:.2f} / ${deployment.config.daily_budget_limit:.2f})")

    print("\n📊 QUALITY METRICS:")
    print(f"   Avg Retrieval Relevance: {deployment.metrics.avg_retrieval_relevance:.3f}")
    ...
    if deployment.customer_usage:
        for customer_id, stats in list(deployment.customer_usage.items())[:3]:  # Show top 3
            print(f"   {customer_id}: {stats['requests']} requests, ${stats['daily_cost']:.6f}, {stats['avg_response_time']:.0f}ms avg")
...
def demonstrate_compliance_reporting(deployment: ProductionRAGDeployment):
    ...
    report = {
        ...
        "system_metrics": {
            ...
            "performance_sla_compliance": deployment.metrics.p95_response_time_ms < deployment.config.max_response_time_ms,
            "security_controls": {
        ...
        "cost_governance": {
            ...
            "daily_budget_compliance": deployment.metrics.daily_budget_utilization <= 1.0,
    ...
    print(f"   SOC 2 Controls: {'PASS' if report['system_metrics']['security_controls']['audit_logging'] else 'FAIL'}")
    print(f"   GDPR Compliance: {'PASS' if report['data_governance']['privacy_controls'] == 'GDPR compliant' else 'FAIL'}")
    print(f"   Performance SLA: {'PASS' if report['system_metrics']['performance_sla_compliance'] else 'FAIL'}")
    print(f"   Budget Controls: {'PASS' if report['cost_governance']['daily_budget_compliance'] else 'FAIL'}")
    ...
    if deployment.request_history:
        ...
        print(f"   Timestamp: {datetime.fromtimestamp(sample_request['start_time']).isoformat()}")
...
def main():
    ...
    try:
        ...
        print(f"   Auto-scaling: {'Enabled' if config.auto_scale_threshold else 'Disabled'}")
        print(f"   Fallback Models: {'Enabled' if config.fallback_model_enabled else 'Disabled'}")

        # Initialize deployment
        deployment = ProductionRAGDeployment(config)
        print(f"✅ Production deployment initialized with {len(deployment.provider_configs)} providers")
        ...
        print(f"   • Average cost per request: ${deployment.metrics.cost_per_request:.6f}")
        print(f"   • System success rate: {deployment.metrics.calculate_success_rate():.1f}%")
        print(f"   • Average response time: {deployment.metrics.avg_response_time_ms:.0f}ms")
        ...
        if "api key" in str(e).lower():
            print("\n🔧 API KEY ISSUE:")
            print("   Production deployment requires: OPENAI_API_KEY or ANTHROPIC_API_KEY")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("   → Integrate with your existing observability stack (Datadog, Grafana)")
