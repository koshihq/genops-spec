...
try:
    from genops.providers.together import (  # noqa: F401
...
class CostOptimizer:
    ...
    def find_cheapest_model_for_task(
        self, task_type: str, max_budget: float = 0.001, min_context_length: int = 8192
    ) -> dict[str, Any]:
    ...
    def compare_model_performance_costs(
        self, models: list[str], test_prompt: str, max_tokens: int = 100
    ) -> list[dict[str, Any]]:
                    ...
                    results.append(
                        {
                            ...
                            "tokens_per_second": result.tokens_used
...
def demonstrate_cost_comparison():
    ...
    results = optimizer.compare_model_performance_costs(
        models=models_to_compare, test_prompt=test_prompt, max_tokens=120
    )
...
def demonstrate_task_based_optimization():
        ...
        if recommendation["recommended_model"]:
            ...
            try:
                ...
                task_results[task_type] = {
                    ...
                    "cost_accuracy": abs(
                        float(result.cost) - recommendation["estimated_cost"]
                    ),
                ...
                print(
                    f"   📊 Cost estimation accuracy: ±${abs(float(result.cost) - recommendation['estimated_cost']):.6f}"
                )
    ...
    if task_results:
        ...
        avg_accuracy = sum(tr["cost_accuracy"] for tr in task_results.values()) / len(
            task_results
        )

        print(f"   Total cost for all task types: ${total_cost:.6f}")
        print(f"   Average cost estimation accuracy: ±${avg_accuracy:.6f}")
        print(f"   Models used: {len({tr['model'] for tr in task_results.values()})}")
...
def demonstrate_budget_constrained_operations():
    ...
    operations_possible = int(
        adapter.daily_budget_limit / comparisons[0]["estimated_cost"]
    )
        ...
        for i, query in enumerate(test_queries[:operations_possible], 1):
            try:
                ...
                if cost_summary["daily_budget_utilization"] > 80:
                    print(
                        f"   ⚠️  Budget utilization: {cost_summary['daily_budget_utilization']:.1f}%"
                    )
    ...
    print(
        f"   Remaining budget: ${adapter.daily_budget_limit - cost_summary['daily_costs']:.6f}"
    )
...
def demonstrate_cost_projection_analysis():
    ...
    for scenario in usage_scenarios:
        ...
        if analysis["potential_savings"]["best_alternative"]:
            ...
            print(
                f"   Monthly savings: ${analysis['potential_savings']['potential_monthly_savings']:.2f}"
            )
...
def main():
    ...
    try:
        ...
        if adapter:
            ...
            print("   • Task-based optimization configured")
            print("   • Budget constraints successfully managed")
            print("   • Cost projections analyzed")

            print("\n💰 Session Totals:")
            print(f"   Total spending: ${cost_summary['daily_costs']:.6f}")
            print("   Models tested: Multiple across all price tiers")
            print("   Optimization focus: Cost-effectiveness and budget control")
