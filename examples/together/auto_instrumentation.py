    ...
    try:
        ...
        result = adapter.chat_with_governance(
            messages=[
                {
                    "role": "user",
                    "content": "Compare auto vs manual instrumentation approaches",
                }
            ],
        ...
        response = client.chat.completions.create(
            model="meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo",
            messages=[
                {"role": "user", "content": "This is automatically tracked too!"}
            ],
            max_tokens=50,
        )
...
def main():
    ...
    if adapter:
        ...
        print(
            f"   • Budget monitoring: {cost_summary['daily_budget_utilization']:.1f}% utilized"
        )
