    ...
    with adapter.track_session("code-generation-session") as session:
        for task in coding_tasks:
            ...
            try:
                result = adapter.chat_with_governance(
                    messages=[
                        {"role": "system", "content": f"You are an expert {task['language']} developer. Write clean, well-documented code."},
...
async def demonstrate_async_batch_processing():
    ...
    if successful_tasks:
        ...
        print(f"   🎯 Throughput: {len(successful_tasks) / total_batch_time:.1f} tasks/second")
...
def demonstrate_reasoning_models():
    ...
    with adapter.track_session("reasoning-analysis") as session:
        for task in reasoning_tasks:
            ...
            try:
                result = adapter.chat_with_governance(
                    messages=[
                        {"role": "system", "content": "Think step-by-step and provide detailed reasoning for your analysis."},
                        {"role": "user", "content": task['prompt']}
                    ],
                    model=task['model'],
                    max_tokens=400,
    ...
    if reasoning_results:
        ...
        print(f"   Average reasoning depth: {avg_reasoning_depth:.1f} indicators per response")
        print(f"   Models used: {len(set(r['model'] for r in reasoning_results))}")
