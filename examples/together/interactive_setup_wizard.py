...
class TogetherSetupWizard:
    ...
    def gather_api_credentials(self):
        ...
        while True:
            ...
            if not api_key.startswith(("sk-", "pk-")):
                print(
                    "⚠️ Warning: API key format may be incorrect (should start with 'sk-' or 'pk-')"
                )
    ...
    def gather_governance_config(self):
        ...
        self.config["team"] = (
            input(f"Team name [{default_team or 'my-team'}]: ").strip()
            or default_team
            or "my-team"
        )
        ...
        self.config["project"] = (
            input(
                f"Project name [{default_project or 'together-ai-project'}]: "
            ).strip()
            or default_project
            or "together-ai-project"
        )
        ...
        self.config["environment"] = (
            input(f"Environment [{default_env}]: ").strip() or default_env
        )
        ...
        while True:
            try:
                ...
                self.config["daily_budget_limit"] = (
                    float(daily_budget) if daily_budget else 50.0
                )
                break
            except ValueError:
        ...
        while True:
            try:
                ...
                self.config["monthly_budget_limit"] = (
                    float(monthly_budget) if monthly_budget else 1000.0
                )
                break
            except ValueError:
                print("❌ Please enter a valid number")
        ...
        while True:
            policy = (
                input("Governance policy [advisory]: ").strip().lower() or "advisory"
            )
    ...
    def gather_preferences(self):
        ...
        while True:
            try:
                retry_attempts = input(
                    "Retry attempts for failed requests [3]: "
                ).strip()
                self.config["retry_attempts"] = (
                    int(retry_attempts) if retry_attempts else 3
                )
                break
            except ValueError:
                print("❌ Please enter a valid number")
...
"""
        ...
        if self.config.get("api_key") and self.config["api_key"].startswith(
            ("sk-", "pk-")
        ):
            print(
