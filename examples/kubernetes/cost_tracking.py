...
try:
    from genops.core.cost import BudgetManager, CostSummary, CostTracker  # noqa: F401

    from genops.core.governance import create_governance_context  # noqa: F401
...
class KubernetesCostDemo:
    ...
    async def demonstrate_basic_cost_tracking(
        ...
        with self.adapter.create_governance_context(**governance_attrs) as ctx:
            ...
            for provider, model, cost, tokens_in, tokens_out, operation in operations:
                print(
                    f"   💸 {provider} {model}: ${cost:.4f} ({tokens_in} → {tokens_out} tokens)"
                )
            ...
            print(
                f"   Total Cost: ${cost_summary.get('total_cost', total_simulated_cost):.4f}"
            )
    ...
    async def demonstrate_budget_management(self, budget_limit: float = 100.0) -> bool:
        ...
        for i, (description, cost) in enumerate(operations, 1):
            ...
            print(
                f"   Would bring total to: ${potential_total:.2f} ({percentage:.1f}%)"
            )
            ...
            if potential_total <= budget_limit:
                ...
                for threshold in budget_config["alert_thresholds"]:
                    if (
                        ...
                        and (current_spend - cost) / budget_limit * 100 < threshold
                    ):
                ...
                print(
                    f"   🛑 This operation would exceed the daily budget of ${budget_limit:.2f}"
                )
                break
    ...
    async def demonstrate_multi_provider_cost_aggregation(self) -> bool:
        ...
        with self.adapter.create_governance_context(**governance_attrs) as ctx:
            ...
            print(
                f"     Potential savings: ${potential_savings:.4f} ({potential_savings / total_cost * 100:.1f}%)"
            )
            ...
            print(
                f"     This model accounts for ${most_expensive_model[1]:.4f} ({most_expensive_model[1] / total_cost * 100:.1f}%)"
            )
        ...
        for scenario in optimization_scenarios:
            ...
            for model, cost in models:
                ...
                if savings_vs_expensive > 0:
                    print(
                        f"       Savings vs most expensive: ${savings_vs_expensive:.4f}"
                    )
...
Examples:
    ...
    parser.add_argument("--team", type=str, help="Team name for cost attribution")

    parser.add_argument("--project", type=str, help="Project name for cost tracking")
    ...
    if args.multi_provider:
        ...
        basic_success = await demo.demonstrate_basic_cost_tracking(
            team=args.team, project=args.project
        )
