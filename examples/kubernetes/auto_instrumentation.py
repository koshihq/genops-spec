...
async def demonstrate_auto_instrumentation():
    ...
    if validation.is_kubernetes_environment:
        ...
        print(
            "⚠️  Not in Kubernetes - auto-instrumentation will work with limited context"
        )
    ...
    print(
        f"   Environment detected: {'Kubernetes' if detector.is_kubernetes() else 'Local'}"
    )
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Hello! This request is automatically tracked by GenOps.",
                }
            ],
            max_tokens=50,
        )
    ...
    try:
        ...
        response = await client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Hello! This Anthropic request is automatically tracked.",
                }
            ],
        )
...
Examples:
    ...
    parser.add_argument(
        "--test-openai", action="store_true", help="Test OpenAI auto-instrumentation"
    )
