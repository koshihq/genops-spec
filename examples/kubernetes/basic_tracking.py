...
async def basic_tracking_example(
    ...
    if not validation.is_kubernetes_environment:
        print(
            "⚠️  Not running in Kubernetes - governance will work but with limited context"
        )
    else:
    ...
    with adapter.create_governance_context(**governance_attrs) as governance_context:
            ...
            try:
                ...
                response = await client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": "Hello from Kubernetes with GenOps governance!",
                        }
                    ],
                    max_tokens=50,
                )
                ...
                print(
                    f"   ✅ OpenAI response: {response.choices[0].message.content[:100]}..."
                )
                ...
                if cost_info:
                    print(
                        f"   💰 Estimated cost: ${cost_info.get('total_cost', 0):.4f}"
                    )
        ...
        if validation.has_resource_monitoring:
            ...
            if resource_usage:
                print(
                    f"   CPU usage: {resource_usage.get('cpu_usage_millicores', 'N/A')}m"
                )
                print(
                    f"   Memory usage: {resource_usage.get('memory_usage_bytes', 'N/A')} bytes"
                )
    ...
    try:
        ...
        try:
            ...
            if usage.memory_usage_bytes is not None:
                print(
                    f"   Current Memory: {usage.memory_usage_bytes / 1024 / 1024:.1f} MB"
                )
...
Examples:
    ...
    parser.add_argument("--project", type=str, help="Project name for tracking")

    parser.add_argument(
        "--customer-id", type=str, help="Customer ID for billing attribution"
    )

    parser.add_argument(
        "--show-patterns", action="store_true", help="Show available tracking patterns"
    )
    ...
    if not args.show_patterns and not args.show_k8s_features:
        success = await basic_tracking_example(
            team=args.team, project=args.project, customer_id=args.customer_id
        )
