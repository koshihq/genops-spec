...
class FireworksCostOptimizer:
    ...
    def compare_batch_vs_standard_pricing(
        self, model: FireworksModel, operations_count: int, avg_tokens: int = 500
    ) -> dict[str, Any]:
    ...
    def optimize_model_selection_for_budget(
        self, prompts: list[str], total_budget: float, prefer_quality: bool = False
    ) -> list[dict[str, Any]]:
...
def main():
    ...
    test_prompt = (
        "Explain the benefits of cost-optimized AI inference in business applications."
    )
    ...
    if model_results:
        best_value = max(
            model_results,
            key=lambda x: len(x.response.split()) / float(x.cost) if x.cost > 0 else 0,
        )
        print(
    ...
    for model in batch_test_models:
        ...
        print(
            f"      Savings: ${batch_analysis['savings']:.2f} ({batch_analysis['savings_percentage']:.0f}%)"
        )
    ...
    task_examples = [
        ...
        (
            "Conduct a detailed competitive analysis of AI inference providers",
            "complex",
        ),
    ]
    ...
    for scenario, ops_per_day, avg_tokens, complexity in usage_scenarios:
        ...
        rec = optimizer.pricing_calc.recommend_model(
            task_complexity=complexity, budget_per_operation=0.01, prefer_batch=True
        )
        ...
        if rec.recommended_model:
            ...
            print(
                f"      Batch savings: ${analysis['optimization']['batch_optimization_potential']:.2f}/month"
            )
            ...
            if analysis["optimization"]["best_alternative"]:
                ...
                print(
                    f"      Potential savings: ${analysis['optimization']['potential_monthly_savings']:.2f}/month"
                )
    ...
    total_operations = (
        len(model_results) + successful_operations + 3
    )  # +3 for task examples
    average_cost = (
        float(cost_summary["daily_costs"]) / total_operations
