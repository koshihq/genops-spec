...
try:
    from genops.providers.fireworks import (  # noqa: F401
        FireworksModel,
...
class InteractiveSetupWizard:
    ...
    def welcome(self):
        ...
        print("• Configure Fireworks AI with optimal settings for your use case")
        print("• Set up governance and cost management")
        print("• Get personalized model recommendations")
        print("• Generate working code examples")
        print("• Plan your production deployment")
        print()
        print("🔥 Why Fireworks AI?")
        print("• 4x faster inference with Fireattention optimization")
        print("• 100+ models across all modalities (text, vision, audio)")
        print("• Parameter-based pricing from $0.10-$3.00 per 1M tokens")
    ...
    def collect_team_info(self) -> dict[str, str]:
        ...
        team_name = input(
            "Team name (e.g., 'ml-engineering', 'product-team'): "
        ).strip()
        if not team_name:
            team_name = "demo-team"
        ...
        project_name = input(
            "Project name (e.g., 'chat-bot', 'content-generation'): "
        ).strip()
        if not project_name:
    ...
    def setup_api_key(self) -> bool:
        ...
        print(
            "   You can continue setup and test later with: python setup_validation.py"
        )
    ...
    def collect_use_case_info(self) -> dict[str, Any]:
        ...
        volumes = {
            ...
            "2": (1000, "Moderate usage (≤1K operations/day)"),
            "3": (10000, "Heavy usage (≤10K operations/day)"),
        ...
        complexity_preference = complexity_options.get(
            complexity_choice, ("balanced", "Balanced")
        )[0]
        print(
            f"Selected: {complexity_options.get(complexity_choice, ('balanced', 'Balanced'))[1]}"
        )
    ...
    def setup_budgets_governance(self, daily_volume: int) -> dict[str, Any]:
        ...
        # Daily budget
        daily_input = input(
            f"\nDaily budget limit (${suggested_daily_budget:.2f}): "
        ).strip()
        try:
        ...
        # Monthly budget
        monthly_input = input(
            f"Monthly budget limit (${daily_budget * 30:.2f}): "
        ).strip()
        try:
            monthly_budget = (
                float(monthly_input) if monthly_input else daily_budget * 30
            )
        except ValueError:
    ...
    def recommend_models(
        self, use_case: str, volume: int, complexity: str
    ) -> dict[str, list[str]]:
        ...
        try:
            ...
            if recommendation.recommended_model:
                ...
                print(
                    f"   Estimated cost: ${recommendation.estimated_cost:.6f} per operation"
                )
        ...
        recommended = fallback_recommendations.get(
            use_case, ["accounts/fireworks/models/llama-v3p1-8b-instruct"]
        )
        ...
        return {"recommended_models": recommended, "fallback_models": fallbacks}
    ...
    def configure_features(self, use_case: str, volume: int) -> dict[str, bool]:
        ...
        if volume >= 1000:
            enable_batch = True
            print("✅ Batch processing: Auto-enabled (recommended for your volume)")
        else:
        ...
        if use_case == "multimodal":
            ...
            print("✅ Multimodal: Auto-enabled (matches your use case)")
        else:
...
""",
        ...
        example_code = use_case_examples.get(
            config.primary_use_case, use_case_examples["chat"]
        )
...
"""
    ...
    def show_next_steps(self, config: WizardConfig, files_created: list[str]):
        ...
        if monthly_estimate < config.monthly_budget_limit:
            buffer_percentage = (
                (config.monthly_budget_limit - monthly_estimate)
