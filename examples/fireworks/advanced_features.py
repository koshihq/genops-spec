...
try:
    ...
    from genops.providers.fireworks_pricing import (
        FireworksPricingCalculator,  # noqa: F401
    )
...
def demonstrate_function_calling():
    ...
    adapter = GenOpsFireworksAdapter(
        team="function-calling-team", project="tool-usage", governance_policy="advisory"
    )
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "optimization_percentage": {
                        "type": "number",
                        "description": "Expected savings percentage",
                    },
                },
...
def demonstrate_structured_output():
    ...
    adapter = GenOpsFireworksAdapter(
        team="structured-output-team", project="json-generation"
    )
    ...
    try:
        ...
        try:
            ...
            print(
                f"      Cost Effectiveness: {analysis.get('cost_effectiveness', 'N/A')}/10"
            )
            print(
                f"      Key Advantages: {', '.join(analysis.get('key_advantages', []))}"
            )
...
async def demonstrate_async_batch_processing():
    ...
    adapter = GenOpsFireworksAdapter(
        team="async-team", project="batch-processing", governance_policy="advisory"
    )
    ...
    try:
        print(
            f"🚀 Processing {len(batch_tasks)} tasks concurrently with batch pricing:"
        )
        ...
        with adapter.track_session("async-batch-processing") as session:
            for i, (task, model) in enumerate(batch_tasks):
                ...
                print(
                    f"      ✅ Completed in {result.execution_time_seconds:.2f}s, cost: ${result.cost:.6f}"
                )
        ...
        print(
            f"   Average speed per task: {avg_speed:.2f}s (🔥 Fireattention optimized)"
        )
...
def demonstrate_complex_reasoning():
    ...
    adapter = GenOpsFireworksAdapter(team="reasoning-team", project="complex-analysis")
    ...
    for i, task_info in enumerate(reasoning_tasks, 1):
        try:
            ...
            quality_score = min(
                len(result.response.split()) / 50, 10
            )  # Up to 10 for comprehensive responses
            print(
                f"   Quality indicator: {quality_score:.1f}/10 (based on comprehensiveness)"
            )
    ...
    if reasoning_results:
        ...
        avg_cost = sum(float(r.cost) for r in reasoning_results) / len(
            reasoning_results
        )
        avg_speed = sum(r.execution_time_seconds for r in reasoning_results) / len(
            reasoning_results
        )
        ...
        print(
            f"   Words per dollar: {total_words / sum(float(r.cost) for r in reasoning_results):.0f}"
        )
