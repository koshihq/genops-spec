...
def main():
    ...
    try:
        messages = [
            ...
            {
                "role": "user",
                "content": "Explain what makes Fireworks AI unique in 2-3 sentences.",
            },
        ]
    ...
    if model_results:
        ...
        avg_time = sum(r.execution_time_seconds for r in model_results) / len(
            model_results
        )
        ...
        print(
            f"   Most cost-effective: {cheapest.model_used.split('/')[-1]} (${cheapest.cost:.6f})"
        )

        # Find fastest (should all be fast with Fireattention)
        fastest = min(model_results, key=lambda x: x.execution_time_seconds)
        print(
            f"   Fastest: {fastest.model_used.split('/')[-1]} ({fastest.execution_time_seconds:.2f}s)"
        )
    ...
    try:
        ...
        with adapter.track_session(
            ...
            creative_tasks = [
                (
                    "Write a haiku about fast AI inference",
                    FireworksModel.LLAMA_3_1_8B_INSTRUCT,
                ),
                (
                    "Create a story opening about lightning-fast robots",
                    FireworksModel.LLAMA_3_1_70B_INSTRUCT,
                ),
                (
                    "Generate creative names for a speed-focused AI company",
            ...
            print(
                f"   Average cost/operation: ${session.total_cost / len(session_results):.6f}"
            )
            print(
                f"   Average speed: {sum(r.execution_time_seconds for r in session_results) / len(session_results):.2f}s"
            )
    ...
    try:
        ...
        embedding_result = adapter.embeddings_with_governance(
            input_texts=[
                "Fast AI inference is revolutionary",
                "Fireworks AI provides 4x speed improvements",
            ],
