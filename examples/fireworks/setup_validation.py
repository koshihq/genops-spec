    ...
    try:
        ...
        if result.is_valid and result.model_access:
            ...
            comparisons = pricing_calc.compare_models(
                accessible_models, estimated_tokens=1000
            )
            ...
            for comp in comparisons:
                ...
                print(
                    f"      Cost: ${comp['estimated_cost']:.4f} ({comp['tier']} tier)"
                )
                print(f"      Context: {comp['context_length']:,} tokens")
                if comp.get("batch_cost"):
                    print(
                        f"      Batch: ${comp['batch_cost']:.4f} (saves ${comp['batch_savings']:.4f})"
                    )
                print()
            ...
            print(
                f"   Cost per operation: ${analysis['cost_analysis']['cost_per_operation']:.4f}"
            )
            print(
                f"   Batch savings: ${analysis['optimization']['batch_optimization_potential']:.2f}"
            )
            ...
            if analysis["optimization"]["best_alternative"]:
                ...
                print(
                    f"   Potential monthly savings: ${analysis['optimization']['potential_monthly_savings']:.2f}"
                )
