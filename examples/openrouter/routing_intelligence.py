...
class ProviderMetrics:
    ...
    def get_health_score(self) -> float:
        ...
        if self.last_failure is None:
            ...
            availability_component = min(1.0, minutes_since_failure / 60)  # Full recovery after 1 hour
...
class IntelligentRouter:
    ...
    def select_optimal_model(
        ...
        for model in available_models:
            ...
            if routing_strategy == "performance":
                score = self._calculate_performance_score(model, provider, metrics, task_requirements)
            elif routing_strategy == "cost":
                score = self._calculate_cost_score(model, provider, metrics, task_requirements)
            elif routing_strategy == "reliability":
                score = self._calculate_reliability_score(model, provider, metrics, task_requirements)
            else:  # balanced
                score = self._calculate_balanced_score(model, provider, metrics, task_requirements)
    ...
    def _calculate_performance_score(self, model: str, provider: str, metrics: ProviderMetrics, requirements: Dict) -> float:
    ...
    def _calculate_cost_score(self, model: str, provider: str, metrics: ProviderMetrics, requirements: Dict) -> float:
        """Calculate cost-optimized score.""" 
        health_score = metrics.get_health_score()
        
        # Simple cost preference (lower cost = higher score)
        cost_bonus = max(0, 0.01 - metrics.avg_cost) / 0.01 if metrics.avg_cost > 0 else 0.5
        
        return (health_score * 0.4) + (cost_bonus * 0.6)
    
    def _calculate_reliability_score(self, model: str, provider: str, metrics: ProviderMetrics, requirements: Dict) -> float:
        """Calculate reliability-optimized score."""
        health_score = metrics.get_health_score()
        
        # Heavy weight on success rate and availability
        success_bonus = metrics.success_rate
        availability_bonus = 1.0 if metrics.last_failure is None else max(0, (time.time() - metrics.last_failure) / 3600)
        
        return (health_score * 0.3) + (success_bonus * 0.4) + (availability_bonus * 0.3)
    
    def _calculate_balanced_score(self, model: str, provider: str, metrics: ProviderMetrics, requirements: Dict) -> float:
    ...
    try:
        ...
        task_scenarios = [
            ...
            },
            {
                ...
                "messages": [{"role": "user", "content": "Explain the differences between transformer architectures and RNNs for sequence modeling."}],
                "requirements": {
                    "complexity": "complex",
                    "budget_limit": 0.05,
                    "max_latency": 10.0,
            ...
            },
            {
                ...
                "messages": [{"role": "user", "content": "I need help with my account billing issue."}],
                "requirements": {
                    "complexity": "medium",
                    "budget_limit": 0.01,
                    "max_latency": 3.0,
            ...
            },
            {
                ...
                "messages": [{"role": "user", "content": "Urgent: System status check needed."}],
                "requirements": {
                    "complexity": "simple", 
                    "budget_limit": 0.1,
                    "max_latency": 1.0,
        ...
        for provider_name, metrics in router.provider_metrics.items():
            ...
            print(f"      • {provider_name}: {health:.2f} ({metrics.request_count} requests)")
        ...
        for load_scenario in load_test_scenarios:
            ...
            avg_latency = statistics.mean([r["routing_info"]["latency"] for r in successful_requests]) if successful_requests else 0
        ...
        if routing_results:
            ...
            for strategy, results in strategies_analysis.items():
                if results:
                    ...
                    print(f"        Models used: {list(set([r['selected_model'] for r in results]))}")
