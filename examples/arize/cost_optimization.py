...
def demonstrate_cost_aggregation():
    ...
    try:
        ...
        for model_id, version, environment, predictions, quality_checks, alerts in models_config:
            ...
            print(f"       - Predictions: {predictions:,} (${session_cost.prediction_logging_cost:.2f})")
            print(f"       - Data Quality: {quality_checks} checks (${session_cost.data_quality_cost:.2f})")
            print(f"       - Alerts: {alerts} active (${session_cost.alert_management_cost:.2f})")
            print(f"       - Efficiency: {session_cost.efficiency_score:.1f} pred/hour")
        
        print(f"\n  💰 Cost Summary:")
        print(f"     • Total Monthly Cost: ${total_cost:.2f}")
        print(f"     • Budget Utilization: {(total_cost / cost_aggregator.budget_limit) * 100:.1f}%")
        ...
        for env, cost in cost_by_environment.items():
            print(f"     • {env.capitalize()}: ${cost:.2f} ({(cost/total_cost)*100:.1f}%)")
...
def demonstrate_cost_optimization_recommendations(cost_aggregator):
    ...
    try:
        ...
        print(f"     • Optimized Estimate: ${monthly_summary.total_cost - total_potential_savings:.2f}")
        print(f"     • Savings Percentage: {(total_potential_savings / monthly_summary.total_cost) * 100:.1f}%")
...
def demonstrate_pricing_optimization():
    ...
    try:
        ...
        print(f"     • Predictions per Model: {usage_scenario['predictions_per_model']:,}")
        print(f"     • Total Monthly Predictions: {usage_scenario['models'] * usage_scenario['predictions_per_model']:,}")
        print(f"     • Quality Checks per Model: {usage_scenario['quality_checks_per_model']}")
        ...
        tiers_to_compare = [PricingTier.STARTER, PricingTier.PROFESSIONAL, PricingTier.ENTERPRISE]
        tier_costs = {}
        ...
        for tier in tiers_to_compare:
            ...
            print(f"     • {tier.value.capitalize()}: ${estimate.total_estimated_cost:.2f}")
            print(f"       - Recommended: {'✅' if estimate.recommended_tier == tier else '❌'}")
        ...
        if savings > 0:
            print(f"     • Savings vs Starter: ${savings:.2f} ({(savings/starter_cost)*100:.1f}%)")
        ...
        for volume in volume_levels:
            pricing_breakdown = enterprise_calculator.calculate_prediction_logging_cost(
                prediction_count=volume,
                time_period_days=30
            )
            
            discount_tier = enterprise_calculator.get_volume_discount_tier(volume)
            
            print(f"     {volume:,} → ${pricing_breakdown.effective_rate:.6f} → {discount_tier.tier_name} ({discount_tier.discount_percentage:.0f}%)")
...
def demonstrate_cost_forecasting(cost_aggregator):
    ...
    try:
        ...
        print(f"     • Confidence Range: ${forecast.confidence_interval[0]:.2f} - ${forecast.confidence_interval[1]:.2f}")
        ...
        for i, month in enumerate(historical_months):
            ...
            if i == 0:
                ...
                prev_cost = base_cost * (1 + (i-1) * 0.08 + 0.02)  # Approximate previous
        ...
        print(f"     • Recommended Q4 Budget: ${current_monthly * 3 * 1.15:.2f} (+15% buffer)")
        print(f"     • Annual Budget Estimate: ${current_monthly * 12 * 1.20:.2f} (+20% growth)")
...
def demonstrate_dynamic_cost_monitoring():
    ...
    try:
        ...
        for period, usage, strategy in scenarios:
            ...
            print(f"     {period:<15}   ${usage:>5.1f}    {budget_pct:>6.1f}%    {sampling_color} {sampling_rate}")
...
def demonstrate_enterprise_cost_patterns():
    ...
    for team, models, predictions, cost in teams_config:
        ...
        print(f"     {team:<12}  {models:>6}    {predictions:>11,}    ${cost:>10.2f}    {pct_total:>8.1f}%")
    
    print(f"     {'TOTAL':<12}  {sum(m for _, m, _, _ in teams_config):>6}    {sum(p for _, _, p, _ in teams_config):>11,}    ${total_cost:>10.2f}    {'100.0':>8}%")
    ...
    print(f"     • Quarterly Review: Adjust allocations based on usage trends")
    print(f"     • Annual Planning: Forecast growth and optimization opportunities")
    print(f"     • Cost Controls: Automated alerts at 80% budget utilization")
    print(f"     • Chargeback Model: Teams charged based on actual usage")
