...
def check_advanced_prerequisites():
    ...
    try:
        ...
        from genops.providers.arize import (  # noqa: F401
            GenOpsArizeAdapter,
            auto_instrument,
        )
        from genops.providers.arize_cost_aggregator import (
            ArizeCostAggregator,  # noqa: F401
        )
        from genops.providers.arize_pricing import ArizePricingCalculator  # noqa: F401
        from genops.providers.arize_validation import ArizeSetupValidator  # noqa: F401
...
def create_sample_model_data():
    ...
    for model_id, config in models.items():
        ...
        for i in range(min(1000, config["volume"])):  # Sample of actual volume
            prediction = {
                ...
                if "fraud" in model_id or "churn" in model_id
    ...
    print(
        f"  📈 Total sample predictions: {sum(len(data['predictions']) for data in model_data.values())}"
    )
    print()
...
def demonstrate_multi_model_monitoring(model_data):
    ...
    cost_aggregator = ArizeCostAggregator(  # noqa: F821
        ...
        try:
            ...
            with adapter.track_model_monitoring_session(
                ...
                session.log_prediction_batch(
                    predictions_df, cost_per_prediction=cost_per_prediction
                )
            ...
            cost_aggregator.add_cost_record(
                model_id=model_id,
                environment=model_info["config"]["environment"],
                prediction_logging_cost=session_results["prediction_count"]
                * cost_per_prediction,
            ...
            print(
                f"  ✅ {model_id}: ${session_results['total_cost']:.3f} cost, {session_results['active_alerts']} alerts"
            )
...
def demonstrate_cost_intelligence(cost_aggregator):
    ...
    print(
        f"  🔍 Cost per data quality check: ${efficiency.cost_per_data_quality_check:.3f}"
    )
    ...
    if efficiency.model_efficiency_scores:
        sorted_models = sorted(
            efficiency.model_efficiency_scores.items(), key=lambda x: x[1], reverse=True
        )
...
def demonstrate_dynamic_budget_management():
    ...
    for scenario in budget_scenarios:
        ...
        if total_estimated_cost <= scenario["max_session"]:
            ...
            recommendation = (
                "Implement sampling strategy or defer non-critical monitoring"
            )
...
def demonstrate_enterprise_governance():
    ...
    for tenant in tenants:
        ...
        print(
            f"  👤 {tenant['customer_id']}: {tier} tier, {sla} SLA, ${tenant['budget']} budget"
        )
...
def main():
    ...
    try:
        ...
        if model_results:
            ...
            print(
                "❌ Multi-model monitoring demo failed. Check configuration and try again."
            )
