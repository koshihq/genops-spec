    ...
    for var in required_env_vars:
        if os.getenv(var):
            ...
            missing_requirements.append(
                f"export {var}='your-{var.lower().replace('_', '-')}'"
            )
...
def demonstrate_basic_monitoring():
    ...
    try:
        ...
        with adapter.track_model_monitoring_session(
            model_id=model_id, model_version=model_version, environment="development"
        ) as session:
            ...
            quality_metrics = {
                "missing_values_rate": predictions_df.isnull().sum().sum()
                / (len(predictions_df) * len(predictions_df.columns)),
                "duplicate_rate": predictions_df.duplicated().sum()
                / len(predictions_df),
                "fraud_rate": (predictions_df["prediction_label"] == "fraud").mean(),
                "accuracy": (
                    predictions_df["prediction_label"] == predictions_df["actual_label"]
                ).mean(),
            ...
            session.create_performance_alert(
                metric="accuracy", threshold=0.85, cost_per_alert=0.10
            )
        ...
        if session_cost:
            ...
            print(
                f"     • Prediction Logging: ${session_cost.prediction_logging_cost:.2f}"
            )
            ...
            print(
                f"     • Efficiency: {session_cost.efficiency_score:.2f} predictions/hour"
            )
...
def demonstrate_manual_arize_integration():
    ...
    try:
        ...
        # Create Arize client
        Client(
            api_key=os.getenv("ARIZE_API_KEY"), space_key=os.getenv("ARIZE_SPACE_KEY")
        )
        ...
        sample_predictions = [
            {
                ...
                "model_version": "1.0",
                "features": {"amount": 100.0 + i * 50, "merchant": "online_store"},
            }
            for i in range(5)
        ]

        print(
            f"  📊 Prepared {len(sample_predictions)} sample predictions for manual logging"
        )
        ...
        with adapter.track_model_monitoring_session("manual-logging") as session:
            for pred in sample_predictions:
                ...
                print(
                    f"     • Logged prediction: {pred['prediction_id']} -> {pred['prediction_label']}"
                )
...
def main():
    ...
    if not check_prerequisites():
        print(
            "❌ Prerequisites not met. Please install dependencies and set environment variables."
        )
        return 1
    ...
    if monitoring_success:
        ...
        demonstrate_manual_arize_integration()
