...
def check_python_version() -> Dict[str, Any]:
    ...
    return {
        ...
        "version": version_str,
        "required": "3.9+",
        "message": f"Python {version_str}" + (" (compatible)" if is_compatible else " (too old)"),
        "recommendation": "Upgrade to Python 3.9 or higher" if not is_compatible else None
    }

def check_package_installation(package_name: str, import_name: Optional[str] = None) -> Dict[str, Any]:
...
def check_griptape_structures() -> Dict[str, Any]:
    ...
    try:
        ...
        # Test Pipeline
        try:
            pipeline_test = Pipeline.__name__
        ...
        # Test Workflow
        try:
            workflow_test = Workflow.__name__
...
def check_genops_griptape_provider() -> Dict[str, Any]:
    ...
    try:
        from genops.providers.griptape import auto_instrument, GenOpsGriptapeAdapter
        
        # Test adapter creation (without actual instrumentation)
        try:
            adapter = GenOpsGriptapeAdapter(team="test", project="validation")
...
def check_environment_variables() -> Dict[str, Any]:
    ...
    return {
        ...
        "set_vars": set_vars,
        "message": f"Set variables: {list(set_vars.keys())}" if set_vars else "No GenOps variables set",
        "recommendation": "Set recommended variables: export GENOPS_TEAM='your-team' GENOPS_PROJECT='your-project'" if not has_recommended else None
    }
...
def test_basic_instrumentation() -> Dict[str, Any]:
    ...
    try:
        ...
        from genops.providers.griptape.registration import is_instrumented, disable_auto_instrument
...
def print_validation_results(checks: List[Dict[str, Any]]) -> None:
    ...
    # Recommendations
    recommendations = [check.get("recommendation") for check in checks if not check["passed"] and check.get("recommendation")]
...
def main():
    ...
    try:
        ...
        critical_failures = sum(1 for check in checks if not check["passed"] and check["name"] in [
            "Python Version", "Genops Package", "Griptape Package", "GenOps Griptape Provider"
        ])
...
if __name__ == "__main__":
    ...
    if not success:
        print("\n🔧 For more help:")
        print("  • Check the troubleshooting guide: docs/integrations/griptape.md#troubleshooting")
