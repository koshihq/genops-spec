...
class EnterpriseGovernanceManager:
    ...
    def register_tenant(self, config: TenantConfiguration) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Registered tenant {config.tenant_id} with {config.compliance_level.value} compliance"
            )
            return True
    ...
    def validate_operation(
        self, tenant_id: str, operation: str, estimated_cost: float
    ) -> dict[str, Any]:
        ...
        if config.compliance_level in [ComplianceLevel.HIPAA, ComplianceLevel.GDPR]:
            validation_result["warnings"].append(
                "PII data handling compliance required"
            )
    ...
    def log_operation(
        ...
        audit_entry = AuditLogEntry(
            ...
            data_classification=metadata.get("data_classification", "standard")
            if metadata
            else "standard",
    ...
    def generate_compliance_report(self, tenant_id: str) -> dict[str, Any]:
        ...
        tenant_violations = [
            v for v in self.sla_violations if v["tenant_id"] == tenant_id
        ]
        ...
        report = {
            ...
            "budget_compliance": {
                ...
                "utilization_percentage": (
                    self.get_tenant_cost_usage(tenant_id) / config.daily_budget_limit
                )
                * 100,
            },
            "sla_compliance": {
                ...
                "average_response_time": sum(
                    log.metadata.get("response_time", 0) for log in tenant_logs
                )
            ...
            "audit_trail": {
                ...
                "sensitive_operations": len(
                    [
                        ...
                        if log.data_classification == "sensitive"
                    ]
                ),
                "last_sensitive_access": max(
                    [
                        log.timestamp
                        for log in tenant_logs
                        if log.data_classification == "sensitive"
            ...
            "recommendations": self.generate_recommendations(
                tenant_id, tenant_logs, tenant_violations
            ),
        }
    ...
    def generate_recommendations(
        self, tenant_id: str, logs: list[AuditLogEntry], violations: list[dict]
    ) -> list[str]:
        ...
        if len(violations) > 0:
            recommendations.append(
                "Consider upgrading SLA tier or optimizing pipeline performance"
            )
        ...
        if len(logs) > 100:
            ...
            if avg_cost > 0.01:
                recommendations.append(
                    "Review cost optimization opportunities - high average cost per operation"
                )

        sensitive_ops = [log for log in logs if log.data_classification == "sensitive"]
        if len(sensitive_ops) > 10:
            recommendations.append(
                "Consider additional security controls for sensitive data operations"
            )
...
def demo_multi_tenant_operations():
    ...
    for tenant_ops in tenant_operations:
        ...
        with adapter.track_session(
            f"tenant-{tenant_id}-operations", use_case="multi-tenant-enterprise"
        ) as session:
            for i, request in enumerate(tenant_ops["requests"], 1):
                ...
                validation = governance_manager.validate_operation(
                    tenant_id, "generation", estimated_cost
                )

                if not validation["allowed"]:
                    print(
                        f"         ❌ Operation denied: {validation.get('reason', 'Unknown')}"
                    )
                    continue
                ...
                with adapter.track_pipeline(
                    ...
                    data_classification=validation.get("metadata", {}).get(
                    ...
                    # Execute pipeline
                    result = pipeline.run({"prompt_builder": {"request": request}})

                    operation_time = time.time() - start_time
                    response = result["llm"]["replies"][0]

                    # Check SLA compliance
                    sla_compliant = governance_manager.check_sla_compliance(
                        tenant_id, operation_time
                    )
                    ...
                    print(
                        f"         ⏱️ Time: {operation_time:.2f}s {'✅' if sla_compliant else '❌'}"
                    )
            ...
            print(
                f"         Budget utilization: {(session.total_cost / tenant_config.daily_budget_limit * 100):.1f}%"
            )
...
def demo_compliance_reporting(governance_manager: EnterpriseGovernanceManager):
    ...
    for tenant_id in governance_manager.tenants.keys():
        ...
        print(
            f"      📈 Operations: {report['operations_summary']['total_operations']}"
        )
        print(f"      💰 Total Cost: ${report['operations_summary']['total_cost']:.6f}")
        print(
            f"      📊 Budget Utilization: {report['budget_compliance']['utilization_percentage']:.1f}%"
        )
        print(
            f"      ⚡ SLA Violations: {report['sla_compliance']['total_violations']}"
        )
        print(
            f"      🔒 Sensitive Operations: {report['audit_trail']['sensitive_operations']}"
        )
    ...
    print(
        f"   Violation rate: {(total_violations / max(total_operations, 1) * 100):.2f}%"
    )
