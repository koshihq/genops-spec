...
def demo_rag_governance():
    ...
    with rag_adapter.track_session(
        "rag-qa-session", use_case="knowledge-base-qa"
    ) as session:
        ...
        print(
            f"   Average cost per query: ${session.total_cost / session.total_pipelines:.6f}"
        )
...
def analyze_rag_performance(rag_adapter, session_results):
    ...
    if session_results:
        ...
        avg_response_time = sum(r["time"] for r in session_results) / len(
            session_results
        )
    ...
    for i, result in enumerate(session_results[:3], 1):  # Show first 3 for brevity
        if hasattr(rag_adapter, "monitor"):
            ...
            print(
                f"     Cost efficiency: ${result['cost'] / result['docs_retrieved']:.6f} per document"
            )
...
def demo_advanced_rag_features(rag_adapter):
    ...
    with rag_adapter.track_session(
        "multi-turn-conversation", use_case="conversational-rag"
    ) as session:
        for i, question in enumerate(follow_up_questions, 1):
            ...
            with rag_adapter.track_pipeline(
                ...
                conversation_history.append(
                    {"question": question, "answer": answer, "turn": i}
                )

                metrics = context.get_metrics()
                print(
                    f"        Cost: ${metrics.total_cost:.6f} | Time: {metrics.total_execution_time_seconds:.2f}s"
                )
        ...
        print(f"     Total cost: ${session.total_cost:.6f}")
        print(
            f"     Average cost per turn: ${session.total_cost / session.total_pipelines:.6f}"
        )
    ...
    with rag_adapter.track_session(
        "batch-rag-processing", use_case="batch-qa"
    ) as batch_session:
        batch_results = []

        for i, question in enumerate(batch_questions, 1):
            with rag_adapter.track_pipeline(
                f"batch-query-{i}", batch_position=i
            ) as context:
                temp_store = setup_knowledge_base()
                temp_pipeline = create_rag_pipeline(temp_store)
        ...
        print(
            f"   Efficiency: ${batch_session.total_cost / len(batch_questions):.6f} per question"
        )
