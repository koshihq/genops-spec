...
class IntelligentCache:
    ...
    def __init__(self, max_size: int = 1000, default_ttl: int = 3600):
        ...
        self.stats = {"hits": 0, "misses": 0, "evictions": 0, "expires": 0}
    ...
    def put(self, key: str, value: Any, ttl: Optional[int] = None) -> None:
        ...
        with self.lock:
            ...
            entry = CacheEntry(value=value, timestamp=datetime.now(), ttl_seconds=ttl)
...
class BatchProcessor:
    ...
    def __init__(
        self, batch_size: int = 10, batch_timeout: float = 1.0, max_workers: int = 4
    ):
...
class OptimizedPipelineManager:
    ...
    def initialize_pipelines(self):
        ...
        fast_pipeline.add_component(
            "prompt_builder",
            PromptBuilder(template="Provide a concise answer: {{query}}"),
        )
        fast_pipeline.add_component(
            "llm",
            OpenAIGenerator(
        ...
        balanced_pipeline.add_component(
            "prompt_builder",
            PromptBuilder(template="Provide a detailed and accurate answer: {{query}}"),
        )
        balanced_pipeline.add_component(
            "llm",
            OpenAIGenerator(
        ...
        quality_pipeline.add_component(
            "prompt_builder",
            PromptBuilder(
                template="""
    ...
    def process_query_cached(
        self, query: str, pipeline_name: str, request_id: str
    ) -> dict[str, Any]:
    ...
    def process_query(
        self, query: str, priority: str = "balanced", request_id: Optional[str] = None
    ) -> dict[str, Any]:
...
def demo_caching_optimization():
    ...
    test_queries = [
        ...
        {
            "query": "Provide a comprehensive analysis of deep learning architectures including CNNs, RNNs, and Transformers",
            "priority": "quality",
            "repeat": 2,
        },
        {"query": "How do neural networks work?", "priority": "speed", "repeat": 4},
        {
            "query": "What are the best practices for MLOps?",
            "priority": "balanced",
            "repeat": 2,
        },
    ]
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for test_case in test_queries:
            ...
            for _i in range(repeat_count):
                ...
                future = executor.submit(
                    pipeline_manager.process_query, query, priority, request_id
                )
        ...
        for request_id, query_preview, future in futures:
            try:
                ...
                print(
                    f"   {cached_indicator} {request_id}: {query_preview} ({processing_time:.3f}s)"
                )
    ...
    print(
        f"   Average Response Time: {metrics['processing_metrics']['average_response_time']:.3f}s"
    )
    print(
        f"   P95 Response Time: {metrics['processing_metrics']['p95_response_time']:.3f}s"
    )
    print(
        f"   Estimated Time Saved: {metrics['optimization_impact']['time_saved_seconds']:.1f}s"
    )
    print(
        f"   Cache Utilization: {metrics['cache_performance']['cache_utilization']:.1f}%"
    )
...
def demo_parallel_processing():
    ...
    adapter = GenOpsHaystackAdapter(
        team="parallel-processing", project="concurrency-demo", daily_budget_limit=150.0
    )
    ...
    pipeline.add_component(
        "prompt_builder",
        PromptBuilder(template="Answer this question concisely: {{question}}"),
    )
    pipeline.add_component(
        "llm",
        OpenAIGenerator(
    ...
    with adapter.track_session(
        "sequential-processing", use_case="performance-comparison"
    ) as seq_session:
    ...
    with adapter.track_session(
        "parallel-processing", use_case="performance-comparison"
    ):
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {}
    ...
    print(
        f"   Average per query: {parallel_time / len(parallel_results):.2f}s"
        if parallel_results
        else "N/A"
    )
    ...
    if sequential_time > 0:
        ...
        print(
            f"   Time saved: {sequential_time - parallel_time:.2f}s ({((sequential_time - parallel_time) / sequential_time * 100):.1f}%)"
        )
...
def demo_pipeline_optimization():
    ...
    with adapter.track_session(
        "model-optimization", use_case="configuration-testing"
    ) as session:
        for config in model_configs:
            ...
            pipeline.add_component(
                "llm",
                OpenAIGenerator(
                    model=config["model"], generation_kwargs=config["params"]
                ),
            )
            pipeline.connect("prompt_builder", "llm")
        ...
        print("   📊 Model Optimization Session:")
    ...
    prompt_variations = [
        ...
        {"name": "Concise", "template": "Explain {{topic}} concisely:"},
        {
            "name": "Structured",
            "template": """
            Topic: {{topic}}
    ...
    for prompt_config in prompt_variations:
        pipeline = Pipeline()
        pipeline.add_component(
            "prompt_builder", PromptBuilder(template=prompt_config["template"])
        )
        pipeline.add_component(
            "llm",
            OpenAIGenerator(
        ...
        print(
            f"         Efficiency ratio: {response_length / processing_time:.1f} chars/sec"
        )
...
def demo_load_testing():
    ...
    adapter = GenOpsHaystackAdapter(
        team="load-testing", project="capacity-planning", daily_budget_limit=200.0
    )

    # Create simple pipeline for load testing
    pipeline = Pipeline()
    pipeline.add_component(
        "prompt_builder", PromptBuilder(template="Answer briefly: {{question}}")
    )
    pipeline.add_component(
        "llm",
        OpenAIGenerator(
    ...
    for scenario in load_scenarios:
        ...
        with adapter.track_session(
            f"load-test-{scenario['name'].lower().replace(' ', '-')}",
            ...
            with ThreadPoolExecutor(
                ...
                for user in range(scenario["concurrent_users"]):
                    for req in range(scenario["requests_per_user"]):
                        ...
                        future = executor.submit(
                            lambda q, rid: self._execute_load_test_request(  # noqa: F821
                                pipeline, q, rid
                            ),
                            question,
                            request_id,
            ...
            print(
                f"         Successful requests: {successful_requests}/{total_requests}"
            )
...
def _execute_load_test_request(
    pipeline, question: str, request_id: str
) -> dict[str, Any]:
    """Execute individual load test request."""
    start_time = time.time()
    try:
        pipeline.run({"prompt_builder": {"question": question}})
