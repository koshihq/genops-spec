...
try:
    ...
    from haystack.components.preprocessors import (  # noqa: F401
        DocumentCleaner,
        DocumentSplitter,
    )
    from haystack.components.writers import DocumentWriter  # noqa: F401
...
try:
    from genops.providers.haystack import (
        GenOpsHaystackAdapter,  # noqa: F401
        analyze_pipeline_costs,
        create_agent_adapter,
        get_agent_insights,  # noqa: F401
...
class AgentToolSimulator:
    ...
    def use_tool(
        self, tool_name: str, input_data: str, complexity: str = "medium"
    ) -> dict[str, Any]:
    ...
    def generate_tool_result(self, tool_name: str, input_data: str) -> str:
        ...
        results = {
            ...
            "document_analysis": "Analyzed document content. Key themes: machine learning, AI governance, cost optimization. Confidence: 0.87",
            "data_extraction": "Extracted 42 data points from source material. Structured format available.",
        ...
        return results.get(
            tool_name,
            f"Tool {tool_name} executed successfully with input: {input_data[:50]}...",
        )
...
def demo_agent_workflow_tracking():
    ...
    with agent_adapter.track_session(
        "agent-research-session", use_case="multi-agent-workflow"
    ) as session:
        ...
        for task_num, task_config in enumerate(agent_tasks, 1):
            ...
            with agent_adapter.track_pipeline(
                ...
                while (
                    not task_state["completed"]
                    and task_state["iterations"] < max_iterations
                ):
                    ...
                    decision_result = decision_pipeline.run(
                        {
                            "decision_maker": {
                                ...
                                "previous_results": [
                                    r["result"] for r in task_state["tool_results"][-2:]
                                ],
                            }
                        }
                    )
                    ...
                    if (
                        "COMPLETE" in decision_text.upper()
                        or "complete" in decision_text.lower()
                    ):
                    ...
                    # Use selected tool
                    tool_input = (
                        f"Process: {task_description[:50]}... (iteration {iteration})"
                    )
                    tool_result = tool_simulator.use_tool(
                        selected_tool, tool_input, task_config["complexity"]
                    )
                    ...
                    context.add_custom_metric(
                        f"iteration_{iteration}_cost", tool_result["cost"]
                    )
                    context.add_custom_metric(
                ...
                success_rate = sum(
                    1 for r in task_state["tool_results"] if r["success"]
                ) / max(total_tools_used, 1)
                ...
                session_results.append(
                    {
                        ...
                        "tool_breakdown": {
                            r["tool_name"]: r["cost"]
                            for r in task_state["tool_results"]
                        },
        ...
        print(
            f"   Average cost per task: ${session.total_cost / session.total_pipelines:.6f}"
        )
...
def analyze_agent_performance(agent_adapter, session_results):
    ...
    if session_results:
        ...
        avg_success_rate = sum(r["success_rate"] for r in session_results) / len(
            session_results
        )
        ...
        print(
            f"   Task completion rate: {completed_tasks}/{len(session_results)} ({completed_tasks / len(session_results):.1%})"
        )
        print(
            f"   Average iterations per task: {total_iterations / len(session_results):.1f}"
        )
        ...
        for result in session_results:
            ...
            complexity_performance[complexity]["iterations"].append(
                result["iterations"]
            )
            complexity_performance[complexity]["tools"].append(result["tools_used"])
            complexity_performance[complexity]["success_rates"].append(
                result["success_rate"]
            )
            complexity_performance[complexity]["completion_rates"].append(
        ...
        for complexity, perf_data in complexity_performance.items():
            ...
            avg_success = sum(perf_data["success_rates"]) / len(
                perf_data["success_rates"]
            )
            completion_rate = sum(perf_data["completion_rates"]) / len(
                perf_data["completion_rates"]
            )
        ...
        for tool_name, usage_data in sorted(
            all_tools_used.items(), key=lambda x: x[1]["total_cost"], reverse=True
        ):
    ...
    for i, result in enumerate(session_results, 1):
        ...
        print(
            f"     Decision-making efficiency: {result['tools_used'] / result['iterations']:.1f} tools per iteration"
        )
        print(
            f"     Cost efficiency: ${result['cost'] / result['tools_used']:.6f} per tool"
        )
        print(
            f"     Time efficiency: {result['time'] / result['iterations']:.2f}s per iteration"
        )
...
def demo_multi_agent_coordination():
    ...
    coordinator_adapter = create_agent_adapter(
        team="agent-coordination", project="multi-agent-system", daily_budget_limit=75.0
    )
    ...
    with coordinator_adapter.track_session(
        "multi-agent-coordination", use_case="collaborative-research"
    ) as session:
        agent_results = {}
        ...
        for i, agent_config in enumerate(coordination_tasks, 1):
            ...
            with coordinator_adapter.track_pipeline(
                ...
                agent_pipeline.add_component(
                    "llm",
                    OpenAIGenerator(
                        model="gpt-3.5-turbo", generation_kwargs={"max_tokens": 150}
                    ),
                )
        ...
        print(
            f"   Average cost per agent: ${session.total_cost / session.total_pipelines:.6f}"
        )
