    ...
    try:
        ...
        with cohere_workflow_context(
            ...
            query_embedding = ctx.embed(
                texts=[query], model="embed-english-v4.0", input_type="search_query"
            )

            if query_embedding.success:
                print(
                    f"✅ Query embedding created: ${query_embedding.usage.total_cost:.6f}"
                )
            ...
            if doc_embeddings.success:
                print(
                    f"✅ Document embeddings created: ${doc_embeddings.usage.total_cost:.6f}"
                )
                print(f"   Documents processed: {len(documents)}")
                print(
                    f"   Cost per document: ${doc_embeddings.usage.total_cost / len(documents):.6f}"
                )

            # Step 3: Rerank documents by relevance
            print("\n🔍 Step 3: Reranking documents by relevance...")
            rerank_result = ctx.rerank(
                query=query, documents=documents, model="rerank-english-v3.0", top_n=3
            )

            if rerank_result.success:
                print(
                    f"✅ Document reranking completed: ${rerank_result.usage.total_cost:.6f}"
                )
            ...
            if summary_result.success:
                ...
                print(
                    f"   Generation speed: {summary_result.usage.tokens_per_second:.1f} tokens/sec"
                )
            ...
            print(
                f"   Average Cost/Operation: ${ctx.get_total_cost() / ctx.get_operation_count():.6f}"
            )
...
def cost_optimization_workflow():
    ...
    try:
        ...
        documents = [
            "AI diagnostic tools improve accuracy",
            "Machine learning predicts treatment outcomes",
        ]
        ...
        for strategy in strategies:
            ...
            with cohere_workflow_context(
                ...
                ctx.embed(texts=documents, model=strategy["embed_model"])

                ctx.rerank(
                    query=task, documents=documents, model=strategy["rerank_model"]
                )
                ...
                strategy_results.append(
                    {
                        ...
                        "cost_per_operation": ctx.get_total_cost()
                        / ctx.get_operation_count(),
                        "breakdown": ctx.get_cost_breakdown(),
                        "quality_score": len(chat_result.content)
                        if chat_result.success
                        else 0,  # Simple quality metric
                    }
                )

                print(f"   Total Cost: ${ctx.get_total_cost():.6f}")
                print(
                    f"   Cost/Operation: ${ctx.get_total_cost() / ctx.get_operation_count():.6f}"
                )
        ...
        for result in strategy_results:
            print(
                f"{result['strategy']:<20} ${result['total_cost']:<11.6f} ${result['cost_per_operation']:<11.6f} {result['quality_score']:<10}"
            )

        # Find best value
        best_value = min(
            strategy_results, key=lambda x: x["total_cost"] / max(x["quality_score"], 1)
        )
        print(f"\n🏆 Best Value Strategy: {best_value['strategy']}")
        print(f"   Cost: ${best_value['total_cost']:.6f}")
        print(
            f"   Cost Efficiency: ${best_value['total_cost'] / max(best_value['quality_score'], 1):.6f} per quality unit"
        )
...
def error_handling_workflow():
    ...
    try:
        ...
        with cohere_workflow_context(
            "error_handling_test", team="testing", project="error-scenarios"
        ) as (ctx, workflow_id):
            # Valid operation
            print("   ✅ Executing valid operation...")
            valid_result = ctx.chat(message="Test message", model="command-light")
            ...
            try:
                invalid_result = ctx.chat(
                    message="Test with invalid model", model="non-existent-model"
                )

                if not invalid_result.success:
                    print(
                        f"   ✅ Error handled gracefully: {invalid_result.error_message[:50]}..."
                    )
