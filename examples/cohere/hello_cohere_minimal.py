...
def main():
    ...
    try:
        ...
        if quick_validate():
            ...
            print(
                "   3. Test connection: python -c \"import cohere; client = cohere.ClientV2(); print('OK')\""
            )
            return False
    ...
    try:
        ...
        adapter = instrument_cohere(team="quickstart-demo", project="30-second-test")
    ...
    try:
        ...
        if response.usage:
            print(
                f"   🔢 Tokens: {response.usage.input_tokens} in + {response.usage.output_tokens} out = {response.usage.total_tokens} total"
            )
            print(f"   💰 Cost: ${response.usage.total_cost:.6f}")
            if response.usage.tokens_per_second > 0:
                print(
                    f"   ⚡ Speed: {response.usage.tokens_per_second:.1f} tokens/second"
                )
    ...
    try:
        ...
        if embed_response.usage:
            print(
                f"✅ Embedding successful: ${embed_response.usage.total_cost:.6f} cost"
            )
    ...
    try:
        ...
        if summary:
            ...
            print(
                f"      Average Cost/Operation: ${summary.get('average_cost_per_operation', 0):.6f}"
            )

            if summary.get("budget_limit"):
                utilization = (
                    summary.get("total_cost", 0) / summary["budget_limit"]
                ) * 100
...
if __name__ == "__main__":
    try:
        ...
        print(
            '   3. Run diagnostics: python -c "from genops.providers.cohere_validation import validate_setup, print_validation_result; print_validation_result(validate_setup(), detailed=True)"'
        )
