...
def demonstrate_kubernetes_workflow():
    ...
    try:
        from genops.providers.huggingface import (
            ...
            create_huggingface_cost_context,  # noqa: F401
        ...
        with production_workflow_context(
            ...
            for i, task in enumerate(k8s_ai_tasks, 1):
                workflow.record_step(
                    f"k8s_task_{i}_{task['name']}_start",
                    {
                ...
                try:
                    ...
                    if task.get("task_type") == "embedding":
                        ...
                        print(
                            f"✅ Task {i}: Generated embeddings for {len(task['inputs'])} K8s policies ({execution_time:.2f}s)"
                        )
                        ...
                        execution_time = time.time() - start_time
                        print(
                            f"✅ Task {i}: {task['description']} completed ({execution_time:.2f}s)"
                        )

                    workflow.record_step(
                        f"k8s_task_{i}_{task['name']}_complete",
                        {
                    ...
                    workflow.record_performance_metric(
                        f"task_{i}_k8s_latency", execution_time, "seconds"
                    )
                    workflow.record_performance_metric(
                        f"task_{i}_pod_memory", 75.0, "percentage"
                    )  # Mock metric
                    workflow.record_performance_metric(
                        f"task_{i}_pod_cpu", 40.0, "percentage"
                    )  # Mock metric
                    ...
                    workflow.record_alert(
                        f"k8s_task_{task['name']}_error", str(e), "error"
                    )
                    workflow.record_step(
                        f"k8s_task_{i}_{task['name']}_failed",
                        {
                            "error": str(e),
                            "execution_time": execution_time,
                            "success": False,
                ...
                workflow.record_performance_metric(
                    f"pod_memory_usage_after_task_{i}", 78.0 + (i * 2), "percentage"
                )
                workflow.record_performance_metric(
                    f"pod_cpu_usage_after_task_{i}", 35.0 + (i * 5), "percentage"
                )
            ...
            if final_summary:
                ...
                workflow.record_performance_metric(
                    "total_k8s_workflow_cost", final_summary.total_cost, "USD"
                )
                workflow.record_performance_metric(
                    "k8s_cost_per_pod", final_summary.total_cost, "USD"
                )
                workflow.record_performance_metric(
                    "k8s_provider_diversity",
                    len(final_summary.unique_providers),
                    "count",
                )
                workflow.record_performance_metric(
                    "k8s_model_diversity", len(final_summary.unique_models), "count"
                )
                ...
                if (
                    final_summary.total_cost > 0.05
                ):  # Threshold for HPA scaling decisions
                    workflow.record_alert(
                ...
                for provider, breakdown in cost_breakdown.items():
                    workflow.record_performance_metric(
                        f"k8s_cost_{provider}", breakdown["cost"], "USD"
                    )
            ...
            workflow.set_governance_attribute(
                "k8s_workflow_cost", final_summary.total_cost if final_summary else 0
            )
            workflow.set_governance_attribute("k8s_deployment_healthy", True)

            workflow.record_checkpoint(
                "k8s_workflow_complete",
                {
                    "namespace": k8s_context.namespace,
            ...
            print(f"   Namespace: {k8s_context.namespace}")
            print(
                f"   Final cost: ${final_summary.total_cost if final_summary else 0:.4f}"
            )
...
def demonstrate_kubernetes_health_checks():
        ...
        try:
            ...
            try:
                ...
                readiness_status["checks"]["genops_adapter"] = {
                    ...
                    "message": "GenOps adapter ready"
                    if adapter.is_available()
                    else "Adapter not ready",
                }
            ...
            readiness_status["checks"]["kubernetes_context"] = {
                ...
                "message": "K8s context ready"
                if not missing_k8s_vars
                else f"Missing K8s vars: {missing_k8s_vars}",
                "namespace": k8s_context.namespace,
                "pod_name": k8s_context.pod_name,
            }
            ...
            readiness_status["checks"]["telemetry_export"] = {
                ...
                "message": f"OTLP ready: {otlp_endpoint}"
                if otlp_endpoint
                else "No OTLP endpoint",
            }
            ...
            readiness_status["checks"]["storage"] = {
                "ready": storage_ready,
                "message": f"Storage ready: {hf_home}"
                if storage_ready
                else f"Storage not writable: {hf_home}",
            }
    ...
    def kubernetes_liveness_probe() -> dict[str, Any]:
        ...
        liveness_status = {"alive": True, "timestamp": time.time(), "checks": {}}
        ...
        try:
            ...
            liveness_status["checks"]["process_resources"] = {
                "alive": memory_percent < memory_threshold
...
data:
  ...
  HF_HOME: "/tmp/.cache/huggingface"'''
    ...
    print("```yaml")
    print(
        "# Create a Secret for Hugging Face token (replace XXXX with your base64 encoded token)"
    )
    print("apiVersion: v1")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="GenOps Hugging Face Kubernetes Integration Example"
    )
    parser.add_argument(
        "--readiness", action="store_true", help="Run readiness probe only"
    )
    parser.add_argument(
        "--liveness", action="store_true", help="Run liveness probe only"
    )
