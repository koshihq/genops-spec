...
def demonstrate_manual_adapter():
    ...
    try:
        ...
        print(f"   → Available tasks: {', '.join(adapter.get_supported_tasks()[:5])}...")
        print()
        ...
        try:
            messages = [
                ...
                {"role": "user", "content": "I'm having trouble with my order. Can you help?"},
                {"role": "assistant", "content": "I'd be happy to help you with your order. What specific issue are you experiencing?"},
                {"role": "user", "content": "My package was supposed to arrive yesterday but it hasn't shown up."}
            ]
        ...
        try:
            ...
            if embeddings:
                ...
                print(f"   📐 Embedding dimensions: {len(embeddings[0]) if isinstance(embeddings, list) and embeddings else 'N/A'}")
...
def demonstrate_cost_tracking():
    ...
    try:
        ...
        models_to_compare = ["gpt-3.5-turbo", "microsoft/DialoGPT-medium", "claude-3-haiku"]
        comparison = compare_model_costs(models_to_compare, input_tokens=1000, output_tokens=500)
        
        for model, info in comparison.items():
            relative_cost = info["relative_cost"]
            cost_indicator = "💰" if relative_cost > 2 else "💚" if relative_cost < 1.5 else "💛"
            print(f"   {cost_indicator} {model[:35]:35} → ${info['cost']:.6f} ({relative_cost:.1f}x)")
