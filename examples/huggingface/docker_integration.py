...
def setup_container_configuration():
    ...
    for key, value in container_config.items():
        ...
        display_value = (
            value
            if not any(secret in key.lower() for secret in ["token", "key", "secret"])
            else "***"
        )
...
def demonstrate_containerized_workflow():
    ...
    try:
        ...
        with production_workflow_context(
            ...
            for i, task in enumerate(tasks, 1):
                ...
                try:
                    if task["name"] == "embedding_generation":
                        ...
                        print(
                            f"✅ Task {i}: Generated embeddings for {len(task['inputs'])} items"
                        )
                    ...
                    workflow.record_step(
                        f"task_{i}_{task['name']}_complete",
                        {"model_used": task["model"], "success": True},
                    )
                ...
                workflow.record_performance_metric(
                    f"task_{i}_memory_usage", 85.0, "percentage"
                )
                workflow.record_performance_metric(
                    f"task_{i}_cpu_usage", 45.0, "percentage"
                )
            ...
            if final_summary:
                workflow.record_performance_metric(
                    "total_container_cost", final_summary.total_cost, "USD"
                )
                workflow.record_performance_metric(
                    "container_efficiency_score",
                    max(0, 100 - (final_summary.total_cost * 100)),
                    "score",
                )
        ...
        try:
            ...
            try:
                ...
                health_status["checks"]["genops_adapter"] = {
                    ...
                    "message": "GenOps Hugging Face adapter available"
            ...
            health_status["checks"]["telemetry"] = {
                ...
                if otlp_endpoint
                else "No OTLP endpoint configured",
            }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="GenOps Hugging Face Docker Integration Example"
    )
    parser.add_argument(
