...
def demonstrate_advanced_multi_task_pipeline():
    ...
    try:
        ...
        with production_workflow_context(
            ...
            content_models = ["gpt-3.5-turbo", "claude-3-haiku", "microsoft/DialoGPT-medium"]
            ...
            for model in content_models:
                try:
                    ...
                    estimated_cost = adapter._calculate_cost(
                        ...
                        model=model,
                        input_tokens=adapter._estimate_tokens("Create a comprehensive guide about sustainable energy solutions for small businesses"),
                    ...
                    workflow.record_hf_operation(
                        ...
                        provider=detected_provider,
                        model=model,
                        tokens_input=adapter._estimate_tokens("Create a comprehensive guide about sustainable energy solutions for small businesses"),
                    ...
                    print(f"✅ Content generation with {model} ({detected_provider}): ${estimated_cost:.4f}")
            ...
            if best_content_result:
                ...
                workflow.record_checkpoint("content_generation_complete", {
                    "best_model": best_content_result.model,
                    "best_cost": best_content_result.cost
                })
                print(f"🎯 Best content model: {best_content_result.model} (${best_content_result.cost:.4f})")
            ...
            for model in embedding_models:
                try:
                    ...
                    detected_provider = adapter._detect_provider(model)
                    total_input_tokens = sum(adapter._estimate_tokens(doc) for doc in documents)
                    ...
                    print(f"✅ Embedding with {model}: {len(documents)} docs, {len(embeddings[0]) if embeddings else 0}D")
            ...
            workflow.record_checkpoint("embedding_pipeline_complete", {
            ...
            with create_huggingface_cost_context(f"{workflow_id}_intelligence_analysis") as intelligence_context:
                ...
                if best_content_result and embedding_results:
                    ...
                    analysis_result = adapter.text_generation(
                    ...
                    workflow.record_step("intelligence_analysis_complete", {
                        "analysis_cost": intelligence_summary.total_cost if intelligence_summary else 0,
                        "content_source_cost": best_content_result.cost,
                        "total_intelligence_cost": (intelligence_summary.total_cost if intelligence_summary else 0) + best_content_result.cost
                    })
                    
                    print(f"🧠 Cross-task intelligence analysis complete")
                    print(f"   Content cost: ${best_content_result.cost:.4f}")
                    print(f"   Analysis cost: ${intelligence_summary.total_cost if intelligence_summary else 0:.4f}")
            ...
            try:
                ...
                workflow.record_hf_operation(
                    ...
                    model="runwayml/stable-diffusion-v1-5",
                    tokens_input=adapter._estimate_tokens("Professional infographic showing sustainable energy solutions for small businesses, modern design"),
                    tokens_output=0,
                    task="text-to-image"
                )
                
                print(f"✅ Generated professional infographic (size: {len(image_result) if isinstance(image_result, bytes) else 'unknown'})")
            ...
            if current_cost_summary:
                ...
                print(f"   Providers Used: {list(current_cost_summary.unique_providers)}")
                print(f"   Models Used: {len(current_cost_summary.unique_models)}")
                print(f"   Tasks Performed: {list(current_cost_summary.tasks_performed)}")
                
                # Record performance metrics
                workflow.record_performance_metric("total_workflow_cost", current_cost_summary.total_cost, "USD")
                workflow.record_performance_metric("provider_diversity", len(current_cost_summary.unique_providers), "count")
                workflow.record_performance_metric("model_diversity", len(current_cost_summary.unique_models), "count")
                
                # Cost optimization recommendations
                provider_breakdown = current_cost_summary.get_provider_breakdown()
                most_expensive_provider = max(provider_breakdown.keys(), 
                ...
                print(f"   Most Expensive Provider: {most_expensive_provider} (${provider_breakdown[most_expensive_provider]['cost']:.4f})")
                
                # Generate cost optimization recommendations
                if current_cost_summary.total_cost > 0.01:  # Threshold for expensive operations
                    workflow.record_alert(
                        "high_cost_workflow",
            ...
            workflow.set_governance_attribute("final_cost", workflow_metadata['current_cost'])
            workflow.set_governance_attribute("efficiency_score", 
                                            min(100, max(0, 100 - (workflow_metadata['current_cost'] * 1000))))
            
            workflow.record_checkpoint("workflow_complete", workflow_metadata)
            
            print(f"✅ Advanced multi-task pipeline completed successfully!")
        ...
        print("Please ensure GenOps is properly installed with: pip install genops-ai[huggingface]")
    except Exception as e:
...
def demonstrate_advanced_provider_detection_and_routing():
    ...
    try:
        ...
        text_generation_models = [
            "gpt-3.5-turbo", "claude-3-haiku", "microsoft/DialoGPT-medium", "llama-2-7b-chat"
        ]
        ...
        for model, cost_data in cost_comparison.items():
            print(f"   {model:<30} ${cost_data['cost']:.6f} ({cost_data['provider']}) - {cost_data['relative_cost']:.1f}x")
        
        # Cost optimization suggestions
        print(f"\n🎯 Cost Optimization Suggestions for GPT-4:")
        optimization = get_cost_optimization_suggestions("gpt-4", "text-generation")
        
        print(f"   Current: {optimization['current_model']['model']} - ${optimization['current_model']['cost_per_1k']['input']:.6f}/1k input")
        print(f"   Alternatives:")
        for alt in optimization['alternatives'][:3]:  # Top 3 alternatives
            print(f"     {alt['model']:<25} - ${alt['cost_per_1k']['input']:.6f}/1k input ({alt['savings']:.1f}% savings)")
    ...
    try:
        ...
        task_recommendations = {
            "High-volume text classification": {
                "recommended": ["distilbert-base-uncased", "cardiffnlp/twitter-roberta-base-sentiment-latest"],
