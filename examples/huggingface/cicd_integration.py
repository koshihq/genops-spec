    ...
    return CICDContext(
        ...
        commit_sha=os.getenv('GITHUB_SHA',
                           os.getenv('CI_COMMIT_SHA', 
                                   os.getenv('GIT_COMMIT', 'unknown')))[:8],  # Short SHA
...
def setup_cicd_configuration():
    ...
    cicd_config = {
        ...
        'OTEL_RESOURCE_ATTRIBUTES': f'cicd.pipeline.id={cicd_context.pipeline_id},'
                                   ...
                                   (f',cicd.pr.number={cicd_context.pr_number}' if cicd_context.pr_number else ''),
        
        # OTLP Configuration (CI/CD-specific endpoints)
        'OTEL_EXPORTER_OTLP_ENDPOINT': os.getenv('CICD_OTEL_ENDPOINT', 'http://localhost:4317'),
        ...
        'CI_PERFORMANCE_BASELINE': os.getenv('CI_PERF_BASELINE', '2.0'),  # 2 seconds baseline
    }
...
def run_cicd_tests():
    ...
    try:
        ...
        for i, test_case in enumerate(ci_test_cases, 1):
            ...
            try:
                print(f"   Test {i}/{len(ci_test_cases)}: {test_case['description']}...", end=" ")
                
                if test_case["name"] == "test_adapter_initialization":
                    adapter = GenOpsHuggingFaceAdapter()
                    test_result["status"] = "passed" if adapter.is_available() else "failed"
                    test_result["message"] = "Adapter initialized successfully" if adapter.is_available() else "Adapter not available"
                    ...
                    test_result["message"] = f"Generated {len(str(result)) if result else 0} characters"
                    test_result["data"]["response_length"] = len(str(result)) if result else 0
                    ...
                    test_result["status"] = "passed" if len(detected_providers) >= 2 else "skipped"
                    test_result["message"] = f"Detected {len(detected_providers)} providers: {detected_providers}"
                    ...
                    try:
                        ...
                        test_result["message"] = f"Error handled correctly: {type(e).__name__}"
                        test_result["data"]["error_type"] = type(e).__name__
                        
                elif test_case["name"] == "test_telemetry_export":
                    with create_huggingface_cost_context(f"ci_test_{cicd_context.pipeline_id}") as context:
                        context.add_hf_call(
                            provider="huggingface_hub",
                            model="test-model",
                            tokens_input=10,
                    ...
                    test_result["message"] = f"Telemetry context works" if summary else "Telemetry context failed"
                    ...
                    baseline_threshold = float(os.getenv('CI_PERFORMANCE_BASELINE', '2.0'))
                    
                    test_result["status"] = "passed" if perf_duration < baseline_threshold else "failed"
                    test_result["message"] = f"Duration: {perf_duration:.2f}s (baseline: {baseline_threshold}s)"
                    ...
                    test_result["status"] = "passed" if cost < cost_threshold else "failed"
                    test_result["message"] = f"Cost: ${cost:.4f} (threshold: ${cost_threshold})"
        ...
        critical_failures = [t for t in test_results["tests"] if t["critical"] and t["status"] == "failed"]
    ...
    try:
        ...
        try:
            ...
            validation_results["checks"]["service_availability"] = {
                ...
                "message": "Service available" if service_available else "Service not available"
            }
        except Exception as e:
            validation_results["checks"]["service_availability"] = {
                "status": "fail",
        ...
        try:
            ...
            validation_results["checks"]["end_to_end"] = {
                ...
                "message": f"E2E test completed in {e2e_duration:.2f}s" if result else "E2E test failed",
        ...
        try:
            ...
            if "end_to_end" in validation_results["checks"]:
                e2e_duration = validation_results["checks"]["end_to_end"].get("duration", 999)
        ...
        print()
        print(f"🚀 Deployment Status: {'READY' if validation_results['deployment_ready'] else 'NOT READY'}")
    ...
    try:
        ...
        for scenario in perf_scenarios:
            ...
            try:
                ...
                performance_results["tests"][scenario["name"]] = {
                    ...
                    "regression_percent": ((duration - baseline) / baseline) * 100 if baseline > 0 else 0,
                    "status": "fail" if regression else "pass"
                }
                
                if regression:
                    performance_results["regression_detected"] = True
                    print(f"❌ REGRESSION ({duration:.2f}s vs {baseline:.2f}s baseline)")
                else:
        ...
        for test_name, test_result in performance_results["tests"].items():
            if test_result["status"] == "pass":
                ...
                print(f"   ❌ {test_name}: {test_result['duration']:.2f}s ({test_result['regression_percent']:+.1f}% vs baseline)")
            else:
                print(f"   ❌ {test_name}: Error - {test_result.get('error', 'Unknown error')}")
        
        print()
        print(f"⚡ Performance Status: {'REGRESSION DETECTED' if performance_results['regression_detected'] else 'NO REGRESSION'}")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="GenOps Hugging Face CI/CD Integration")
    parser.add_argument("--mode", choices=["test", "deploy-validate", "perf-test"], 
                       default="test", help="CI/CD mode to run")
