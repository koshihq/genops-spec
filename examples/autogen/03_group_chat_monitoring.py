...
def main():
    ...
    try:
        ...
        print(f"✅ Multi-agent adapter configured:")
        print(f"   Team: research-team")
        print(f"   Project: collaborative-analysis") 
    ...
    try:
        with adapter.track_group_chat(
            group_chat_id="research-analysis",
            ...
            if use_real_llm:
                ...
                print("   Data Analyst: Based on benchmarks, cost efficiency correlates with...")
            ...
            print(f"   ✅ Group Chat Session 1 completed:")
    ...
    try:
        with adapter.track_group_chat(
            ...
            if use_real_llm:
                ...
                print("   Research Lead: We need to decide on implementation approach...")
                context.add_turn(Decimal('0.002'), 150, "research_lead")
                
                print("   Data Analyst: The data suggests approach A has 23% better ROI...")
                context.add_turn(Decimal('0.005'), 320, "data_analyst")
                
                print("   Critic: However, approach A has significant risks including...")
                context.add_turn(Decimal('0.004'), 280, "critic")
                
                print("   Summarizer: Weighing the analysis, I recommend a hybrid approach...")
                context.add_turn(Decimal('0.006'), 400, "summarizer")
                
                print("   Research Lead: Excellent. Let's proceed with the hybrid approach.")
                context.add_turn(Decimal('0.002'), 100, "research_lead")
                
                time.sleep(2)
            
            print(f"   ✅ Group Chat Session 2 completed:")
            print(f"      Total cost: ${context.total_cost:.6f}")
            print(f"      Turns: {context.turns_count}")
    ...
    try:
        ...
        if summary['active_agents']:
            ...
            for agent in summary['active_agents']:
                ...
                participation = f"~{100/len(summary['active_agents']):.1f}%"  # Simulated
    ...
    print("\n🚀 Next Steps:")
    print("1. Code execution monitoring: python examples/autogen/04_code_execution_tracking.py")
