...
def main():
    ...
    try:
        ...
        print("✅ Governance adapter configured:")
    ...
    try:
        with adapter.track_conversation(
            conversation_id="basic-qa", participants=["assistant", "user_proxy"]
        ) as context:
    ...
    try:
        with adapter.track_conversation(
            conversation_id="problem-solving", participants=["assistant", "user_proxy"]
        ) as context:
            if use_real_llm:
                user_proxy.initiate_chat(
                    assistant,
                ...
                print(
                    "   [Simulated] Assistant: I'll design a workflow with specialized agents..."
                )
        ...
        if monitor:
            ...
            for conv_id in ["basic-qa", "problem-solving"]:
                ...
                if "error" not in insights:
                    ...
                    print(
                        f"      Avg response time: {insights.get('avg_response_time_ms', 0):.1f}ms"
                    )
                    print(
                        f"      Quality score: {insights.get('conversation_quality_score', 0):.2f}"
                    )
                else:
    ...
    print("\n🚀 Next Steps:")
    print(
        "1. Try group conversations: python examples/autogen/03_group_chat_monitoring.py"
    )
    print(
        "2. Explore code execution: python examples/autogen/04_code_execution_tracking.py"
    )
