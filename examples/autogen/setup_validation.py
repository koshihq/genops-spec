...
def quick_validate_environment() -> dict[str, Any]:
    ...
    result = {"success": True, "issues": [], "fixes": [], "score": 100}
    ...
    try:
        from genops.providers.autogen import validate_autogen_setup  # noqa: F401
    ...
    if not found_keys:
        ...
        result["fixes"].append(
            "Set at least one API key: export OPENAI_API_KEY=your_key"
        )
        result["score"] -= 15
        print("⚠️  WARN")
    else:
    ...
    try:
        ...
        GenOpsAutoGenAdapter(team="validation-test", project="quick-test")
        print("✅ PASS")
    except Exception as e:
        result["success"] = False
        result["issues"].append(f"GenOps integration error: {str(e)}")
        result["fixes"].append(
            "Check GenOps installation: pip install --upgrade genops"
        )
        result["score"] -= 20
        print("❌ FAIL")
...
def comprehensive_validate() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "issues": [
                issue.title for issue in result.issues if issue.severity == "error"
            ],
            "warnings": [
                issue.title for issue in result.issues if issue.severity == "warning"
            ],
            "full_result": result,
        }
...
def print_results(
    quick_result: dict[str, Any], comprehensive_result: dict[str, Any] = None
):
    ...
    # Overall status
    if quick_result["success"] and (
        not comprehensive_result or comprehensive_result["success"]
    ):
...
def main():
    ...
    parser.add_argument(
        "--ci", action="store_true", help="CI/CD mode (exit codes only)"
    )
    ...
    if args.ci:
        success = quick_result["success"] and (
            not comprehensive_result or comprehensive_result["success"]
        )
