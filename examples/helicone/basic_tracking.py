...
def basic_multi_provider_example():
    ...
    for i, example in enumerate(examples, 1):
        ...
        try:
            ...
            provider_cost = (
                response.usage.provider_cost if hasattr(response, "usage") else 0.0
            )
            gateway_cost = (
                response.usage.helicone_cost if hasattr(response, "usage") else 0.0
            )
    ...
    if results:
        ...
        for result in results:
            print(
                f"   • {result['provider'].title():>10}: ${result['cost']:.6f} "
        ...
        if len(results) > 1:
            ...
            print(
                f"   🥇 Most cost-effective: {cheapest['provider']} (${cheapest['cost']:.6f})"
            )
            print(
                f"   💎 Most expensive: {most_expensive['provider']} (${most_expensive['cost']:.6f})"
            )
...
def main():
    ...
    if success:
        ...
        print(
            "   • Try 'python advanced_features.py' for streaming & advanced features"
        )
    else:
