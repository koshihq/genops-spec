...
class DustCostOptimizer:
            ...
            if attr in metadata:
                ...
                if key in self.usage_stats:
                    ...
                    self.usage_stats[key][value] = (
                        self.usage_stats[key].get(value, 0) + 1
                    )
    ...
    def get_cost_breakdown(self, user_count: int = 1) -> dict[str, Any]:
        ...
        monthly_conversations = int(
            self.usage_stats["conversations"] * (24 * 30 / hours)
        )
        ...
        return {
            ...
            "cost_analysis": {
                ...
                "cost_difference": enterprise_cost["total_monthly_cost"]
                - pro_cost["total_monthly_cost"],
...
def main():
    ...
    genops.init(service_name="dust-cost-optimization", enable_console_export=True)
    ...
    # Calculate costs for different scenarios
    scenarios = [(5, "Small Team"), (25, "Medium Team"), (100, "Large Organization")]
    ...
    try:
        ...
        for i in range(5):
            ...
            optimizer.track_operation(
                "searches", customer_id=f"customer-{i % 2 + 1}", estimated_tokens=150
            )
    ...
    for team_size in team_sizes:
        ...
        print(
            f"   Pro Plan: €{analysis['cost_analysis']['pro_plan']['total_monthly_cost']:.2f}/month"
        )
        print(
            f"   Enterprise: €{analysis['cost_analysis']['enterprise_plan']['total_monthly_cost']:.2f}/month"
        )
    ...
    if customer_usage:
        ...
        for customer_id, ops_count in customer_usage.items():
            ...
            print(
                f"  • {customer_id}: {ops_count} ops ({percentage:.1f}%) = €{allocated_cost:.2f}"
            )
    else:
...
def demonstrate_advanced_cost_scenarios():
    ...
    for season, usage in seasonal_patterns.items():
        cost_estimate = engine.estimate_monthly_cost(
            user_count=20, usage_forecast=usage, plan_type="pro"
        )
        print(
            f"   {season.replace('_', ' ').title()}: €{cost_estimate['total_monthly_cost']:.2f}"
        )
    ...
    print(f"   Cost per conversation: €{29.0 / (250 * 30):.4f}")  # Rough estimate
    print(f"   Cost per agent execution: €{29.0 / (375 * 30):.4f}")
