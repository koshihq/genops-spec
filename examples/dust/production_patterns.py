...
from genops.providers.dust_validation import print_validation_result, validate_setup
...
class DustProductionManager:
    ...
    def customer_operation_context(self, customer_id: str, operation_type: str):
        ...
        if self.circuit_breaker_state["dust_api"] == "open":
            ...
            if last_failure and datetime.now() - last_failure < timedelta(minutes=5):
                raise RuntimeError(
                    "Dust API circuit breaker is open - service degraded"
                )
            else:
        ...
        with set_customer_context(
            ...
            try:
                ...
                customer.current_usage[operation_type] = (
                    customer.current_usage.get(operation_type, 0) + 1
                )
                ...
                logger.info(
                    f"Completed {operation_type} for {customer_id} in {duration.total_seconds():.2f}s, cost: €{operation_cost:.4f}"
                )

    def _calculate_operation_cost(
        self, customer: CustomerConfig, operation_type: str
    ) -> float:
    ...
    def _handle_operation_failure(
        self, customer_id: str, operation_type: str, error: Exception
    ):
        """Handle operation failures and circuit breaker logic."""

        logger.error(
            f"Operation failed for customer {customer_id}, operation {operation_type}: {error}"
        )
    ...
    def create_customer_conversation(
        self, customer_id: str, title: str, **kwargs
    ) -> dict[str, Any]:
    ...
    def send_customer_message(
        self, customer_id: str, conversation_id: str, content: str, **kwargs
    ) -> dict[str, Any]:
    ...
    def run_customer_agent(
        self, customer_id: str, agent_id: str, inputs: dict[str, Any], **kwargs
    ) -> dict[str, Any]:
    ...
    def search_customer_datasources(
        self, customer_id: str, query: str, **kwargs
    ) -> dict[str, Any]:
        ...
        return {
            "system_status": {
                ...
                "active_customers": len(
                    [c for c in self.customers.values() if c.current_usage]
                ),
            },
            "policy_compliance": {
            ...
            "recent_violations": [
                ...
                for v in sorted(
                    self.policy_violations, key=lambda x: x.timestamp, reverse=True
                )[:10]
            ],
        }
...
def main():
    ...
    for customer_id in customers_to_demo:
        ...
        try:
            ...
            if conversation and "conversation" in conversation:
                ...
                if message:
                    print("   ✅ Message sent with governance tracking")
    ...
    try:
        ...
        print("System Status:")
        print(
            f"   Circuit Breaker: {health['system_status']['circuit_breaker']['dust_api']}"
        )
        ...
        print(
            f"   Critical Violations: {health['policy_compliance']['critical_violations']}"
        )
