...
class AdvancedDustWorkflows:
        ...
        self.workflow_templates = {
            "customer_onboarding": [
                ...
                },
                {
                    ...
                    "content": "Let me help you get started with our AI assistant.",
                },
                {"operation": "datasource_search", "query": "onboarding documentation"},
                {
                    "operation": "agent_run",
                    "agent_type": "onboarding_assistant",
                    "personalized": True,
                },
            ],
            "support_escalation": [
                ...
                {
                    "operation": "message_send",
                    "content": "Based on analysis, here are the recommended next steps...",
                },
            ],
            "content_analysis": [
                {
                    "operation": "datasource_search",
                    "query": "content to analyze",
                    "comprehensive": True,
                },
                {
                    "operation": "agent_run",
                ...
                {
                    "operation": "message_send",
                    "content": "Analysis complete with insights and recommendations",
                },
            ],
        }
            ...
            **metadata,
            ...
            with track_enhanced(
                ...
                **metadata,
                ...
                try:
                    logger.info(
                        f"🚀 Starting workflow {workflow_name} for customer {customer_id}"
                    )
                    ...
                    logger.info(
                        f"✅ Completed workflow {workflow_name} in {duration:.2f}s, cost: €{workflow.total_cost:.4f}"
                    )
    ...
    async def execute_step(
        self, workflow: WorkflowExecution, step_config: dict[str, Any]
    ) -> WorkflowStep:
        ...
        with track(
            ...
            try:
                logger.info(
                    f"Executing step {operation_type} in workflow {workflow.workflow_name}"
                )
                ...
                if operation_type == "conversation_create":
                    ...
                    conversation_id = self._get_conversation_from_previous_steps(
                        workflow
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Agent execution failed (expected in demo): {agent_error}"
                        )
                        ...
                        result = {
                            "run": {
                                ...
                                "results": [
                                    {
                                        "output": f"Mock agent result for {operation_type}"
                                    }
                                ],
                            }
                        }
                ...
                logger.info(
                    f"✅ Step {operation_type} completed in {step.duration_ms:.0f}ms, cost: €{step.cost:.4f}"
                )
                ...
                logger.error(
                    f"❌ Step {operation_type} failed after {step.duration_ms:.0f}ms: {e}"
                )
                raise

    def _get_conversation_from_previous_steps(
        self, workflow: WorkflowExecution
    ) -> Optional[str]:
        """Extract conversation ID from previous workflow steps."""
        for step in reversed(workflow.steps):
            if (
                step.operation_type == "conversation_create"
                and step.outputs
                and "conversation" in step.outputs
            ):
    ...
    async def execute_workflow_template(
        self, template_name: str, customer_id: str, **customizations
    ) -> WorkflowExecution:
            ...
            **customizations,
            ...
            for step_config in steps_config:
                try:
                    ...
                    logger.error(
                        f"Workflow {template_name} failed at step {step_config['operation']}: {e}"
                    )
                    ...
                    if step_config.get("critical", True):
                        ...
                        logger.warning(
                            "Continuing workflow despite non-critical step failure"
                        )
    ...
    def get_workflow_analytics(self) -> dict[str, Any]:
        ...
        if completed_workflows:
            ...
            avg_cost = sum(w.total_cost for w in completed_workflows) / len(
                completed_workflows
            )
            avg_steps = sum(len(w.steps) for w in completed_workflows) / len(
                completed_workflows
            )
        ...
        return {
            "summary": {
                ...
                "success_rate": f"{len(completed_workflows) / total_workflows * 100:.1f}%"
            ...
            "workflow_types": {
                workflow_name: len(
                    [w for w in all_workflows if w.workflow_name == workflow_name]
                )
                for workflow_name in {w.workflow_name for w in all_workflows}
            },
        }
...
async def main():
    ...
    try:
        async with workflows.workflow_context(
            ...
            print(
                f"✅ Custom workflow completed with {len(custom_workflow.steps)} steps"
            )
