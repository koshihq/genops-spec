...
def smart_model_selection():
    ...
    try:
        ...
        for task in tasks:
            ...
            print(
                f"{task['description']:<20} {config.name:<20} ${estimated_cost:.4f}      {config.use_case[:30]}"
            )
            ...
            actual_cost = (
                response.usage.prompt_tokens / 1000 * config.cost_per_1k_input
            ...
            print(
                f"   Response ({actual_tokens} tokens, ${actual_cost:.4f}): {response.choices[0].message.content[:60]}...\n"
            )

        print(f"\n💰 Total cost for optimized model selection: ${total_cost:.4f}")
        print("🎯 Estimated savings vs using GPT-4 for all: ~60-80%")
...
def budget_constrained_completion():
    ...
    try:
        ...
        def complete_within_budget(prompt: str, max_budget: float = 0.01) -> dict:
            ...
            for tier in performance_order:
                ...
                if estimated_cost <= max_budget:
                    print(
                        f"✅ Selected {config.name} (${estimated_cost:.4f} <= ${max_budget} budget)"
                    )
        ...
        print(
            f"{'Scenario':<20} {'Budget':<10} {'Model':<20} {'Actual Cost':<12} {'Status'}"
        )
        print("-" * 80)
        ...
        for scenario in test_scenarios:
            try:
                ...
                print(
                    f"{scenario['scenario']:<20} ${scenario['budget']:<9.3f} {result['model']:<20} ${result['actual_cost']:<11.4f} {status}"
                )
                print(f"   Response: {result['response'][:60]}...\n")

            except ValueError as e:
                print(
                    f"{scenario['scenario']:<20} ${scenario['budget']:<9.3f} {'None':<20} {'N/A':<12} ❌ No Model"
                )
                print(f"   Error: {e}\n")
...
def cost_comparison_analysis():
    ...
    try:
        ...
        print(
            f"{'Model':<20} {'Actual Cost':<12} {'Tokens':<10} {'Cost per Token':<15} {'Response Quality'}"
        )
        print("-" * 85)
        ...
        for tier, config in configs.items():
            try:
                ...
                print(
                    f"{config.name:<20} ${actual_cost:<11.4f} {response.usage.total_tokens:<10} ${cost_per_token:<14.6f} {quality_rating}"
                )
        ...
        if results:
            ...
            print("\n🏆 Analysis Summary:")
            print(
                f"   • Best value (cost per token): {best_value['model']} (${best_value['cost_per_token']:.6f}/token)"
            )
            print(
                f"   • Cheapest total cost: {cheapest['model']} (${cheapest['cost']:.4f})"
            )
            print(
                f"   • Most expensive: {most_expensive['model']} (${most_expensive['cost']:.4f})"
            )
            print(
