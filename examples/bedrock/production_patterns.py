    ...
    try:
        ...
        with production_workflow_context(
            ...
            workflow.record_performance_metric(
                "classification_accuracy", 0.95, "percentage"
            )
            ...
            workflow.record_performance_metric(
                "extraction_completeness", 0.88, "percentage"
            )
            ...
            workflow.record_step(
                "report_generation",
                {"report_format": "executive_summary", "target_audience": "c_level"},
            )
            ...
            print(
                f"      Executive summary generated ({len(report_result.content)} chars)"
            )
            ...
            workflow.record_performance_metric(
                "total_workflow_cost", final_cost_summary.total_cost, "USD"
            )
            ...
            print(
                f"      Budget Utilization: {(final_cost_summary.total_cost / 5.00) * 100:.1f}%"
            )
            print(f"      Models Used: {len(final_cost_summary.unique_models)}")
            print("      SOC2 Compliance: ✅ Maintained")
        ...
        with create_bedrock_cost_context(
            ...
            inquiries = [
                {
                    "type": "billing",
                    "priority": "high",
                    "text": "Question about my invoice",
                },
                {
                    "type": "technical",
                    "priority": "medium",
                ...
                },
                {
                    "type": "billing",
                    "priority": "high",
                    "text": "Refund request processing",
                },
                {
                    "type": "technical",
                    "priority": "high",
                    "text": "Critical system error report",
                },
            ...
            for batch_idx in range(0, len(inquiries), batch_size):
                ...
                if remaining_budget > 0.20 and high_priority_count > 2:
                    ...
                    print(
                        f"      Using premium model for {high_priority_count} high-priority items"
                    )
                    ...
                    print("      Using cost-effective model (low budget)")
                ...
                current_summary = cost_context.get_current_summary()
                print(
                    f"      Processed: {len(batch)} inquiries, Cost: ${current_summary.total_cost:.6f}"
                )
            ...
            print(
                f"   Average cost per inquiry: ${final_summary.get_average_cost_per_operation():.6f}"
            )
            print(
                f"   Budget utilization: {(final_summary.total_cost / 1.00) * 100:.1f}%"
            )
            print(f"   Models used: {list(final_summary.unique_models)}")

            # Performance metrics
            high_priority_ops = sum(
                1
                for op in cost_context.operations
                if op.governance_attributes.get("priority") == "high"
            )
    ...
    try:
        ...
        for i, scenario in enumerate(error_scenarios, 1):
            ...
            if circuit_open:
                ...
                print("      🔄 Would retry after cooldown period")
                continue
            ...
            try:
                ...
                print(
                    f"      ✅ Success: ${result.cost_usd:.6f}, {result.latency_ms:.0f}ms"
                )
                ...
                print("      📝 Error logged for alerting and analysis")
...
def demonstrate_monitoring_integration():
    ...
    for metric in monitoring_metrics:
        ...
        if "cost" in name or "utilization" in name:
            status = (
                "🟢" if value < threshold else "🟡" if value < threshold * 1.2 else "🔴"
            )
            print(f"   {status} {name}: {value:.4f} (threshold: {threshold:.4f})")
        elif "latency" in name:
            status = (
                "🟢" if value < threshold else "🟡" if value < threshold * 1.5 else "🔴"
            )
            print(f"   {status} {name}: {value}ms (threshold: {threshold}ms)")
        else:
            status = (
                "🟢" if value > threshold else "🟡" if value > threshold * 0.9 else "🔴"
            )
