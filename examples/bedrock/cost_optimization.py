...
def demonstrate_multi_model_optimization():
    ...
    try:
        ...
        for scenario in task_scenarios:
            ...
            print(
                f"   Volume: {scenario['input_tokens']} → {scenario['output_tokens']} tokens"
            )
        ...
        with create_bedrock_cost_context(
            ...
            for i, request in enumerate(content_requests, 1):
                ...
                if remaining_budget <= 0.001:  # Less than $0.001 remaining
                    print("   ⚠️  Budget exhausted, switching to cheapest model")
            ...
            print("\n   📊 Final Budget Analysis:")
            print("      Budget limit: $0.05")
            print(f"      Actual spend: ${final_summary.total_cost:.6f}")
            print(
                f"      Budget utilization: {(final_summary.total_cost / 0.05) * 100:.1f}%"
            )
            print(
                f"      Operations completed: {successful_operations}/{total_operations}"
            )
            print(
                f"      Average cost per operation: ${final_summary.get_average_cost_per_operation():.6f}"
            )
...
def demonstrate_regional_optimization():
    ...
    try:
        ...
        for i, (region, cost, multiplier) in enumerate(regional_costs, 1):
            ...
            print(
                f"      {emoji} {i}. {region}: ${cost:.6f} (multiplier: {multiplier:.2f})"
            )
        ...
        print(
            f"      Most expensive: {most_expensive_region} (${most_expensive_cost:.6f})"
        )
        print(
            f"      Potential savings: ${savings:.6f} per operation ({percentage_savings:.1f}%)"
        )
...
def demonstrate_provisioned_vs_ondemand():
    ...
    try:
        ...
        print(
            f"   Average: {avg_input_tokens} input → {avg_output_tokens} output tokens"
        )
        print()
        ...
        for scenario in usage_scenarios:
            ...
            if analysis["provisioned_available"]:
                print(
                    f"      Provisioned cost: ${analysis['provisioned_monthly']:.2f}/month"
                )
                ...
                print(
                    f"      Break-even point: {analysis['breakeven_operations']:,.0f} ops/month"
                )
            else:
                print("      ⚠️  Provisioned throughput not available for this model")
        ...
        with create_bedrock_cost_context(
            ...
            for i, task in enumerate(batch_tasks, 1):
                ...
                cost_context.add_operation(
                    ...
                    model_id=task["model"],
                    provider="anthropic"
                    if "claude" in task["model"]
                    else "amazon"
                    if "titan" in task["model"]
                    else "ai21",
                    region="us-east-1",
                ...
                if current_summary.optimization_recommendations:
                    print(
                        f"      💡 Recommendation: {current_summary.optimization_recommendations[0]}"
                    )
            ...
            print("      Export formats: JSON, CSV, Summary")
