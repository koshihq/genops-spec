...
try:
    from genops.providers.raindrop import (  # noqa: F401
        GenOpsRaindropAdapter,
        auto_instrument,
    )
    from genops.providers.raindrop_validation import (
        print_validation_result,  # noqa: F401
        validate_setup,
    )
...
def main():
    ...
    try:
        ...
        print(
            f"  {'✅' if api_key else '❌'} RAINDROP_API_KEY: {'configured' if api_key else 'MISSING'}"
        )
        ...
        if not api_key:
            ...
            print(
                "  1. Get your API key from https://app.raindrop.ai → Settings → API Keys"
            )
        ...
        if not validation_result.is_valid:
            ...
            print(
                '  • Run interactive setup: python -c "from genops.providers.raindrop_validation import validate_setup_interactive; validate_setup_interactive()"'
            )
    ...
    print(
        f"  {'✅' if api_key else '❌'} RAINDROP_API_KEY {'configured' if api_key else 'required'}"
    )
    print(f"  ✅ Team: {team}")
    ...
    with adapter.track_agent_monitoring_session(session_name) as session:
        ...
        for i, agent_id in enumerate(agents, 1):
            ...
            print(
                f"  💬 Agent interaction logged: {agent_id} - ${cost_result.total_cost:.3f}"
            )
        ...
        cost_result = session.create_alert(
            alert_name="agent_performance_degradation", alert_config=alert_config
        )
        print(
            f"  🔔 Alert created: agent_performance_degradation - ${cost_result.total_cost:.3f}"
        )
        ...
        print(
            f"  Efficiency: {session.operation_count / max(session.duration_seconds / 3600, 1 / 3600):.1f} operations/hour"
        )
    ...
    if summary.cost_by_operation_type:
        ...
        for op_type, cost in summary.cost_by_operation_type.items():
            percentage = (
                float(cost / summary.total_cost * 100) if summary.total_cost > 0 else 0
            )
            print(f"    • {op_type}: ${cost:.3f} ({percentage:.1f}%)")

    print("\n✅ Basic tracking example completed successfully!")
    print("\n🚀 Next Steps:")
    print("  1. Try auto-instrumentation: python auto_instrumentation.py")
    print("  2. Explore advanced features: python advanced_features.py")
    print("  3. Check cost optimization: python cost_optimization.py")
    print("  4. Review production patterns: python production_patterns.py")
