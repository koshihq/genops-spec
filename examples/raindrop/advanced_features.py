    ...
    with adapter.track_agent_monitoring_session(session_name) as session:
        ...
        for i in range(interaction_count):
            ...
            if agent_type == "customer-service":
                ...
                interaction_data = {
                    "input": "User profile and behavior data",
                ...
                interaction_data = {
                    "input": "Transaction data with risk indicators",
            ...
            if (
                i % 25 == 0 and random.random() > 0.7
            ):  # 30% chance every 25 interactions
                alert_config = {
                    "conditions": [
...
def main():
    ...
    adapter.pricing_calculator.update_monthly_volume(
        15000
    ...
    print(
        f"🔄 Starting concurrent monitoring for {len(agent_fleet)} production agents..."
    )
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        future_to_agent = {
            executor.submit(
                simulate_production_agent, agent_config, adapter
            ): agent_config
        ...
        for future in as_completed(future_to_agent):
            ...
            try:
                ...
                print(
                    f"  ✅ {result['agent_id']}: ${result['total_cost']:.3f} cost, {result['alerts_created']} alerts"
                )
    ...
    adapter.cost_aggregator.get_summary()
    ...
    for agent_type, cost in sorted(
        agent_type_costs.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    if volume_info["next_tier_threshold"]:
        ...
        savings_potential = total_monitoring_cost * (
            volume_info["next_tier_discount_rate"]
            - volume_info["current_discount_rate"]
        )
    ...
    print("  ✅ Multi-agent cost attribution")
    print("  ✅ Real-time budget enforcement")
    print("  ✅ Volume-based pricing optimization")
    print("  ✅ Performance-based cost intelligence")
    print("  ✅ Automated policy compliance")
    ...
    print("  1. Connect to your observability dashboard (Grafana, Datadog)")
    print("  2. Set up automated cost alerts and budget enforcement")
    print("  3. Integrate with your FinOps and procurement workflows")
    print("  4. Deploy governance policies across development teams")
