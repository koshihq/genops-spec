...
def main():
    ...
    try:
        from genops.providers.perplexity import (
            GenOpsPerplexityAdapter,  # noqa: F401
...
def create_production_adapters() -> dict[str, Any]:
    ...
    adapters["enterprise"] = GenOpsPerplexityAdapter(  # noqa: F821
        ...
        default_search_context=SearchContext.HIGH,  # noqa: F821
        tags={
    ...
    print(
        f"   ✅ Strict governance | Budget: ${adapters['enterprise'].daily_budget_limit}/day"
    )

    # Multi-tenant adapter
    print("\n🏢 Multi-Tenant Adapter:")
    adapters["multitenant"] = GenOpsPerplexityAdapter(  # noqa: F821
    ...
    print(
        f"   ✅ Multi-tenant isolation | Budget: ${adapters['multitenant'].daily_budget_limit}/day"
    )

    # High-performance adapter
    print("\n⚡ Performance-Optimized Adapter:")
    adapters["performance"] = GenOpsPerplexityAdapter(  # noqa: F821
        ...
        enable_governance=True,
        governance_policy="enforced",
        default_search_context=SearchContext.MEDIUM,  # Balanced for performance  # noqa: F821
        tags={
    ...
    print(
        f"   ✅ Performance optimized | Budget: ${adapters['performance'].daily_budget_limit}/day"
    )

    # Compliance-focused adapter
    print("\n🔐 Compliance-Focused Adapter:")
    adapters["compliance"] = GenOpsPerplexityAdapter(  # noqa: F821
    ...
    print(
        f"   ✅ Compliance controls | Budget: ${adapters['compliance'].daily_budget_limit}/day"
    )

    # Resilient error handling adapter
    print("\n🛡️ Resilient Error Handling Adapter:")
    adapters["resilient"] = GenOpsPerplexityAdapter(  # noqa: F821
    ...
    print(
        f"   ✅ Resilience patterns | Budget: ${adapters['resilient'].daily_budget_limit}/day"
    )
    ...
    with adapter.track_search_session("enterprise_governance") as session:
        for scenario in governance_scenarios:
            ...
            try:
                ...
                print(
                    f"   🏷️ Cost Attribution: {scenario['governance_tags'].get('cost_code', 'N/A')}"
                )
                print(f"   📊 Citations: {len(result.citations)}")

                # Log enterprise metrics
                logger.info(
                    f"Enterprise search completed: {scenario['name']}",
                    extra={
                ...
                logger.error(
                    f"Enterprise governance scenario failed: {scenario['name']}: {e}"
                )
...
def demonstrate_multi_tenant_architecture(adapters):
    ...
    tenants = [
        ...
        },
        {
            ...
            "tier": "starter",
            "budget_limit": 50.0,
            "searches": ["Digital marketing trends 2024"],
        },
    ]
    ...
    for tenant in tenants:
        ...
        with tenant_adapter.track_search_session(
            ...
            for search_query in tenant["searches"]:
                try:
                    ...
                    print(
                        f"      ✅ Cost: ${result.cost:.6f} | Running total: ${tenant_cost:.6f}"
                    )

                    # Tenant budget check
                    if tenant_cost > tenant["budget_limit"]:
                        print(
                            f"      ⚠️ Budget limit exceeded for {tenant['tenant_id']}"
                        )
                        break
...
def demonstrate_performance_optimization(adapter):
    ...
    for scenario in performance_scenarios:
        ...
        with adapter.track_search_session(
            f"perf_{scenario['name'].lower().replace(' ', '_')}"
        ) as session:
                ...
                for query in scenario["queries"]:
                    try:
                        result = adapter.search_with_governance(
                            query=query,
                            model=PerplexityModel.SONAR,  # noqa: F821
                            search_context=SearchContext.LOW,  # Optimized for performance  # noqa: F821
                            session_id=session.session_id,
                            max_tokens=150,
        ...
        print(
            f"      Queries processed: {successful_queries}/{len(scenario['queries'])}"
        )
        print(f"      Total time: {execution_time:.2f}s")
        print(
            f"      Avg time per query: {execution_time / max(successful_queries, 1):.2f}s"
        )
        print(f"      Total cost: ${total_cost:.6f}")
        print(
            f"      Cost efficiency: ${total_cost / max(successful_queries, 1):.6f} per query"
        )
        ...
        for search in compliance_searches:
            ...
            try:
                result = adapter.search_with_governance(
                    query=search["query"],
                    model=PerplexityModel.SONAR_PRO,  # noqa: F821
                    search_context=SearchContext.HIGH,  # noqa: F821
                    session_id=session.session_id,
                    max_tokens=300,
                ...
                print(f"   💰 Cost: ${result.cost:.6f}")
                print(
                    f"   📋 Audit ID: {session.session_id}-{hash(search['query']) % 10000}"
                )

                # Log compliance event
                logger.info(
                    "Compliance search executed",
                    extra={
                        "session_id": session.session_id,
