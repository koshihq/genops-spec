...
class InteractiveSetupWizard:
            ...
            try:
                ...
                if 0 <= use_case_idx < len(use_cases):
                    ...
                    if selected_use_case in self.use_case_templates:
                        ...
                        print("   Applied template with recommended settings")
    ...
    def _configure_basic_settings(self):
        ...
        self.config.team = (
            input(f"   Team name [{self.config.team}]: ").strip() or self.config.team
        )
        self.config.project = (
            input(f"   Project name [{self.config.project}]: ").strip()
        ...
        env_input = (
            input(f"   Environment [{self.config.environment}]: ").strip().lower()
        )
    ...
    def _configure_governance(self):
        ...
        daily_budget = input(
            f"   Daily budget limit (${self.config.daily_budget_limit}): "
        ).strip()
        if daily_budget:
            try:
        ...
        monthly_budget = input(
            f"   Monthly budget limit (${self.config.monthly_budget_limit}): "
        ).strip()
        if monthly_budget:
            try:
        ...
        policies = {
            "1": (
                "advisory",
                "Warn about budget/policy violations but allow operations",
            ),
        ...
        alert_input = (
            input(
                f"   Enable cost alerts? [{'Y' if self.config.enable_cost_alerts else 'N'}/n/y]: "
            )
            .strip()
            .lower()
        )
        ...
        print(
            f"   Cost Alerts: {'Enabled' if self.config.enable_cost_alerts else 'Disabled'}"
        )
    ...
    def _configure_advanced_settings(self):
        ...
        self.config.customer_id = (
            input("   Customer ID (for multi-tenant): ").strip() or None
        )
        self.config.cost_center = (
            input("   Cost Center (for financial reporting): ").strip() or None
        )
        ...
        model_input = (
            input(f"   Default model [{self.config.default_model}]: ").strip().lower()
        )
        ...
        context_input = (
            input(f"   Default search context [{self.config.default_search_context}]: ")
            .strip()
            .lower()
        )
        ...
        cache_input = (
            input(
                f"   Enable result caching? [{'Y' if self.config.enable_caching else 'N'}/n/y]: "
            )
            .strip()
            .lower()
        )
        ...
        print(
            f"   Performance: Caching {'enabled' if self.config.enable_caching else 'disabled'}"
        )
    ...
    def _validate_configuration(self):
        ...
        if self.config.environment in ["development", "staging", "production"]:
            validation_results.append(
                ("✅", f"Environment '{self.config.environment}' recognized")
            )
        else:
            validation_results.append(
                ("⚠️", f"Custom environment '{self.config.environment}'")
            )
        ...
        if self.config.environment == "production":
            if not self.config.customer_id and not self.config.cost_center:
                validation_results.append(
                    ("⚠️", "Consider adding customer_id or cost_center for production")
                )
            else:
        ...
        test_connection = (
            input("   Test Perplexity API connection? [y/N]: ").strip().lower()
        )
    ...
    def _generate_configuration_outputs(self):
        ...
        print("\nGenerated files:")
        print("   • .env.perplexity - Environment variables")
        print("   • perplexity_example.py - Working example code")
        print("   • perplexity_config.json - Complete configuration")
        print("\nNext steps:")
        print("   1. Review generated files")
        print("   2. Run: python perplexity_example.py")
        print("   3. Explore examples/perplexity/ for more patterns")
...
"""
        ...
        print("   ✅ Generated .env.perplexity")
        if self.config.api_key and self.config.api_key.startswith("pplx-"):
            print(
                "   🔐 Security: API key not written to file - please set it manually"
            )
            print("   💡 Run: export PERPLEXITY_API_KEY='your-actual-key'")
...
'''
        ...
        print("   ✅ Generated perplexity_example.py")
    ...
    def _generate_config_summary(self):
        ...
        print("   ✅ Generated perplexity_config.json")
...
def main():
    ...
    # Check prerequisites
    try:
        from genops.providers.perplexity import GenOpsPerplexityAdapter  # noqa: F401
