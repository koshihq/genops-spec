...
try:
    ...
    try:
        ...
        from genops.providers.databricks_unity_catalog_pricing import (
...
def demonstrate_catalog_operations(adapter, governance_attrs: dict[str, str]) -> None:
    ...
    catalog_metadata = adapter.track_catalog_operation(
        operation="read", catalog_name="demo_catalog", **governance_attrs
    )
...
def demonstrate_table_operations(adapter, governance_attrs: dict[str, str]) -> None:
    ...
    print(
        f"   Table: {table_metadata['catalog_name']}.{table_metadata['schema_name']}.{table_metadata['table_name']}"
    )
...
def demonstrate_sql_warehouse_operations(
    adapter, governance_attrs: dict[str, str]
) -> None:
...
def demonstrate_cost_calculation() -> None:
    ...
    print("✅ SQL warehouse cost calculation:")
    print("   Warehouse size: Small")
    ...
    print("✅ Compute cluster cost calculation:")
    print("   Cluster type: standard")
    print("   Nodes: 3")
    ...
    storage_cost = pricing_calc.calculate_storage_cost(
        data_size_gb=100.0, storage_duration_days=30, region="us-west-2"
    )

    print("✅ Storage cost calculation:")
    print("   Data size: 100 GB")
...
def demonstrate_data_lineage() -> None:
    ...
    print(
        f"   Source: {lineage_metrics.source_catalog}.{lineage_metrics.source_schema}.{lineage_metrics.source_table}"
    )
    print(
        f"   Target: {lineage_metrics.target_catalog}.{lineage_metrics.target_schema}.{lineage_metrics.target_table}"
    )
...
def demonstrate_workspace_cost_context() -> None:
    ...
    with create_workspace_cost_context(workspace_id, "demo_analysis"):
        ...
        from genops.providers.databricks_unity_catalog.cost_aggregator import (
            get_cost_aggregator,
        )
...
def main():
    ...
    adapter = instrument_databricks_unity_catalog(workspace_url=config["workspace_url"])
