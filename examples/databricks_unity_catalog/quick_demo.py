    ...
    for i, op in enumerate(operations, 1):
        try:
            if op["type"] == "catalog":
                ...
                print(
                    f"     {i}. ✅ {op['description']} ({op['rows']:,} rows, {op['size_mb']} MB)"
                )
                ...
                print(
                    f"     {i}. ✅ {op['description']} ({op['duration_ms']}ms, {op['compute_units']} DBU)"
                )

        except Exception as e:
            print(
                f"     {i}. ⚠️ {op['description']} (simulated due to: {type(e).__name__})"
            )
    ...
    try:
        ...
        for i, lineage in enumerate(lineage_examples, 1):
            if lineage["target"]:
                ...
                print(
                    f"     {i}. ✅ {lineage['type'].title()}: {source_path} → {target_path}"
                )
            else:
                source_path = f"{lineage['source'][0]}.{lineage['source'][1]}.{lineage['source'][2]}"
        ...
        print("   🏛️ Governance Summary:")
        print(f"     • Lineage events: {governance_summary.lineage_events}")
        print(
            f"     • Data classifications: {dict(governance_summary.data_classifications)}"
        )
