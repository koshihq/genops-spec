...
def demonstrate_multi_environment_setup():
    ...
    try:
        ...
        for env_config in environments:
            ...
            print(f"   🔐 Compliance: {', '.join([cf.value for cf in env_config.compliance_frameworks]) or 'None'}")
            print(f"   📈 Scaling: {env_config.scaling_config['min_instances']}-{env_config.scaling_config['max_instances']} instances")
            print()
        ...
        for env_type, adapter in adapters.items():
            ...
            with adapter.track_routing_session(f"env-test-{env_type.value}") as session:
                result = session.track_multi_model_routing(
                    ...
                    routing_strategy="reliability_first" if env_type == Environment.PRODUCTION else "balanced",
                    complexity="enterprise" if env_type == Environment.PRODUCTION else "moderate"
                )
                
                print(f"   🤖 Model: {result.model}")
                print(f"   💰 Cost: ${result.total_cost:.4f}")
                print(f"   🔐 Compliance: {'✅ Verified' if env_type == Environment.PRODUCTION else '⚠️ Basic'}")
                print()
...
def demonstrate_high_availability_patterns():
    ...
    try:
        ...
        for operation in operations:
            with primary_adapter.track_routing_session(f"primary-{operation['type']}") as session:
                result = session.track_multi_model_routing(
                    models=["gpt-4", "claude-3-opus", "claude-3-sonnet"],
                    input_data={
                ...
                print(f"   ✅ {operation['type']}: {result.model} - ${result.total_cost:.4f}")
        ...
        print(f"   📈 Cost difference: ${cost_difference:.4f} ({((cost_difference / primary_total) * 100):.1f}%)")
        print(f"   ⏱️  Failover time: ~2 seconds (within 15m RTO)")
        print(f"   🎯 Availability: 99.9% maintained")
        print()
...
def demonstrate_compliance_frameworks():
    ...
    try:
        ...
        for request in sensitive_requests:
            ...
            with adapter.track_routing_session(f"compliance-{framework.value}") as session:
                result = session.track_multi_model_routing(
    ...
    try:
        ...
        for pattern in workload_patterns:
            ...
            for i in range(pattern["operations"]):
                with production_adapter.track_routing_session(f"{pattern['name'].lower().replace(' ', '_')}-{i}") as session:
                    result = session.track_multi_model_routing(
        ...
        thresholds = {
            ...
            "error_rate": {"threshold": 1.0, "current": monitoring_results['error_rate']},
            "cost_per_hour": {"threshold": 50.0, "current": monitoring_results['total_cost'] * 60},
            "sla_compliance": {"threshold": 99.5, "current": monitoring_results['sla_compliance']}
        }
        
        for metric, data in thresholds.items():
            if metric == "sla_compliance":
                status = "🟢 HEALTHY" if data["current"] >= data["threshold"] else "🔴 ALERT"
            else:
                status = "🟢 HEALTHY" if data["current"] <= data["threshold"] else "🔴 ALERT"
            
            print(f"   {metric.replace('_', ' ').title()}: {status}")
            print(f"     Current: {data['current']:.2f} | Threshold: {data['threshold']}")
...
def demonstrate_auto_scaling_patterns():
    ...
    try:
        ...
        print(f"   📈 Instance range: {scaling_config['min_instances']}-{scaling_config['max_instances']}")
        ...
        for scenario in load_scenarios:
            ...
            required_instances = max(
                ...
                min(
                    ...
                    (scenario["requests"] + requests_per_instance - 1) // requests_per_instance
                )
            )
            
            # Simulate scaling decision
            if required_instances > current_instances:
                print(f"   ⬆️  Scaling up: {current_instances} → {required_instances} instances")
            elif required_instances < current_instances:
                print(f"   ⬇️  Scaling down: {current_instances} → {required_instances} instances")
            else:
            ...
            for i in range(min(scenario["requests"], 10)):  # Process up to 10 requests for demo
                with scaling_adapter.track_routing_session(f"{scenario['name'].lower().replace(' ', '_')}-{i}") as session:
                    result = session.track_multi_model_routing(
        ...
        print(f"   📈 Peak instances: {max([sc['requests'] // 10 + 1 for sc in load_scenarios])}")
        print(f"   📉 Min instances maintained: {scaling_config['min_instances']}")
        print(f"   🎯 Average CPU utilization: ~65% (within target)")
        print()
...
def demonstrate_cost_governance_enterprise():
    ...
    try:
        ...
        usage_patterns = {
            "engineering": {
                "operations": [
                    ...
                    {"type": "automated_testing", "complexity": "moderate", "count": 25},
                    {"type": "deployment_analysis", "complexity": "complex", "count": 8}
                ]
            },
            "product": {
                "operations": [
                    ...
                    {"type": "competitive_intel", "complexity": "moderate", "count": 10}
                ]
            },
            "customer_success": {
                "operations": [
            ...
            "sales": {
                "operations": [
                    ...
                    {"type": "proposal_generation", "complexity": "moderate", "count": 6},
        ...
        for dept_name, adapter in department_adapters.items():
            ...
            for operation in usage_patterns[dept_name]["operations"]:
                for i in range(operation["count"]):
                    with adapter.track_routing_session(f"{dept_name}-{operation['type']}-{i}") as session:
                        result = session.track_multi_model_routing(
                            models=["gpt-4", "claude-3-opus", "claude-3-sonnet", "gpt-3.5-turbo"],
                            input_data={
                                "department": dept_name,
                            ...
                            routing_strategy="cost_optimized" if dept_name == "sales" else "balanced",
            ...
            department_costs[dept_name] = {
                ...
                "utilization": (dept_cost / departments[dept_name]["daily_budget"]) * 100
            }
            
            print(f"   💰 Total cost: ${dept_cost:.4f}")
            print(f"   📊 Operations: {dept_operations}")
            print(f"   📈 Budget utilization: {department_costs[dept_name]['utilization']:.1f}%")
            print()
        ...
        print(f"📊 **Overall Budget Utilization: {(total_spent / total_budget) * 100:.1f}%**")
        print()
        
        # Department ranking by efficiency
        print("🏆 Department Cost Efficiency Ranking:")
        efficiency_ranking = sorted(
            department_costs.items(),
            key=lambda x: x[1]["cost"] / x[1]["operations"]
        )
        ...
        for dept_name, stats in department_costs.items():
            if stats["utilization"] > 90:
                print(f"   🔴 {dept_name.title()}: HIGH - {stats['utilization']:.1f}% budget used")
            elif stats["utilization"] > 80:
                print(f"   🟡 {dept_name.title()}: MEDIUM - {stats['utilization']:.1f}% budget used")
            else:
                print(f"   🟢 {dept_name.title()}: LOW - {stats['utilization']:.1f}% budget used")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("• High-availability patterns maintain 99.9% uptime with automatic failover")
