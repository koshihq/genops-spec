...
def demonstrate_customer_support_workflow():
    ...
    try:
        ...
        print(
            f"📉 Average cost per step: ${total_workflow_cost / len(workflow_steps):.4f}"
        )
        print()
        ...
        sorted_agents = sorted(
            workflow_results.items(), key=lambda x: x[1]["cost"], reverse=True
        )
        for agent_name, result in sorted_agents:
            percentage = (
                (result["cost"] / total_workflow_cost) * 100
                if total_workflow_cost > 0
                else 0
            )
        ...
        return WorkflowResult(
            ...
            steps_failed=0,
            agent_costs={
                name: result["cost"] for name, result in workflow_results.items()
            },
...
def demonstrate_content_creation_pipeline():
    ...
    try:
        ...
        for strategy, total_cost in sorted(
            strategy_costs.items(), key=lambda x: x[1], reverse=True
        ):
...
def demonstrate_parallel_agent_execution():
    ...
    try:
        ...
        parallel_duration = max(
            sequential_duration * 0.3, 1.0
        )  # Simulate 70% time savings
...
def demonstrate_workflow_optimization():
    ...
    print(
        "    'classification': {'model': 'gpt-3.5-turbo', 'strategy': 'cost_optimized'},"
    )
    ...
    print("📊 **Optimization Impact Example:**")
        ...
        if success:
            ...
            if results:
                ...
                if "customer_support" in results:
                    ...
                    print(
                        f"   🤖 Customer Support: ${cs_result.total_cost:.4f} ({cs_result.steps_completed} steps)"
                    )

                if "content_creation" in results:
                    cc_result = results["content_creation"]
                    print(
                        f"   ✍️  Content Creation: ${cc_result['total_cost']:.4f} ({cc_result['stages']} stages)"
                    )

                if "parallel_execution" in results:
                    pe_result = results["parallel_execution"]
                    print(
                        f"   ⚡ Parallel Execution: {pe_result['time_savings_percent']:.1f}% time savings"
                    )
