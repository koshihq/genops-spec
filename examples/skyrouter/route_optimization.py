...
def demonstrate_routing_strategy_optimization():
    ...
    try:
        ...
        from genops.providers.skyrouter_pricing import RaindropPricingConfig
    ...
    print(f"  🎯 Focus: Route optimization across 150+ models")
    print()
    ...
    routing_strategies = ["cost_optimized", "balanced", "latency_optimized", "reliability_first"]
    ...
    for scenario in test_scenarios:
        ...
        for strategy in routing_strategies:
            ...
            with adapter.track_routing_session(f"optimization-{scenario['name'].lower().replace(' ', '_')}-{strategy}") as session:
                ...
                print(f"       💰 ${result.total_cost:.4f} | 🤖 {result.model} | ⚡ {result.route_efficiency_score:.2f}")
        ...
        best_efficiency = max(scenario_results.items(), key=lambda x: x[1]["efficiency_score"])
...
def analyze_optimization_results(optimization_results: Dict[str, Dict[str, Any]]):
    ...
    for strategy in ["cost_optimized", "balanced", "latency_optimized", "reliability_first"]:
    ...
    if best_overall_cost[0] == best_overall_efficiency[0]:
        recommendations.append(f"🎯 **Use '{best_overall_cost[0]}' as your default strategy** - it provides the best balance of cost and efficiency.")
    else:
        recommendations.append(f"💰 **For cost optimization**: Use '{best_overall_cost[0]}' (saves ${(max(total_costs.values()) - best_overall_cost[1]):.4f} vs worst)")
        recommendations.append(f"⚡ **For efficiency optimization**: Use '{best_overall_efficiency[0]}' (efficiency score: {best_overall_efficiency[1]/num_scenarios:.2f})")
    ...
    if len(set(scenario_recommendations.values())) > 1:
        recommendations.append("🎛️ **Use scenario-specific strategies for optimal results:**")
    ...
    cost_savings = sum(
        ...
        for scenario in high_volume_scenarios if scenario in optimization_results
    )
    
    if cost_savings > 0:
        monthly_savings = cost_savings * 1000  # Assuming 1000 operations per month
        recommendations.append(f"📊 **Volume optimization**: Optimizing high-volume scenarios could save ~${monthly_savings:.2f}/month")
...
def demonstrate_volume_discount_optimization():
    ...
    try:
        ...
        for scenario in volume_scenarios:
            ...
            print(f"🏢 **{scenario['name']}** ({scenario['monthly_volume']:,} tokens/month)")
            print(f"   📈 Volume discount: {volume_info['current_discount_percentage']:.1f}%")
            print(f"   💰 Monthly cost: ${cost_estimate['final_monthly_cost']:.2f}")
            print(f"   💾 Discount savings: ${cost_estimate['volume_discount_amount']:.2f}")
            print(f"   📊 Cost per operation: ${cost_estimate['cost_per_operation']:.4f}")
            ...
            if volume_info['next_threshold']:
                ...
                print(f"   🎯 Next tier: {tokens_to_next:,} more tokens for {next_discount:.1f}% discount")
...
def demonstrate_intelligent_route_selection():
    ...
    try:
        ...
        for scenario in intelligent_scenarios:
            ...
            print(f"📋 **{context}**")
            print(f"   📝 Requirements: {', '.join(f'{k}={v}' for k, v in requirements.items())}")
            print(f"   💡 Recommended: {recommended}")
            
            # Test the recommended strategy
            with adapter.track_routing_session(f"intelligent-{context.lower().replace(' ', '_')}") as session:
                result = session.track_multi_model_routing(
                    models=scenario["models"],
                    input_data={
                        "context": context,
        ...
        strategy_usage = {}
        for context, result in route_intelligence.items():
        ...
        for strategy, count in strategy_usage.items():
            ...
            print(f"   📊 {strategy}: {count}/{total_contexts} scenarios ({percentage:.1f}%)")
        ...
        print(f"   💰 Most cost-effective: {sorted_contexts[0][0]} (${sorted_contexts[0][1]:.4f})")
        print(f"   💎 Most premium: {sorted_contexts[-1][0]} (${sorted_contexts[-1][1]:.4f})")
        print()
...
def demonstrate_cost_optimization_recommendations():
    ...
    try:
        ...
        usage_patterns = [
            ...
            {"type": "premium_complex", "operations": 5, "strategy": "reliability_first"},
            {"type": "mixed_workload", "operations": 20, "strategy": "latency_optimized"}
        ]
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations, 1):
                priority_icon = "🔥" if rec["priority_score"] >= 80 else "⭐" if rec["priority_score"] >= 60 else "💡"
                effort_icon = "🟢" if rec["effort_level"] == "low" else "🟡" if rec["effort_level"] == "medium" else "🔴"
                
                print(f"{priority_icon} **Recommendation {i}: {rec['title']}**")
                print(f"   📝 {rec.get('description', 'Optimize your routing for better cost efficiency')}")
                ...
                if rec["optimization_type"] == "model_optimization":
                    print(f"   🔧 Implementation: Review model selection for high-cost operations")
                elif rec["optimization_type"] == "route_optimization":
                    print(f"   🔧 Implementation: Switch to cost_optimized routing strategy")
                elif rec["optimization_type"] == "volume_optimization":
                    print(f"   🔧 Implementation: Consolidate operations to unlock volume discounts")
        ...
        if budget_limit:
            ...
            print(f"📊 Budget Utilization: {status_icon} {utilization:.1f}% (${current_cost:.4f}/${budget_limit:.2f})")
...
def main():
    ...
    try:
        ...
        if success:
            ...
            print("• Automated recommendations help identify optimization opportunities")
