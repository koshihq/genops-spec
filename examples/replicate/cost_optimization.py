...
def demonstrate_intelligent_model_selection():
    ...
    pricing_calc = ReplicatePricingCalculator()
    ...
    text_models = {
        ...
        "meta/llama-2-13b-chat": {"speed": "medium", "quality": "better", "cost": "medium"},
    ...
    for task in tasks:
        print(f"\n   📋 Task: {task.name}")
        print(f"      Complexity: {task.complexity} | Quality req: {task.quality_threshold}")
        print(f"      Latency: {task.latency_requirement} | Cost sensitivity: {task.cost_sensitivity}")
        ...
        try:
            ...
            print(f"      📊 Quality estimate: {text_models[selected_model]['quality']}")
...
def demonstrate_batch_processing_optimization():
    ...
    for i, content_request in enumerate(content_requests[:3], 1):  # Limit to 3 for demo
        try:
            ...
            print(f"      Request {i}: ${response.cost_usd:.6f} ({response.latency_ms:.0f}ms)")
    ...
    try:
        ...
        if individual_total_cost > 0:
            cost_savings = ((individual_total_cost - response.cost_usd) / individual_total_cost) * 100
            time_savings = ((individual_total_time - batch_total_time) / individual_total_time) * 100
...
def demonstrate_advanced_cost_monitoring():
    ...
    with create_replicate_cost_context("production_workflow", budget_limit=2.0) as context:
        ...
        print(f"   🎯 Budget Utilization: {(final_summary.total_cost / 2.0) * 100:.1f}%")
