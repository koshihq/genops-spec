    ...
    try:
        with adapter.trace_with_governance(
            ...
            for step_name, resp in steps:
                print(
                    f"   {step_name}: ${resp.usage.cost:.6f} ({resp.usage.latency_ms:.0f}ms)"
                )
    ...
    for team_name in teams:
        try:
            ...
            team_adapter = instrument_langfuse(
                team=team_name, project="team-comparison", environment="development"
            )
            ...
            print(
                f"   📊 {team_name.title()} Team: ${response.usage.cost:.6f} "
...
def demonstrate_error_handling():
    ...
    try:
        ...
        adapter = instrument_langfuse(team="error-handling", project="resilience-demo")
