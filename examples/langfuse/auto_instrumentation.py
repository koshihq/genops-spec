...
def demonstrate_existing_langfuse_enhanced():
    ...
    try:
        ...
        @observe()
        def existing_translation_service(
            ...
            print(
                f"   ✅ Translation complete: {translation_result['confidence']:.0%} confidence"
            )
        ...
        translation_result = existing_translation_service(
            "GenOps makes governance transparent and automatic", "spanish"
        )
...
def demonstrate_langchain_auto_enhancement():
    ...
    try:
        ...
        print(
            "   (Your existing LangChain + Langfuse code gets governance automatically)"
        )
...
def demonstrate_multi_provider_governance():
    ...
    try:
        ...
        print(
            f"   🌐 Providers used: {', '.join([op['provider'].title() for op in operations])}"
        )
...
def show_next_steps():
    ...
    next_steps = [
        (
            "🔍 Custom Governance",
            "Add custom team/project attribution to specific functions",
            "adapter.trace_with_governance(name='my_operation', customer_id='abc123')",
        ),
        (
            "💰 Budget Management",
            "Set specific budget limits for different operations",
            "instrument_langfuse(budget_limits={'daily': 5.0, 'monthly': 100.0})",
        ),
        (
            "📊 Advanced Tracking",
            "Use manual tracking for complex multi-step workflows",
            "python basic_tracking.py",
        ),
        (
            "🎯 Evaluation Governance",
            "Add governance to your LLM evaluation workflows",
            "python evaluation_integration.py",
        ),
        (
            "🏭 Production Setup",
            "Configure auto-instrumentation for production deployment",
