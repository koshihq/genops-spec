...
class AdvancedObservabilityManager:
        ...
        **governance_attrs,
        ...
        metrics = ObservabilityMetrics(
            ...
            **{
                ...
                if hasattr(ObservabilityMetrics, k)
            },
        )
        ...
        with self.adapter.trace_with_governance(
            ...
            **governance_attrs,
            ...
            try:
                ...
                print(
                    f"   🔗 Parent: {parent_operation_id[:12] if parent_operation_id else 'None'}"
                )
                ...
                if (
                    parent_operation_id
                    and parent_operation_id in self.active_operations
                ):
                    parent_metrics = self.active_operations[parent_operation_id]
    ...
    def add_operation_cost(
        ...
        if operation_id in self.active_operations:
            ...
            metrics.provider_costs[provider] = (
                metrics.provider_costs.get(provider, 0.0) + cost
            )

            for token_type, count in tokens.items():
                metrics.token_usage[token_type] = (
                    metrics.token_usage.get(token_type, 0) + count
                )
    ...
    def _check_compliance(self, metrics: ObservabilityMetrics):
        ...
        if (
            metrics.duration_ms
            and metrics.duration_ms > self.alert_thresholds["latency_ms"]
        ):
            violations.append(
                f"Latency exceeded threshold: {metrics.duration_ms:.0f}ms"
            )
    ...
    def _check_alerts(self, metrics: ObservabilityMetrics):
        ...
        if metrics.warnings:
            print(
                f"🚨 Alert conditions detected for operation {metrics.operation_type}"
            )
    ...
    def get_operation_analytics(
        ...
        for op in recent_operations:
            ...
            if op.customer_id:
                customer_costs[op.customer_id] = (
                    customer_costs.get(op.customer_id, 0.0) + op.total_cost
                )
        ...
        return {
            ...
            "compliance": {
                "operations_with_warnings": sum(
                    1 for op in recent_operations if op.warnings
                ),
            ...
            with obs_manager.observe_complex_operation(
                ...
                analysis_tasks = [
                    ...
                    },
                    {
                        ...
                        "prompt": "Generate a concise summary of this document",
                    },
                ]
                ...
                for task in analysis_tasks:
                    with obs_manager.observe_complex_operation(
                        ...
                        print(
                            f"       ✅ {task['task']} complete, cost: ${response.usage.cost:.6f}"
                        )

                print(
                    f"   ✅ All analysis tasks complete, total cost: ${total_analysis_cost:.6f}"
                )
        ...
        with obs_manager.observe_complex_operation(
            ...
            for config in provider_configs:
                ...
                with obs_manager.observe_complex_operation(
                    ...
                    print(
                        f"   🤖 Testing {provider_name} ({model_name}) for {use_case}"
                    )
                    ...
                    time.sleep(
                        0.2 + (0.1 if "gpt-4" in model_name else 0)
                    ...
                    print(
                        f"     ✅ Cost: ${mock_cost:.6f}, Latency: {latency_ms:.0f}ms"
                    )

            # Analyze provider performance
            print("\n   📊 Multi-Provider Performance Analysis:")
            print(
                "   Provider/Model        | Cost      | Latency   | Tokens    | Cost/Token"
            )
            print("   " + "-" * 70)
            ...
            for config_name, result in provider_results.items():
                ...
                print(
                    f"   {config_name:<20} | ${result['cost']:>8.6f} | {result['latency_ms']:>6.0f}ms | {sum(result['tokens'].values()):>8} | ${cost_per_token:>9.7f}"
                )
            ...
            print("\n   🏆 Performance Winners:")
            print(
                f"   💰 Most Cost Effective: {cheapest[0]} (${cheapest[1]['cost']:.6f})"
            )
...
def demonstrate_real_time_analytics():
    ...
    try:
        ...
        if summary["average_cost_per_operation"] > 0.03:
            print(
                "   ⚠️  Average cost per operation is elevated - consider optimization"
            )
...
def demonstrate_anomaly_detection():
    ...
    try:
        ...
        print(
            f"   Cost threshold: ${obs_manager.alert_thresholds['cost_per_operation']:.2f}"
        )
        print(f"   Latency threshold: {obs_manager.alert_thresholds['latency_ms']}ms")
        print(
            f"   Error rate threshold: {obs_manager.alert_thresholds['error_rate']:.1%}"
        )
        ...
        for scenario in test_scenarios:
            ...
            try:
                with obs_manager.observe_complex_operation(
                    ...
                    print(
                        f"   ✅ Completed - Cost: ${scenario['cost']:.6f}, Latency: {actual_latency:.0f}ms"
                    )
                    if context["metrics"].warnings:
                        print(
                            f"   🚨 Alerts triggered: {len(context['metrics'].warnings)}"
                        )
        ...
        scenarios_with_alerts = sum(
            1 for r in anomaly_results if r.get("alerts_triggered", False)
        )

        print(f"   📊 Total scenarios tested: {total_scenarios}")
        print(f"   🚨 Scenarios triggering alerts: {scenarios_with_alerts}")
        print(
            f"   📈 Alert detection rate: {(scenarios_with_alerts / total_scenarios) * 100:.1f}%"
        )
        ...
        actual_alerts = [
            r["scenario"] for r in anomaly_results if r.get("alerts_triggered", False)
        ]
...
def demonstrate_enterprise_governance():
    ...
    try:
        ...
        obs_manager.compliance_rules = {
            ...
            "access_controls": {
                "approved_teams": ["enterprise-governance", "ai-research"]
            },
            "audit_requirements": {
        ...
        for result in governance_results:
            ...
            classification_breakdown[classification] = (
                classification_breakdown.get(classification, 0.0) + result["cost"]
            )

            # Customer breakdown
            customer = result["customer"]
            customer_breakdown[customer] = (
                customer_breakdown.get(customer, 0.0) + result["cost"]
            )
...
def show_next_steps():
    ...
    production_steps = [
        (
            "🏭 Production Patterns",
            "Deploy advanced observability in production environments",
            "python production_patterns.py",
        ),
        (
            "📊 Custom Dashboards",
            "Build custom monitoring dashboards for your organization",
            "Integrate with Grafana, Datadog, or your observability platform",
        ),
        (
            "🔄 CI/CD Integration",
            "Integrate governance checks into deployment pipelines",
            "Add GenOps governance to your continuous integration workflows",
        ),
        (
            "📈 Business Intelligence",
            "Connect observability to business intelligence platforms",
            "Export governance data to BI tools for executive reporting",
        ),
        (
            "🌐 Multi-Region Deployment",
            "Deploy governance across multiple geographic regions",
