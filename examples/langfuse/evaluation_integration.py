...
class GovernanceEvaluator:
    ...
    def evaluate_response_quality(
        ...
        **governance_attrs,
        ...
        cost_efficiency = max(
            0.0, 1.0 - (cost / 0.10)
        )  # Normalized to 10 cent baseline

        # Latency scoring (faster = better)
        latency_score = max(
            0.0, 1.0 - (latency_ms / 5000.0)
        )  # Normalized to 5 second baseline
    ...
    def batch_evaluate(
        self, evaluation_dataset: list[dict[str, Any]], **governance_attrs
    ) -> dict[str, Any]:
        ...
        for i, example in enumerate(evaluation_dataset, 1):
            print(
                f"   📊 Evaluating example {i}/{total_evaluations}: {example['prompt'][:40]}..."
            )
                ...
                **governance_attrs,
                ...
                evaluation_result = self.adapter.evaluate_with_governance(
                    ...
                    evaluator_function=lambda: {
                        "score": metrics.overall_score,  # noqa: B023
                        "comment": f"Quality: {metrics.overall_score:.3f}, Cost-efficiency: {metrics.cost_efficiency:.3f}",  # noqa: B023
                        "metrics": metrics.to_dict(),  # noqa: B023
                    },
                    **governance_attrs,
                )
    ...
    try:
        ...
        for scenario in evaluation_scenarios:
            ...
            with adapter.trace_with_governance(
                ...
                print(
                    f"   🛡️  Governance Compliance: {metrics.governance_compliance:.3f}"
                )
...
def demonstrate_batch_evaluation():
    ...
    try:
        ...
        print(
            f"   💡 Average cost per example: ${batch_results['average_cost_per_example']:.6f}"
        )
        ...
        print(
            f"   🛡️  Governance Compliance: {avg_metrics['governance_compliance']:.3f}"
        )
...
def demonstrate_model_comparison():
    ...
    try:
        ...
        for model in models_to_compare:
            ...
            with adapter.trace_with_governance(
                ...
                evaluation_result = adapter.evaluate_with_governance(
                    ...
                    evaluator_function=lambda m=metrics: {
                        "score": m.overall_score,
                        "comment": f"Model: {model} | Score: {m.overall_score:.3f}",  # noqa: B023
        ...
        best_quality = max(
            comparison_results.items(), key=lambda x: x[1]["metrics"].overall_score
        )
        best_cost = min(
            comparison_results.items(), key=lambda x: x[1]["response"].usage.cost
        )
        best_speed = min(
            comparison_results.items(), key=lambda x: x[1]["response"].usage.latency_ms
        )

        print(
            f"🥇 Best Quality: {best_quality[0]} (Score: {best_quality[1]['metrics'].overall_score:.3f})"
        )
        print(
            f"💰 Most Cost Effective: {best_cost[0]} (${best_cost[1]['response'].usage.cost:.6f})"
        )
        print(
            f"⚡ Fastest: {best_speed[0]} ({best_speed[1]['response'].usage.latency_ms:.0f}ms)"
        )
        ...
        for model, result in comparison_results.items():
            ...
            print(
                f"{model:<18} | {metrics.overall_score:>7.3f} | ${cost:>7.6f} | {latency:>6.0f}ms | {metrics.cost_efficiency:>7.3f}"
            )
    ...
    try:
        ...
        for scenario in automation_scenarios:
            ...
            with adapter.trace_with_governance(
                ...
                print(
                    f"   📊 Budget utilization: {automation_result['budget_utilization']:.1%}"
                )

        # Summary of automation results
        total_automation_cost = sum(
            result["total_cost"] for result in automation_results
        )
        ...
        print(
            f"   📊 Average cost per scenario: ${total_automation_cost / len(automation_results):.6f}"
        )
...
def show_next_steps():
    ...
    advanced_patterns = [
        ...
        (
            ...
            "Scale evaluation workflows across teams",
