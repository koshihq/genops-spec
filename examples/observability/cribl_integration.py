...
def demonstrate_cost_tracking():
    ...
    for op in operations:
        with track_enhanced(
            ...
            print(f"  🤖 Processing: {op['customer']} - {op['feature']} ({op['model']})")
...
def demonstrate_policy_enforcement():
    ...
    for policy in policies:
        with track_enhanced(
            ...
            print(f"  {status_emoji} Policy: {policy['name']} → {policy['result'].upper()}")
...
def demonstrate_budget_alerting():
    ...
    for budget in budgets:
        ...
        with track_enhanced(
            ...
            team=budget["name"].split("-")[1] if "team" in budget["name"] else "platform",
            budget_name=budget["name"]
        ) as span:
            print(f"  📊 Budget: {budget['name']} ({budget['period']})")
            print(f"     💰 ${budget['used']:.2f} / ${budget['limit']:.2f} USD ({utilization:.1f}%)")
            ...
            if utilization >= 100:
                print(f"     🚨 CRITICAL ALERT: Budget exceeded by ${budget['used'] - budget['limit']:.2f}")
                print(f"        → PagerDuty incident created (P1)")
                print(f"        → Slack alert: #budget-alerts-critical")
                ...
                print(f"        → Slack alert: #budget-alerts")
            else:
...
def demonstrate_compliance_audit_trail():
    ...
    for op in regulated_operations:
        with track_enhanced(
            ...
            span.set_attribute("genops.compliance.data_classification", op["data_classification"])
            span.set_attribute("genops.compliance.audit_trail_required", True)
            span.set_attribute("genops.compliance.retention_years", op["retention_years"])

            # Cribl routes to long-term storage (S3/Snowflake) for compliance
            print(f"     📤 Routed to:")
            print(f"        → S3 (compliance bucket with {op['retention_years']}-year retention)")
            print(f"        → Snowflake (audit database)")
            print(f"        → Cribl Lake (searchable archive)")
            print()
