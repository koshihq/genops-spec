...
def basic_promptlayer_with_genops():
    ...
    try:
        with adapter.track_prompt_operation(
            ...
            span.update_token_usage(
                input_tokens=45, output_tokens=78, model="gpt-3.5-turbo"
            )

            print("✅ Prompt executed with governance tracking")
            print(
                f"📝 Response: {result.get('response', {}).get('mock', 'Demo response generated')}"
            )
    ...
    try:
        ...
        with adapter.track_prompt_operation(
            ...
            print("🏷️ Cost attributed to team: marketing")
    ...
    try:
        ...
        with adapter.track_prompt_operation(
            ...
            for prompt_info in evaluation_prompts:
                ...
                print(
                    f"   📊 {prompt_name}: Performance {performance_score:.3f}, Cost ${cost_efficiency:.6f}"
                )

            # Find best performing prompt
            best_prompt = min(
                evaluation_results, key=lambda x: x["cost_per_quality_point"]
            )

            print(f"🏆 Best prompt: {best_prompt['prompt']}")
            print(
                f"   💰 Cost per quality point: ${best_prompt['cost_per_quality_point']:.6f}"
            )
            print("   🏷️ Evaluation attributed to team: product")

            eval_span.add_attributes(
                {
                    "evaluation.best_prompt": best_prompt["prompt"],
                    "evaluation.prompts_tested": len(evaluation_prompts),
                    "evaluation.cost_per_quality": best_prompt[
...
def demonstrate_governance_features():
    ...
    try:
        ...
        with adapter.track_prompt_operation(
            ...
            print("📊 Governance context captured:")
...
def show_promptlayer_integration():
    ...
    try:
        ...
        adapter = GenOpsPromptLayerAdapter(
            team="integration-demo", project="feature-showcase"
        )
...
async def main():
    ...
    if success:
        ...
        print(
            "   • Explore 'python evaluation_integration.py' for evaluation workflows"
        )
        ...
        print(
            "   adapter = instrument_promptlayer(team='your-team', project='your-project')"
        )
