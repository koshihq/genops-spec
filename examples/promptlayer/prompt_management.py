            ...
            for version_info in prompt_versions:
                ...
                if expected_cost <= adapter.max_operation_cost:
                    ...
                    with adapter.track_prompt_operation(
                        ...
                        print(
                            f"   ✅ Version {version}: Cost ${expected_cost:.6f}, Quality {quality_score:.3f}, CPQ {metrics.cost_per_quality_point:.6f}"
                        )

                else:
                    print(
                        f"   ⚠️ Version {version}: Exceeds cost limit ${expected_cost:.6f} > ${adapter.max_operation_cost:.6f}"
                    )

            # Select optimal version based on cost-per-quality
            if selected_metrics:
                optimal_version = min(
                    selected_metrics, key=lambda x: x.cost_per_quality_point
                )
                print(f"🏆 Optimal version selected: {optimal_version.version}")
                print(
                    f"   💰 Cost efficiency: ${optimal_version.cost_per_quality_point:.6f} per quality point"
                )
        ...
        with adapter.track_prompt_operation(
            ...
            for config in test_configurations:
                ...
                with adapter.track_prompt_operation(
                    ...
                    print(
                        f"   📊 {variant}: Cost ${base_cost:.6f}, Quality {quality_score:.3f}, Satisfaction {user_satisfaction:.3f}"
                    )
            ...
            print("\n🏆 A/B Test Results:")
            print(
                f"   Best Overall: {best_overall['variant']} (Governance Score: {best_overall['governance_score']:.3f})"
            )
            print(
                f"   Most Cost-Effective: {most_cost_effective['variant']} (CPQ: ${most_cost_effective['cost_per_quality']:.6f})"
            )

            # Governance recommendation
            if best_overall["variant"] == most_cost_effective["variant"]:
                print(
                    f"   ✅ Recommendation: Deploy {best_overall['variant']} (optimal on both metrics)"
                )
            else:
                cost_diff = abs(best_overall["cost"] - most_cost_effective["cost"])
                if cost_diff < 0.01:  # Less than 1 cent difference
                    print(
                        f"   ✅ Recommendation: Deploy {best_overall['variant']} (minimal cost difference)"
                    )
                else:
                    print(
                        f"   ⚖️ Trade-off Decision: {best_overall['variant']} (quality) vs {most_cost_effective['variant']} (cost)"
                    )
    ...
    try:
        ...
        with adapter.track_prompt_operation(
            ...
            for prompt in urgent_prompts:
                ...
                prompt["value_per_dollar"] = (
                    prompt["priority_score"] / prompt["estimated_cost"]
                )
            ...
            sorted_prompts = sorted(
                urgent_prompts, key=lambda x: x["value_per_dollar"], reverse=True
            )

            print(f"   💰 Budget constraint: ${remaining_budget:.2f} remaining")
            print("   🎯 Selecting prompts by value per dollar:")
            ...
            for prompt in sorted_prompts:
                if current_cost + prompt["estimated_cost"] <= remaining_budget:
                    ...
                    with adapter.track_prompt_operation(
                        ...
                        print(
                            f"      ✅ {prompt['name']}: ${prompt['estimated_cost']:.6f} ({prompt['priority']} priority)"
                        )
            ...
            print(
                f"      Budget utilization: {(current_cost / remaining_budget) * 100:.1f}%"
            )
            ...
            planning_span.add_attributes(
                {
                    ...
                    "total_value_score": sum(
                        p["priority_score"] for p in selected_prompts
                    ),
                }
            )
        ...
        for stage_info in lifecycle_stages:
            ...
            with adapter.track_prompt_operation(
                ...
                if test_cost <= cost_limit:
                    print("      ✅ Stage passed governance policies")
                else:
                    print(
                        f"      ❌ Stage exceeds cost limit (${test_cost:.6f} > ${cost_limit:.6f})"
                    )

        print("\n   🎯 Lifecycle Management Benefits:")
        print("      • Cost control at every development stage")
        print("      • Progressive governance enforcement")
        print("      • Automatic budget allocation by stage")
        print("      • Team accountability and attribution")
...
async def main():
    ...
    if success:
        ...
        print(
            "   ✅ Complete prompt lifecycle management with stage-appropriate policies"
        )
