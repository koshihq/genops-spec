...
class AdvancedObservabilityManager:
    ...
    def trace_operation(self, operation_name: str, **kwargs):
        ...
        try:
            if self.enable_detailed_tracing:
                logger.info(
                    f"Starting traced operation: {operation_name} (ID: {metrics.operation_id})"
                )
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        avg_duration = (
            sum(m.duration_ms or 0 for m in self.metrics_buffer) / total_operations
        )

        error_count = sum(m.error_count for m in self.metrics_buffer)
        success_rate = (
            (total_operations - error_count) / total_operations
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "p95_duration_ms": self._calculate_percentile(
                    [m.duration_ms or 0 for m in self.metrics_buffer], 0.95
                ),
                "p99_duration_ms": self._calculate_percentile(
                    [m.duration_ms or 0 for m in self.metrics_buffer], 0.99
                ),
            },
            "governance_context": {
                "team": self.adapter.team,
                "project": self.adapter.project,
                "environment": self.metrics_buffer[-1].environment
...
def demonstrate_distributed_tracing():
    ...
    def cost_efficiency_collector(metrics: ObservabilityMetrics):
        ...
        if metrics.quality_score and metrics.cost_usd > 0:
            metrics.custom_attributes["cost_per_quality_point"] = (
                metrics.cost_usd / metrics.quality_score
            )
        ...
        with obs_manager.trace_operation(
            ...
            with obs_manager.trace_operation(
                ...
                with adapter.track_prompt_operation(
                    ...
                    span.update_token_usage(
                        intent_metrics.input_tokens,
                        intent_metrics.output_tokens,
                        "gpt-3.5-turbo",
                    )

                    print(
                        "   ✅ Intent Analysis: Billing issue detected (Confidence: 87%)"
                    )
            ...
            with obs_manager.trace_operation(
                ...
                with adapter.track_prompt_operation(
                    ...
                    span.update_token_usage(
                        ...
                        context_metrics.output_tokens,
                        "gpt-3.5-turbo",
                    )

                    print(
                        "   ✅ Context Enrichment: Customer profile enhanced (Completeness: 94%)"
                    )
            ...
            with obs_manager.trace_operation(
                ...
                with adapter.track_prompt_operation(
                    ...
                    span.update_token_usage(
                        response_metrics.input_tokens,
                        response_metrics.output_tokens,
                        "gpt-3.5-turbo",
                    )

                    print(
                        "   ✅ Response Generation: Personalized response created (Quality: 91%)"
                    )

            # Aggregate workflow metrics
            workflow_metrics.cost_usd = (
                intent_metrics.cost_usd
                + context_metrics.cost_usd
                + response_metrics.cost_usd
            )
            ...
            workflow_metrics.quality_score = (
                intent_metrics.quality_score
                + context_metrics.quality_score
                + response_metrics.quality_score
            ) / 3
            ...
            print(
                f"      Total Tokens: {workflow_metrics.input_tokens + workflow_metrics.output_tokens}"
            )
    ...
    try:
            ...
            with obs_manager.trace_operation(
                ...
                with adapter.track_prompt_operation(
                    ...
                    adapter.run_prompt_with_governance(
                        ...
                        tags=[
                            "performance_test",
                            f"complexity_{scenario['load_factor']}",
                        ],
                    )
                    ...
                    perf_metrics.quality_score = min(
                        0.95, 0.80 + (0.15 / scenario["load_factor"])
                    )
                    perf_metrics.custom_attributes["load_factor"] = scenario[
                        "load_factor"
                    ]
                    perf_metrics.custom_attributes["expected_duration"] = scenario[
                        "expected_duration"
                    ]

                    span.update_cost(perf_metrics.cost_usd)
                    span.update_token_usage(
                        perf_metrics.input_tokens,
                        perf_metrics.output_tokens,
                        "gpt-3.5-turbo",
                    )
                    ...
                    print(
                        f"   {performance_status} {scenario['name']}: "
                        f"{actual_duration:.0f}ms (expected: {scenario['expected_duration']}ms)"
                    )

        # Performance summary
        print("\n   📊 Performance Monitoring Summary:")
        avg_ratio = sum(r["ratio"] for r in performance_results) / len(
            performance_results
        )
        ...
        acceptable_count = sum(
            1 for r in performance_results if "ACCEPTABLE" in r["status"]
        )
        degraded_count = sum(
            1 for r in performance_results if "DEGRADED" in r["status"]
        )
    ...
    try:
        ...
        print("\n   💰 Cost Analysis:")
        print(
            f"   Avg Cost/Op: ${cost_breakdown.get('average_cost_per_operation', 0):.6f}"
        )
