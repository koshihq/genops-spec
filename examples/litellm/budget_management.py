...
class BudgetManager:
        ...
        with self._lock:
            ...
            if team_or_project not in self.usage_tracking:
                self.usage_tracking[team_or_project] = BudgetUsage(
                    config=config, last_reset_date=datetime.now().strftime("%Y-%m-%d")
                )
    ...
    def check_budget_allowance(
        self, team_or_project: str, estimated_cost: float
    ) -> dict[str, Any]:
        """
        ...
        with self._lock:
            ...
            if config.policy == BudgetPolicy.HARD_LIMIT:
                ...
                overrun_daily = daily_limit * (
                    1 + config.allowed_overrun_percentage / 100
                )
                overrun_monthly = monthly_limit * (
                    1 + config.allowed_overrun_percentage / 100
                )

                if (
                    new_daily_total > overrun_daily
                    or new_monthly_total > overrun_monthly
                ):
                    return {
                        "allowed": False,
            ...
            monthly_usage_pct = (
                new_monthly_total / monthly_limit if monthly_limit > 0 else 0
            )
            ...
            for alert_level, threshold in config.alert_thresholds.items():
                if (
                    max_usage_pct >= threshold
                    and alert_level not in usage.current_alerts
                ):
                    self._trigger_alert(
                        team_or_project,
                        alert_level,
    ...
    def get_budget_summary(
        self, team_or_project: Optional[str] = None
    ) -> dict[str, Any]:
        ...
        with self._lock:
            if team_or_project:
                ...
                return {
                    ...
                    "daily_usage_pct": (usage.daily_spent / config.daily_limit) * 100
                    if config.daily_limit > 0
                    else 0,
                    "monthly_limit": config.monthly_limit,
                    "monthly_spent": usage.monthly_spent,
                    "monthly_remaining": config.monthly_limit - usage.monthly_spent,
                    "monthly_usage_pct": (usage.monthly_spent / config.monthly_limit)
                    * 100
        ...
        with self._lock:
            if team_or_project in self.usage_tracking:
                ...
                self._trigger_alert(
                    ...
                    {"block_reason": reason, "action": "emergency_block_activated"},
                )
...
def setup_alert_system(manager: BudgetManager):
    ...
    def console_alert_handler(team: str, level: AlertLevel, details: dict):
        ...
        if level == AlertLevel.EMERGENCY:
            ...
            print("   Consider immediate budget increase or usage review")
    ...
    def email_alert_handler(team: str, level: AlertLevel, details: dict):
        ...
        if level in [AlertLevel.CRITICAL, AlertLevel.EMERGENCY]:
            ...
            print("   Recipients: budget-alerts@company.com, team-leads@company.com")
...
def demo_budget_configuration():
    ...
    for team, summary in budget_summary.items():
        ...
        print(
            f"   Current usage: ${summary['daily_spent']:.4f} daily, ${summary['monthly_spent']:.4f} monthly"
        )
    ...
    for scenario in test_scenarios:
        ...
        for i, request in enumerate(requests):
            ...
            if allowance["allowed"]:
                ...
                print(
                    f"   ✅ Approved - Remaining daily: ${allowance.get('remaining_daily', 0):.2f}"
                )
                print(
                    f"       Usage: {allowance.get('daily_usage_pct', 0):.1f}% daily, {allowance.get('monthly_usage_pct', 0):.1f}% monthly"
                )
            else:
        ...
        print(
            f"       Daily spent: ${final_summary['daily_spent']:.2f}/${final_summary['daily_limit']:.2f}"
        )
        print(
            f"       Alerts active: {', '.join(final_summary['current_alerts']) if final_summary['current_alerts'] else 'None'}"
        )
...
def demo_real_time_tracking():
    ...
    def budget_aware_request(
        team: str, model: str, messages: list[dict], estimated_cost: float = 0.01
    ):
        ...
        try:
            ...
            response = litellm.completion(
                model=model, messages=messages, max_tokens=30, timeout=10
            )
            ...
            print("   ⚠️ Request failed: [Error details redacted for security]")
            return None

    # Enable GenOps tracking
    auto_instrument(
        team="budget-demo", project="real-time-tracking", daily_budget_limit=50.0
    )
    ...
    total_budget_spent = sum(
        summary["daily_spent"]
        for summary in all_budgets.values()
...
def demo_emergency_controls():
    ...
    block_result = manager.set_emergency_block(
        emergency_team, "Security incident detected - suspicious API usage pattern"
    )
    ...
    for i, cost in enumerate(rapid_costs):
        ...
        if allowance["allowed"]:
            ...
            print(
                f"         ✅ Approved ({allowance.get('daily_usage_pct', 0):.1f}% usage)"
            )
        else:
            print(f"         ❌ Blocked: {allowance['reason']}")
            break
    ...
    print("\n   📊 Final Status:")
    print(
        f"       Spent: ${final_status['daily_spent']:.2f}/${final_status['daily_limit']:.2f}"
    )
    ...
    if len(hourly_costs) > 1:
        ...
        hours_to_limit = (
            (limit - spent) / (hourly_costs[-1] + trend)
            if (hourly_costs[-1] + trend) > 0
            else float("inf")
        )
        ...
        if hours_to_limit < 8:  # Less than 8 hours
            print("      🚨 WARNING: Budget may be exhausted within business day!")
            print("      💡 Recommendation: Review spending or increase budget")
