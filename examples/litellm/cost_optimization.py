...
class ModelProfile:
    ...
    def get_value_score(
        self, complexity: TaskComplexity, budget_priority: float = 0.5
    ) -> float:
        ...
        complexity_fit = (
            1.0 - (max_level - required_level) * 0.1
        )  # Penalty for over-capability
        ...
        value_score = budget_priority * cost_efficiency + (1 - budget_priority) * (
            0.4 * quality_normalized + 0.3 * complexity_fit + 0.3 * performance_score
        )
...
class CostOptimizer:
    ...
    def select_optimal_model(
        ...
        if max_cost is not None:
            available_models = [
                ...
                if model.estimate_cost(estimated_input_tokens, estimated_output_tokens)
                <= max_cost
            ]
            ...
            comparisons.append(
                {
                    ...
                    "cost_per_quality_point": cost / model.quality_score
    ...
    def get_cost_savings_recommendation(
        ...
        savings_percentage = (
            (potential_savings / current_cost) * 100 if current_cost > 0 else 0
        )
        ...
        return {
            ...
            "quality_impact": optimal_model.quality_score
...
def demo_intelligent_model_selection():
    ...
    for model in available_models[:3]:  # Show first 3
        print(
            f"   • {model.model} ({model.provider}) - Quality: {model.quality_score}/10"
        )
    ...
    for scenario in test_scenarios:
        ...
        print(
            f"   Tokens: {scenario['tokens'][0]} input, {scenario['tokens'][1]} output"
        )
...
def demo_cost_comparison_analysis():
    ...
    for complexity, description, tokens in complexity_tests:
        ...
        for i, comp in enumerate(comparisons[:5]):  # Show top 5
            ...
            print(
                f"      🔥 Value: ${comp['cost_per_quality_point']:.6f}/quality point"
            )
            ...
            if i == 0 and len(comparisons) > 1:
                ...
                print(
                    f"      💡 Saves ${savings:.6f} ({savings_pct:.1f}%) vs most expensive"
                )
...
def demo_real_time_optimization():
    ...
    for i, scenario in enumerate(request_scenarios):
        ...
        optimal_model = optimizer.select_optimal_model(
            task_complexity=scenario["complexity"],
            estimated_input_tokens=len(scenario["prompt"].split())
            * 1.3,  # Rough estimate
        ...
        try:
            ...
            if optimal_model.model != baseline_model:
                ...
                if (
                    "potential_savings" in savings_rec
                    and savings_rec["potential_savings"] > 0
                ):
                    total_savings += savings_rec["potential_savings"]
                    print(
                        f"   💰 Saved: ${savings_rec['potential_savings']:.6f} vs {baseline_model}"
                    )
    ...
    if total_savings > 0:
        print(
            f"   🎉 Cost optimization achieved {(total_savings / (stats['total_cost'] + total_savings)) * 100:.1f}% savings!"
        )
    ...
    if cost_breakdown["cost_by_provider"]:
        ...
        for provider, cost in cost_breakdown["cost_by_provider"].items():
            percentage = (
                (cost / cost_breakdown["total_cost"]) * 100
...
def demo_budget_constrained_optimization():
    ...
    budget_scenarios = [
        {
            "name": "Tight budget",
            "max_cost": 0.001,
            "description": "Maximum $0.001 per request",
        },
        {
            "name": "Moderate budget",
            "max_cost": 0.01,
            "description": "Maximum $0.01 per request",
        },
        {
            "name": "Flexible budget",
            "max_cost": 0.1,
            "description": "Maximum $0.10 per request",
        },
    ]
    ...
    for scenario in budget_scenarios:
        ...
        if optimal_model:
            ...
            print(f"   ⭐ Quality: {optimal_model.quality_score}/10")
            print(
                f"   💡 Efficiency: {optimal_model.quality_score / (actual_cost * 1000):.1f} quality/cost ratio"
            )
        else:
            ...
            if available_models:
                costs = [
                    (model, model.estimate_cost(*test_task["tokens"]))
                    for model in available_models
                ]
                cheapest_model, cheapest_cost = min(costs, key=lambda x: x[1])

                print(
                    f"   💡 Cheapest option: {cheapest_model.model} at ${cheapest_cost:.6f}"
                )
                print(
                    f"   📈 Budget increase needed: ${cheapest_cost - scenario['max_cost']:.6f}"
                )
