...
MODEL_EQUIVALENTS = {
    "fast_chat": [
        ModelBenchmark("openai", "gpt-3.5-turbo", 0.0015, 0.002, 800, 8.0, "Fast general chat"),
        ModelBenchmark("anthropic", "claude-3-haiku", 0.00025, 0.00125, 900, 8.2, "Fast thoughtful responses"),
        ModelBenchmark("google", "gemini-pro", 0.0005, 0.0015, 1200, 7.8, "Fast multimodal"),
        ModelBenchmark("cohere", "command-light", 0.0003, 0.0006, 700, 7.5, "Fast enterprise")
    ],
    "powerful_reasoning": [
        ModelBenchmark("openai", "gpt-4", 0.03, 0.06, 2000, 9.2, "Advanced reasoning"),
        ModelBenchmark("anthropic", "claude-3-sonnet", 0.003, 0.015, 2200, 9.0, "Balanced power/speed"),
        ModelBenchmark("google", "gemini-1.5-pro", 0.0035, 0.01, 2500, 8.8, "Advanced multimodal"),
        ModelBenchmark("anthropic", "claude-3-opus", 0.015, 0.075, 3000, 9.5, "Maximum capability")
    ],
    "coding": [
        ModelBenchmark("openai", "gpt-4", 0.03, 0.06, 2000, 9.0, "Code generation"),
        ModelBenchmark("anthropic", "claude-3-sonnet", 0.003, 0.015, 2200, 8.8, "Code analysis"),
        ModelBenchmark("google", "gemini-1.5-pro", 0.0035, 0.01, 2500, 8.5, "Multimodal coding")
    ]
}
...
class CostOptimizationEngine:
    ...
    def select_optimal_model(self, use_case: str, budget_priority: float = 0.7) -> Optional[ModelBenchmark]:
        """
        ...
        for model in available_models:
            ...
            estimated_cost = (model.cost_per_1k_input * 0.15) + (model.cost_per_1k_output * 0.05)
            ...
            quality_score = (10 - model.quality_score) / 10  # Invert quality (lower is better)
            
            # Weighted optimization score
            optimization_score = (
                budget_priority * cost_score +
                (1 - budget_priority) * 0.5 * (latency_score + quality_score)
            )
    ...
    def compare_costs(self, use_case: str, input_tokens: int = 150, output_tokens: int = 50):
        """Compare costs across all available models for a use case."""
        available_models = self.get_available_models(use_case)
        
        comparisons = []
        for model in available_models:
            cost = (model.cost_per_1k_input * input_tokens / 1000 + 
...
def demo_cost_comparison():
    ...
    for use_case in use_cases:
        ...
        for i, comp in enumerate(comparisons):
            ...
            print(f"      Quality: {comp['quality_score']}/10, Latency: {comp['latency_ms']}ms")
            
            if i == 0 and len(comparisons) > 1:
                savings = ((comparisons[-1]['cost'] - comp['cost']) / comparisons[-1]['cost']) * 100
...
def demo_smart_model_selection():
    ...
    scenarios = [
        ...
        ("powerful_reasoning", 0.3, "Quality-focused: Complex reasoning, quality over cost"),
    ...
    for use_case, budget_priority, description in scenarios:
        ...
        print(f"   📍 Selected: {optimal_model.provider.title()} ({optimal_model.model})")
        print(f"   💡 Reason: {optimal_model.use_case}")
        
        # Test with actual LiteLLM call
        try:
            print(f"   🔄 Testing with real API call...")
            ...
            response = litellm.completion(
                ...
                messages=[
                    {"role": "user", "content": f"Test message for {use_case.replace('_', ' ')}. Respond briefly."}
                ],
                max_tokens=20,
                timeout=10
            )
    ...
    for team in teams:
        ...
        if comparisons:
            ...
            if len(comparisons) > 1:
                ...
                print(f"   🎯 Potential savings: {requests_cheapest - requests_expensive:.0f} extra requests")
