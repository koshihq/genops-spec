...
def check_setup():
    ...
    try:
        ...
        print("❌ Import error: [Error details redacted for security]")
    ...
    for scenario in user_scenarios:
        ...
        if scenario["track_detailed"]:
            ...
            tracking_context = {
                ...
                "customer_id": scenario["customer_id"],
                "custom_tags": {"tier": scenario["tier"], "tracking_level": "basic"},
            }
        ...
        try:
            ...
            print(
                "   ⚠️ Request simulation failed: [Error details redacted for security]"
            )
...
def demo_performance_patterns():
    ...
    print(
        f"   💰 Tracking overhead reduced by {((requests_processed - requests_tracked) / requests_processed) * 100:.0f}%"
    )
...
def demo_custom_attribution():
    ...
    for example in attribution_examples:
        ...
        try:
            ...
            print(
                "   ⚠️ Attribution setup failed: [Error details redacted for security]"
            )
...
def demo_usage_analytics():
    ...
    from genops.providers.litellm import get_cost_summary, get_usage_stats
    ...
    print(
        f"   Average cost per request: ${usage_stats.get('avg_cost_per_request', 0):.6f}"
    )
    ...
    if usage_stats.get("provider_usage"):
        ...
        for provider, stats in usage_stats["provider_usage"].items():
            print(
                f"   • {provider}: {stats.get('requests', 0)} requests, ${stats.get('cost', 0):.6f}"
            )
    ...
    if cost_by_team.get("cost_by_team"):
        ...
        for team, cost in cost_by_team["cost_by_team"].items():
            percentage = (
                (cost / cost_by_team["total_cost"]) * 100
                if cost_by_team["total_cost"] > 0
                else 0
            )
    ...
    if cost_by_project.get("cost_by_project"):
        ...
        for project, cost in cost_by_project["cost_by_project"].items():
            percentage = (
                (cost / cost_by_project["total_cost"]) * 100
                if cost_by_project["total_cost"] > 0
                else 0
            )
    ...
    if usage_stats.get("total_requests", 0) > 0:
        print("   • Average request cost optimized for tracking efficiency")
        print("   • Multi-dimensional attribution enables cost allocation")
        print("   • Performance patterns minimize operational overhead")
    else:
        print("   • No tracked requests in current session")
        print("   • Run with valid API keys for live tracking data")


@contextmanager
def custom_tracking_context(
    model: str, business_context: dict[str, Any], performance_mode: str = "balanced"
):
    """
    ...
    for scenario in business_scenarios:
        ...
        try:
            with custom_tracking_context(
                ...
                print("   ✅ Custom tracking context active")

        except Exception:
            print("   ⚠️ Advanced pattern failed: [Error details redacted for security]")
