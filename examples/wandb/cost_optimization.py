...
class CostOptimizedMLExperiment:
    ...
    def estimate_experiment_cost(
        ...
        model_factors = {"small": 0.7, "medium": 1.0, "large": 1.8, "xlarge": 3.2}

        # Dataset size factors
        dataset_factors = {"small": 0.5, "medium": 1.0, "large": 1.8, "xlarge": 3.5}
    ...
    def estimate_performance(
        ...
        # Optimizer impact
        optimizer_boost = {"sgd": -0.01, "adam": 0.0, "adamw": 0.015}
...
def run_cost_optimization_analysis(adapter, budget_limit: float) -> dict[str, Any]:
    ...
    print("   • Optimization Target: Maximize performance within budget")
    print()
    ...
    for config in base_configs:
        for resource_profile in ResourceProfile:
            ...
            duration = (
                estimated_cost / 0.75
            )  # Rough estimate based on standard GPU cost
    ...
    affordable_profiles = [
        p for p in experiment_profiles if p.estimated_cost <= budget_limit
    ]
    ...
    most_efficient_resource = max(
        profile_counts.keys(), key=lambda x: profile_counts[x]
    )
    ...
    cheap_perf_ratio = (
        cheapest_profile.expected_performance / cheapest_profile.estimated_cost
    )
    ...
    if budget_efficiency < 0.8:
        recommendations.append(
            "💰 Budget underutilized: Consider running multiple experiments or upgrading configurations"
        )
    elif budget_efficiency > 0.95:
        recommendations.append(
            "⚠️  Budget nearly exhausted: Consider smaller configurations or staged experiments"
        )
    ...
    if most_common_model != "medium":
        recommendations.append(f"🧠 Optimal model size for budget: {most_common_model}")
    ...
    return {
        ...
        "performance_distribution": {
            ...
            "avg_performance": np.mean(
                [p.expected_performance for p in affordable_profiles]
            ),
        },
    }
...
def run_budget_monitoring_simulation(adapter, initial_budget: float) -> dict[str, Any]:
    ...
    print("📊 Simulating Budget Monitoring & Alerts...")
    print(f"   • Initial Budget: ${initial_budget:.2f}")
    print("   • Monitoring Period: 5 simulated experiments")
    print()
    ...
    for i in range(5):
        ...
        if experiment_cost > current_budget:
            ...
            print(
                f"   🚫 Experiment {i + 1} blocked: Cost ${experiment_cost:.2f} > Budget ${current_budget:.2f}"
            )
            break
        ...
        if experiment_cost > current_budget * 0.8:
            ...
            print(
                f"   ⚠️  High cost warning for Experiment {i + 1}: {(experiment_cost / current_budget) * 100:.1f}% of budget"
            )
            ...
            print(
                f"   💡 Medium cost alert for Experiment {i + 1}: {(experiment_cost / current_budget) * 100:.1f}% of budget"
            )
        ...
        print(
            f"   ✅ Experiment {i + 1}: Cost ${actual_cost:.2f}, Performance {estimated_performance:.3f}"
        )
        ...
        if current_budget < initial_budget * 0.1:
            ...
            print(
                f"   🚨 CRITICAL: Only ${current_budget:.2f} remaining ({100 - budget_used_pct:.1f}% of budget)"
            )
    ...
    if len(spent_amounts) > 0:
        cost_variation = (
            np.std(spent_amounts) / avg_cost_per_experiment
            if avg_cost_per_experiment > 0
            else 0
        )
        if cost_variation > 0.3:
            insights.append(
                "High cost variation detected - consider standardizing experiment configurations"
            )
        ...
        if len(alert_events) > 3:
            insights.append(
                "Multiple budget alerts triggered - implement tighter cost controls"
            )
    ...
    return {
        ...
        "budget_utilization": (total_spent / initial_budget) * 100
        if initial_budget > 0
        else 0,
        "experiments_completed": len(experiment_results),
        "experiments_blocked": len(
            [a for a in alert_events if a.get("action") == "experiment_blocked"]
        ),
        ...
        "alert_summary": {
            ...
            "critical_alerts": len(
                [a for a in alert_events if a["type"] == "budget_critical"]
            ),
            "blocked_experiments": len(
                [a for a in alert_events if a.get("action") == "experiment_blocked"]
            ),
        },
    }
...
def generate_cost_forecast(
    adapter, historical_data: list[dict], target_experiments: int
) -> CostForecast:
    ...
    if len(model_costs) > 1:
        ...
        cost_drivers.append(
            f"Model size: {max_model} models cost {avg_by_model[max_model] / avg_by_model[min_model]:.1f}x more than {min_model}"
        )

    # Analyze epoch impact
    epoch_costs = [
        (exp["config"].get("epochs", 10), exp["actual_cost"]) for exp in historical_data
    ]
    if len(epoch_costs) > 5:
        ...
        if high_epoch and low_epoch:
            cost_drivers.append(
                f"Training epochs: 15+ epochs average ${np.mean(high_epoch):.2f} vs ≤10 epochs ${np.mean(low_epoch):.2f}"
            )
    ...
    if trend_factor > 1.2:
        recommendations.append(
            "📈 Costs trending upward - review recent configuration changes"
        )
    elif trend_factor < 0.8:
        recommendations.append(
            "📉 Costs trending downward - good optimization progress"
        )

    if cost_std > avg_cost * 0.5:
        recommendations.append(
            "📊 High cost variability - standardize experiment configurations for predictable budgeting"
        )

    if avg_cost > 2.0:
        recommendations.append(
            "💰 High average experiment cost - consider smaller models or shorter training runs"
        )
...
def main():
    ...
    try:
        ...
        print("   • Max Experiment Cost: $15.00")
        print(f"   • Current Usage: ${initial_metrics['daily_usage']:.2f}")
        print(
            f"   • Cost Alerts: {'Enabled' if initial_metrics['cost_alerts_enabled'] else 'Disabled'}"
        )
        ...
        if "error" in optimization_analysis:
            ...
            print(
                f"💡 Minimum budget required: ${optimization_analysis['min_cost_required']:.2f}"
            )
            return False

        print("📈 Cost Optimization Results:")
        print(
            f"   • Configurations Evaluated: {optimization_analysis['total_configurations_evaluated']}"
        )
        print(
            f"   • Affordable Options: {optimization_analysis['affordable_configurations']}"
        )
        print(
            f"   • Budget Utilization: {optimization_analysis['budget_utilization'] * 100:.1f}%"
        )
        ...
        print(
            f"   • Cost Efficiency: {best_config['cost_efficiency']:.1f} performance/dollar"
        )
        ...
        print(
            f"   • Expected Performance: {cheapest_config['expected_performance']:.3f}"
        )
        ...
        print(
            f"   • Cost Efficiency: {perf_config['cost_efficiency']:.1f} performance/dollar"
        )

        # Optimization recommendations
        print("\n💡 Optimization Recommendations:")
        for i, rec in enumerate(
            optimization_analysis["optimization_recommendations"], 1
        ):
            print(f"   {i}. {rec}")
        ...
        print(
            f"   • Budget Utilization: {budget_simulation['budget_utilization']:.1f}%"
        )
        print(
            f"   • Experiments Completed: {budget_simulation['experiments_completed']}"
        )
        print(f"   • Experiments Blocked: {budget_simulation['experiments_blocked']}")
        print(
            f"   • Average Cost/Experiment: ${budget_simulation['average_cost_per_experiment']:.2f}"
        )
        ...
        print("📊 Cost Forecast for 20 Future Experiments:")
        print(f"   • Estimated Total Cost: ${forecast.estimated_total_cost:.2f}")
        print(
            f"   • Confidence Interval: ${forecast.confidence_interval[0]:.2f} - ${forecast.confidence_interval[1]:.2f}"
        )
        ...
        for component, cost in forecast.cost_breakdown.items():
            ...
            print(
                f"   • {component.replace('_', ' ').title()}: ${cost:.2f} ({percentage:.1f}%)"
            )
        ...
        print("⚡ Running experiment with optimal cost-efficiency configuration...")
        ...
        with adapter.track_experiment_lifecycle(
            ...
            for epoch in range(epochs):
                ...
                accuracy = base_perf * (1 - np.exp(-3 * progress)) + random.uniform(
                    -0.01, 0.01
                )
                ...
                epoch_cost = (
                    best_config["estimated_cost"] / epochs * random.uniform(0.95, 1.05)
                )
                experiment.estimated_cost += epoch_cost

                print(
                    f"      📊 Epoch {epoch + 1:2d}: accuracy={accuracy:.3f}, loss={loss:.3f}, cost=${epoch_cost:.3f}"
                )
            ...
            print("   ✅ Cost-optimized experiment completed!")
            print(f"   📊 Final Cost: ${experiment.estimated_cost:.3f}")
            print(f"   🎯 Final Performance: {accuracy:.3f}")
            print(
                f"   💰 Cost Efficiency: {accuracy / experiment.estimated_cost:.1f} performance/dollar"
            )
        ...
        print("\n📚 What you learned:")
        print("   ✅ How to perform cost optimization analysis for ML experiments")
        print("   ✅ Real-time budget monitoring with alerts and interventions")
        print("   ✅ Resource efficiency analysis and cost-per-accuracy optimization")
        print("   ✅ Cost forecasting based on historical experiment patterns")
        print("   ✅ Budget-constrained experiment planning and execution")
        print("   ✅ Cost-aware ML workflow design and governance")

        print("\n🚀 Next Steps:")
        print("   • Explore advanced features: python advanced_features.py")
        print("   • Deploy in production: python production_patterns.py")
        print("   • Review complete documentation: docs/integrations/wandb.md")

        print("\n💡 Key Cost Optimization Insights:")
        print(
            f"   • Most efficient configuration achieved {best_config['cost_efficiency']:.1f} performance/dollar"
        )
        print(
            f"   • Budget monitoring prevented {alert_summary['blocked_experiments']} over-budget experiments"
        )
        print(
            f"   • Cost forecasting predicts ${forecast.estimated_total_cost:.2f} for 20 future experiments"
        )
        print(
            f"   • Resource profile '{best_config['resource_profile']}' shows best cost efficiency"
        )
