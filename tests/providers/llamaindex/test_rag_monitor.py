...
class TestRAGOperationMonitoring:
    ...
    def test_monitor_rag_operation_timing(self, monitor):
        ...
        with monitor.monitor_rag_operation("timing test") as operation_context:
    ...
    def test_monitor_rag_operation_exception_handling(self, monitor):
        ...
        with pytest.raises(ValueError):
            with monitor.monitor_rag_operation("error test") as operation_context:
...
class TestRAGAnalytics:
    ...
    def monitor_with_data(self):
        ...
        # Add quality metrics
        monitor.record_quality_metrics("query_1", RAGQualityMetrics(
            retrieval_relevance=0.85,
            response_faithfulness=0.90
        ))
        monitor.record_quality_metrics("query_2", RAGQualityMetrics(
            retrieval_relevance=0.80,
            response_faithfulness=0.85
        ))

        # Add performance metrics
        monitor.record_performance_metrics("query_1", RAGPerformanceMetrics(
            total_latency_ms=1200.0,
            tokens_per_second=20.0
        ))
        monitor.record_performance_metrics("query_2", RAGPerformanceMetrics(
