...
class TestAPIErrorHandling:
    ...
    def setup_method(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer") as mock_pl:
            ...
            self.adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-error-test", team="error-test-team"
            )

    def test_api_connection_error(self):
        """Test handling of API connection errors."""
        self.mock_client.run.side_effect = ConnectionError(
            "Failed to connect to PromptLayer API"
        )
    ...
    def test_api_rate_limit_error(self):
        ...
        with pytest.raises(Exception, match="Rate limit exceeded"):
            with self.adapter.track_prompt_operation("rate_limit_test"):
                self.adapter.run_prompt_with_governance(
    ...
    def test_api_timeout_error(self):
        ...
        with pytest.raises(TimeoutError):
            with self.adapter.track_prompt_operation("timeout_test"):
                self.adapter.run_prompt_with_governance(
                    prompt_name="timeout_test", input_variables={"test": "timeout"}
                )
    ...
    def test_malformed_api_response(self):
        ...
        with self.adapter.track_prompt_operation("malformed_test"):
            result = self.adapter.run_prompt_with_governance(
                prompt_name="malformed_test", input_variables={"test": "malformed"}
            )
...
class TestGracefulDegradation:
    ...
    def test_promptlayer_sdk_unavailable(self):
        ...
        with patch("genops.providers.promptlayer.HAS_PROMPTLAYER", False):
            ...
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-unavailable-test", team="degradation-test-team"
            )
    ...
    def test_missing_api_key_graceful_handling(self):
        ...
        with patch(
            "genops.providers.promptlayer.PromptLayer",
            side_effect=Exception("Missing API key"),
        ):
            adapter = GenOpsPromptLayerAdapter(
    ...
    def test_network_unavailable_handling(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer") as mock_pl:
            ...
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-network-test", team="network-test-team"
            )

            with pytest.raises(OSError):
                with adapter.track_prompt_operation("network_test") as span:
                    adapter.run_prompt_with_governance(
                        prompt_name="network_test", input_variables={"test": "network"}
                    )
...
class TestContextManagerErrorHandling:
    ...
    def setup_method(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer"):
            self.adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-context-error-test", team="context-error-team"
            )
...
class TestGovernanceErrorHandling:
    ...
    def test_budget_violation_with_enforced_policy(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer"):
            ...
            with pytest.raises(ValueError, match="Daily budget limit"):
                with adapter.track_prompt_operation("budget_violation_test"):
                    pass
    ...
    def test_cost_calculation_overflow(self):
        ...
        span = EnhancedPromptLayerSpan(
            operation_type="overflow_test", operation_name="cost_overflow"
        )
    ...
    def test_concurrent_access_errors(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer"):
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-concurrent-error", team="concurrent-team"
            )
            ...
            assert (
                len(adapter.active_spans) <= 3
            )  # May have cleanup issues but shouldn't crash
...
class TestRecoveryPatterns:
    ...
    def test_retry_after_transient_error(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer") as mock_pl:
            ...
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-retry-test", team="retry-team"
            )

            # First attempt should fail
            with pytest.raises(ConnectionError):
                with adapter.track_prompt_operation("retry_test_1"):
                    adapter.run_prompt_with_governance(
                        prompt_name="retry_test_1", input_variables={"attempt": 1}
                    )

            # Second attempt should succeed
            with adapter.track_prompt_operation("retry_test_2"):
                result = adapter.run_prompt_with_governance(
                    prompt_name="retry_test_2", input_variables={"attempt": 2}
                )
    ...
    def test_fallback_to_mock_after_persistent_failure(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer") as mock_pl:
            ...
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-fallback-test", team="fallback-team"
            )
            ...
            with adapter.track_prompt_operation("fallback_test") as span:
                result = adapter.run_prompt_with_governance(
                    prompt_name="fallback_test", input_variables={"test": "fallback"}
                )
                span.update_cost(0.01)
    ...
    def test_partial_operation_recovery(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer") as mock_pl:
            ...
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-partial-recovery", team="recovery-team"
            )
...
class TestRobustnessUnderLoad:
    ...
    def test_memory_pressure_handling(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer"):
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-memory-pressure", team="memory-team"
            )
    ...
    def test_long_running_operation_stability(self):
        ...
        with patch("genops.providers.promptlayer.PromptLayer"):
            adapter = GenOpsPromptLayerAdapter(
                promptlayer_api_key="pl-long-running", team="stability-team"
            )
