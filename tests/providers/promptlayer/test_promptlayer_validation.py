...
@pytest.mark.skipif(
    not VALIDATION_AVAILABLE, reason="PromptLayer validation not available"
)
class TestValidationDataClasses:
    ...
    def test_validation_result_properties(self):
        ...
        result = ValidationResult(
            overall_status=ValidationStatus.WARNING, checks=checks
        )
...
class TestIndividualValidationChecks:
    ...
    def test_check_genops_installation_failure(self):
        ...
        with patch(
            ...
            side_effect=ImportError("No module named 'genops'"),
        ):
            result = check_genops_installation()
            assert result.status == ValidationStatus.FAILED
    ...
    def test_check_promptlayer_installation_failure(self):
        ...
        with patch(
            "importlib.import_module",
            side_effect=ImportError("No module named 'promptlayer'"),
        ):
            result = check_promptlayer_installation()
            assert result.status == ValidationStatus.FAILED
            assert "not installed" in result.message.lower()
...
class TestIntegrationValidationChecks:
    ...
    @patch("genops.providers.promptlayer_validation.GenOpsPromptLayerAdapter")
    def test_check_genops_promptlayer_integration_import_failure(
        self, mock_adapter_class
    ):
        """Test integration check when import fails."""
        with patch(
            "genops.providers.promptlayer_validation.GenOpsPromptLayerAdapter",
...
class TestValidationSuite:
    ...
    def test_validate_setup_comprehensive(self):
        ...
        with (
            ...
            mock_span = Mock()
            mock_adapter_instance.track_prompt_operation.return_value.__enter__ = Mock(
                return_value=mock_span
            )
            mock_adapter_instance.track_prompt_operation.return_value.__exit__ = Mock(
                return_value=None
            )
...
class TestValidationResultFormatting:
    ...
    def test_print_validation_result_success(self, capsys):
        ...
        checks = [
            ValidationCheck(
                "Check 1", ValidationStatus.PASSED, "Success", category="test"
            ),
            ValidationCheck(
                "Check 2", ValidationStatus.PASSED, "Success", category="test"
            ),
        ]
...
class TestRealValidation:
    ...
    @pytest.mark.skipif(
        not os.getenv("PROMPTLAYER_API_KEY"), reason="PROMPTLAYER_API_KEY not set"
    )
