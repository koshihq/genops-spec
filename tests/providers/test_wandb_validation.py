...
class TestWandbValidation(unittest.TestCase):
    ...
    def test_check_environment_variables(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            self.assertIn(
                "WANDB_API_KEY",
                [check.name for check in result if check.status == "FAILED"],
            )

        # Test with present variables
        with patch.dict(
            os.environ, {"WANDB_API_KEY": "test-key", "GENOPS_TEAM": "test-team"}
        ):
            result = check_environment_variables()
    ...
    def test_validate_governance_setup(self):
        ...
        for expected_check in expected_checks:
            ...
            any(expected_check in name.lower() for name in check_names)
    ...
    def test_validate_setup_basic(self):
        ...
        with patch.dict(
            ...
            with patch(
                "genops.providers.wandb_validation.validate_wandb_connection"
            ) as mock_wandb_check:
    ...
    def test_validate_setup_with_governance(self):
        ...
        result = validate_setup(
            include_connectivity_tests=False, include_governance_tests=True
        )

        # Should include governance-related checks
        check_names = [check.name for check in result.checks]
        [name for name in check_names if "governance" in name.lower()]
    ...
    def test_print_validation_result_basic(self):
        ...
        result = ValidationResult(
            ...
            checks=[
                ValidationCheck(
                    name="test_check_1", status="PASSED", message="First check passed"
                ),
                ValidationCheck(
                    name="test_check_2",
                    status="WARNING",
        ...
        result = ValidationResult(
            ...
            checks=[
                ValidationCheck(
                    ...
                    details={
                        "issue": "Minor configuration issue",
                        "suggestion": "Set GENOPS_TEAM",
                    },
                )
            ],
