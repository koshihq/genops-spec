...
class TestGeminiEndToEndWorkflow:
    ...
    def test_complete_text_generation_workflow(self, mock_calculate_cost):
        ...
        with patch('genops.providers.gemini.genai.Client', return_value=mock_client):
            with patch('genops.providers.gemini.GenOpsTelemetry', return_value=mock_telemetry):
                ...
                assert call_args[1]["contents"] == "Explain artificial intelligence in simple terms"
    ...
    def test_workflow_without_telemetry(self):
        ...
        with patch('genops.providers.gemini.genai.Client', return_value=mock_client):
            with patch('genops.providers.gemini.calculate_gemini_cost', return_value=0.001):
...
class TestGeminiAutoInstrumentation:
    ...
    def test_auto_instrumentation_setup(self):
        ...
        with patch('genops.providers.gemini.genai') as mock_genai:
    ...
    def test_validation_print_integration(self, capsys):
        ...
        result = GeminiValidationResult(
            ...
            checks=[
                ...
                ValidationCheck("api_key", ValidationLevel.SUCCESS, "API key configured"),
                ValidationCheck("connectivity", ValidationLevel.SUCCESS, "API connectivity OK")
            ],
...
class TestGeminiRealWorldScenarios:
    ...
    def test_content_generation_pipeline(self):
        ...
        with patch('genops.providers.gemini.genai.Client', return_value=mock_client):
            with patch('genops.providers.gemini.calculate_gemini_cost', side_effect=[0.0005, 0.003, 0.001, 0.0003]):
                with create_gemini_cost_context(
                    "content_pipeline",
                    budget_limit=0.01,
                    ...
                    context.add_operation("title_gen", "gemini-2.5-flash-lite", 50, 10, 400.0)
                    ...
                    context.add_operation("content_gen", "gemini-2.5-pro", 200, 600, 2500.0)
                    ...
                    context.add_operation("summary_gen", "gemini-2.5-flash", 100, 50, 800.0)
                    ...
                    context.add_operation("tags_gen", "gemini-2.5-flash-lite", 30, 8, 350.0)
                ...
                # Verify governance attributes
                assert final_summary.governance_attributes["team"] == "content-marketing"
                assert final_summary.governance_attributes["project"] == "ai-blog-series"
