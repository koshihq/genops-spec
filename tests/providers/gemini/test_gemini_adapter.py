...
class TestGeminiAdapterInitialization:
    ...
    def test_adapter_init_without_gemini_sdk_raises_error(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", False):
            with pytest.raises(
                ImportError, match="Google Gemini dependencies not available"
            ):
    ...
    def test_adapter_init_with_custom_model(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", True):
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", True):
                adapter = GenOpsGeminiAdapter(
                    api_key="test_key", default_model="gemini-2.5-pro"
                )
...
class TestGeminiAdapterAvailability:
    ...
    def test_is_available_with_successful_api_call(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", True):
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", True):
                ...
                with patch(
                    "genops.providers.gemini.genai.Client", return_value=mock_client
                ):
                    adapter = GenOpsGeminiAdapter(api_key="test_key")
    ...
    def test_is_available_when_gemini_not_available(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", False):
            ...
            adapter = Mock()
            adapter.is_available = GenOpsGeminiAdapter.is_available.__get__(
                adapter, GenOpsGeminiAdapter
            )
...
class TestGeminiAdapterTextGeneration:
    ...
    def test_text_generation_basic(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", True):
            with patch(
                "genops.providers.gemini.GENOPS_AVAILABLE", False
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        result = adapter.text_generation(
                            prompt="Test prompt", model="gemini-2.5-flash"
                        )
            ...
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", False):
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert (
                            result.governance_attributes["customer_id"]
                            == "customer-123"
                        )
            ...
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", False):
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        adapter.chat_completion(messages=messages)
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", True):
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", True):
                ...
                with patch(
                    ...
                    with patch(
                        "genops.providers.gemini.GenOpsTelemetry",
                        ...
                        with patch(
                            ...
                            assert (
                                call_args["genops.operation_type"] == "text_generation"
                            )
    ...
    def test_extract_response_content(self):
        ...
        with patch("genops.providers.gemini.GEMINI_AVAILABLE", True):
            with patch("genops.providers.gemini.GENOPS_AVAILABLE", True):
                ...
                content, tokens = adapter._extract_response_content(
                    mock_response, "gemini-2.5-flash"
                )
