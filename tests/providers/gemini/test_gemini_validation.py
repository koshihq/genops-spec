...
class TestValidationCheck:
    ...
    def test_validation_check_minimal(self):
        ...
        check = ValidationCheck(
            name="minimal_check", level=ValidationLevel.ERROR, message="Error message"
        )
...
class TestGeminiValidationResult:
    ...
    def test_has_errors(self):
        ...
        result_with_errors = GeminiValidationResult(success=False, errors=["Error 1"])
    ...
    def test_has_warnings(self):
        ...
        result_with_warnings = GeminiValidationResult(
            success=True, warnings=["Warning 1"]
        )
...
class TestValidateGeminiSetup:
    ...
    def test_validation_missing_gemini_sdk(self):
        ...
        assert result.get_error_count() > 0
        assert any(
            "Google Gemini SDK not installed" in error for error in result.errors
        )
    ...
    def test_validation_missing_genops_core(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test_key"}, clear=True):
            ...
            assert any(
                "GenOps core not available" in warning for warning in result.warnings
            )
    ...
    def test_validation_with_explicit_api_key(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch(
                ...
                result = validate_gemini_setup(
                    api_key="explicit_key_123", test_connectivity=True
                )
    ...
    def test_validation_invalid_api_key_format(self):
        ...
        assert result.get_warning_count() > 0
        assert any(
            "format appears unusual" in check.message
    ...
    def test_validation_correct_api_key_format(self):
        ...
        format_checks = [
            check for check in result.checks if check.name == "api_key_format"
        ]
    ...
    def test_validation_connectivity_success(self):
        ...
        with patch(
            ...
            connectivity_checks = [
                check for check in result.checks if check.name == "api_connectivity"
            ]
    ...
    def test_validation_connectivity_auth_error(self):
        ...
        with patch(
            ...
            assert result.get_error_count() > 0
            assert any(
                "API key authentication failed" in error for error in result.errors
            )
    ...
    def test_validation_model_access_testing(self):
        ...
        with patch(
            ...
            result = validate_gemini_setup(
                test_model_access=True, test_connectivity=False
            )
    ...
    def test_validation_performance_testing(self):
        ...
        with patch(
            ...
            result = validate_gemini_setup(
                test_connectivity=False, test_model_access=False, performance_test=True
            )

            # Should have performance metrics
            assert (
                len([k for k in result.performance_metrics.keys() if "latency_ms" in k])
                > 0
            )
            assert (
                len([k for k in result.performance_metrics.keys() if "tokens" in k]) > 0
            )
...
class TestPrintValidationResult:
    ...
    def test_print_validation_result_with_quick_fixes(self, capsys):
        ...
        with patch("genops.providers.gemini_validation.GEMINI_AVAILABLE", False):
            with patch.dict(os.environ, {}, clear=True):
                result = GeminiValidationResult(
                    success=False, errors=["SDK not available", "API key missing"]
                )
