...
class TestValidationIssue:
    ...
    def test_validation_issue_optional_fix(self):
        ...
        issue = ValidationIssue(
            level="warning", component="test", message="Warning message"
        )
...
class TestEnvironmentVariableValidation:
    ...
    def test_check_environment_variables_no_tokens(self):
        ...
        hf_token_warning = next(
            (i for i in warning_issues if "Hugging Face token" in i.message), None
        )
    ...
    def test_check_environment_variables_with_otel(self):
        ...
        otel_info_issues = [
            i for i in issues if i.level == "info" and "OpenTelemetry" in i.message
        ]
...
class TestDependencyValidation:
    ...
    def test_check_dependencies_missing_huggingface(self, mock_import):
        ...
        hf_error = next(
            (i for i in error_issues if "huggingface_hub" in i.message), None
        )
    ...
    def test_check_dependencies_missing_optional(self, mock_import):
        ...
        optional_missing = next(
            (i for i in info_issues if "Optional AI/ML dependencies" in i.message), None
        )
...
class TestConnectivityValidation:
    ...
    def test_check_huggingface_connectivity_success(self, mock_inference_client):
        ...
        success_info = next(
            (
                ...
                if "InferenceClient created successfully" in i.message
            ),
            None,
        )
    ...
    @patch("genops.providers.huggingface_validation.InferenceClient")
    def test_check_huggingface_connectivity_no_text_generation(
        self, mock_inference_client
    ):
        ...
        method_warning = next(
            (
                ...
                if "text_generation method not available" in i.message
            ),
            None,
        )
    ...
    def test_check_huggingface_connectivity_import_error(self, mock_inference_client):
        ...
        with patch(
            "genops.providers.huggingface_validation.InferenceClient",
    ...
    def test_check_huggingface_connectivity_creation_error(self, mock_inference_client):
        ...
        creation_warning = next(
            (
                i
                for i in warning_issues
                if "Issue creating Hugging Face client" in i.message
            ),
            None,
        )
...
class TestGenOpsIntegrationValidation:
    ...
    def test_check_genops_integration_success(self, mock_adapter_class):
        ...
        adapter_success = next(
            (i for i in info_issues if "adapter working" in i.message), None
        )
        ...
        provider_detection_success = next(
            (
                i
                for i in info_issues
                if "Provider detection working correctly" in i.message
            ),
            None,
        )
    ...
    def test_check_genops_integration_no_tasks(self, mock_adapter_class):
        ...
        no_tasks_warning = next(
            (i for i in warning_issues if "no supported tasks found" in i.message), None
        )
        assert no_tasks_warning is not None

    @patch("genops.providers.huggingface_validation.GenOpsHuggingFaceAdapter")
    def test_check_genops_integration_partial_provider_detection(
        ...
        partial_warning = next(
            (
                i
                for i in warning_issues
                if "working for" in i.message and "test models" in i.message
            ),
            None,
        )
    ...
    def test_check_genops_integration_import_error(self):
        ...
        with patch(
            "genops.providers.huggingface_validation.GenOpsHuggingFaceAdapter",
            side_effect=ImportError,
        ):
            issues = check_genops_integration()
        ...
        import_error = next(
            (
                ...
                if "Cannot import GenOps Hugging Face adapter" in i.message
            ),
            None,
        )
        assert import_error is not None
    ...
    def test_check_genops_integration_creation_error(self, mock_adapter_class):
        ...
        creation_error = next(
            (
                i
                for i in error_issues
                if "Failed to create GenOps Hugging Face adapter" in i.message
            ),
            None,
        )
...
class TestCostCalculationValidation:
    ...
    def test_check_cost_calculation_success(
        self, mock_get_provider_info, mock_calculate_cost, mock_detect_provider
    ):
        ...
        calculation_success = next(
            (i for i in info_issues if "Cost calculation working" in i.message), None
        )
    ...
    def test_check_cost_calculation_import_error(self):
        ...
        # Test with patched import to simulate missing module
        with patch(
            "genops.providers.huggingface_validation.detect_model_provider",
            side_effect=ImportError,
        ):
            issues = check_cost_calculation()
        ...
        import_error = next(
            (
                i
                for i in error_issues
                if "Cannot import Hugging Face pricing utilities" in i.message
            ),
            None,
        )
        assert import_error is not None
    ...
    @patch("genops.providers.huggingface_validation.detect_model_provider")
    @patch("genops.providers.huggingface_validation.calculate_huggingface_cost")
    def test_check_cost_calculation_partial_detection(
        self, mock_calculate_cost, mock_detect_provider
    ):
        ...
        warning_issues = [i for i in issues if i.level == "warning"]
        partial_warning = next(
            (i for i in warning_issues if "working for 2/4 models" in i.message), None
        )
        assert partial_warning is not None

    @patch("genops.providers.huggingface_validation.detect_model_provider")
    @patch("genops.providers.huggingface_validation.calculate_huggingface_cost")
    def test_check_cost_calculation_error(
        self, mock_calculate_cost, mock_detect_provider
    ):
...
class TestMainValidationFunction:
    ...
    def test_validate_huggingface_setup_all_pass(
        self, mock_cost, mock_integration, mock_connectivity, mock_deps, mock_env
    ):
        ...
        mock_integration.return_value = [
            ValidationIssue("info", "integration", "Working")
        ]
    ...
    @patch("genops.providers.huggingface_validation.check_genops_integration")
    @patch("genops.providers.huggingface_validation.check_cost_calculation")
    def test_validate_huggingface_setup_with_errors(
        self, mock_cost, mock_integration, mock_connectivity, mock_deps, mock_env
    ):
        ...
        mock_deps.return_value = [
            ValidationIssue("error", "deps", "Missing dependency")
        ]
        mock_connectivity.return_value = [
            ValidationIssue("error", "conn", "Cannot connect")
        ]
        mock_integration.return_value = [
            ValidationIssue("warning", "integration", "Partial working")
        ]
    ...
    def test_validate_huggingface_setup_with_exception(self, mock_env):
        ...
        validation_error = next(
            (
                i
                for i in error_issues
                if "Validation check check_environment_variables failed" in i.message
            ),
            None,
        )
...
class TestValidationReporting:
    ...
    def test_quick_validate_success(self, capsys):
        ...
        with patch(
            "genops.providers.huggingface_validation.validate_huggingface_setup"
        ) as mock_validate:
    ...
    def test_quick_validate_failure(self, capsys):
        ...
        with patch(
            ...
            captured = capsys.readouterr()
            assert (
                "❌ Hugging Face setup validation failed with 1 error(s)"
                in captured.out
            )
...
class TestValidationScriptExecution:
    ...
    def test_main_execution_success(self):
        ...
        with patch(
            ...
            with patch(
                "genops.providers.huggingface_validation.print_huggingface_validation_result"
            ):
