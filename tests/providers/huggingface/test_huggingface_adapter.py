...
class TestGenOpsHuggingFaceAdapter:
    ...
    def test_adapter_initialization_default(
        self, mock_telemetry_class, mock_inference_client
    ):
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_governance_attributes_extraction(
        self, mock_telemetry_class, mock_inference_client
    ):
    ...
    def test_provider_detection(self, mock_telemetry_class, mock_inference_client):
        ...
        for model, expected_provider in test_cases:
            ...
            assert detected_provider == expected_provider, (
                f"Failed for {model}: expected {expected_provider}, got {detected_provider}"
            )
    ...
    def test_token_estimation(self, mock_telemetry_class, mock_inference_client):
        ...
        for text, expected_tokens in test_cases:
            ...
            assert estimated_tokens == expected_tokens, (
                f"Failed for '{text}': expected {expected_tokens}, got {estimated_tokens}"
            )

    @patch("genops.providers.huggingface.HAS_HUGGINGFACE", True)
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_cost_calculation_with_pricing_module(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with patch(
            "genops.providers.huggingface_pricing.calculate_huggingface_cost"
    ...
    def test_cost_calculation_fallback(
        ...
        with patch(
            "genops.providers.huggingface.calculate_huggingface_cost",
            ...
            expected_cost = (100 / 1000) * 0.0015 + (
                50 / 1000
            ) * 0.002  # OpenAI fallback rates
...
class TestTextGeneration:
    ...
    def test_text_generation_success(self, mock_telemetry_class, mock_inference_client):
        ...
        mock_telemetry_class.return_value = mock_telemetry_instance
        mock_span = Mock()
        mock_telemetry_instance.trace_operation.return_value.__enter__.return_value = (
            mock_span
        )
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_text_generation_with_complex_response(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with patch(
            ...
            mock_span.set_attribute.assert_any_call(
                "genops.tokens.output", 6
            )  # "Complex generated response" ≈ 6 tokens

    @patch("genops.providers.huggingface.HAS_HUGGINGFACE", True)
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_text_generation_error_handling(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with pytest.raises(Exception, match="API Error"):
            adapter.text_generation(
                prompt="Test prompt", model="microsoft/DialoGPT-medium"
            )
    ...
    def test_chat_completion_success(self, mock_telemetry_class, mock_inference_client):
        ...
        with patch(
            ...
            result = adapter.chat_completion(
                messages=messages, model="gpt-3.5-turbo", team="chat-team"
            )

            assert result == mock_response
            mock_span.set_attribute.assert_any_call("genops.messages.count", 2)
            mock_span.set_attribute.assert_any_call(
                "genops.tokens.output", 3
            )  # "Chat response" ≈ 3 tokens

    @patch("genops.providers.huggingface.HAS_HUGGINGFACE", True)
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_chat_completion_input_token_estimation(
        self, mock_telemetry_class, mock_inference_client
    ):
...
class TestFeatureExtraction:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_feature_extraction_string_input(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with patch(
            ...
            mock_span.set_attribute.assert_any_call(
                "genops.task.type", "feature-extraction"
            )

            # Verify input tokens were estimated
            mock_span.set_attribute.assert_any_call(
                "genops.tokens.input", 6
            )  # "Test text for embedding" ≈ 6 tokens

    @patch("genops.providers.huggingface.HAS_HUGGINGFACE", True)
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_feature_extraction_list_input(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        result = adapter.feature_extraction(
            inputs=inputs, model="sentence-transformers/all-MiniLM-L6-v2"
        )
        ...
        mock_span.set_attribute.assert_any_call(
            "genops.tokens.input", 6
        )  # "First text Second text" ≈ 6 tokens
...
class TestTextToImage:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_text_to_image_multiple_images(
        self, mock_telemetry_class, mock_inference_client
    ):
...
class TestAutoInstrumentation:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_create_instrumented_client(
        self, mock_telemetry_class, mock_inference_client
    ):
