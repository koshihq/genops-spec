...
class TestEndToEndWorkflows:
    ...
    def test_complete_text_generation_workflow(
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_multi_task_workflow_integration(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with patch(
            "genops.providers.huggingface_pricing.calculate_huggingface_cost",
            side_effect=mock_calculate_cost,
        ):
            adapter = GenOpsHuggingFaceAdapter()
            ...
            cost_calls = [
                call[1]["cost"]
                for call in mock_telemetry_instance.record_cost.call_args_list
            ]
    ...
    def test_provider_detection_integration(
        ...
        with patch(
            "genops.providers.huggingface_pricing.calculate_huggingface_cost",
            side_effect=mock_cost_calculation,
        ):
            adapter = GenOpsHuggingFaceAdapter()
            ...
            for model, expected_provider, expected_cost in test_scenarios:
                ...
                adapter.text_generation(
                    prompt="Test prompt", model=model, team="integration-test"
                )

                # Verify provider detection
                mock_span.set_attribute.assert_any_call(
                    "genops.provider.detected", expected_provider
                )
...
class TestAutoInstrumentationIntegration:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_auto_instrumentation_with_governance(
        self, mock_telemetry_class, mock_inference_client
    ):
...
class TestValidationIntegration:
    ...
    def test_validation_with_real_components(self):
        ...
        with (
            ...
            with (
                ...
                patch(
                    "genops.providers.huggingface_validation.calculate_huggingface_cost",
                    return_value=0.002,
                ),
                patch(
                    "genops.providers.huggingface_validation.get_provider_info",
    ...
    def test_validation_integration_with_missing_components(self):
        ...
        with patch(
            "genops.providers.huggingface_validation.GenOpsHuggingFaceAdapter",
            side_effect=ImportError("Missing GenOps"),
        ):
            result = validate_huggingface_setup()
            ...
            genops_error = next(
                (i for i in error_issues if "GenOps" in i.message), None
            )
...
class TestErrorHandlingIntegration:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_error_handling_with_telemetry_integration(
        self, mock_telemetry_class, mock_inference_client
    ):
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_cost_calculation_error_handling_integration(
        self, mock_telemetry_class, mock_inference_client
    ):
        ...
        with patch(
            ...
            result = adapter.text_generation(
                prompt="Test prompt", model="test-model", team="cost-error-team"
            )
...
class TestComplexWorkflowIntegration:
    ...
    @patch("genops.providers.huggingface.InferenceClient")
    @patch("genops.providers.huggingface.GenOpsTelemetry")
    def test_content_pipeline_workflow(
        self, mock_telemetry_class, mock_inference_client
    ):
