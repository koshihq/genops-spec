...
class TestProviderDetection:
    ...
    def test_detect_google_models(self):
        ...
        google_models = ["gemma-7b-it", "flan-t5-xxl"]
    ...
    def test_detect_huggingface_hub_models(self):
        ...
        for model in hub_models:
            provider = detect_model_provider(model)
            assert provider == "huggingface_hub", (
                f"Failed to detect Hub model for {model}"
            )
    ...
    def test_detect_unknown_models(self):
        ...
        unknown_models = ["unknown-model-123", "some-random-model", ""]

        for model in unknown_models:
            provider = detect_model_provider(model)
            assert provider == "huggingface_hub", (
                f"Should default to huggingface_hub for {model}"
            )
...
class TestModelSizeCategorization:
    ...
    def test_medium_model_detection(self):
        ...
        for model in medium_models:
            category = estimate_model_size_category(model)
            assert category == "medium_models", (
                f"Failed to categorize {model} as medium"
            )
    ...
    def test_small_model_detection(self):
        ...
        small_models = ["gemma-2b-it", "model-3b-base", "tiny-1b-model"]
    ...
    def test_embedding_model_detection(self):
        ...
        for model in embedding_models:
            category = estimate_model_size_category(model)
            assert category == "embedding_models", (
                f"Failed to categorize {model} as embedding"
            )
    ...
    def test_image_model_detection(self):
        ...
        for model in image_models:
            category = estimate_model_size_category(model)
            assert category == "image_models", (
                f"Failed to categorize {model} as image model"
            )
    ...
    def test_default_categorization(self):
        ...
        unclear_models = ["gpt-3.5-turbo", "claude-3-haiku", "mysterious-model"]

        for model in unclear_models:
            category = estimate_model_size_category(model)
            assert category in ["small_models", "medium_models"], (
                f"Unexpected category for {model}: {category}"
            )
...
class TestCostOptimization:
    ...
    def test_optimization_tips_content(self):
        ...
        for keyword in expected_keywords:
            assert keyword in tips_text, (
                f"Expected keyword '{keyword}' not found in tips"
            )
...
class TestProviderInfo:
    ...
    def test_provider_info_cost_estimates(self):
        ...
        for _estimate_type, cost in estimates.items():
...
class TestErrorHandling:
    ...
    def test_get_model_pricing_fallback(self):
        ...
        pricing = get_model_pricing(
            provider="unknown-provider", model="unknown-model", task="unknown-task"
        )
