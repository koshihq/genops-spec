...
class TestFireworksPricingCalculator:
    ...
    def test_pricing_data_integrity(self):
        ...
        for _model_id, pricing in calc.model_pricing.items():
...
class TestCostEstimation:
    ...
    def test_cost_estimation_different_tiers(self):
        ...
        for i in range(len(costs) - 1):
            assert costs[i][1] < costs[i + 1][1], (
                f"{costs[i][0]} should cost less than {costs[i + 1][0]}"
            )
...
class TestModelRecommendations:
    ...
    def test_recommend_model_simple_task(self):
        ...
        assert (
            "1b" in recommendation.recommended_model.lower()
            or "8b" in recommendation.recommended_model.lower()
        )
    ...
    def test_recommend_model_complex_task(self):
        ...
        assert (
            "70b" in recommendation.recommended_model.lower()
            or "405b" in recommendation.recommended_model.lower()
        )
    ...
    def test_recommend_model_budget_constraints(self):
        ...
        # Very tight budget
        recommendation = calc.recommend_model(
            task_complexity="moderate", budget_per_operation=0.0001
        )
    ...
    def test_recommend_model_with_preferences(self):
        ...
        recommendation = calc.recommend_model(
            task_complexity="moderate", budget_per_operation=0.005, prefer_batch=True
        )

        assert isinstance(recommendation, ModelRecommendation)
        assert (
            "batch" in recommendation.reasoning.lower()
            or "50%" in recommendation.reasoning
        )
    ...
    def test_recommend_model_no_suitable_options(self):
        ...
        assert (
            recommendation.recommended_model is None
            or len(recommendation.alternatives) > 0
        )
...
class TestModelComparisons:
    ...
    def test_compare_models_with_batch_analysis(self):
        ...
        comparisons = calc.compare_models(
            models, estimated_tokens=1000, include_batch_analysis=True
        )
        ...
        for comparison in comparisons:
            ...
            assert abs(comparison["batch_cost"] - expected_batch_cost) < Decimal(
                "0.00001"
            )
    ...
    def test_compare_models_sorting(self):
        ...
        comparisons = calc.compare_models(
            models, estimated_tokens=1000, sort_by_cost=True
        )

        # Should be sorted by cost (ascending)
        for i in range(len(comparisons) - 1):
            assert (
                comparisons[i]["estimated_cost"] <= comparisons[i + 1]["estimated_cost"]
            )
...
class TestCostAnalysis:
    ...
    def test_analyze_costs_different_volumes(self):
        ...
        for i in range(len(analyses) - 1):
            ...
            if (
                analyses[i + 1]["optimization"].get("batch_optimization_potential", 0)
                > 0
            ):
...
class TestPricingEdgeCases:
    ...
    def test_zero_tokens_pricing(self):
        ...
        cost = calc.estimate_chat_cost(
            "accounts/fireworks/models/llama-v3p1-8b-instruct", tokens=0
        )
    ...
    def test_negative_tokens_handling(self):
        ...
        with pytest.raises(ValueError):
            calc.estimate_chat_cost(
                "accounts/fireworks/models/llama-v3p1-8b-instruct", tokens=-100
            )
    ...
    def test_extremely_large_token_counts(self):
        ...
        # Test with 1M tokens
        cost = calc.estimate_chat_cost(
            "accounts/fireworks/models/llama-v3p1-8b-instruct", tokens=1000000
        )
...
class TestBatchOptimizationCalculations:
    ...
    def test_batch_threshold_recommendations(self):
        ...
        calc.recommend_model(
            task_complexity="simple", budget_per_operation=0.001, operations_per_day=10
        )
        ...
        assert (
            "batch" in high_volume_rec.reasoning.lower()
            or "50%" in high_volume_rec.reasoning
        )
