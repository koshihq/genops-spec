...
class TestFireworksAdapterInitialization:
    ...
    def test_adapter_initialization_budget_validation(self):
        ...
        with pytest.raises(ValueError, match="Daily budget must be positive"):
            GenOpsFireworksAdapter(
                team="test", project="test", daily_budget_limit=-100.0
            )

        with pytest.raises(
            ValueError, match="Monthly budget must be greater than daily"
        ):
            GenOpsFireworksAdapter(
                team="test",
                project="test",
...
class TestChatCompletionsWithGovernance:
    ...
    def test_chat_with_governance_basic(
        ...
        assert (
            call_args[1]["model"] == "accounts/fireworks/models/llama-v3p1-8b-instruct"
        )
        ...
        assert (
            result.response
            == "Test response from Fireworks AI with 4x speed optimization"
        )
    ...
    def test_chat_error_handling(
        self, mock_fireworks_class, sample_fireworks_config, sample_chat_messages
    ):
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_chat_budget_enforcement(
        self,
        mock_fireworks_class,
        sample_fireworks_config,
        sample_chat_messages,
...
class TestEmbeddingOperations:
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_embeddings_with_governance_basic(
        self,
        mock_fireworks_class,
        sample_fireworks_config,
        ...
        assert (
            call_args[1]["model"] == "accounts/fireworks/models/nomic-embed-text-v1p5"
        )
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_embeddings_error_handling(
        self, mock_fireworks_class, sample_fireworks_config, sample_embedding_texts
    ):
...
class TestSessionBasedTracking:
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_session_operation_tracking(
        self,
        mock_fireworks_class,
        sample_fireworks_config,
        sample_chat_messages,
...
class TestCostManagement:
    ...
    def test_cost_calculation_accuracy(self, sample_fireworks_config):
        ...
        for model, tokens, expected_cost in test_cases:
            calculated_cost = adapter._calculate_cost(
                model.value, tokens, is_batch=False
            )
...
class TestPerformanceAndOptimization:
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_fireattention_speed_tracking(
        self,
        mock_fireworks_class,
        sample_fireworks_config,
        sample_chat_messages,
...
class TestErrorHandlingAndResilience:
    ...
    def test_invalid_model_handling(
        self, sample_fireworks_config, sample_chat_messages
    ):
...
class TestFireworksModels:
    ...
    def test_model_pricing_tiers(self):
        ...
        large_models = [
            FireworksModel.LLAMA_3_1_70B_INSTRUCT,
            FireworksModel.LLAMA_3_1_405B_INSTRUCT,
        ]
