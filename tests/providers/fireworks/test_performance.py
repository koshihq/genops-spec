...
class TestFireattentionOptimization:
    ...
    def test_fireattention_vs_baseline_comparison(
        ...
        # Verify 4x speedup
        speedup_ratio = (
            baseline_result.execution_time_seconds
    ...
    def test_fireattention_across_model_sizes(
        ...
        for _model, response_time in performance_results.items():
...
class TestThroughputPerformance:
    ...
    def test_concurrent_operations(
        ...
        def single_operation(operation_id):
            ...
            with patch("time.time") as mock_time:
                ...
                return adapter.chat_with_governance(
                    messages=[
                        {"role": "user", "content": f"Concurrent test {operation_id}"}
                    ],
                    model=FireworksModel.LLAMA_3_1_8B_INSTRUCT,
                    max_tokens=50,
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(single_operation, i) for i in range(num_concurrent)
            ]
    ...
    def test_batch_processing_throughput(
        ...
        sum(r.cost for r in batch_results)
...
class TestMemoryAndResourceEfficiency:
    ...
    def test_adapter_initialization_efficiency(self, sample_fireworks_config):
        ...
        for i in range(10):
            adapter = GenOpsFireworksAdapter(
                ...
                **{
                    k: v
                    for k, v in sample_fireworks_config.items()
    ...
    @patch("genops.providers.fireworks.Fireworks")
    def test_session_memory_efficiency(
        self, mock_fireworks_class, sample_fireworks_config, mock_fireworks_client
    ):
    ...
    def test_token_length_scaling(
        ...
        for max_tokens in token_lengths:
            with patch("time.time") as mock_time:
                ...
                result = adapter.chat_with_governance(
                    messages=[
                        {"role": "user", "content": f"Token scaling test {max_tokens}"}
                    ],
                    model=FireworksModel.LLAMA_3_1_8B_INSTRUCT,
        ...
        thousand_token_time = next(
            time for tokens, time in performance_data if tokens == 1000
        )
...
class TestLoadTestingScenarios:
    ...
    def test_mixed_workload_performance(
        ...
        large_costs = [
            r.cost for r in results[2::4]
        ]  # Every 4th offset by 2 (large model)

        assert statistics.mean(tiny_costs) < statistics.mean(
            large_costs
        )  # Cost scaling
