...
class TestDependencyValidation:
    ...
    @patch(
        ...
        side_effect=lambda name, *args: (
            Mock() if name == "openllmetry" else __import__(name, *args)
        ),
    )
    def test_openllmetry_availability_success(self):
        ...
        openllmetry_results = [
            r for r in results if r.check_name == "openllmetry_availability"
        ]
    ...
    def test_openllmetry_availability_failure(self):
        ...
        with patch(
            "builtins.__import__",
            side_effect=ImportError("No module named 'openllmetry'"),
        ):
            results = validate_dependencies()
    ...
    @patch(
        "builtins.__import__",
        side_effect=lambda name, *args: (
            Mock() if name == "traceloop.sdk" else __import__(name, *args)
        ),
    )
    def test_traceloop_sdk_availability_success(self):
        ...
        traceloop_results = [
            r for r in results if r.check_name == "traceloop_sdk_availability"
        ]
    ...
    def test_genops_integration_availability(self):
        ...
        genops_results = [
            r for r in results if r.check_name == "genops_traceloop_integration"
        ]
...
class TestConfigurationValidation:
    ...
    def test_api_key_validation_openai_present(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-openai-key"}):
            ...
            provider_results = [
                r for r in results if r.check_name == "ai_provider_available"
            ]
    ...
    def test_traceloop_platform_config_present(self):
        ...
        with patch.dict(os.environ, {"TRACELOOP_API_KEY": "test-traceloop-key"}):
            ...
            traceloop_results = [
                r for r in results if r.check_name == "traceloop_platform_config"
            ]
            assert len(traceloop_results) == 1
            assert traceloop_results[0].status == ValidationStatus.PASSED
    ...
    def test_genops_governance_config_complete(self):
        ...
        with patch.dict(
            os.environ, {"GENOPS_TEAM": "test-team", "GENOPS_PROJECT": "test-project"}
        ):
            results = validate_configuration()

            # Find governance config check
            governance_results = [
                r for r in results if r.check_name == "genops_governance_config"
            ]
...
class TestConnectivityValidation:
    ...
    def test_openai_connectivity_success(self, mock_openai_class):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            ...
            openai_results = [
                r for r in results if r.check_name == "openai_connectivity"
            ]
            assert len(openai_results) == 1
            assert openai_results[0].status == ValidationStatus.PASSED
    ...
    def test_anthropic_connectivity_success(self, mock_anthropic_class):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            anthropic_results = [
                r for r in results if r.check_name == "anthropic_connectivity"
            ]
...
class TestGovernanceValidation:
    ...
    def test_genops_adapter_creation_success(self, mock_instrument):
        ...
        adapter_results = [
            r for r in results if r.check_name == "genops_adapter_creation"
        ]
    ...
    def test_auto_instrumentation_availability(self, mock_auto_instrument):
        ...
        auto_results = [
            r for r in results if r.check_name == "auto_instrumentation_available"
        ]
...
class TestPerformanceValidation:
    ...
    def test_governance_overhead_acceptable(self, mock_instrument):
        ...
        assert overhead_results[0].status in [
