...
class TestAutoInstrumentation:
    ...
    @patch('genops.providers.griptape.registration._is_griptape_available', return_value=False)
    ...
    @patch('genops.providers.griptape.registration._is_griptape_available', return_value=True)
    ...
    def test_disable_auto_instrument(self, mock_apply, mock_available):
        ...
        adapter = auto_instrument(team="test-team")
        assert is_instrumented() is True
    ...
    @patch('genops.providers.griptape.registration._is_griptape_available', return_value=True)
    @patch('genops.providers.griptape.registration._apply_instrumentation', side_effect=Exception("Apply failed"))
...
class TestManualInstrumentation:
    ...
    def test_instrument_griptape_success(self, mock_workflow, mock_pipeline, mock_agent, mock_available):
        ...
        with patch('griptape.structures.Agent', mock_agent), \
...
class TestClassWrapping:
    ...
    def test_wrap_structure_method(self, mock_available):
        ...
        with patch.object(adapter, 'track_agent', return_value=mock_context):
            # Wrap the method
            wrapped_method = _wrap_structure_method(original_method, "agent", "run", adapter)
...
class TestValidationFunctionality:
    ...
    @patch('genops.providers.griptape.registration._is_griptape_available', return_value=True)
    @patch('genops.providers.griptape.registration._detect_griptape_version', return_value='1.0.0')
    @patch('griptape.structures.Agent')
    @patch('griptape.structures.Pipeline')
    @patch('griptape.structures.Workflow')
    def test_validate_griptape_setup_success(self, mock_workflow, mock_pipeline, mock_agent, mock_version, mock_available):
    ...
    @patch('genops.providers.griptape.registration._is_griptape_available', return_value=True)
    @patch('genops.providers.griptape.registration._detect_griptape_version', return_value='unknown')
    @patch('griptape.structures.Agent')
    def test_validate_griptape_setup_unknown_version(self, mock_agent, mock_version, mock_available):
    ...
    def test_validate_griptape_setup_import_failure(self, mock_agent, mock_version, mock_available):
    ...
    def test_validate_griptape_setup_with_instrumentation(self, mock_instrumented, mock_available):
...
class TestIntegrationScenarios:
    ...
    def test_multiple_enable_disable_cycles(self, mock_remove, mock_apply, mock_available):
