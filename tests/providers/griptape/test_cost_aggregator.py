...
class TestGriptapeCostAggregator:
    ...
    def test_calculate_cost_with_provider_calculator(self, mock_openai_calc, aggregator):
    ...
    def test_calculate_cost_generic_fallback(self, aggregator):
        ...
        result = aggregator.calculate_cost("unknown-provider", "unknown-model", 500, 250)
    ...
    def test_get_cost_summary_no_filters(self, aggregator):
        ...
        aggregator.add_structure_cost("pipeline-1", "pipeline", "anthropic", "claude-3", 150, 300)
        aggregator.add_structure_cost("workflow-1", "workflow", "google", "gemini-pro", 80, 160)
    ...
    def test_get_cost_summary_with_filters(self, aggregator):
        ...
        aggregator.add_structure_cost(
            "agent-1", "agent", "openai", "gpt-4", 100, 200,
        ...
        aggregator.add_structure_cost(
            "pipeline-1", "pipeline", "openai", "gpt-4", 80, 160,
    ...
    def test_get_cost_summary_with_time_filter(self, aggregator):
        ...
        recent_summary = aggregator.get_cost_summary(start_time=now - timedelta(minutes=5))
        assert recent_summary.total_requests == 1  # Only recent one
        
        # Filter to include old breakdown
        all_summary = aggregator.get_cost_summary(start_time=yesterday - timedelta(hours=1))
    ...
    def test_get_daily_costs(self, aggregator):
        ...
        aggregator.add_structure_cost("agent-1", "agent", "openai", "gpt-4", 100, 200)
        aggregator.add_structure_cost("agent-2", "agent", "anthropic", "claude-3", 150, 300)
    ...
    def test_clear_old_data(self, aggregator):
        ...
        aggregator.add_structure_cost("recent-agent", "agent", "openai", "gpt-4", 100, 200)
