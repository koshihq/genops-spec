...
@pytest.mark.skipif(not SKYROUTER_PRICING_AVAILABLE, reason="SkyRouter pricing module not available")
...
class TestSkyRouterPricingCalculator:
    ...
    def test_multi_model_cost_aggregation(self):
        ...
        assert total_cost > 0
        assert len(cost_breakdown) <= len(set(op['model'] for op in operations))
    ...
    def test_routing_strategy_cost_impact(self):
        ...
        strategy_costs = {}
        strategies = ['cost_optimized', 'balanced', 'latency_optimized', 'reliability_first']
