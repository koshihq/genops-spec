...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if (
            "enterprise" in item.name
            or "compliance" in item.name
            or "governance" in item.name
        ):
...
class SkyRouterTestHelper:
    ...
    @staticmethod
    def assert_cost_within_range(
        actual_cost: float, expected_min: float, expected_max: float
    ):
