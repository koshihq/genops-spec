...
class TestModelInfo:
    ...
    def test_model_info_creation(self):
        ...
        model = ModelInfo(name="llama3.2:1b", size_gb=1.3, parameter_count="1.2B")
    ...
    def test_performance_stats_update(self):
        ...
        model.update_performance_stats(
            inference_time_ms=1500.0, tokens=30, memory_mb=2048.0, cost=0.001
        )
        ...
        model.update_performance_stats(
            inference_time_ms=2000.0, tokens=50, memory_mb=2560.0, cost=0.0015
        )
...
class TestModelOptimizer:
    ...
    def test_add_recommendation(self):
        ...
        optimizer.add_recommendation(
            "Performance", "Use quantized model for faster inference", "high"
        )
...
class TestModelComparison:
    ...
    def test_add_metric(self):
        ...
        # Add cost metric
        comparison.add_metric("cost_per_inference", {"model1": 0.002, "model2": 0.001})

        assert "cost_per_inference" in comparison.comparison_metrics
        assert comparison.best_for_cost == "model2"  # Lower cost

        # Add speed metric
        comparison.add_metric("avg_tokens_per_second", {"model1": 20.0, "model2": 30.0})
...
class TestOllamaModelManager:
    ...
    def manager(self):
        ...
        with patch("genops.providers.ollama.model_manager.HAS_OLLAMA_CLIENT", False):
            return OllamaModelManager(
                enable_auto_optimization=True, track_performance_history=True
            )
    ...
    def test_discover_models_with_client(self, manager, mock_ollama_client):
        ...
        mock_client.list.return_value = {
            "models": [{"name": "test-model", "size": 5000000000}]
        }
    ...
    def test_optimization_recommendations_generation(self, manager):
        ...
        slow_model.update_performance_stats(
            8000.0, 3, 12000.0, 0.02
        )  # Very slow, low throughput, high cost
        ...
        latency_recs = [
            rec
            for rec in optimizer.optimization_opportunities
        ...
        cost_recs = [
            rec for rec in optimizer.optimization_opportunities if "cost" in rec.lower()
        ]
...
class TestModelManagerIntegration:
    ...
    def test_full_model_lifecycle(self, mock_requests):
        ...
        mock_requests.get.return_value.json.return_value = {
            "models": [{"name": "lifecycle-model", "size": 3000000000}]
        }
        ...
        assert model_info.success_rate == pytest.approx(
            83.33, abs=0.1
        )  # 10/(10+2) * 100
