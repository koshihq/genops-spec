...
class TestGenOpsBedrockAdapter:
    ...
    def test_error_handling(self, mock_boto_client):
        ...
        with pytest.raises(Exception) as exc_info:
            self.adapter.text_generation(
                prompt="Test prompt", **self.sample_governance_attrs
            )

        assert "AccessDeniedException" in str(exc_info.value) or "Access denied" in str(
            exc_info.value
        )
    ...
    def test_performance_metrics(self, mock_boto_client):
        ...
        result = self.adapter.text_generation(
            prompt="Performance test prompt", **self.sample_governance_attrs
        )
    ...
    def test_different_model_providers(self):
        ...
        for _provider, model in providers_models.items():
            adapter = GenOpsBedrockAdapter(default_model=model)
            assert adapter.default_model == model
...
class TestResultObject:
    ...
    def test_cost_calculation_consistency(self):
        ...
        test_cases = [
            {"input_cost": 0.001, "output_cost": 0.002, "expected_total": 0.003},
            {"input_cost": 0.0005, "output_cost": 0.0015, "expected_total": 0.002},
        ]
        ...
        try:
            with self.adapter as ctx_adapter:
                result = ctx_adapter.text_generation(
                    prompt="Context test", team="context-test"
                )
...
class TestEdgeCases:
    ...
    def test_empty_prompt(self):
        ...
        try:
            self.adapter.text_generation(prompt="", team="empty-test")
    ...
    def test_very_long_prompt(self):
        ...
        try:
            self.adapter.text_generation(
                prompt=very_long_prompt, team="long-prompt-test"
            )
    ...
    def test_none_values_in_governance_attrs(self):
        ...
        try:
            self.adapter.text_generation(
                prompt="Test with None values", **governance_with_nones
            )
        except Exception:
