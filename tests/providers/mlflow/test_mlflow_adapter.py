...
def mock_tracer():
    ...
    mock = MagicMock()
    mock.start_as_current_span = MagicMock(return_value=MagicMock(__enter__=Mock(), __exit__=Mock()))
    return mock
...
def test_track_mlflow_run_basic(adapter):
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        with patch('src.genops.providers.mlflow.adapter.create_mlflow_cost_context') as mock_cost_ctx:
            ...
            with adapter.track_mlflow_run(experiment_name="test-exp", run_name="test-run") as run:
                assert run == mock_run
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        with patch('src.genops.providers.mlflow.adapter.create_mlflow_cost_context'):
            with pytest.raises(Exception, match="Connection error"):
                with adapter.track_mlflow_run(experiment_name="test-exp") as run:
                    pass
...
def test_track_mlflow_run_without_experiment_name(adapter):
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        with patch('src.genops.providers.mlflow.adapter.create_mlflow_cost_context'):
            with adapter.track_mlflow_run(run_name="test-run") as run:
                pass
...
def test_track_mlflow_run_nested_contexts(adapter):
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        with patch('src.genops.providers.mlflow.adapter.create_mlflow_cost_context'):
            with adapter.track_mlflow_run(run_name="outer") as outer_run:
                assert 'run-1' in adapter.active_runs

                with adapter.track_mlflow_run(run_name="inner") as inner_run:
                    assert 'run-1' in adapter.active_runs
...
def test_patched_method_calls_original(adapter):
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        with patch.object(adapter, 'tracer'):
            result = mock_mlflow.log_metric("accuracy", 0.95)
...
def test_patch_cycle_preserves_functionality(adapter):
    ...
    with patch('src.genops.providers.mlflow.adapter.mlflow') as mock_mlflow:
        ...
        # Patch
        adapter._patch_log_metric(mock_mlflow)
        patched_method = mock_mlflow.log_metric
