...
class TestThroughputPerformance:
    ...
    def test_sequential_requests_throughput(self, mock_together_client, performance_adapter):
    ...
    def test_concurrent_requests_performance(self, mock_together_client, performance_adapter):
        ...
        assert concurrent_throughput > 30  # Should handle >30 requests/second concurrently
    ...
    def test_batch_operation_performance(self, mock_together_client, performance_adapter):
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_multiple_operations(self, mock_together_client, performance_adapter):
    ...
    def test_memory_cleanup_after_session(self, mock_together_client, performance_adapter):
...
class TestScalabilityPerformance:
    ...
    def test_session_scalability(self, mock_together_client, performance_adapter):
        ...
        def session_worker(session_id):
            ...
            session_results = []
            with performance_adapter.track_session(f"scale-session-{session_id}") as session:
                for i in range(operations_per_session):
                    result = performance_adapter.chat_with_governance(
                        messages=[{"role": "user", "content": f"Session {session_id} Op {i}"}],
                        model=TogetherModel.LLAMA_3_1_8B_INSTRUCT,
                        max_tokens=10,
                        session_id=session.session_id
                    )
        ...
        print(f"Multi-session scalability throughput: {scalability_throughput:.1f} ops/second")
    ...
    def test_cost_calculation_performance(self, performance_adapter):
        ...
        print(f"Cost calculation throughput: {calculations_per_second:.0f} calculations/second")

    def test_governance_attribute_performance(self, mock_together_client, performance_adapter):
...
class TestStressTestScenarios:
    ...
    def test_rapid_fire_requests(self, mock_together_client, performance_adapter):
        ...
        print(f"Rapid fire throughput: {successful_requests / total_time:.1f} requests/second")
