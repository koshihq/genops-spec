...
class TestApiKeyValidation:
    ...
    def test_check_api_key_missing(self):
        ...
        assert isinstance(error, ValidationError)
        assert (
            "missing" in error.message.lower() or "not found" in error.message.lower()
        )
...
class TestDependencyValidation:
    ...
    def test_check_dependencies_missing_together(self, mock_import):
        ...
        together_error = next(
            (e for e in errors if "together" in e.message.lower()), None
        )
    ...
    def test_check_dependencies_missing_opentelemetry(self, mock_import):
        ...
        otel_error = next(
            (e for e in errors if "opentelemetry" in e.message.lower()), None
        )
...
class TestComprehensiveValidation:
    ...
    def test_validate_setup_missing_api_key(self, mock_together):
        ...
        api_key_error = next(
            (e for e in result.errors if "api" in e.message.lower()), None
        )
...
class TestValidationResultPrinting:
    ...
    def test_print_validation_result_failure(self, capsys):
        ...
        errors = [
            ValidationError(
                "API_KEY_MISSING", "API key not found", "Set TOGETHER_API_KEY"
            ),
            ValidationError(
                "DEPENDENCY_MISSING", "Missing dependency", "pip install together"
            ),
        ]
        result = ValidationResult(
            is_valid=False,
