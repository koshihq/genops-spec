...
try:
    ...
    from src.genops.providers.together_pricing import (
        TogetherPricingCalculator,  # noqa: F401
    )
    ...
    def get_cost_summary(self):
        ...
        return {
            ...
            "daily_budget_utilization": (
                float(self.daily_costs) / self.daily_budget_limit
            )
            * 100,
...
class TestCrossProviderGovernance:
    ...
    def test_governance_attribute_consistency(
        self, mock_together_client, together_adapter
    ):
    ...
    def test_unified_cost_tracking(self, mock_together_client):
        ...
        together_adapter.chat_with_governance(
            messages=[{"role": "user", "content": "Together test"}],
            model=TogetherModel.LLAMA_3_1_8B_INSTRUCT,
        )

        openai_adapter.chat_with_governance(
            messages=[{"role": "user", "content": "OpenAI test"}], model="gpt-3.5-turbo"
        )

        anthropic_adapter.chat_with_governance(
    ...
    def test_multi_provider_session_tracking(self, mock_together_client):
        ...
        together_adapter = GenOpsTogetherAdapter(
            team="session-test", project="multi-provider"
        )
        openai_adapter = MockOpenAIAdapter(
            team="session-test", project="multi-provider"
        )
...
class TestProviderMigration:
    ...
    def test_migration_cost_comparison(self, mock_together_client):
        ...
        test_message = [
            {
                "role": "user",
                "content": "Generate a product description for an AI chatbot",
            }
        ]

        # Together AI
        together_adapter = GenOpsTogetherAdapter(
            team="cost-comparison", project="migration-analysis"
        )
        ...
        openai_result = openai_adapter.chat_with_governance(
            messages=test_message, model="gpt-3.5-turbo", max_tokens=100
        )
    ...
    def test_feature_parity_migration(self, mock_together_client):
        ...
        together_adapter = GenOpsTogetherAdapter(
            team="parity-test", project="feature-migration"
        )
...
class TestMultiProviderOperations:
    ...
    def test_provider_fallback_scenario(self, mock_together_client):
        ...
        primary_adapter = GenOpsTogetherAdapter(
            team="fallback-test", project="resilience-test"
        )
    ...
    def test_cost_aggregation_across_providers(self, mock_together_client):
        ...
        together_result = together_adapter.chat_with_governance(
            messages=test_message, model=TogetherModel.LLAMA_3_1_8B_INSTRUCT
        )

        openai_result = openai_adapter.chat_with_governance(
            messages=test_message, model="gpt-3.5-turbo"
        )
        ...
        assert (
            together_result.governance_attributes["customer_id"]
            == "customer-aggregation"
        )
        assert (
            openai_result.governance_attributes["customer_id"] == "customer-aggregation"
        )
