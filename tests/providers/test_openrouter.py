...
class TestGenOpsOpenRouterAdapter:
    ...
    def test_adapter_initialization_missing_openai(self):
        ...
        with patch("genops.providers.openrouter.HAS_OPENROUTER_DEPS", False):
            ...
            assert "OpenAI package not found (required for OpenRouter compatibility)" in str(exc_info.value)

    def test_chat_completions_create_basic(self, mock_openai_import, mock_span_recorder):
    ...
    def test_chat_completions_create_with_openrouter_attributes(self, mock_openai_import, mock_span_recorder):
    ...
    def test_cost_calculation_with_pricing_engine(self, mock_openai_import):
        ...
        with patch("genops.providers.openrouter.calculate_openrouter_cost") as mock_calc:
            mock_calc.return_value = 0.001234
            
            cost = adapter._calculate_cost("anthropic/claude-3-sonnet", "anthropic", 100, 50)
            
            assert cost == 0.001234
            mock_calc.assert_called_once_with("anthropic/claude-3-sonnet", "anthropic", 100, 50)
    ...
    def test_cost_calculation_fallback(self, mock_openai_import):
        ...
        with patch("genops.providers.openrouter.calculate_openrouter_cost", side_effect=ImportError):
    ...
    def test_error_handling_in_chat_completion(self, mock_openai_import, mock_span_recorder):
    ...
    def test_governance_attribute_extraction(self, mock_openai_import):
        ...
        expected_governance = {"team": "ai-team", "project": "chatbot", "customer_id": "customer-123"}
    ...
    def test_telemetry_attributes_in_chat_completion(self, mock_openai_import, mock_span_recorder):
    ...
    def test_openrouter_specific_attributes_in_telemetry(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterValidation:
    ...
    def test_validate_setup_import(self, mock_openai_import):
        ...
        with patch("genops.providers.openrouter.validate_openrouter_setup") as mock_validate:
    ...
    def test_validate_setup_missing_validation_module(self, mock_openai_import):
        ...
        with patch("genops.providers.openrouter.validate_openrouter_setup", side_effect=ImportError):
            result = validate_setup()
    ...
    def test_print_validation_result_import(self, mock_openai_import):
        ...
        # Mock the validation module
        with patch("genops.providers.openrouter.print_openrouter_validation_result") as mock_print:
            print_validation_result(mock_result)
            
            mock_print.assert_called_once_with(mock_result)

    def test_print_validation_result_missing_validation_module(self, mock_openai_import):
        ...
        # Mock ImportError for validation module
        with patch("genops.providers.openrouter.print_openrouter_validation_result", side_effect=ImportError):
...
class TestOpenRouterIntegrationPatterns:
    ...
    def test_multi_provider_routing_telemetry(self, mock_openai_import, mock_span_recorder, mock_openrouter_response):
    ...
    def test_cost_calculation_with_actual_provider(self, mock_openai_import, mock_openrouter_response):
    ...
    def test_fallback_detection_in_telemetry(self, mock_openai_import, mock_span_recorder):
        ...
        assert attributes.get("genops.openrouter.actual_provider") == "openai"  # Fallback provider
...
class TestOpenRouterPricingEngineIntegration:
    ...
    def test_pricing_engine_model_coverage(self):
        ...
        expected_providers = {"openai", "anthropic", "google", "meta", "mistral", "cohere"}
    ...
    def test_cost_breakdown_detailed_info(self):
        ...
        required_keys = {
            "total_cost", "input_cost", "output_cost", "provider", 
            "model_family", "currency", "model_name"
        }
...
class TestOpenRouterValidationUtilities:
    ...
    def test_validation_setup_structure(self):
        ...
        from genops.providers.openrouter_validation import ValidationResult, ValidationIssue
...
class TestOpenRouterMultiProviderScenarios:
    ...
    def test_model_provider_mapping_accuracy(self, mock_openai_import):
        ...
        for model, expected_provider in test_mappings:
            ...
            assert predicted_provider == expected_provider, f"Model {model} mapped to {predicted_provider}, expected {expected_provider}"
    
    def test_cost_attribution_across_providers(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterErrorHandlingScenarios:
    ...
    def test_authentication_error_handling(self, mock_openai_import, mock_span_recorder):
    ...
    def test_model_not_found_error_handling(self, mock_openai_import, mock_span_recorder):
        """Test handling of model not found errors.""" 
        from openai import NotFoundError
        
        mock_client = MockOpenAIClient()
        mock_client.chat.completions.create.side_effect = NotFoundError("Model not found")
...
class TestOpenRouterGovernanceIntegration:
    ...
    def test_governance_attribute_propagation(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterPerformanceAndScaling:
    ...
    def test_concurrent_request_handling(self, mock_openai_import, mock_span_recorder):
        ...
        request_ids = [
            attr.value for span in spans 
            for attr in span.attributes 
            if attr.key == "request_id"
        ]
        assert len(set(request_ids)) == 5  # All unique
    
    def test_memory_efficiency_large_requests(self, mock_openai_import, mock_span_recorder):
    ...
    def test_attribute_extraction_performance(self, mock_openai_import):
        ...
        many_attributes = {
            f"custom_attr_{i}": f"value_{i}" for i in range(50)
        }
...
class TestOpenRouterStreamingSupport:
    ...
    def test_streaming_parameter_passthrough(self, mock_openai_import, mock_span_recorder):
    ...
    def test_streaming_with_governance_tracking(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterProductionScenarios:
    ...
    def test_production_context_preservation(self, mock_openai_import, mock_span_recorder):
    ...
    def test_production_memory_efficiency(self, mock_openai_import, mock_span_recorder):
        ...
        for batch in range(3):
            for request in range(10):
                response = adapter.chat_completions_create(
                    model="openai/gpt-3.5-turbo",
                    messages=[{"role": "user", "content": f"Batch {batch}, Request {request}"}],
                    team="production",
...
class TestOpenRouterSecurityAndCompliance:
    ...
    def test_sensitive_data_filtering(self, mock_openai_import, mock_span_recorder):
        ...
        response = adapter.chat_completions_create(
            ...
            messages=[{
                "role": "user", 
                "content": "Process this data: sensitive-info"
            }],
            team="security-team",
    ...
    def test_compliance_attribute_handling(self, mock_openai_import, mock_span_recorder):
    ...
    def test_encryption_in_transit_support(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterAdvancedFeatures:
    ...
    def test_budget_constraint_enforcement(self, mock_openai_import, mock_span_recorder):
...
class TestOpenRouterIntegrationRobustness:
    ...
    def test_network_interruption_recovery(self, mock_openai_import, mock_span_recorder):
...
if __name__ == "__main__":
    pytest.main([__file__])
        assert attributes.get("project") == "streaming-project"