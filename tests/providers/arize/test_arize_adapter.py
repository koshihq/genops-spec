...
class TestArizeAdapterCore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client_patch = patch(
            "genops.providers.arize.ArizeClient", return_value=self.arize_client_mock
        )
    ...
    def test_track_model_monitoring_session_context_manager(self):
        ...
        with patch.object(adapter.tracer, "start_as_current_span") as mock_span:
            ...
            with adapter.track_model_monitoring_session(
                model_id="test-model", model_version="1.0", environment="production"
            ) as session:
        ...
        with patch.object(adapter.tracer, "start_as_current_span") as mock_span:
            ...
            with adapter.track_model_monitoring_session("test-model") as session:
                ...
                session.create_performance_alert(
                    metric="accuracy", threshold=0.80, cost_per_alert=0.10
                )
    ...
    def test_get_metrics(self):
        ...
        adapter = GenOpsArizeAdapter(daily_budget_limit=100.0, enable_cost_alerts=True)
    ...
    def test_get_monitoring_cost_summary(self):
        ...
        with patch.object(adapter.tracer, "start_as_current_span"):
            with adapter.track_model_monitoring_session(
                "test-model", "v1", "production"
            ) as session:
                # Add some costs
                session.log_prediction_batch(
                    pd.DataFrame({"pred": [1, 0, 1]}), cost_per_prediction=0.001
                )
    ...
    def test_budget_validation(self):
        ...
        adapter = GenOpsArizeAdapter(daily_budget_limit=10.0, enable_governance=True)
...
class TestArizeInstrumentation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env_patcher = patch.dict(
            os.environ,
            {"ARIZE_API_KEY": "test-key", "ARIZE_SPACE_KEY": "test-space"},
            clear=False,
        )
        self.env_patcher.start()
...
class TestArizeCostTracking(unittest.TestCase):
    ...
    def test_cost_alerts_when_enabled(self):
        ...
        adapter = GenOpsArizeAdapter(
            daily_budget_limit=1.0, max_monitoring_cost=0.5, enable_cost_alerts=True
        )

        with patch("genops.providers.arize.logger"):
            with patch.object(adapter.tracer, "start_as_current_span"):
                with adapter.track_model_monitoring_session(
                    "expensive-model"
                ) as session:
    ...
    def test_cost_alerts_when_disabled(self):
        ...
        adapter = GenOpsArizeAdapter(
            daily_budget_limit=1.0, max_monitoring_cost=0.5, enable_cost_alerts=False
        )
