...
class TestCohereIntegrationWorkflow:
        ...
        with patch("genops.providers.cohere.ClientV2") as mock_client_class:
            ...
            mock_client.rerank.return_value = Mock(
                results=[
                    ...
                    Mock(
                        index=1, relevance_score=0.87, document={"text": "Second doc"}
                    ),
                ],
    ...
    def test_complete_setup_workflow(self, mock_cohere_environment):
        ...
        adapter = instrument_cohere(team="integration-team", project="workflow-test")

        # Step 3: Perform operations
        chat_response = adapter.chat(
            message="Integration test message", model="command-r-08-2024"
        )

        embed_response = adapter.embed(
            texts=["integration", "test"], model="embed-english-v4.0"
        )
    ...
    def test_multi_operation_workflow_integration(self, mock_cohere_environment):
        ...
        aggregator = CohereCostAggregator(
            enable_detailed_tracking=True, cost_alert_threshold=0.10
        )
        ...
        assert (
            len(workflow_results["cost_breakdown"]) == 4
        )  # embed_query, embed_docs, rerank, summarize
    ...
    def test_enterprise_deployment_integration(self, mock_cohere_environment):
        ...
        enterprise_config = {
            "teams": {
                "ml-team": {
                    "budget": 50.0,
                    "models": ["command-r-08-2024", "embed-english-v4.0"],
                },
                "search-team": {
                    "budget": 30.0,
                    "models": ["command-light", "rerank-english-v3.0"],
                },
                "research-team": {
                    "budget": 100.0,
        ...
        global_aggregator = CohereCostAggregator(
            cost_alert_threshold=enterprise_config["global_budget"]
            * enterprise_config["cost_alert_threshold"]
        )
        ...
        for team, model, operation, content in team_operations:
            ...
            if operation == CohereOperation.CHAT:
                ...
                response = adapter.rerank(
                    query="test query", documents=["doc1", "doc2"], model=model
                )
        ...
        assert (
            abs(enterprise_summary.overview.total_cost - total_enterprise_cost) < 0.001
        )
    ...
    def test_performance_monitoring_integration(self, mock_cohere_environment):
        ...
        adapter = instrument_cohere(team="performance-team", project="monitoring-test")
        ...
        def delayed_embed(*args, **kwargs):
            ...
            return Mock(
                embeddings=[[0.1, 0.2]], usage=Mock(input_tokens=20, output_tokens=0)
            )
        ...
        embed_response = adapter.embed(
            texts=["performance"], model="embed-english-v4.0"
        )
    ...
    def test_telemetry_export_integration(self, mock_cohere_environment):
        ...
        with patch("genops.providers.cohere.trace") as mock_trace:
            ...
            adapter = instrument_cohere(team="telemetry-team", project="export-test")

            # Execute operation
            response = adapter.chat(message="Telemetry test", model="command-light")
    ...
    def test_cost_optimization_workflow_integration(self, mock_cohere_environment):
        ...
        adapter = GenOpsCohereAdapter(api_key="test-key", cost_aggregator=aggregator)
    ...
    def _execute_intelligent_search_workflow(
        self, adapter: GenOpsCohereAdapter, query: str, documents: list[str]
    ) -> dict[str, Any]:
...
class TestCohereErrorHandlingIntegration:
    ...
    def test_budget_exceeded_handling(self):
        ...
        with patch("genops.providers.cohere.ClientV2") as mock_client_class:
            ...
            with patch.object(
                adapter, "_calculate_cost", return_value=(0.001, 0.0, 0.0)
            ):
                response = adapter.chat(message="test", model="command-light")
                ...
                )  # GenOps doesn't block by default, just warns
