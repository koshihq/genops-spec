...
class TestCohereCalculator:
    ...
    def test_cost_breakdown_creation(self, calculator):
        ...
        assert breakdown.total_cost == breakdown.input_cost + breakdown.output_cost + breakdown.operation_cost
    ...
    def test_bulk_cost_calculation(self, calculator):
        ...
        operations = [
            ...
            {"model": "embed-english-v4.0", "operation": "EMBED", "input_tokens": 200, "operation_units": 2},
            {"model": "rerank-english-v3.0", "operation": "RERANK", "operation_units": 1}
        ]
    ...
    def test_multi_operation_workflow_costing(self, calculator):
        ...
        assert abs(total_workflow_cost - expected_total) < 0.000001  # Allow for float precision
    ...
    def test_pricing_data_freshness(self, calculator):
        ...
        with patch('warnings.warn') as mock_warn:
            ...
            assert "outdated" in warning_message.lower() or "stale" in warning_message.lower()
...
class TestPricingIntegration:
    ...
    def test_pricing_with_adapter_integration(self):
        ...
        with patch('genops.providers.cohere_pricing.CohereCalculator') as mock_calc_class:
            mock_calc = Mock()
    ...
    def test_pricing_data_validation(self):
        ...
        for model in major_models:
            try:
                calculator.calculate_cost(
                    model=model,
                    operation="CHAT" if "embed" not in model and "rerank" not in model else
                             "EMBED" if "embed" in model else "RERANK",
                    input_tokens=100,
                    output_tokens=50 if "rerank" not in model and "embed" not in model else 0,
