...
try:
    ...
    from genops.providers.databricks_unity_catalog.registration import (
        ...
        configure_unity_catalog_governance,  # noqa: F401
    )
...
class TestEndToEndIntegration:
    ...
    def test_complete_etl_pipeline_tracking(
        ...
        assert (
            "storage" in cost_summary.cost_by_resource_type
            or "table" in cost_summary.cost_by_resource_type
        )

    @patch("databricks.sdk.WorkspaceClient")
    def test_auto_instrumentation_to_telemetry_export(
        self, mock_client_class, mock_databricks_client
    ):
    ...
    @patch("databricks.sdk.WorkspaceClient")
    def test_multi_workspace_governance_coordination(
        self, mock_client_class, mock_databricks_client
    ):
    ...
    @patch("databricks.sdk.WorkspaceClient")
    def test_compliance_workflow_integration(
        self, mock_client_class, mock_databricks_client
    ):
...
class TestPerformanceIntegration:
    ...
    @patch("databricks.sdk.WorkspaceClient")
    def test_high_volume_operation_tracking(
        self, mock_client_class, mock_databricks_client
    ):
...
class TestErrorRecoveryIntegration:
    ...
    def test_partial_service_degradation_handling(self, mock_client_class):
        ...
        try:
            ...
            assert (
                "service unavailable" in str(e).lower() or "graceful" in str(e).lower()
            )

    def test_telemetry_export_failure_resilience(self):
        """Test resilience when telemetry export fails."""
        with patch(
            "opentelemetry.sdk.trace.export.SpanProcessor.on_end",
...
class TestCrossProviderCompatibility:
    ...
    def test_mixed_provider_cost_aggregation(self, mock_client_class):
        ...
        databricks_adapter.track_sql_warehouse_operation(
        ...
        with patch.object(
            cost_aggregator, "add_external_provider_cost"
        ) as mock_add_external:
