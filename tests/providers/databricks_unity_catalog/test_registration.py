...
@pytest.mark.skipif(
    not REGISTRATION_AVAILABLE, reason="Registration module not available"
)
class TestProviderRegistration:
    ...
    def test_auto_register_with_databricks_available(self, mock_client_class):
        ...
        with patch(
            "genops.providers.databricks_unity_catalog.registration.register_databricks_unity_catalog_provider"
        ) as mock_register:
            mock_register.return_value = True
...
class TestAutoInstrumentation:
    ...
    @patch("databricks.sdk.WorkspaceClient")
    @patch(
        "genops.providers.databricks_unity_catalog.registration._detect_databricks_configuration"
    )
    def test_auto_instrumentation_success(self, mock_detect_config, mock_client_class):
        ...
        with patch(
            "genops.providers.databricks_unity_catalog.registration.instrument_databricks_unity_catalog"
        ) as mock_instrument:
...
class TestConfigurationDetection:
    ...
    def test_detect_databricks_configuration_full(self):
        ...
        with patch.dict("os.environ", env_vars):
            ...
            assert (
                config["workspace_url"] == "https://test-workspace.cloud.databricks.com"
            )
    ...
    def test_detect_databricks_configuration_minimal(self):
        ...
        with patch.dict("os.environ", env_vars, clear=True):
            ...
            assert (
                ...
                == "https://minimal-workspace.cloud.databricks.com"
            )
    ...
    def test_feature_toggle_detection(self):
        ...
        with patch.dict("os.environ", env_vars, clear=True):
            ...
            assert not config["enable_auto_patching"]
...
class TestUnityGovernanceConfiguration:
    ...
    @patch(
        "genops.providers.databricks_unity_catalog.registration.get_governance_monitor"
    )
    @patch("genops.providers.databricks_unity_catalog.registration.get_cost_aggregator")
    def test_configure_unity_catalog_governance_success(
        self, mock_cost_agg, mock_gov_monitor, mock_instrument
    ):
...
class TestEnvironmentHandling:
    ...
    def test_empty_environment_variables(self):
        ...
        env_vars = {"DATABRICKS_HOST": "", "GENOPS_TEAM": "", "GENOPS_PROJECT": ""}
...
class TestIntegrationPatterns:
    ...
    @patch(
        "genops.providers.databricks_unity_catalog.registration.patch_databricks_operations"
    )
