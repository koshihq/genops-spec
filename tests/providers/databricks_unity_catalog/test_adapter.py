...
@pytest.mark.skipif(not DATABRICKS_AVAILABLE, reason="Databricks Unity Catalog provider not available")
class TestGenOpsDatabricksUnityCatalogAdapter:
    ...
    @patch('databricks.sdk.WorkspaceClient')
    def test_catalog_operation_tracking(self, mock_client_class, mock_databricks_client):
    ...
    @patch('databricks.sdk.WorkspaceClient')
    def test_sql_warehouse_operation_tracking(self, mock_client_class, mock_databricks_client):
    ...
    def test_multi_workspace_support(self, mock_client_class):
        ...
        assert len(set(adapter.workspace_url for adapter in adapters)) == 3

    @patch('databricks.sdk.WorkspaceClient')
    def test_error_handling_scenarios(self, mock_client_class):
        """Test error handling for various failure scenarios."""
        # Test connection error
        mock_client_class.side_effect = ConnectionError("Failed to connect to Databricks")
        ...
        assert any(keyword in error_msg for keyword in ["connect", "connection", "databricks"])

    @patch('databricks.sdk.WorkspaceClient')
    def test_authentication_error_handling(self, mock_client_class, mock_databricks_client):
        """Test handling of authentication errors."""
        # Mock authentication failure
        mock_databricks_client.current_user.me.side_effect = Exception("Authentication failed")
        mock_client_class.return_value = mock_databricks_client
    ...
    def test_governance_attributes_validation(self):
        ...
        normalized_minimal = adapter._normalize_governance_attributes(**minimal_governance)
        assert normalized_minimal['team'] == 'minimal-team'

    @patch('databricks.sdk.WorkspaceClient')
    def test_performance_metrics_capture(self, mock_client_class, mock_databricks_client):
    ...
    def test_different_operation_types(self):
        ...
        operation_types = [
            ...
            "query", "transform", "aggregate", "join", "union"
        ]
    ...
    def test_cost_attribution_accuracy(self):
        ...
        attribution_cases = [
            {"team": "data-engineering", "project": "etl-pipeline", "expected_cost_factor": 1.0},
            {"team": "analytics", "project": "reporting", "expected_cost_factor": 0.8},
            {"team": "ml-platform", "project": "model-training", "expected_cost_factor": 1.5}
        ]
    ...
    @patch('databricks.sdk.WorkspaceClient')
    def test_operation_context_management(self, mock_client_class, mock_databricks_client):
        ...
        try:
            with create_unity_catalog_operation_context(
                ...
                operation_type="analytics_pipeline"
            ) as context:
    ...
    def test_real_world_usage_patterns(self, mock_client_class, mock_databricks_client):
        ...
        etl_operations = [
            # Extract phase
            {"op": "table", "action": "read", "catalog": "raw", "schema": "ingestion", "table": "events"},
            {"op": "table", "action": "read", "catalog": "raw", "schema": "ingestion", "table": "users"},
            
            # Transform phase  
            {"op": "warehouse", "warehouse_id": "etl-medium", "query_type": "join"},
            {"op": "warehouse", "warehouse_id": "etl-medium", "query_type": "aggregate"},
            
            # Load phase
            {"op": "table", "action": "write", "catalog": "processed", "schema": "analytics", "table": "user_metrics"}
        ]
...
class TestInstrumentationFunction:
    ...
    def test_multiple_instrumentation_calls(self):
        ...
        try:
            adapter1 = instrument_databricks_unity_catalog()
            adapter2 = instrument_databricks_unity_catalog()
