...
class TestReplicateCostSummary:
    ...
    def test_cost_summary_initialization(self):
        ...
        summary = ReplicateCostSummary(total_cost=1.234, operation_count=5)
...
class TestReplicateCostAggregatorInitialization:
    ...
    def test_aggregator_with_budget_limit(self):
        ...
        aggregator = ReplicateCostAggregator(
            context_name="budget-context", budget_limit=50.0, enable_alerts=True
        )
    ...
    def test_aggregator_with_pricing_calculator(self):
        ...
        with patch(
            "src.genops.providers.replicate_cost_aggregator.ReplicatePricingCalculator"
        ) as mock_calc:
            mock_instance = Mock()
...
class TestOperationTracking:
    ...
    def test_add_multiple_operations(self, aggregator):
        ...
        assert (
            aggregator._cost_by_model["meta/llama-2-7b-chat"] == 0.0025
        )  # 0.001 + 0.0015
...
class TestBudgetMonitoring:
    ...
    def test_budget_alerts_disabled(self):
        ...
        aggregator = ReplicateCostAggregator(
            "no-alerts-context", budget_limit=10.0, enable_alerts=False
        )

        # Add expensive operation
        aggregator.add_operation(
            model="expensive-model", category="video", cost_usd=8.0
        )
    ...
    def test_budget_warning_alert(self):
        ...
        aggregator = ReplicateCostAggregator(
            "warning-context", budget_limit=10.0, enable_alerts=True
        )

        # Add operation that reaches 75% of budget
        aggregator.add_operation(model="test-model", category="text", cost_usd=7.5)
    ...
    def test_budget_critical_alert(self):
        ...
        aggregator = ReplicateCostAggregator(
            "critical-context", budget_limit=10.0, enable_alerts=True
        )

        # Add operation that reaches 90% of budget
        aggregator.add_operation(model="test-model", category="text", cost_usd=9.0)
    ...
    def test_budget_exceeded_alert(self):
        ...
        aggregator = ReplicateCostAggregator(
            "exceeded-context", budget_limit=10.0, enable_alerts=True
        )

        # Add operation that exceeds budget
        aggregator.add_operation(
            model="expensive-model", category="video", cost_usd=12.0
        )
    ...
    def test_budget_alert_updates(self):
        ...
        aggregator = ReplicateCostAggregator(
            "update-context", budget_limit=10.0, enable_alerts=True
        )

        # First operation - under threshold
        aggregator.add_operation(model="test-model", category="text", cost_usd=5.0)
        assert len(aggregator.alerts) == 0

        # Second operation - triggers warning
        aggregator.add_operation(model="test-model", category="text", cost_usd=3.0)
        assert len(aggregator.alerts) == 1
        assert aggregator.alerts[0].alert_type == "warning"

        # Third operation - escalates to critical
        aggregator.add_operation(model="test-model", category="text", cost_usd=2.0)
...
class TestCostSummaryGeneration:
    ...
    def test_get_current_summary_with_budget(self):
        ...
        aggregator = ReplicateCostAggregator(
            "budget-summary-context", budget_limit=10.0, enable_alerts=True
        )

        aggregator.add_operation(model="test-model", category="text", cost_usd=8.0)
...
class TestOptimizationRecommendations:
    ...
    def test_generate_recommendations_many_images(self):
        ...
        for _i in range(8):  # More than 5 images
            aggregator.add_operation(
                model="black-forest-labs/flux-schnell", category="image", cost_usd=0.003
            )
    ...
    def test_generate_recommendations_budget_limit(self):
        ...
        aggregator = ReplicateCostAggregator(
            "recommendations-budget", budget_limit=10.0
        )

        # Add operation that uses 85% of budget
        aggregator.add_operation(
            model="expensive-model", category="video", cost_usd=8.5
        )
    ...
    def test_generate_recommendations_with_pricing_calculator(self):
        ...
        with patch(
            ...
            aggregator.add_operation(
                model="expensive-model", category="text", cost_usd=0.01
            )
...
class TestModelPerformanceAnalysis:
    ...
    def aggregator_with_model_data(self):
        ...
        aggregator.add_operation(
            model="different-model", category="text", cost_usd=0.005, latency_ms=3000.0
        )
    ...
    def test_get_model_performance_existing_model(self, aggregator_with_model_data):
        ...
        performance = aggregator_with_model_data.get_model_performance(
...
class TestExportFunctionality:
    ...
    def populated_aggregator(self):
        ...
        aggregator = ReplicateCostAggregator("export-context", budget_limit=5.0)

        aggregator.add_operation(
            model="test-model-1", category="text", cost_usd=0.001, team="team-1"
        )

        aggregator.add_operation(
            model="test-model-2", category="image", cost_usd=0.003, team="team-2"
        )
...
class TestCreateReplicateCostContext:
    ...
    def test_cost_context_with_budget(self, mock_tracer):
        ...
        with create_replicate_cost_context(
            "budget-context", budget_limit=25.0
        ) as context:
            assert context.budget_limit == 25.0

            # Add operation to test functionality within context
            context.add_operation(model="test-model", category="text", cost_usd=0.01)
    ...
    def test_cost_context_success_logging(self, mock_logger, mock_tracer):
        ...
        with create_replicate_cost_context("success-context") as context:
            context.add_operation(model="test-model", category="text", cost_usd=0.005)
        ...
        with pytest.raises(ValueError):
            with create_replicate_cost_context("error-context") as context:
                context.add_operation(
                    model="test-model", category="text", cost_usd=0.001
                )
...
class TestContextManagerIntegration:
    ...
    def test_context_telemetry_attributes(self, mock_tracer):
        ...
        with create_replicate_cost_context(
            "telemetry-test", budget_limit=15.0
        ) as context:
