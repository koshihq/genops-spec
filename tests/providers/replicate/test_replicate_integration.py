...
class TestFullWorkflowIntegration:
    ...
    def mock_replicate_environment(self):
        ...
        with patch('src.genops.providers.replicate.replicate') as mock_replicate, \
             patch('src.genops.providers.replicate_validation.replicate') as mock_val_replicate:

            # Mock successful API responses
            mock_replicate.run.return_value = "Integration test response"
            mock_replicate.stream.return_value = iter(["Integration", " test", " streaming"])
            ...
            pricing_patch = patch('src.genops.providers.replicate_pricing.ReplicatePricingCalculator')
    ...
    def test_complete_multimodal_workflow(self, mock_replicate_environment):
        ...
        with create_replicate_cost_context("integration-workflow", budget_limit=1.0) as context:
...
class TestValidationIntegration:
    ...
    def validation_environment(self):
        ...
        with patch.dict('os.environ', {'REPLICATE_API_TOKEN': 'r8_validation_test_token'}):
            yield

    @patch('src.genops.providers.replicate_validation.replicate')
    @patch('requests.get')
    def test_complete_validation_workflow(self, mock_requests, mock_replicate, validation_environment):
    ...
    def test_quick_validate_integration(self):
        ...
        with patch('src.genops.providers.replicate_validation.validate_setup') as mock_validate:
...
class TestErrorHandlingIntegration:
    ...
    def test_graceful_degradation_without_pricing(self):
        ...
        with patch('src.genops.providers.replicate.replicate') as mock_replicate, \
             patch('src.genops.providers.replicate.ReplicatePricingCalculator', side_effect=ImportError):
...
class TestPerformanceIntegration:
    ...
    def performance_environment(self):
        ...
        with patch('src.genops.providers.replicate.replicate') as mock_replicate:
            ...
            with patch('src.genops.providers.replicate_pricing.ReplicatePricingCalculator') as mock_calc:
                mock_instance = Mock()
    ...
    def test_memory_usage_with_large_context(self, performance_environment):
        ...
        adapter = GenOpsReplicateAdapter(api_token="r8_memory_test")
...
class TestRealWorldScenarios:
    ...
    def realistic_environment(self):
        ...
        with patch('src.genops.providers.replicate.replicate') as mock_replicate:
            ...
            responses = {
                ...
                "black-forest-labs/flux-schnell": ["https://example.com/generated_image_1.png"],
                "black-forest-labs/flux-pro": ["https://example.com/professional_image.png"],
            ...
            with patch('src.genops.providers.replicate_pricing.ReplicatePricingCalculator') as mock_calc:
                ...
                def realistic_model_info(model_name):
                    ...
                    pricing_map = {
                        ...
                        "black-forest-labs/flux-schnell": ("output", 0.003, None, "image", 0.003),
                        "black-forest-labs/flux-pro": ("output", 0.04, None, "image", 0.04),
                        "openai/whisper": ("time", 0.0001, None, "audio", 0.0001)
                    }

                    if model_name in pricing_map:
                        pricing_type, input_cost, output_cost, category, base_cost = pricing_map[model_name]
                        return ReplicateModelInfo(
                            name=model_name,
                ...
                def realistic_cost_calculation(model_info, input_data, output, latency_ms):
                    ...
                    if model_info.pricing_type == "token":
                        ...
                        input_cost = (prompt_tokens / 1000) * (model_info.input_cost or 0)
                        output_cost = (output_tokens / 1000) * (model_info.output_cost or 0)
    ...
    def test_marketing_campaign_workflow(self, realistic_environment):
        ...
        with create_replicate_cost_context("marketing-campaign", budget_limit=5.0) as context:
            ...
            assert summary.operation_count == 8  # 1 strategy + 3 visuals + 4 copy pieces
