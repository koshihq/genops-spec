...
def mock_kubernetes_files(temp_dir):
    ...
    service_account_dir = (
        temp_dir / "var" / "run" / "secrets" / "kubernetes.io" / "serviceaccount"
    )
    ...
    return {"service_account_dir": service_account_dir, "cgroup_dir": cgroup_dir}
...
def mock_genops_modules():
    ...
    mock_validation_result.get_summary.return_value = (
        "✅ Kubernetes environment valid (namespace: test-namespace)"
    )
    ...
    mock_get_active_instrumentations = Mock(
        return_value={
...
skip_without_genops = pytest.mark.skipif(
    not _check_genops_available(), reason="GenOps not installed"
)

skip_without_kubernetes = pytest.mark.skipif(
    not _is_kubernetes_available(), reason="Kubernetes not available"
)
