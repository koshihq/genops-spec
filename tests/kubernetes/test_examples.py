...
def mock_genops_imports():
    ...
    mock_adapter.create_governance_context.return_value.__aenter__ = AsyncMock(
        return_value=mock_governance_context
    )
    mock_adapter.create_governance_context.return_value.__aexit__ = AsyncMock(
        return_value=None
    )
    ...
    with patch.dict(
        "sys.modules",
        {
            ...
            "genops.core.instrumentation": Mock(
                get_active_instrumentations=Mock(
                    return_value={
...
class TestSetupValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_environment_with_options(
        self, test_env, mock_genops_imports
    ):
...
class TestAutoInstrumentation:
    ...
    @pytest.mark.asyncio
    async def test_demonstrate_auto_instrumentation(
        self, test_env, mock_genops_imports
    ):
...
class TestProductionPatterns:
    ...
    async def test_high_availability_pattern(self, test_env, mock_genops_imports):
        ...
        try:
            ...
            with (
                patch("production_patterns.PerformanceMonitor"),
    ...
    def test_production_config(self, test_env):
        ...
        try:
            ...
            # Test custom values
            config = production_patterns.ProductionConfig(
                max_concurrent_requests=100, log_level="DEBUG"
            )
    ...
    def test_example_documentation(self, test_env):
        ...
        for example_path in examples:
            ...
            assert "Usage:" in content or "usage:" in content, (
                f"{example_path.name} missing usage examples"
            )

            # Should have clear descriptions
            assert any(
                word in content.lower() for word in ["demonstrates", "shows", "example"]
