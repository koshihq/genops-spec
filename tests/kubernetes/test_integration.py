...
class TestKubernetesIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_tracking_workflow(
        self, mock_genops_modules, sample_governance_attributes
    ):
        ...
        try:
            ...
            with adapter.create_governance_context(
                **sample_governance_attributes
            ) as ctx:
    ...
    @pytest.mark.skipif(
        not os.getenv("TEST_WITH_REAL_CLUSTER"), reason="Real cluster testing disabled"
    )
...
class TestExampleIntegration:
    ...
    @pytest.mark.asyncio
    async def test_setup_validation_example_integration(
        self, mock_genops_modules, examples_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_basic_tracking_example_integration(
        self, mock_genops_modules, examples_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_cost_tracking_example_integration(
        self, mock_genops_modules, examples_dir
    ):
        ...
        try:
            ...
            with (
                patch("cost_tracking.CostTracker"),
                patch("cost_tracking.BudgetManager"),
            ):
                ...
                multi_provider_result = (
                    await demo.demonstrate_multi_provider_cost_aggregation()
                )
                assert multi_provider_result is True

                optimization_result = (
                    await demo.demonstrate_cost_optimization_strategies()
                )
        ...
        for example_file in example_files:
            ...
            try:
                ...
                assert (
                    ...
                    or "examples" in result.stdout.lower()
                )
...
class TestPerformanceIntegration:
    ...
    async def test_high_volume_operations(
        self, mock_genops_modules, sample_governance_attributes, performance_timer
    ):
        ...
        try:
            ...
            async def run_batch_operations():
                ...
                for i in range(operation_count):
                    ...
                    async def single_operation():
                        with adapter.create_governance_context(**attrs) as ctx:  # noqa: B023
                            ctx.add_cost_data(
                                provider="openai",
                                model="gpt-3.5-turbo",
                                cost=0.001,
            ...
            assert performance_timer.elapsed < 5.0, (
                f"High volume test took {performance_timer.elapsed:.2f}s"
            )
