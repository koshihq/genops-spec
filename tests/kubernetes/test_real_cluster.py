...
class RealClusterTestFramework:
    ...
    async def setup_cluster_info(self) -> ClusterInfo:
        ...
        try:
            ...
            node_count = (
                len(nodes_result.stdout.strip().split("\n"))
...
"""
            ...
            self.cleanup_resources.append(
                ("deployment", f"{self.test_namespace}/genops-ai-test")
            )
            self.cleanup_resources.append(
                ("service", f"{self.test_namespace}/genops-ai-test")
            )
    ...
    async def _wait_for_deployment_ready(
        self, deployment_name: str, timeout: int = 60
    ) -> bool:
        """Wait for deployment to be ready."""
...
"""
            ...
            success = (
                is_kubernetes
                and namespace != "None"
                and namespace == self.test_namespace
            )
...
"""
            ...
            has_accessible_cgroups = (
                "Has accessible cgroup files: True" in result.stdout
            )
    ...
    def generate_test_report(self) -> dict[str, Any]:
        ...
        report = {
            "cluster_info": {
                ...
                "context": self.cluster_info.context
...
@pytest.mark.skipif(
    not os.getenv("TEST_REAL_CLUSTER"), reason="Real cluster testing disabled"
)
...
class TestRealClusterIntegration:
    ...
    async def test_real_network_connectivity(self, cluster_framework):
        ...
        assert result.success, (
            f"Basic network connectivity failed: {result.error_message}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--namespace", help="Test namespace (auto-generated if not provided)"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Cleanup resources after tests",
    )
    parser.add_argument(
        "--no-cleanup", action="store_false", dest="cleanup", help="Skip cleanup"
    )
    ...
    try:
        ...
        if args.output:
            ...
            print("\n📊 Test Report:")
            print(
                f"   Cluster: {report['cluster_info']['name']} ({report['cluster_info']['version']})"
            )
            print(
                f"   Tests: {report['summary']['successful_tests']}/{report['summary']['total_tests']} passed"
            )
