    ...
    def test_quickstart_guides_exist(self, docs_dir):
        ...
        for guide in quickstart_guides:
            ...
            assert "5 minutes" in content or "5 minute" in content, (
                f"Missing 5-minute promise in {guide}"
            )
        ...
        for md_file in md_files:
            ...
            for example_ref in example_refs:
                ...
                assert example_path.exists(), (
                    f"Referenced example {example_ref} not found (referenced in {md_file.name})"
                )
    ...
    def test_kubectl_commands_are_valid(self, docs_dir):
        ...
        for md_file in md_files:
            ...
            for cmd in kubectl_commands:
                ...
                any(re.search(pattern, cmd) for pattern in valid_patterns)

                # Or check basic syntax
                parts = cmd.split()
                assert len(parts) >= 2, (
                    f"Invalid kubectl command: {cmd} (in {md_file.name})"
                )
                assert parts[0] == "kubectl", (
                    f"Invalid kubectl command: {cmd} (in {md_file.name})"
                )
    ...
    def test_helm_commands_are_valid(self, docs_dir):
        ...
        for md_file in md_files:
            ...
            for cmd in helm_commands:
                ...
                parts = cmd.split()
                assert len(parts) >= 2, (
                    f"Invalid helm command: {cmd} (in {md_file.name})"
                )
                assert parts[0] == "helm", (
                    f"Invalid helm command: {cmd} (in {md_file.name})"
                )
                ...
                assert parts[1] in valid_subcommands, (
                    f"Unknown helm subcommand: {cmd} (in {md_file.name})"
                )
        ...
        for md_file in md_files:
            ...
            for language, code in code_blocks:
                ...
                if language == "yaml":
                    ...
                    for line in lines:
                        if line.strip() and not line.startswith("#"):
                            ...
                            assert "\t" not in line, (
                                f"YAML should use spaces, not tabs: {line} (in {md_file.name})"
                            )
                ...
                # Check bash blocks
                    ...
                    assert "\r\n" not in code, (
                        f"Bash code should use Unix line endings (in {md_file.name})"
                    )
...
class TestExampleDocumentation:
    ...
    def test_all_examples_have_docstrings(self, examples_dir):
        ...
        for example_file in example_files:
            ...
            assert "Usage:" in content, (
                f"Example {example_file.name} missing usage information"
            )
    ...
    def test_examples_have_help_text(self, examples_dir):
        ...
        example_files = [
            f for f in examples_dir.glob("*.py") if not f.name.startswith("__")
        ]
        ...
        for example_file in example_files:
            try:
                ...
                assert result.returncode == 0, (
                    f"Example {example_file.name} --help failed"
                )
                assert len(result.stdout) > 50, (
                    f"Example {example_file.name} help text too short"
                )
                assert "usage" in result.stdout.lower(), (
                    f"Example {example_file.name} missing usage in help"
                )
    ...
    def test_examples_have_proper_error_handling(self, examples_dir):
        ...
        for example_file in example_files:
            ...
            assert "try:" in content, (
                f"Example {example_file.name} missing error handling"
            )
            assert "except" in content, (
                f"Example {example_file.name} missing exception handling"
            )

            # Should check for ImportError specifically
            assert "ImportError" in content, (
                f"Example {example_file.name} missing ImportError handling"
            )
    ...
    def test_examples_readme_exists_and_comprehensive(self, examples_dir):
        ...
        example_files = [
            f.name for f in examples_dir.glob("*.py") if not f.name.startswith("__")
        ]

        for example_file in example_files:
            assert example_file in content, (
                f"Example {example_file} not described in README"
            )

        # Should have learning progression
        assert "5-minute" in content or "5 minutes" in content, (
            "README missing 5-minute progression"
        )
        assert "30-minute" in content or "30 minutes" in content, (
            "README missing 30-minute progression"
        )
                ...
                for heading in headings:
                    for alt in alternatives:
                        if alt.lower() in heading.lower() and preferred not in heading:
                            ...
                            print(
                                f"Warning: Consider using '{preferred}' instead of '{alt}' in heading: {heading} ({md_file.name})"
                            )
    ...
    def test_consistent_command_formatting(self, docs_dir):
        ...
        for md_file in md_files:
            ...
            if len(kubectl_inline) > len(kubectl_blocks) * 2:
                print(
                    f"Warning: Consider using code blocks for kubectl commands in {md_file.name}"
                )
    ...
    def test_links_are_consistent(self, docs_dir):
        ...
        for md_file in md_files:
            ...
            for _link_text, link_url in internal_links:
                ...
                assert not link_url.startswith("http"), (
                    f"Internal link should be relative: {link_url} (in {md_file.name})"
                )
                ...
                if not link_url.startswith("#") and "/" not in link_url:
                    ...
                    if not referenced_file.exists():
                        print(
                            f"Warning: Referenced file may not exist: {link_url} (in {md_file.name})"
                        )
