...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "examples", "observability")
)
...
class TestValidationResult:
    ...
    def test_validation_result_has_warnings(self):
        ...
        result = OTelCollectorValidationResult(valid=True, warnings=["Warning 1"])
...
class TestValidateSetup:
    ...
    def test_validate_setup_with_invalid_endpoint_url(self):
        ...
        result = validate_setup(
            collector_endpoint="invalid-url", check_connectivity=False
        )
    ...
    def test_validate_setup_without_connectivity_check(self):
        ...
        result = validate_setup(
            collector_endpoint="http://localhost:4318", check_connectivity=False
        )
...
class TestEnvironmentVariables:
    ...
    @patch.dict(
        os.environ, {"OTEL_EXPORTER_OTLP_ENDPOINT": "http://env-collector:4318"}
    )
    @patch("otel_collector_validation.HAS_REQUESTS", True)
