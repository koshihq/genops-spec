...
class TestHaystackInstallationValidation:
    ...
    def test_validate_haystack_not_installed(self):
        ...
        with patch(
            "genops.providers.haystack_validation.importlib.import_module"
        ) as mock_import:
...
class TestAIProvidersValidation:
    ...
    def test_validate_providers_invalid_key_format(self):
        ...
        warning_issues = [issue for issue in issues if issue.severity == "warning"]
        assert any(
            "key format appears invalid" in issue.message for issue in warning_issues
        )
    ...
    def test_validate_providers_key_no_library(self):
        ...
        warning_issues = [issue for issue in issues if issue.severity == "warning"]
        assert any(
            "API key found but library not installed" in issue.message
...
class TestPerformanceBenchmarking:
    ...
    def test_benchmark_performance_slow_import(self, mock_adapter_class):
        ...
        with patch(
            "genops.providers.haystack_validation.time.perf_counter", mock_perf_counter
        ):
            metrics, issues = benchmark_performance()
...
class TestValidateHaystackSetup:
    ...
    def test_validate_haystack_setup_python_failure(self, mock_validate_python):
        ...
        mock_validate_python.return_value = (
            ...
            [
                ValidationIssue(
                    "error", "dependency", "Python too old", "Upgrade Python"
                )
            ],
        )
    ...
    def test_validate_haystack_setup_haystack_failure(self, mock_validate_haystack):
        ...
        mock_validate_haystack.return_value = (
            False,
            [
                ValidationIssue(
                    "error", "dependency", "Haystack not found", "Install Haystack"
                )
            ],
            None,
        )
...
class TestPrintValidationResult:
    ...
    def test_print_validation_result_with_errors(self, capsys):
        ...
        result = ValidationResult(
            is_valid=False, overall_score=0.3, python_version="3.7.0", platform="linux"
        )
    ...
    def test_print_validation_result_with_warnings(self, capsys):
        ...
        result = ValidationResult(
            is_valid=True, overall_score=0.8, python_version="3.8.5", platform="darwin"
        )
    ...
    def test_print_validation_result_needs_fixes(self, capsys):
        ...
        result = ValidationResult(is_valid=False, overall_score=0.5)
