...
class TestCLIParser:
    ...
    def test_parser_policy_register_command(self):
        ...
        args = parser.parse_args([
            'policy', 'register', 'cost_limit',
            '--description', 'Cost limit policy',
            '--enforcement', 'warning',
...
class TestMainFunction:
    ...
    def test_main_keyboard_interrupt(self, capsys):
        ...
        with patch('sys.argv', ['genops', 'status']):
            with patch('genops.cli.main.cmd_status', mock_cmd_that_raises_keyboard_interrupt):
                result = main()
                ...
                with patch('genops.cli.main.register_policy') as mock_register:
                    with patch('genops.cli.main.track_usage') as mock_track_usage:
                        with patch('genops.cli.main.track') as mock_track:
                            ...
                            mock_init.return_value = mock_instrumentor
                            mock_status.return_value = mock_instrumentor.status.return_value
                            
                            # Test init command
                            with patch('sys.argv', ['genops', 'init', '--team', 'test-team']):
                                result = main()
                                assert result == 0
