...
def benchmark_telemetry_overhead():
    ...
    if result.mean < 50:  # Expect <50ms overhead
        print(f"✅ Overhead is minimal (<50ms)")
    else:
...
def benchmark_cost_calculation():
    ...
    for model in models:
        for input_tokens, output_tokens in token_counts:
            ...
            result = measure_operation(calculate_cost, iterations=ITERATIONS*10, unit="μs")
            results.append((model.split("/")[-1], input_tokens, output_tokens, result.mean))
...
def benchmark_memory_profiling():
    ...
    print("-" * 50)
    for r in results:
        print(f"{r['requests']:<15} {r['total_kb']:<15.1f} {r['per_request_kb']:<20.2f}")
...
def benchmark_budget_manager():
    ...
    for config_name, config in period_configs:
        ...
        def budget_check():
            budget_manager.check_budget_availability(0.001)

        check_result = measure_operation(budget_check, iterations=ITERATIONS*10, unit="μs")

        # Benchmark cost recording
        def cost_recording():
            budget_manager.record_cost(0.001)

        record_result = measure_operation(cost_recording, iterations=ITERATIONS*10, unit="μs")
        ...
        print(f"  Budget Check: {check_result.mean:.1f} μs (median: {check_result.median:.1f})")
        print(f"  Cost Recording: {record_result.mean:.1f} μs (median: {record_result.median:.1f})")
    ...
    for config_name, res in results.items():
        print(f"{config_name:<15} {res['check'].mean:<15.1f} {res['record'].mean:<15.1f}")
...
def benchmark_circuit_breaker():
    ...
    closed_result = measure_operation(check_closed, iterations=ITERATIONS*10, unit="μs")
...
def benchmark_retry_logic():
    ...
    for attempt in range(3):
        ...
        print(f"  Attempt {attempt + 1}: Expected {wait_time:.1f}s, Actual {actual_wait:.3f}s, Deviation {deviation:.2f}%")
