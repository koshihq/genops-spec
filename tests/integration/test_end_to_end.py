...
class TestEndToEndWorkflows:
    ...
    def test_complete_governance_workflow(self, mock_otel_setup, cleanup_test_state):
        ...
        @genops.track_usage(
            operation_name="customer_support_query",
    ...
    def test_provider_integration_anthropic(self, mock_anthropic_import, mock_otel_setup):
    ...
    def test_multi_provider_governance(self, mock_openai_import, mock_anthropic_import, mock_otel_setup, cleanup_test_state):
        ...
        openai_span = next(s for s in spans if s.attributes.get("genops.provider") == "openai")
        anthropic_span = next(s for s in spans if s.attributes.get("genops.provider") == "anthropic")
    ...
    def test_policy_enforcement_integration(self, mock_otel_setup, cleanup_test_state):
        ...
        with patch('genops.core.policy._global_policy_engine.evaluate_policy') as mock_evaluate:
    ...
    def test_cost_attribution_workflow(self, mock_openai_import, mock_otel_setup):
        ...
        # Group by customer
        customer_a_spans = [s for s in spans if s.attributes.get("genops.customer_id") == "customer_a"]
        customer_b_spans = [s for s in spans if s.attributes.get("genops.customer_id") == "customer_b"]
        ...
        gpt35_spans = [s for s in spans if "gpt-3.5-turbo" in s.attributes.get("genops.model", "")]
        gpt4_spans = [s for s in spans if "gpt-4" in s.attributes.get("genops.model", "")]
    ...
    def test_context_manager_integration(self, mock_otel_setup):
        ...
        with genops.track(
            ...
            # Nested operation
            with genops.track(
                operation_name="individual_inference",
                feature="prediction"
            ) as inner_span:
