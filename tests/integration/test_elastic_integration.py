...
class TestElasticIntegrationBasicWorkflow:
    ...
    def test_complete_tracking_workflow(self, sample_elastic_config, mock_elasticsearch_client):
        ...
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            ...
            assert mock_elasticsearch_client.bulk.called or mock_elasticsearch_client.index.called

    def test_multiple_operations_tracking(self, sample_elastic_config, mock_elasticsearch_client):
        ...
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            ...
            for op in operations:
                with adapter.track_ai_operation(op, operation_type=f"llm.{op}") as span:
                    adapter.record_cost(span=span, cost=0.01, provider="openai", model="text-embedding-ada-002")

            # Flush to ensure all exports
            adapter.exporter.flush()

            # Verify multiple exports
            assert mock_elasticsearch_client.bulk.call_count >= 1 or mock_elasticsearch_client.index.call_count >= 3


class TestElasticIntegrationAutoInstrumentation:
    """Test auto-instrumentation functionality."""

    def test_auto_instrument_with_env_vars(self, mock_env_vars, mock_elasticsearch_client):
    ...
    def test_auto_instrument_returns_singleton(self, mock_env_vars, mock_elasticsearch_client):
...
class TestElasticIntegrationCostAggregation:
    ...
    def test_aggregate_costs_single_provider(self, sample_elastic_config, mock_elasticsearch_client):
        ...
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            ...
            for i in range(5):
                with adapter.track_ai_operation(f"op-{i}") as span:
                    cost = 0.01 * (i + 1)
                    adapter.record_cost(span=span, cost=cost, provider="openai", model="gpt-4")
                    total_cost += cost

            # Verify stats tracking
            stats = adapter.exporter.stats
            # Costs should be aggregated in Elasticsearch

    def test_aggregate_costs_multiple_providers(self, sample_elastic_config, mock_elasticsearch_client):
        ...
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            ...
            for provider, model, cost in providers:
                with adapter.track_ai_operation(f"{provider}-op") as span:
                    adapter.record_cost(span=span, cost=cost, provider=provider, model=model)
...
class TestElasticIntegrationPolicyEnforcement:
    ...
    def test_track_policy_decision(self, sample_elastic_config, mock_elasticsearch_client):
    ...
    def test_track_policy_violation(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationExportModes:
    ...
    def test_batch_mode_workflow(self, sample_elastic_config, mock_elasticsearch_client):
        ...
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            ...
            for i in range(6):
                with adapter.track_ai_operation(f"batch-op-{i}") as span:
                    adapter.record_cost(span=span, cost=0.01, provider="openai", model="gpt-3.5-turbo")

            # Verify bulk export was called
            assert mock_elasticsearch_client.bulk.called

    def test_realtime_mode_workflow(self, sample_elastic_config, mock_elasticsearch_client):
    ...
    def test_hybrid_mode_workflow(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationGovernanceAttributes:
    ...
    def test_default_governance_attributes(self, sample_elastic_config, mock_elasticsearch_client):
        """Test that default governance attributes are propagated."""
        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            adapter = instrument_elastic(**sample_elastic_config, auto_validate=False)

            with adapter.track_ai_operation("gov-test") as span:
                adapter.record_cost(span=span, cost=0.01, provider="openai", model="gpt-4")
            ...
            assert span.attributes.get("genops.cost_center") == sample_elastic_config["cost_center"]

    def test_override_governance_attributes(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationILM:
    ...
    def test_ilm_policy_creation(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationValidation:
    ...
    def test_validation_on_initialization(self, sample_elastic_config, mock_elasticsearch_client):
        """Test that validation runs on initialization when enabled."""
        config = sample_elastic_config.copy()
        config["auto_validate"] = True

        with patch('genops.providers.elastic.client.Elasticsearch', return_value=mock_elasticsearch_client):
            adapter = instrument_elastic(**config)

            # Adapter should initialize successfully with validation

    def test_standalone_validation(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationErrorRecovery:
    ...
    def test_export_failure_recovery(self, sample_elastic_config, mock_elasticsearch_client):
    ...
    def test_operation_error_handling(self, sample_elastic_config, mock_elasticsearch_client):
...
class TestElasticIntegrationShutdown:
    ...
    def test_shutdown_flushes_pending_events(self, sample_elastic_config, mock_elasticsearch_client):
