...
try:
    from genops.providers.haystack.validation import ValidationIssue, ValidationResult
...
class InteractiveValidator:
    ...
    def __init__(
        self,
        provider_focus: Optional[str] = None,
        fix_mode: bool = False,
        detailed: bool = False,
    ):
        ...
        if self.provider_focus and self.provider_focus in provider_specific:
            ...
            for module, package in provider_specific[self.provider_focus]:
                try:
                    ...
                    print(
                        f"   ❌ {module} required for {self.provider_focus} (pip install {package})"
                    )
                    result.missing_dependencies.append(package)
    ...
    def _validate_specific_provider(
        self, result: InteractiveValidationResult, provider: str
    ):
        ...
        for env_var in config["env_vars"]:
            if os.getenv(env_var):
                ...
                masked_value = (
                    f"{os.getenv(env_var)[:8]}..."
                    if len(os.getenv(env_var)) > 8
                    else "***"
                )
    ...
    def _generate_interactive_guidance(self, result: InteractiveValidationResult):
        ...
        for category in category_priority:
            ...
            for fix in fixes:
                ...
                if self.fix_mode and category in [
                    "required_dependency",
                    "provider_dependency",
                ]:
            ...
            if fix.get("category") in ["required_dependency", "provider_dependency"]:
                ...
                if fix_command.startswith("pip install"):
                    ...
                    if self._confirm_action(
                        f"Install {fix['issue'].split(':')[-1].strip()}?"
                    ):
                        try:
                            subprocess.run(
                                fix_command.split(), check=True, capture_output=True
                            )
    ...
    def display_results(self, result: InteractiveValidationResult):
        ...
        # Issue summary
        total_issues = len(result.missing_dependencies) + len(
        ...
        if total_issues == 0:
            ...
            print("\n📚 Next Steps:")
            print(
                "   • Try the quickstart: python examples/haystack/basic_pipeline_tracking.py"
            )
            ...
            if result.suggested_fixes:
                remaining_fixes = [
                    ...
                    if f["issue"] not in self.fixes_applied
                ]
                if remaining_fixes:
                    print(
                        f"📝 {len(remaining_fixes)} fixes still needed (see guidance above)"
                    )
        ...
        print(
            "   Provider-specific:  python scripts/validate_setup.py --provider openai"
        )
...
Examples:
  ...
  python scripts/validate_setup.py --detailed         # Verbose output
    ...
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed environment information"
    )
    ...
    try:
        ...
        if args.json:
            ...
            json_result = {
                ...
                "total_issues": len(result.missing_dependencies)
                + len(result.configuration_issues),
            }
